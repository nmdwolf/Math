...
\subsection{Derivatives}
	...
	\newdef{Derivative of a distribution}{\index{derivative}
...
\section{Fourier series}
	...
	\newformula{Generalized Fourier series}{\index{Fourier!series}\label{transforms:fourier_series}
    	Let $f(x)\in L^2[-l, l]$ be a $2l$-periodic function. This function can be approximated by the following series:
        \begin{gather}
	...
	\newdef{Periodic extension}{\index{periodic!extension}
    	Let $f(x)$ be piecewise $C^1$ on $[-L, L]$. The periodic extension $f^L(x)$ is defined by gluing ''copies'' of $f(x)$ together. The \textbf{normalized periodic extension} is defined as follows:
        \begin{gather}
...
\section{Fourier transformation}\index{Fourier!transformation}

	The Fourier series can be used to expand a $2l$-periodic function as an infinite series of exponentials. For expanding a nonperiodic function we need the Fourier integral:
	\begin{gather}
	...
	\begin{property}[Convergence of the Fourier integral]
	...
	\begin{theorem}[Parceval]\index{Parceval}
		...
		\end{gather}
	\end{theorem}
	\begin{result}[Plancherel]\index{Plancherel}
...
\section{Tempered distributions}
	...
	\newdef{Schwartz space}{\index{Schwartz!space}\label{distribution:schwartz_space}
		The Schwartz space of \textbf{rapidly decreasing functions}\footnotemark\ $\mathscr{S}(\mathbb{R})$ is defined as follows:
		...
		\begin{gather}
		\mathscr{S}(\mathbb{R}) = \left\{f(x)\in C^\infty(\mathbb{R}):\forall i,j\in\mathbb{N},\forall x\in\mathbb{R}:|x^if^{(j)}(x)|<+\infty\right\}.
		\end{gather}
	}
	...
	\begin{property}
		If $f(x)\in\mathscr{S}(\mathbb{R})$ and $a(x)\in N(\mathbb{R})$ then $a(x)f(x)\in\mathscr{S}(\mathbb{R})$.
	\end{property}
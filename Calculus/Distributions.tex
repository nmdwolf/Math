\chapter{Distributions}\label{chapter:distributions}
	...
	The main references for this chapter are \cite{AMP1, AMP2, georgiev}. Although this chapter is technically part of functional analysis and hence uses the language of normed spaces we chose to present it in the part on calculus. For important definitions or theorems we have added references to chapter \ref{chapter:normed_spaces}.
...
\section{Distributions as functionals}
	...
	\begin{example}[Ordinary function as generalized function]
    	...
    	\begin{gather}
	    	\langle f,g \rangle = \int f(x)g(x)d^nx.
    	\end{gather}
	\end{example}
...
\section{\difficult{Analysis on groups}}
    ...
    \newdef{Pontryagin dual}{\index{Pontryagin!dual}
        Let $G$ be a locally compact Abelian group. Its (Pontryagin) dual is defined as the group of continuous homomorphisms from $G$ to the circle:
        \begin{gather}
         G^\vee := \hom(G, S^1).
        \end{gather}
        In general this group is endowed with the compact-open topology.
    }
    ...
    \newdef{$L^p$-space}{
        Consider a locally compact group $G$ together with its canonical Haar measure $\mu$. For all $p>0$ one can define the function space $L^p(G)$ as follows:
        \begin{gather}
            L^p(G) := \left\{(f:G\rightarrow\mathbb{C}):\int_G|f(g)|^pd\mu(g)<\infty\right\}.
        \end{gather}
    }

...
\subsection{\texorpdfstring{$L^p$}{Lp} spaces}
    ...
    \begin{property}
        For all $1\leq p\leq+\infty$, $L^p(E)$ is a Banach space under the norm given by
        \begin{gather}
    ...
    \newformula{H\"{o}lder's inequality}{\index{H\"older's inequality}\label{lebesgue:holders_inequality}
        Let $\frac{1}{p} + \frac{1}{q} = 1$ with $p\geq1$. For every $f\in L^p(E)$ and $g\in L^q(E)$ we have that $fg\in L^1(E)$ and that
        \begin{gather}
            ||fg||_1\leq||f||_p||g||_q.
        \end{gather}
    }
...
\subsection{Real hyperspace \texorpdfstring{$\mathbb{R}^n$}\ }
    ...
    \newdef{Hypercube}{\index{hypercube}
        Let $I_1, ..., I_n$ be a sequence of intervals. The hypercube spanned by them is defined as the following set:
        \begin{gather}
            \mathbf{I} = I_1\times...\times I_n
        \end{gather}
    }
    \newdef{Generalized length}{\index{volume}
        Let $\mathbf{I}$ be a hypercube induced by the set of intervals $I_1,...,I_n$. The generalized length (or \textbf{volume}) of $\mathbf{I}$ is given by
        \begin{gather}

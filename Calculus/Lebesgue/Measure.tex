...
\subsection{General definitions}
    ...
    \newdef{Measure}{\index{measure}\index{outer!measure}\index{$\sigma$-additivity}\label{lebesgue:measure}
        ...
        \begin{enumerate}
            ...
            \item Measure zero: $\mu(\emptyset) = 0$
            \item Countable additivity\footnote{This is also called \textbf{$\sigma$-additivity}.} : $\forall i\neq j:E_i\cap E_j=\emptyset\implies\mu\left(\bigcup_{i=1}^\infty E_i\right) = \sum_{i=1}^\infty \mu(E_i)$.
        \end{enumerate}
    ...
    \newdef{Regular measure}{\label{lebesgue:regular_measure}
        ...
        \begin{align}
            \mu(B) =& \inf\{\mu(O):O \text{ open and measurable}, O\supset B\}\\
            \mu(B) =& \sup\{\mu(F):F \text{ compact and measurable}, F\subset B\}.
        \end{align}
    }
    ...
    \newdef{\texorpdfstring{$\sigma$-}{sigma-}finite measure}{\index{$\sigma$-finite}\label{lebesgue:sigma_finite_measure}
        Let $(X,\Sigma,\mu)$ be a measure space. The measure $\mu$ is said to be $\sigma$-finite if there exists a sequence $(A_i)_{i\in\mathbb{N}}$ of measurable sets such that $\bigcup_{i=1}^{+\infty}A_i = X$ with $\forall A_i:\mu(A_i) < +\infty$.
    }
...
\subsection{Lebesgue measure}
    ...
    \newdef{Null set}{\index{null set}
        A set $A\subset\mathbb{R}$ is called a null set if it can be covered by a sequence of intervals of arbitrarily small length, i.e. $\forall\varepsilon>0$ there exists a sequence $(I_n)_{n\in\mathbb{N}}$ such that
        \begin{gather}
...
\subsection{Limit operations}
    ...
    \begin{property}
        Let $(f_i)_{i\in\mathbb{N}}$ be a sequence of Borel/Lebesgue measurable functions. The following functions are also measurable:
        \begin{itemize}

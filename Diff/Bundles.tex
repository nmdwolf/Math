...
\section{General bundles}
    ...
    \newdef{Cocycle}{\index{cocycle}\label{diff:G_cocycle}
...
\subsection{\texorpdfstring{Pseudogroups $\clubsuit$}{Pseudogroups}}
    ...
    \begin{example}[Diffeomorphism jets]
        Let $M$ be a smooth manifold. Consider the set $\mathcal{D}^{\text{an}}_{\text{local}}(M)$ of local analytic diffeomorphisms $\phi:M\rightarrow M:z\mapsto\phi(z)\equiv Z$. The locality property turns this set into a (smooth) pseudogroup \ref{topology:pseudogroup}.

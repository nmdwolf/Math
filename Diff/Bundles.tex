...
\section{General bundles}
    ...
    \begin{example}[Fibered manifold]\index{fibered manifold}\index{fibre}
...
\subsection{Constructions}
    ...
    \begin{property}[Vector bundles over a sphere]\index{vector!bundle}\label{diff:vector_bundles_over_sphere}
        The clutching theorem and the homotopy invariance imply that vector bundles over the sphere are determined by homotopy classes of functions $S^{n-1}\rightarrow\text{GL}_p(\mathbb{R})$, i.e. they are classified by the homotopy group $\pi_{n-1}(\text{GL}_p(\mathbb{R}))$.
    \end{property}
...
\chapter{Principal Bundles}\label{chapter:principal_bundles}
    ...
    The main reference for this chapter is \cite{principal_bundles}.

...
\section{Bundles}
    ...
    \newdef{Fibre bundle}{\index{fibre!bundle}\index{local!trivialization}\index{structure!group}
        ...
        The \textbf{transition maps} $\varphi_j\circ\varphi_i^{-1}:(U_i\cap U_j)\times F\rightarrow (U_i\cap U_j)\times F$ can be identified with a cocycle $g_{ji}:U_i\cap U_j\rightarrow G$ as follows. The transition maps restrict to the identity on $B$ and, hence, act as
        \begin{gather}
            \varphi_j\circ\varphi_i^{-1}(b,x) = (b,g_{ji}(b)\cdot x).
        \end{gather}
        The compatibility conditions satisfied by the functions $g_{ji}$, obtained by considering triple intersections, are exactly the cocycle conditions \eqref{diff:G_cocycle_condition}. Moreover, it can be shown that this action of $G$ on every fibre is faithful \ref{group:faithful_action}.
    }
    \begin{remark}
...
\section{Constructions}
    ...
    \begin{construct}[Fibre bundle construction theorem\footnotemark]\index{clutching}\label{diff:fibre_bundle_construction_theorem}
    ...
    \newdef{Fibre product}{\index{fibre!product}
        ...
        \begin{gather}
            ...
            F_1\diamond F_2 := \big\{(f,g)\in F_1\times F_2\,\big\vert\,\pi_1(f) = \pi_2(g)\big\}.
        \end{gather}
    }
...
\section{Sections}
    ...
    \newdef{Section}{\index{section}
        A (\textbf{global}) section of a fibre bundle $\pi:E\rightarrow B$ is a morphism $s:B\rightarrow E$ such that $\pi\circ s = \mathbbm{1}_B$. For any open subset $U\subset B$, a \textbf{local} section is defined as a morphism $s_U:U\rightarrow E$ such that $\pi\circ s_U=\mathbbm{1}_U$.
    }
    \begin{notation}

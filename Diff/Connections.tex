...
\subsection{Vertical vectors}
    ...
    Because smooth fibre bundles are also smooth manifolds, one can define the traditional notions such as the tangent bundle. Due to the composite nature of these geometric objects, one decompose the tangent bundle in horizontal and vertical (sub)bundles:
    ...
    \newdef{Fundamental vector field}{\index{vector field!fundamental}
        ...
        \begin{gather}
            \label{diff:fundamental_vector_field}
...
\subsection{Ehresmann connections}
    ...
    \newdef{Tensorial form}{\index{tensorial}\label{diff:tensorial_form}
        Consider a differential form $\theta$ on a principal $G$-bundle $P$ with values in a vector space $V$ equipped with a representation $\rho:G\rightarrow V$. This form is said to be \textbf{tensorial of type $(V, \rho)$} if it is horizontal and if it satisfies the equivariance condition
        \begin{gather}
...
\subsection{Parallel transport}
    ...
    \newdef{Parallel transport}{\index{parallel!transport}
        ...
        \end{gather}
        This map is $G$-equivariant and it is an isomorphism of fibres. The group element given by the path-ordered exponential in equation \ref{diff:g0_to_gt} is generally called the \textbf{holonomy} along the curve $\gamma$.
    }
...
\subsection{Koszul connections}
    ...
    \newdef{Horizontal lifts on associated bundles}{\index{horizontal!lift}
        ...
        \end{gather}
        Although the element $f$ seems to stay constant along the horizontal lift, it in fact changes according to formula \ref{diff:associated_bundle_equivalence}.
    }
    \newdef{Parallel transport}{\index{parallel!transport}
...
\subsection{Exterior covariant derivative}\label{section:exterior_covariant_derivative}
    ...
    \begin{formula}
        ...
        \begin{gather}
            ...
            \end{aligned}
        \end{gather}
        where, as usual, $\hat{v}_i$ means that this vector is omitted. As an example the formula for a one-form $\Phi$ is given:
        \begin{gather}
            D\Phi(X, Y) = \nabla_X(\Phi(Y)) - \nabla_Y(\Phi(X)) - \Phi([X, Y]).
        \end{gather}
        This formula should remind the reader of the analogous formula for the ordinary exterior derivative \ref{diff:k_form_exterior_derivative}.
    \end{formula}
...
\subsection{Curvature}
    ...
    \newdef{Flat connection}{\index{connection!flat}
        A principal connection is said to be flat if its curvature vanishes everywhere.
    }
...
\section{Reduction of the structure group}\label{section:G-structure}
    ...
    \newdef{Reduction}{\index{reduction}
        ...
        More generally, a principal bundle $\prin{H}{\widetilde{P}}{M}$ with structure group $H$ is called an $H$-reduction of $P$ if there exists a bundle isomorphism $\widetilde{P}\rightarrow P$. This allows for morphisms besides inclusions, such as covering maps $\lambda:H\rightarrow G$. (See for example the definition of spinor bundles in section \ref{section:spinor_bundles}.) As such the name ''reduction'' is not the best choice of terminology. For covering maps the term \textbf{lift(ing)} is sometimes used.
    }
    ...
    \begin{property}[Flat connections]

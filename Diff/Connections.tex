...
\subsection{Vertical vectors}
    ...
    \newdef{Fundamental vector field}{\index{vector field!fundamental}\label{diff:fundamental_vector_field}
...
\subsection{Ehresmann connections}
    ...
    \newdef{Ehresmann connection}{\index{connection!Ehresmann}\index{horizontal!vector}\label{diff:connection}
        ...
        \begin{enumerate}
            ...
            \item The choice of subspace depends smoothly on $e\in E$. (This is meant in the sense of distributions \ref{diff:distribution}.)
        \end{enumerate}
        The vectors in $\text{Hor}(T_eE)$ are said to be \textbf{horizontal} (with respect to the chosen connection).
    }
    \newdef{Horizontal bundle}{\index{horizontal}
        The horizontal (sub)bundle $\text{Hor}(TE)$ is defined as $\bigsqcup_{e\in E}\text{Hor}(T_eE)$ with the bundle structure induced from $TE$.
    }

    \newdef{Principal connection}{\index{connection!principal}\label{diff:principal_connection}
    ...
    \begin{remark}
        Note that this condition was automatically satisfied for vertical bundles as in equation \eqref{diff:vert_g_equivariance}.
    \end{remark}

    \begin{property}[Dimension]\label{diff:connection_dimensions}
        Properties \ref{diff:principal_bundle_dimension} and \ref{diff:vertical_dimension}, together with the direct sum decomposition of $TP$, imply the following relation for all $p\in P$:
        \begin{gather}
    ...
    \newdef{Dual connection}{\index{dual!connection}
        ...
        \begin{gather}
            \text{Hor}(T_p^*P) := \big\{h\in T_p^*P\mid h(v)=0, v\in\text{Vert}(T_pP)\big\}.
        \end{gather}
        It is the space of one-forms that vanish on the vertical subspace. A dual connection can then be defined as the selection of a vertical covector bundle $\text{Vert}(T_p^*P)$ satisfying the conditions of Definitions \ref{diff:connection} and \ref{diff:principal_connection} (where $\text{Vert}$ and $\text{Hor}$ should now be interchanged). Note that here the horizontal bundle is canonically defined.
    }

    \newdef{Horizontal and vertical forms}{\index{horizontal!form}\index{vertical!form}\label{diff:horizontal_form}
    ...
    \newdef{Tensorial form}{\index{tensorial}\index{basic}\label{diff:tensorial_form}
        Consider a differential form $\theta$ on a principal $G$-bundle $P$ with values in a vector space $V$ equipped with a representation $\rho:G\rightarrow V$. This form is said to be \textbf{tensorial} or \textbf{basic of type $(V, \rho)$} if it is horizontal and if it satisfies the equivariance condition
        \begin{gather}
...
\subsection{Connection forms}
    ...
    \newdef{Connection one-form}{\index{connection!form}
        ...
        \begin{enumerate}
            ...
            \end{gather}
        \end{enumerate}
        The horizontal subspaces are recovered as the kernel of the connection one-form: $\text{Hor}(T_pP) = \ker\omega|_p$.
    }
...
\subsection{Maurer-Cartan form}
    ...
    \begin{theorem}[Fundamental theorem of calculus]\index{fundamental theorem!of calculus}\index{Maurer-Cartan!equation}\label{diff:mc_equation}
...
\subsection{Local representations}
    ...
    \newdef{Yang-Mills field}{\index{Yang-Mills!field}\label{diff:yang_mills_field}
    ...
    \begin{formula}[Compatibility condition]\label{diff:compatibility_connection}
...
\subsection{Parallel transport}
    ...
    \newdef{Parallel transport}{\index{parallel!transport}
        ...
        \end{gather}
        This map is $G$-equivariant and it is an isomorphism of fibres. The group element given by the path-ordered exponential in equation \eqref{diff:g0_to_gt} is generally called the \textbf{holonomy} along the curve $\gamma$.
    }
...
\subsection{Holonomy group}
    ...
    \newdef{Holonomy group}{\index{holonomy}\label{diff:holonomy_group}
        ...
        \begin{gather}
            \text{Hol}_p(\omega) := \{g\in G\mid p \sim p\cdot g\}
        \end{gather}
    ...
    \newdef{Holonomy bundle}{\index{holonomy!bundle}\label{diff:holonomy_bundle}
        Let $M$ be a smooth path-connected manifold. Consider a principal bundle $P$ over $M$ with principal connection $\omega$. One can equip $P$ with an equivalence relation $\sim$ such that $p\sim q$ if and only if there exists a horizontal curve connecting $p$ and $q$. For every point $p\in P$ one can then construct the following set:
        \begin{gather}
            H(p) := \{q\in P\mid p\sim q\}.
        \end{gather}
        Path-connectedness of the base manifold implies that $H(p)$ and $H(q)$ are isomorphic for all $p,q\in P$. Using this fact one can show that $\sqcup_pH(p)$ is in fact a principal bundle itself. Its structure group is $\text{Hol}_p(\omega)$ for any $p\in P$.
    }
...
\subsection{Koszul connections}
    ...
    \newdef{Horizontal lifts on associated bundles}{\index{horizontal!lift}
        ...
        \end{gather}
        Although the element $f$ seems to stay constant along the horizontal lift, it in fact changes according to equation \eqref{diff:associated_bundle_equivalence}.
    }
    \newdef{Parallel transport}{\index{parallel!transport}
    ...
    \newdef{Covariant derivative}{\index{covariant!derivative}
        Consider a vector bundle $\bundle$ with model fibre space $V$ and its associated principal $\text{GL}(V)$-bundle with principal connection $\omega$. Let $\sigma:M\rightarrow E$ be a section of the vector bundle and let $X$ be a vector field on $M$. The covariant derivative of $\sigma$ with respect to $X$ is defined as
        \begin{gather}
        ...
        One can also rephrase the above definition in terms of the horizontal vector field associated to the lift $\widetilde{\gamma}$ (akin to Definition \ref{diff:lie_derivative_functions}). By Property \ref{diff:section_bijection} every section $\sigma$ of an associated bundle corresponds to a $G$-equivariant map $\phi(\sigma):P\rightarrow V$. In terms of this map one obtains
        \begin{gather}
    ...
    \begin{property}\index{Koszul!connection}
        ...
        \end{gather}
        defines a Koszul connection \ref{diff:koszul_connection}. It follows that every principal connection on a principal bundle induces a Koszul connection on all of its associated vector bundles.
    \end{property}
...
\subsection{Exterior covariant derivative}\label{section:exterior_covariant_derivative}
    ...
    \newdef{Exterior covariant derivative}{\index{exterior!covariant derivative}
        ...
        \end{gather}
        where $d$ is the exterior derivative \ref{diff:exterior_derivative} and $v_i^H$ is the projection of $v_i$ on the horizontal subspace $\text{Hor}(T_pP)$. From this definition it follows that the exterior covariant derivative $D\theta$ is a horizontal form \ref{diff:horizontal_form}.
    }
    \begin{remark}
    ...
    Because of property \ref{diff:section_bijection} one can use the following construction to find an explicit expression for the covariant derivative on an associated vector bundle:
    \begin{construct}\index{covariant!derivative}\label{diff:covariant_derivative_associated_bundle}
...
\subsection{Curvature}
    ...
    \newdef{Flat connection}{\index{connection!flat}\index{bundle!flat}
    ...
    \begin{formula}[Curvature on associated bundles]\label{diff:curvature_associated_bundles}
    ...
    \newformula{Cartan structure equation}{\index{Cartan!structure equation}\label{diff:cartan_structure_equation}
        ...
        \end{gather}
        The Maurer-Cartan equation in the (geometric) fundamental theorem of calculus \ref{diff:mc_equation} exactly states the vanishing of the algebraic curvature associated to a general $\mathfrak{g}$-valued one-form.
    }

    The following property is an immediate consequence of Frobenius's integrability theorem \ref{diff:frobenius} and the fact that a connection vanishes on the horizontal subbundle:
    \begin{property}\index{integrable}
        Let $\omega$ be a principal connection one-form. The associated horizontal distribution \ref{diff:distribution} \[p\mapsto\text{Hor}(T_pP)\] is integrable if and only if the connection $\omega$ is flat. In contrast, the vertical distribution is always integrable.
    \end{property}

    Similar to definition \ref{diff:yang_mills_field} one can also define the Yang-Mills field strength:
    ...
    \begin{theorem}[Ambrose-Singer]\index{Ambrose-Singer}\label{diff:ambrose_singer}
        The Lie algebra of the holonomy group $\text{Hol}_p(\omega)$ (Definition \ref{diff:holonomy_group}) is spanned by the elements of the form $\Omega_q(X,Y)$ where $q$ ranges over the holonomy bundle $H(p)$ (Definition \ref{diff:holonomy_bundle}) and $X,Y$ are horizontal.
    \end{theorem}
...
\subsection{Torsion}
    ...
    \newdef{Torsion}{\index{torsion}\index{Cartan!structure equation}
        ...
        \end{gather}
        where the wedge product is defined analogously to \ref{diff:vector_valued_wedge} and \ref{diff:lie_algebra_valued_wedge} using the induced representation of $\mathfrak{g}$ on $V$:
        \begin{gather}
...
\section{Reduction of the structure group}\label{section:G-structure}
    ...
    \newdef{$G$-structure}{\index{G-structure}
        Consider a manifold $M$. A $G$-structure on $M$ is the reduction of the structure group $\text{GL}(n)$ of the frame bundle $FM$ to the group $\iota:G\rightarrow\text{GL}(n)$.
    }
    \newdef{Integrability}{\index{integrable!$G$-structure}
        A $G$-structure $P\rightarrow M$ is said to be integrable if for every point $m\in M$ there exists a chart $U\ni m$ such that the associated holonomic frame $\{\partial_i\}_{i\leq\dim(M)}$ induces a local section of $P$.
    }
    \begin{property}\label{diff:integrable_torsion_free}
    ...
    \end{example}
    \begin{example}[Riemannian manifold]\label{diff:riemannian_G_structure}
    ...
    \begin{property}[Equivariant morphisms]\label{diff:reduction_classification}
    ...
    \newdef{Reducible connection}{\index{connection!reducible}\label{diff:reducible_connection}
    ...
    \begin{property}[Flat connections]\label{diff:flat_connection_cohomology}
...
\section{Characteristic classes}
    ...
    \newdef{Characteristic class}{\index{characteristic!class}
        Let $M$ be a smooth manifold. A characteristic class is a map from each isomorphism class of vector bundles or principal bundles $E\rightarrow M$ to a cohomology class $c(E)\in H^*(M;R)$ such that if there exists a morphism $f:N\rightarrow M$, then $c(f^*E) = f^*c(E)\in H^*(N;R)$. The coefficient ring $R$ is often assumed to be the base field ($\mathbb{R}$ or $\mathbb{C}$), but this is not always the case (e.g. Stiefel-Whitney classes).
    }
    ...
    Because the statement of the ''splitting principle'' is quite different when given in the language of principal bundles or that of vector bundles, it will be stated twice. First an additional construction is needed:
    \newdef{Flag bundle}{\index{flag}
        Let $\bundle$ be vector bundle. Using the definition of the flag manifold \ref{linalgebra:flag_manifold} one can construct for every fibre $E_p$ a space $Fl(E_p)$ that has the complete flags of $E_p$ as points (expressed as a sequence of one-dimensional subspaces). Using the bundle construction theorem, one can then obtain the flag bundle $\pi_{\mathrm{Fl}}:\mathrm{Fl}(E)\rightarrow M$ that has the flag manifolds as fibres.
    }
...
\subsection{Chern-Weil theory}\label{section:chern_weil}
    ...
    The characteristic classes of a vector bundle can be constructed from the connection and curvature forms on the vector bundle. In fact the expressions are polynomial in the curvature forms as is described in this section.
    ...
    \newdef{Chern-Weil morphism}{\index{Chern-Weil}
        ...
        \begin{gather}
            K[\mathfrak{g}]^G\rightarrow H^*_{dR}(E):P\mapsto P(\Omega),
        \end{gather}
        ...
        \begin{itemize}
            ...
            \item $\overline{P}(\Omega)$ does not depend on the chosen connection, i.e. for two connections one-forms $\omega,\omega'$ the difference $\overline{P}(\Omega) - \overline{P}(\Omega')$ is exact.
        \end{itemize}
    }

\subsection{Complex bundles}

    In this section only complex bundles are considered. This allows the choice of $\mathfrak{u}(n)$-valued connection one-forms. See Chapter \ref{chapter:complex_geometry} for more information.
    ...
    \newdef{Chern class}{\index{Chern!class}
        ...
        \begin{gather}
            \det\left(1 + \frac{it}{2\pi}\Omega\right) =: \sum_{k=1}^nc_k(E)t^k.
        \end{gather}
        The $i^{th}$ Chern class is a cohomology class in $H_{dR}^{2i}(M)$.
    }

    \newdef{Chern polynomial}{
        Let $c_k(E)$ denote the $k^{th}$ Chern class of $E$. The Chern polynomial is defined as follows:
        \begin{gather}
            c_t(E) := \sum_{i=1}^\infty c_i(E)t^i.
        \end{gather}
    ...
    \newdef{Theta characteristic}{\index{theta characteristic}
        Consider a smooth manifold $M$ together with its canonical class $K_M$. The theta characteristic, if it exists, is a characteristic class $\Theta$ such that $\Theta\cup\Theta=K_M$ where $\cup$ is the cup-product in cohomology \ref{diff:cup_product}.
    }
    ...
    \begin{example}[$\hat{A}$-genus\footnotemark]\label{diff:a_roof_genus}
...
\section{\difficult{Differential cohomology}}\index{differential!cohomology}

    In the foregoing sections a multitude of objects were introduced that are related to principal fibre bundles. For example, connections and their associated curvature forms could be used to construct differential quantities, while characteristic classes contained data about the topology of the bundle. However, even in the simple case of $\text{U}(1)$-bundles, neither the (first) Chern class, nor the curvature form are are able to
...
\subsection{Differential characters}
    ...
    \newdef{Differential character}{\index{Cheeger-Simons|see{differential character}}\index{differential!character}
        Consider a positive integer $k\geq1$ and let $M$ be a smooth manifold. A (\textbf{Cheeger-Simons}) differential character of \textbf{degree} $k$ is a group homomorphism $\chi:Z_{k-1}(M;\mathbb{Z})\rightarrow\text{U}(1)$ that is given by integration on boundaries:\footnote{Some authors omit the exponential function by working modulo $\mathbb{Z}$. This just replaces the multiplicative $\text{U}(1)$-group by the isomorphic additive $\mathbb{R}/\mathbb{Z}$-group.}
        \begin{gather}
            \chi(\partial\gamma) = \exp\left(2\pi i\int_\gamma\omega\right)
        \end{gather}
        for some $\omega\in\Omega^k(M)$. The group of differential characters of degree $k$ is denoted by $\hat{H}^k(M;\mathbb{Z})$. For $k=0$ the convention $\hat{H}^0(M;\mathbb{Z}):=H^0(M;\mathbb{Z})$ is used.
    }
    ...
    \begin{property}[Curvature]\index{curvature}\index{flat}
        Every differential character is represented by a unique, closed and integral $k$-form. The map $\text{curv}:\hat{H}^k(M;\mathbb{Z})\rightarrow\Omega^k_{\text{int}}(M)$ is called the curvature map. If $\text{curv}(\chi)=0$, the character $\chi$ is said to be \textbf{flat}.
    \end{property}

    \begin{property}[Characteristic class]\index{Dixmier-Douady class}
        Every differential character gives rise to a characteristic class as follows. The group of cocycles is free and the quotient map $\mathbb{R}\rightarrow\text{U}(1)$ is onto, so every differential character lifts to a group homomorphism $\widetilde{\chi}:Z_{k-1}(M;\mathbb{Z})\rightarrow\mathbb{R}$ such that $h(z)=\exp(2\pi i\widetilde{h}(z))$. The map
        \begin{gather}
            \text{ch}:C_k(M;\mathbb{Z})\rightarrow\mathbb{Z}:\gamma\mapsto\int_\gamma\text{curv}(\chi)-\widetilde{\chi}(\partial\gamma)
        \end{gather}
        induces a well-defined map $\text{ch}:\hat{H}^k(M;\mathbb{Z})\rightarrow H^k(M;\mathbb{Z})$. If $\text{ch}(\chi)=0$, the character $\chi$ is said to be \textbf{topologically trivial}. The characteristic class associated to a differential character is sometimes called the \textbf{Dixmier-Douady} class (see e.g. \cite{brylinski}).
    \end{property}

    \begin{example}[Circle bundles]
        Consider a $\text{U}(1)$-bundle $\pi:P\rightarrow M$ with connection $\omega$. Holonomy around closed curve $\gamma$ gives a parallel transport map
        \begin{gather}
            P\rightarrow P:p\mapsto p\cdot g(p,\gamma)
        \end{gather}
        for a smooth function $g:\Omega_p P\rightarrow\text{U}(1)$. In fact $g$ only depends on the homology of $\gamma$ and the projection $\pi(p)$, so one obtains a map $g\in\hat{H}^2(M;\mathbb{Z})$ with curvature $\text{curv}(g)=\frac{-1}{2\pi i}\Omega$ and characteristic class $\text{ch}(g)$ the first Chern class of $P$. The converse also holds, every different character of degree 2 determines a principal $\text{U}(1)$-bundle with connection (up to connection-preserving isomorphism). This leads to the following equivalence:
        \begin{align}
    ...
    \begin{property}[Curvature exact sequence]
        ...
        \begin{gather}
            0\longrightarrow H^{k-1}(M;\text{U}(1))\longrightarrow\hat{H}^k(M;\mathbb{Z})\overset{\text{curv}}{\longrightarrow}\Omega^k_{\text{int}}(M)\longrightarrow0
        \end{gather}
    ...
    \begin{property}[Characteristic class exact sequence]
        \begin{gather}
            0\longrightarrow \Omega^{k-1}(M)/\Omega^{k-1}_{\text{int}}(M)\longrightarrow\hat{H}^k(M;\mathbb{Z})\overset{\text{ch}}{\longrightarrow} H^k(M;\mathbb{Z})\longrightarrow0
        \end{gather}
        ...
        \end{gather}
        which has the closed integral forms as kernel. This exact sequence says that two connections on the same principal $\text{U}(1)$-bundle differ by a global connection form (up to an integral form).
    \end{property}
...
\subsection{Deligne cohomology}
    ...
    \newdef{Deligne complex}{
        Let $\mathbf{B}^k\text{U}(1)_{\text{conn}}$ denote the cochain complex of Abelian sheaves
        \begin{gather}
        ...
        \begin{gather}
            H^{k+1}_D(M;\mathbb{Z}) := \check{H}^0(M;\mathbf{B}^k\text{U}(1)_{\text{conn}}),
        \end{gather}
        where $\check{H}^\bullet$ denotes \v{C}ech cohomology \ref{sheaf:cech} and the cochain complex $\mathbf{B}^k\text{U}(1)_{\text{conn}}$ is turned into a cochain complex by inverting the degrees.
    }
    \begin{example}[Circle bundles]
    ...
    \end{example}
    \remark{As was the case for differential characters, higher Deligne cohomology classes classify higher $\text{U}(1)$-bundles with connection. The main benefit of this approach is that one gets an ``explicit'' description of the local data. See \cite{hitchin_gerbes} for a good introduction.}

    ?? COMPLETE ??

\section{Cartan connections}\label{section:cartan_geometry}

    In the first part of this section a short overview of Klein's \textit{Erlangen program} which unifies (and generalizes) Euclidean and non-Euclidean geometries will be given. In the second part of this section Cartan's generalization in terms of bundle theory is explained. A reference for this section is \cite{sharpe} (there a definition of Cartan geometries using charts and atlases, similar to the definition of smooth manifolds, is also given).

\subsection{Klein geometry}

    \newdef{Klein geometry}{\index{Klein!geometry}
        Consider a Lie group $G$ together with a closed subgroup $H$. If it is connected, the orbit space $G/H$ is called a Klein geometry, while $G$ is called the \textbf{principal group}. If the principal space is also connected, the Klein geometry is said to be \textbf{geometrically oriented}.
...
\subsection{Cartan geometry}
    ...
    \newdef{Curvature}{\index{curvature}
        By analogy with the Maurer-Cartan condition and the Cartan structure equation \ref{diff:cartan_structure_equation}, the curvature of a Cartan connection is defined as follows:
        \begin{gather}
            \Omega := d\omega + \frac{1}{2}[\omega\wedge\omega].
        \end{gather}
    }

...
\subsection{Weights, roots and Dynkin diagrams}
	...
	\newdef{Weight space}{\index{weight}\index{module!weight}
        	...
        	\end{gather}
        	Non-zero elements of a weight space are called \textbf{weight vectors}. If the representation $V$ can be decomposed as a direct sum of weigth spaces it is called a \textbf{weight module}:
        	\begin{gather}
	...
	We conclude that the root system $\Phi$ is not linearly independent. To introduce some kind of basis we define the following:
        ...
        \begin{property}
        	Every root $\phi\in\Phi$ can be written as $\phi = \sigma(\mu)$ for some $\mu\in\Delta$ and $\sigma\in W$. Furthermore, the root system $\Phi$ is closed under the action of $W$. The Weyl group $W$ is exactly the symmetry group of the root system $\Phi$ and the isometry group of the Killing form (and its dual).
        \end{property}
        ...
        \begin{notation}[Coroot]
        	Sometimes it is more favourable to denote the coroot associated to $\alpha$ by $H^\alpha$.
        \end{notation}
        ...
        \newdef{Fundamental weight}{\index{weight}

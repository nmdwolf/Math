...
\subsection{Left invariant vector fields}
	...
	\newdef{Left invariant vector field\footnotemark}{\index{vector field!left invariant}
		\footnotetext{Often abbreviated as \textbf{LIVF}.}
...
\subsection{Central extensions}\label{section:central_extension_algebra}
	...
	\newdef{Central extension}{\index{central!extension}
		A central extension of $\mathfrak{g}$ by an Abelian Lie algebra $\mathfrak{a}$ is an exact sequence of Lie algebras of the form
		\begin{gather}
	...
	\end{gather}\index{cocycle!Lie algebra}
...
\subsection{Coadjoint orbits}
	...
	\newdef{Coadjoint representation}{
		This is the dual representation of a Lie group $G$ on the dual space $\mathfrak{g}^*$:
		\begin{gather}
			\langle \text{Ad}_g^*(\omega), v \rangle = \langle \omega, \text{Ad}_g^{-1}(v) \rangle
		\end{gather}
		Infinitesimally this induces a representation of $\mathfrak{g}$ on its linear dual. It is given by $K_*(X):\omega\mapsto -\omega\circ\text{ad}_X$.
	}
	...
	\newdef{Kirillov-Kostant-Souriau form}{
		...
		\end{gather}
		Now consider a coadjoint orbit $\Omega_\alpha$. Since $\alpha$ is an element of the coadjoint representation of $G$, the tangent vectors to $\Omega_\alpha$ (at $\alpha$) are naturally elements of the induced representation of $\mathfrak{g}$. Hence for any tangent vector $v$ one can write $v\equiv K_*(X)\alpha$ for some $X\in\mathfrak{g}$. We then define a 2-form on $\Omega_\alpha$ by the following formula:
		\begin{gather}
...
\subsection{Weights, roots and Dynkin diagrams}
	...
	\newdef{Cartan subalgebra}{\index{Cartan!subalgebra}
		...
		\begin{enumerate}
			...
			\begin{gather}
				\exists n\in\mathbb{N}: \underbrace{[\mathfrak{h}, [\mathfrak{h}, [\mathfrak{h}, ...]]]}_{n \text{ times}} = 0
			\end{gather}
	...
	\newdef{Weight space}{\index{weight}\index{module!weight}\label{lie:weight_space}
        ...
        \newdef{Root}{\index{root}
        	...
        	\end{gather}
        	where the one-dimensional $\mathfrak{g}_\lambda$ are the weight spaces associated to the roots $\lambda$ ($\mathfrak{h}$ is equal to $\mathfrak{g}_0$ in this notation).
        }
...
\subsection{Group contractions}
	...
	\newdef{In\"on\"u-Wigner contraction}{\index{In\"on\"u-Wigner}
		Consider an $n$-dimensional Lie group $G$ with Lie algebra $\mathfrak{g}$. Choose a basis $\{e_i\}_{i\leq n}$ for $\mathfrak{g}$. A non-singular transformation of the basis would leave the structure of the group unchanged. However one can rewrite this non-singular transformation in terms of a singular transformation:
		\[U = u + \varepsilon w\]

...
\section{Tangent vectors}\label{diff:section:tangent_space}
	...
	\newdef{Tangent bundle}{\index{bundle}
...
\section{Submanifolds}
	\newdef{Immersion}{\index{immersion}
		Let $f:M\rightarrow N$ be a differentiable function between smooth manifolds. $f$ is called an immersion if its derivative is everywhere injective, or equivalently if it has maximal rank\footnotemark\ everywhere:
		\footnotetext{See definition \ref{manifolds:rank}.}
		\eq{
			\text{rk}_p(f) = \dim(M), \forall p\in M
		}
	}
	\newdef{Submersion}{\index{submersion}\index{regular point}
		Let $f:M\rightarrow N$ be a smooth map between smooth manifolds. A regular point\footnotemark\ of $f$ is a point $x\in M$ such that $T_xf$ is surjective. If every point $x\in M$ is a regular point of $f$ then $f$ is a submersion.

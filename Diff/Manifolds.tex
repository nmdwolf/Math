...
\section{Charts}
	...
	\begin{remark*}
		...
		For an alternative definition of manifolds in the context of \textit{diffeological spaces} see section \ref{section:diffeological_space}.
	\end{remark*}
	...
	\newdef{Structure sheaf}{\index{structure!sheaf}
		...
		From the sheafy point of view one can then define a smooth manifold as a ringed space that is locally isomorphic to $\mathbb{R}^n$ equipped with its standard space of differentiable functions.
	}
	...
	\begin{notation}
	    	...
	    	The set of all $C^\infty$-functions on a manifold $M$, defined on a neighbourhood of $m\in M$, is denoted by $C^\infty_m(M)$. This set forms a commutative ring when equipped with the usual sum and product (composition) of functions.
	\end{notation}	
	...
	\newdef{Finitely generated smooth algebra}{\index{smooth!locus}
		...
		If we denote the category of finitely generated smooth algebras by $\textbf{C}^\infty$\textbf{Alg}$^{\textbf{fin}}$ then we define the category of \textbf{smooth loci} as $(\textbf{C}^\infty$\textbf{Alg}$^{\textbf{fin}})^{op}$.
	}
...
\subsection{Submanifolds}
	...
	\begin{example}[Stiefel manifold]\index{Stiefel!manifold}
		Let $V$ be an inner product space\footnote{See section \ref{linalgebra:innerproduct}.} over a field $K$ (real, complex or quaternionic numbers). The set of orthonormal $k$-frames can be embedded in $K^{n\times k}$. It becomes a compact embedded submanifold, called the Stiefel manifold of $k$-frames over $V$.
	\end{example}
...
\section{Diffeological spaces}\label{section:diffeological_space}

	\newdef{Diffeological space}{\index{diffeological space}\index{plot}
	...
	\newdef{Differentiable map}{\index{differentiable}
		...
		The diffeological spaces together with their differentiable morphisms form a category \textbf{DiffSp}.
	}
	
	\begin{adefinition}\index{manifold}
		Let $M$ be a diffeological space. $M$ is an $n$-manifold if it is locally diffeomorphic to $\mathbb{R}^n$.
	\end{adefinition}

...
\section{Submanifolds}
	...
	\newdef{Submersion}{\index{submersion}\index{regular!value}\label{manifolds:submersion}
		Let $f:M\rightarrow N$ be a differentiable map between smooth manifolds. A \textbf{regular point}\footnotemark\ of $f$ is a point $x\in M$ such that $T_xf$ is surjective. A map $f$ is called a submersion if all $x\in M$ are regular, or equivalently if
	...
	\newdef{Embedded submanifold}{
		Let $M$ be a manifold. A subset $N$ is an embedded\footnotemark\ or \textbf{regular submanifold} submanifold if the inclusion map $f:M\hookrightarrow N$ is a smooth embedding.

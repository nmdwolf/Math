...
\subsection{Metric}
    ...
    \newdef{Codifferential}{\index{codifferential}
        Using the de Rham differential $d$ and the Hodge star operator \ref{tensor:hodge_star} (here extended to manifolds) we can define a boundary operator\footnote{This means that $\delta^2=0$ (see property \ref{topology:boundary_operator_relation}).} $\delta:\Omega^k(M)\rightarrow \Omega^{k-1}(M)$:
        \begin{gather}

...
\section{Curvature}\label{diff:section:curvature}
    ...
    \newformula{Ricci tensor}{\index{Ricci!tensor}
        \begin{gather}
            ...
            R_{\mu\nu} := R^\lambda_{\ \mu\nu\lambda}
        \end{gather}
        The Ricci tensor is also often denoted by $\textbf{Ric}$ (espeically in Riemannian geometry).
    }
...
\subsection{Spin structures}
    ...
    \newdef{Spinor field}{
        ...
        However, if $F_{SO}(M)$ is not trivial then this construction only works on a local chart. To be able to extend thid construction to the whole manifold we need to patch the diferent frame fields together. The required compatibility conditions reads as follows:
        \begin{gather}
    ...
    One can show that manifold admit the definition of a global spin frame field if and only if they admit a spin structure:
...
\section{\texorpdfstring{Sphere bundles $\clubsuit$}{Sphere bundles}}

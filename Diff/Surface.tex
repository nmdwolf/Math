...
\section{Curves}
	...
	\newprop{Regular curve}{Let $\vector{c}(t)$ be a curve defined on an interval $I$. $\vector{c}(t)$ is said to be regular if $\deriv{\vector{c}}{t} \neq \vector{0}$ for all $t\in I$.}
	\newdef{C$^r$-parameter transformation}{A transformation $\varphi:]c,d[\rightarrow]a,b[:u\mapsto t$ such that $\varphi(u)$ is a C$^r$-diffeomorphism\footnote{See definition \ref{linalgebra:diffeomorphism}}.}
    ...
    \begin{theorem}
		Let $\vector{c}(t), \vector{d}(t)$ be two curves with the same image. We then have the following relation:
        \begin{equation}
    ...
    \subsection{Arc length}
		...
		\end{formula}
        \sremark{
        	The arc length as defined above is often denoted by '$s$'.
        }
	...
	\subsection{Frenet-Serret frame}
    	\newdef{Tangent vector}{\index{tangent!vector}
        	\label{diff:tangent_vector}Let $\vector{c}(s)$ be parametrized by arc length. The tangent vector $\vector{t}(s)$ is defined as:
        	\begin{equation}
        ...
        \newdef{Principal normal vector}{\index{normal vector}
        	\label{diff:principal_normal_vector}
        ...
        \newdef{Binormal vector}{
        	\label{diff:binormal_vector}
        ...
        \newdef{Frenet-Serret frame}{\index{Frenet!Frenet-Serret frame}
        	\label{diff:frenet_serret_frame}
        ...
        \newformula{Frenet formulas}{\index{Frenet!formulas}
        	\label{diff:frenet_formulas}
        ...
        \begin{theorem}[Fundamental theorem of curves]\index{Fundamental theorem!of curves}
			\label{diff:theorem:fundamental_theorem_of_curves}
...
\section{Surfaces}
	...
	\subsection{First fundamental form}
    	...
    	\newdef{First fundamental form}{\index{metric}\index{fundamental form!first}
	...
	\subsection{Second fundamental form}\index{fundamental form!second}
    	\newdef{Second fundamental form}{
        ...
        \begin{theorem}[Meusnier's theorem]\index{Meusnier's theorem}
        	Let $\vector{c}, \vector{d}$ be two curves on a surface $\vector{\sigma}$. The curves have the same normal curvature in a point $\left(q^1(t_0), q^2(t_0)\right)$ if $\vector{c}(t_0) =  \vector{d}(t_0)$ and if $\dot{\vector{c}}(t_0)\ ||\ \dot{\vector{d}}(t_0)$. Furthermore, the osculating circles of all curves with the same normal curvature at a given point form a sphere.
        \end{theorem}
	...
	\subsection{Curvature of a surface}
        ...
        \begin{formula}[Matrix elements of $L_P$]
        ...
        \begin{theorem}
        	\label{diff:theorem:principal_directions}
        	For every point $P$ on the surface $\Sigma$ there exists a basis $\{\vector{h}_1, \vector{h}_2\}\subset T_P\Sigma$ of eigenvectors of $L_P$. Furthermore, the corresponding eigenfunctions are given by $II_P(\vector{h}_i, \vector{h}_i)$ and these eigenvalues are the extreme values of the normal curvature at the point $P$.
        \end{theorem}        
        ...
        \sremark{If the principal curvatures are not equal, the principal directions are orthogonal. If they are equal, the point $P$ is said to be an \textbf{umbilical point} or \textbf{umbilic}.}\index{umbilical point}
        
        \begin{property}
        	For the principal directions we have $L_P(\vector{h}_1)\cdot\vector{h}_2 = 0$\footnotemark. If $P$ is an umbilic then every tangent vector in $P$ is a principal direction and the equality is satisfied for every two tangent vectors.
        \end{property}
        \footnotetext{Tangent vectors that satisfy this equation are called \textbf{adjoint} tangent vectors.}
        ...
        \newdef{Mean curvature}{\index{curvature!mean}
        ...
        \begin{property}
        	The principal curvatures are the solutions of the following equation: \[x^2 - 2Hx + K = 0\] This is the characteristic equation (\ref{linalgebra:characteristic_equation}) of the Weingarten map.
        \end{property}
        ...
        \end{theorem}
        \begin{theorem}[Euler's formula]\index{Euler!formula for normal curvature}
        ...
        \newdef{Line of curvature}{\index{curvature!line of curvature}
        	A curve is a line of curvature if the tangent vector in every point $P$ is a principal direction of the surface in $P$.
        }
        \newformula{Rodrigues' formula}{\index{Rodrigues!formula for lines of curvature}
...
\subsection{Christoffel symbols and geodesics}
	...
	\newformula{Gauss' formulas}{\index{Gauss!formulas for surfaces}\index{Christoffel symbols}
    	\begin{equation}
    		\label{diff:gauss_formulas}
            \vector{\sigma}_{ij} = L_{ij}\vector{N} + \Gamma^k_{\ ij}\vector{\sigma}_k
    	\end{equation}
        where the \textbf{Christoffel symbols} $\Gamma^k_{\ ij}$ are defines as:
        \begin{equation}
        	\label{diff:christoffel_symbol}
            \boxed{\Gamma^k_{\ ij} = g^{kl}\vector{\sigma}_l\cdot\vector{\sigma}_{ij}}
        \end{equation}
    }
    \result{
    ...
    \newdef{Geodesic}{\index{geodesic}
    	A geodesic is a curve with zero geodesic curvature.
    }
    \begin{theorem}
    	A curve on a surface is an geodesic if and only if the tangent plane and the osculation plane are orthogonal in every point $P$ of the surface.
    \end{theorem}
...
\subsection{Theorema Egregium}
    ...
    \newformula{Gauss' equations}{\index{Gauss!equations for Riemann curvature}
    	\begin{equation}
    ...
    \begin{theorem}[Theorema Egregium]\index{Theorema Egregium}
    	The Gaussian curvature $K$ (formula \ref{diff:gaussian_curvature}) is completely determined by the metric tensor $g_{ij}$ and its derivatives:
        \begin{equation}
    ...
    \end{property}
    \result{There exists no isometric projection from the sphere to the plane. This also implies that a  perfect (read: isometric) map of the Earth can not be created.}
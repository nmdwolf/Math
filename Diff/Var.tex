...
\section{Jet bundles}\label{section:jet_bundles}
    ...
    \newdef{Jet}{\index{jet}\index{order}
        Consider two smooth manifolds $M,N$. Morphism $\sigma,\xi\in C^\infty(M, N)$ with local coordinates $(\sigma^i)$ and $(\xi^i)$ define the same $r$-jet at a point $p\in M$ if and only if
        \begin{gather}
    ...
    \newdef{Jet manifold}{
        ...
        \begin{gather}
            J^r(\pi) := \{j_p^r\sigma\mid\sigma\in\Gamma(E),p\in B\}.
        \end{gather}
    ...
    \newadef{Differential operator}{\index{differential!operator}\label{diff:differential_operator}
    ...
    \begin{property}[Formal adjoints]\index{adjoint}\label{var:formal_adjoint}
        ...
        \end{gather}
        This formula can be interpreted as a generalization of Green's identities. In the case where $M$ is compact, Stokes's theorem \ref{diff:stokes_theorem} shows that $D$ and $D^\dagger$ are related through integration by parts.
    \end{property}
...
\subsection{Contact structure}
    ...
    Every jet space $J^k$ carries a natural constact structure (Chapter \ref{chapter:contact}):
    ...
    Now, consider a (generalized) vector field $X$ on the total space $E$. There exists a vector field on $J^k(E)$, called the prolongation of $X$:
    \newdef{Prolongation of vector fields}{\index{prolongation}\index{total!derivative}\label{var:prolongation}
        ...
        \end{gather}
        where the ''total derivative'' operator\footnote{In fact this formula is virtually the same as the one for the true total derivative. However, partial derivatives are replaced by jet coordinates.}
        \begin{gather}
...
\subsection{Partial differential equations}\index{PDE}
    ...
    \newdef{Solution}{
        In this framework one can define a solution of the above PDE as a function $\phi$ satisfying $f\circ j^k\phi=0$. This can be rephrased in a geometric way. Every PDE of order $k$ defines a subbundle $\Sigma^0$ of the finite jet bundle $J^k(E)$ and a solution is then simply a section of $\Sigma^0$.
    }
    \begin{remark}
...
\subsection{\texorpdfstring{Pseudogroups $\clubsuit$}{Pseudogroups}}
    ...
    \begin{example}[Diffeomorphism jets]\index{Fa\'a di Bruno formula}
...
\section{\difficult{Variational bicomplex}}\label{section:variational_bicomplex}
    ...
    \newdef{Evolutionary vector field}{\index{evolutionary vector field}
        A generalized vector field on that projects to 0 on the base manifold. The space of evolutionary vector fields is denoted by $\text{Ev}(J^\infty(E))$.
    ...
    \begin{property}[Prolongation of evolutionary vector fields]
        ...
        \end{gather}
        Furthermore, by writing out Cartan's magic formula \ref{diff:cartan_magic_formula} (with respect to $\mathbf{d}$), we can prove that the prolongation of an evolutionary vector field also satisfies this formula with respect to $\delta$ and that $\iota_{j^\infty X}$ and $d$ anticommute.
    \end{property}
...
\subsection{Inverse problem}
    ...
    The inverse problem in the calculus of variations asks when a given system of PDEs can be obtained from a variational problem, i.e. when a source form $\Delta$ can be written in the form $\delta_{\text{EL}}\mathbf{L}$.
...
\subsection{Structure of the bicomplex}
    ...
    An important property of the de Rham differential is its naturality (see property \ref{diff:exterior_derivative_properties}). The following property states the ''naturality'' of the different operators on the variational bicomplex with respect to the projected pullback:
    \begin{property}
    ...
    \newdef{Bessel-Hagen symmetry}{\index{Bessel-Hagen|see{symmetry}}\index{divergence!symmetry}
        A Bessel-Hagen or \textbf{divergence} symmetry of a Euler-Lagrange form $\Delta\equiv\delta_{\text{EL}}(\mathbf{L})$ is a generalized vector field $X$ such that
        \begin{gather}

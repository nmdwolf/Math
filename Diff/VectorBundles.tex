...
\section{Vector bundles}
    ...
    \begin{property}[Zero section]\label{diff:zero_section}
        For every smooth vector bundle $\pi:E\rightarrow M$ one can embed the base manifold $M$ in the bundle $E$ through the zero section $\mathbf{0}_E:M\rightarrow E$. The complement of the image of this section is often denoted by $E_0$.
    \end{property}
...
\subsection{Vector-valued differential forms}\label{section:vector_valued_forms}
    ...
    \begin{formula}
        ...
        \end{gather}
        where $\wedge$ is the wedge product on $\Omega^\bullet(M)$ and $[\cdot, \cdot]$ is the Lie bracket on $\mathfrak{g}$.
    \end{formula}
...
\subsection{Integration}\index{cochain}\index{de Rham!cohomology}
...
\subsection{Cohomology with compact support}
    ...
    \begin{property}[Relation to reduced cohomology]\label{diff:reduced_compact_cohomology}
        ...
        If we choose $X=\widehat{M}$ to be the one-point compactification of $M$ and $U=M$, then the aforementioned long exact sequence implies that
        \begin{gather}
            H^\bullet_c(M)\cong H^\bullet(\widehat{M}, \ast)\cong\widetilde{H}^\bullet(\widehat{M})
        \end{gather}
        where we used that both $\ast,\widehat{M}$ are compact.\footnote{This is true for Hausdorff spaces.}
    \end{property}
    ...
    ?? COMPLETE (ADD NON-ORIENTABLE EXTENSIONS) ??

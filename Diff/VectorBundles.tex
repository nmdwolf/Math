...
\subsection{Associated vector bundles}
    ...
    \end{example}
    \begin{example}[Cotangent bundle]\index{covariant}\index{co-!vector}\label{manifolds:cotangent_bundle}
...
\subsection{Frobenius' theorem}
    ...
    \newdef{Distribution}{\index{distribution!of $k$-planes}\label{manifolds:distribution}
        A smooth section of the Grassmann $k$-plane bundle \ref{manifolds:grassmann_bundle} is called a distribution of $k$-planes.
    }
...
\subsection{Interior product}
    ...
    \begin{notation}
        In certain situations the above notation might become cumbersome. For this reason we also introduce the notation $X\intmul\omega$.
    \end{notation}

    \newformula{Cartan's magic formula\footnotemark}{\index{Cartan!magic formula}\label{forms:cartan_magic_formula}
...
\section{Differential operators}
    ...
    \newdef{Differential operator}{\index{differential!operator}\index{elliptic!operator}
        ...
        The principal symbol of this operator is then defined as the principal symbol \ref{pde:principal_symbol} of the associated PDE. By extension we say the differential operator is \textbf{elliptic} if its associated PDE is elliptic.
    }

    \newdef{Normally hyperbolic operators}{\index{hyperbolic}
        Consider a vector bundle $E$ equipped with a (pseudo)Riemannian metric (see chapter \ref{chapter:riemann} further on). A linear differential operator on $E$ is said to be normally hyperbolic if its principal symbol is proportional to the given metric.
    }

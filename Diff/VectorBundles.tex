...
\subsection{Vector bundles}
	...
	\end{example}
	\begin{example}[Cotangent bundle]\index{covariant}
	...
	\begin{example}[Pseudovectors]\index{pseudovector}
		If we consider the representation \eq{\rho:A\mapsto \sgn\det(A)A} we can construct a bundle similar to the tangent bundle. The sign of the cocycle functions $t_{ji}$ now has an influence on the fibres. Elements of these fibres are called \textbf{pseudovectors}.
	\end{example}
...
\subsection{Metric}	
	\begin{definition}[Metric]\index{metric}
	...
	\newdef{Musical isomorphisms}{\index{musical isomorphism}
		...
		such that $p(\cdot) = g(v, \cdot)$. These \textit{musical} isomorphisms can be used to lower and raise tensor indices.
	}
	
	\begin{theorem}[Whitney's theorem]\index{Whitney's theorem}
		Every paracompact\footnotemark\ smooth manifold admits a Riemannian metric.
	\end{theorem}
...
\subsection{Lie derivative}
	...
	\begin{property}\index{Lie!bracket}
		...
		\begin{itemize}
			...
			\item The Lie derivative is antisymmetric:\eq{\mathcal{L}_XY = -\mathcal{L}_YX} This follows from the previous property.
		\end{itemize}
	\end{property}
...
\subsection{Exterior derivative}
	...
	\begin{property}\index{Leibniz!rule}
		...
		\begin{itemize}
			...
			\item Graded Leibniz rule: \eq{d(\omega_1\wedge\omega_2) = d\omega_1\wedge\omega_2 + (-1)^j\omega_1\wedge d\omega_2} where $\omega_1\in\Omega^j(M), \omega_2\in\Omega^k(M)$.
...
\subsection{de Rham Cohomology}
	...
	\begin{remark}
		\label{forms:remark:closed_exact}
		From the first item of property \ref{forms:exterior_derivative_properties} it follows that every exact form is closed. The converse however is not true\footnotemark.
	\end{remark}
	\footnotetext{See result \ref{forms:theorem:poincare} for more information.}
	...
	\newdef{de Rham cohomology}{\index{de Rham!cohomology}\index{Poincar\'e}
	...
	\begin{theorem}[Poincar\'e's lemma\footnotemark]\label{forms:theorem:poincare}

...
\subsection{Exterior derivative}
	...
	\begin{example}
		...
		\begin{align}
			d\Phi(X_1, ...n X_{k+1}) = \sum_{i=0}^{k+1} &(-1)^{i+1} X_i(\Phi(X_1, ..., \hat{X}_i, ..., X_{k+1}))\nonumber\\
...
\subsection{de Rham Cohomology}
	...
	\begin{theorem}[Poincar\'e's lemma\footnotemark]\index{Poincar\'e!lemma}\label{forms:theorem:poincare}
...
\section{Linear connections}
	...
	\begin{formula}[Connection on differential forms]
		Using the Leibniz property of a Koszul connection on the contraction $\omega(X_1, ..., X_k)$, where $X_i\in\mathfrak{X}(M)$ and $\omega\in\Omega^k(M)$, gives us the following form of the connection on the $(s, k)$-tensor bundle ($s=0,1$):
		\begin{equation}

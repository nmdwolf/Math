...
\subsection{Associated vector bundles}
	...
	\begin{example}[Pseudovectors]\index{pseudo!vector}
		...
		\end{gather}
		we can construct a bundle similarly to the tangent bundle. The sign of the cocycle functions $t_{ji}$ now has an influence on the fibres. Elements of these fibres are called \textbf{pseudovectors}.
	\end{example}
...
\subsection{de Rham complex}
	...
	\newdef{Integral form}{\label{diff:integral_form}\index{integral!form}
		A differential $k$-form $\omega$ is said to be integral if it lies in the image of the inclusion $H^k(M, \mathbb{Z})\hookrightarrow H^k(M, \mathbb{R})$. Equivalently a differential $k$-form is integral if integrating it over any $k$-cycle with integral coefficients gives an integer.
	}
	...
	\begin{theorem}[Poincar\'e lemma\footnotemark]\index{Poincar\'e!lemma}\label{forms:theorem:poincare}
		...
		\end{gather}
		This implies that every closed form is locally exact, i.e. if $d\omega=0$ at the point $p\in M$ then there exist a neighbourhood $x\in U\subseteq M$ and differential form $\lambda$ such that
		\begin{gather}
			\omega = d\lambda
		\end{gather}

\chapter{Algebra}\label{chapter:algebra}
    ...
    For the later sections on (co)homology theory, some terminology from \labelref{chapter:hom_alg} is used. Basic introductions to group theory can be found in~\citet{jeevanjee_introduction_2015,wigner_group_2012}.
...
\section{Algebraic structures}
    ...
    \newdef{Semigroup}{\index{semi-!group}\index{magma}\label{algebra:semigroup}
        ...
        \end{enumerate}
        If the associativity axiom is dropped, the notion of a \textbf{magma} is obtained.
    }
...
\section{Group theory}\label{section:groups}
    ...
    \begin{theorem}[Lagrange]\index{Lagrange}
        Let $G$ be a finite group. If $H$ is a subgroup, then $|H|$ is a divisor of $|G|$.
    \end{theorem}
    \begin{result}
    ...
    \end{construct}
    \begin{uproperty}
        Consider an Abelian monoid $A$ with its Grothendieck completion $G(A)$. Every monoid morphism $m:A\rightarrow B$ between $A$ and an Abelian group factors uniquely through a group morphism $\varphi:G(A)\rightarrow B$.
    \end{uproperty}
...
\subsection{Symmetric groups}
    ...
    \newdef{Automorphism group}{\index{automorphism}\label{algebra:automorphism}
        Consider a set $X$ with some extra structure (see \labelref{chapter:cat} for a more formal treatment), e.g.~a monoid structure or group structure. The subset of $\Sym(X)$ consisting of bijections that preserve this structure, the \textbf{automorphisms} of $X$, inherits the group structure of $\Sym(X)$ and is denoted by $\Aut(X)$.
    }
...
\subsection{Direct products}
    ...
    \begin{notation}
        The direct sum is often denoted by $\oplus$ (in accordance with the notation for \textit{vector spaces} ((see \cref{section:vector_spaces})) and other algebraic structures that will be introduced further on).
    \end{notation}

    \newdef{Inner semidirect product}{\index{semi-!direct product}\index{split!product}\label{group:inner_semidirect_product}
...
\subsection{Group actions}\label{section:group_actions}
    ...
    \begin{example}[Adjoint action]\index{adjoint!action}\label{algebra:adjoint_action}
        Consider a group $G$. This group acts on itself by conjugation:
        \begin{gather}
            \Ad:G\times G\rightarrow G:(g,h)\mapsto g^{-1}hg\,.
        \end{gather}
    \end{example}
    ...
    \newdef{Equivariant map}{\index{equivariant!map}\index{morphism!of $G$-sets}\label{group:equivariant}
        ...
        \end{gather}
        where, by abuse of notation, the symbol $\cdot$ represents the group actions on both $X$ and $Y$. An equivariant map is sometimes called a \textbf{$G$-map}.\footnote{$G$-spaces together with the $G$-maps constitute a category $G\text{-}\symbfsf{Set}$.}
    }
    ...
    \newdef{Fixed-point space}{\index{fixed point}
        Consider a $G$-space $X$. Its fixed-point set or invariant subset $X^G$ is defined as follows:
        \begin{gather}
...
\subsection{Group extensions}\label{section:group_extensions}

    \newdef{Group extension}{\index{extension!group}
        Consider a group $G$ together with an Abelian group $A$. A group extension of $G$ by $A$ is a group $E$ such that:
        \begin{enumerate}
            \item $E/A\cong G$, and
            \item $A$ is (isomorphic to) a normal subgroup of $E$.
        \end{enumerate}
    }

    Group extensions can also be defined using the notion of \textit{(short) exact sequences} (see \cref{homalg:short_exact_sequence}).
    \newadef{Group extension}{
        A short exact sequence of groups
        \begin{gather}
            0\longrightarrow A\overset{\iota}{\longrightarrow}E\overset{\pi}{\longrightarrow}G\longrightarrow0
        \end{gather}
        where $A$ is Abelian. This definition automatically implies that $\iota$ is a mono and $\pi$ is an epi.
    }
    \begin{remark}
        This definition in terms of group extensions easily allows to define non-Abelian group extensions by dropping the condition on $A$. The remainder of this section uses this general approach following the work by \indexauthor{Schreier}.
    \end{remark}

    Now, how to construct such a group extension? First, note that, given the extension $E$ and a (set-theoretic) section $\sigma:G\rightarrow E$ of the projection $\pi:E\twoheadrightarrow G$, i.e.~$\pi\circ \sigma=\mathbbm{1}_G$, one can obtain an action of $G$ on $A$ as follows:
    \begin{gather}
        g\cdot a := \iota^{-1}\bigl(\sigma(g)\iota(a)\sigma(g)^{-1}\bigr)\,,
    \end{gather}
    where $\iota:A\hookrightarrow E$ is the obvious inclusion. Note that this essentially comes down to the adjoint action of $G$ on $A$, both embedded as subgroups of $E$.

    The structure of $E$ can actually be recovered from the action $\varphi:G\rightarrow\Aut(A)$ defined above and the section $\sigma$. Since $\pi$ is a group morphism, $\pi(\sigma(g)\sigma(h))=gh=\pi(\sigma(gh))$ for all $g,h\in G$. It follows that
    \begin{gather}
        \sigma(gh)^{-1}\sigma(g)\sigma(h)\in A\,.
    \end{gather}    
    This gives a well-defined function $\omega:G\times G\rightarrow A$, called the \textit{2-cocycle} associated to the extension $E$. The reason for this terminology (and its relation to \cref{homalg:short_exact_sequence}), is the following. Writing out the associativity condition on the multiplication in $E$ gives:
    \begin{gather}
        \omega(g,hk)\omega(h,k) = \omega(gh,k)k^{-1}\cdot\omega(g,h)\,.
    \end{gather}
    This is exactly the 2-cocycle condition in group cohomology.

    Now, given two groups $A,G$, together with an action $\varphi:G\rightarrow\Aut(A)$ and a 2-cocycle $\omega\in H^2_\varphi(G;A)$, can one always construct a group extension such that this data corresponds to the data found above? The answer is yes!
    \begin{construct}[Group extension]\label{group:group_extension_construction}
        Construct the Cartesian product $E:=G\times A$. On this product, one defines the following multiplication:
        \begin{gather}
            (g,a)(h,b) := \bigl(gh,a\varphi(g)b\omega(g,h)\bigr)\,.
        \end{gather}
    \end{construct}

    \begin{theorem}[Schur cover]\index{Schur!cover}
        \todo{INSERT}
    \end{theorem}

    \newdef{Split extension}{\index{extension!split}
        A group extension is split if the projection $\pi:E\rightarrow G$ admits a section $\sigma:G\rightarrow E$ in $\symbfsf{Grp}$.
    }
    \newdef{Central extension}{\index{extension!central}
        A group extension is central if $\im(\iota)\subseteq Z(E)$.
    }

    \begin{property}
        Whereas general group extensions are classified by both the automorphism action $\varphi:G\rightarrow\Aut(A)$ and the 2-cocycle $\omega:G\times G\rightarrow A$, the situation for split and central extensions is much easier. An extension is split if and only if $\omega$ is trivial or, equivalently (by \cref{group:group_extension_construction} above), when $E$ is the semidirect product $G\times_\varphi A$. Similarly, an extension is central if and only if $\varphi$ is trivial.
    \end{property}
...
\subsection{General}
    ...
    \newdef{Field}{\index{field}\label{algebra:field}
    ...
    \begin{property}[Freshman's dream]\index{freshman's dream}\label{algebra:freshmans_dream}
        Let $R$ be a ring of characteristic $p>0$. For all $x,y\in R$, the following statement holds:\footnote{See \cref{calculus:binomial_theorem} for the statement in characteristic 0.}
        \begin{gather}
            (x+y)^p = x^p + y^p\,.
        \end{gather}
    \end{property}
    \newdef{Frobenius endomorphism}{\index{Frobenius!endomorphism}
        Let $R$ be a ring of characteristic $p>0$. The Frobenius endomorphism sends an element $x\in R$ to its $p^{\text{th}}$ power:
        \begin{gather}
            F_p:R\rightarrow R:x\mapsto x^p\,.
        \end{gather}
        By the preceding property, this is a ring morphism.
    }
...
\subsection{Domains}
    ...
    \newdef{Rational fractions}{\index{rational!fractions}
        Consider an integral domain $I$. The commutative ring of rational fractions $I(x)$ over $I$ is given by the field of fractions of the polynomial ring $I[x]$.
    }

    \newdef{Reduced ring}{\index{reduced!ring}\label{algebra:reduced_ring}
    ...
    \newdef{Irreducible element}{\index{irreducible}\label{algebra:irreducible_element}
        Let $I$ be an integral domain. An element $r\in I$ is said to be irreducible if:
        \begin{enumerate}
            \item\textbf{Invertibility}: $r$ is not a unit, i.e.~it is not invertible.
            \item\textbf{Decomposition}: $r$ cannot be decomposed as a product of other noninvertible elements.
        \end{enumerate}
    }
    \newdef{Unique factorization domain}{\index{domain!unique factorization}\label{algebra:ufd}
        An integral domain in which every element admits a unique factorization into irreducible elements. These are often abbreviated as \textbf{UFD}s.
    }

\subsection{Ideals}\label{section:ideals}
    ...
    \newdef{Principal ideal domain}{\index{domain!principal ideal}\label{algebra:pid}
    ...
    \newdef{Fractional ideal}{\index{ideal!fractional}\label{algebra:fractional_ideal}
        Let $R$ be an integral domain. A fractional ideal $I$ of $R$ is a submodule of $\mathrm{Frac}(R)$ such that $rI\subseteq R$ for some $r\in R$. In the case of $R=\mathbb{Z}$, $r$ would correspond to the common denominator of the fractions in $I$.
    }
...
\subsection{Valuations}
    ...
    \newdef{Absolute value}{\index{absolute value}\index{valuation}\index{ultrametric}\index{Archimedean}\label{algebra:absolute_value}
        Let $\mathfrak{K}$ be a ring (usually a field). An absolute value (sometimes called an \textbf{exponential} or \textbf{multiplicative valuation}) is a multiplicative \textit{seminorm} (see \cref{functional:seminorm}), i.e.~a function $|\cdot|:\mathfrak{K}\rightarrow\mathbb{R}$ satisfying the following conditions for all $x,y\in\mathfrak{K}$:
        \begin{enumerate}
    ...
    \newdef{Place}{\index{place}
        Two absolute values $|\cdot|_1:\mathfrak{K}\rightarrow\mathbb{R}$, $|\cdot|_2:\mathfrak{K}\rightarrow\mathbb{R}$ are equivalent if
        \begin{gather}
            |x|_1<1\iff|x|_2<1
        \end{gather}
        for all $x\in\mathfrak{K}$. An equivalence class of absolute values is called a place.
    }
    ...
    \newdef{Valuation}{\index{valuation}\label{algebra:valuation}
        Let $\mathfrak{K}$ be a field and let $\Gamma$ be a totally ordered Abelian group (\cref{set:total_order}). The group law and the order relation on $\Gamma$ can be extended to the union $\Gamma\cup\{\infty\}$ in the following way (the notation $\infty$ is only a convention):
        \begin{itemize}
            \item $g+\infty:=\infty+g:=\infty$ for all $g\in\Gamma$, and
            \item $g\leq\infty$ for all $g\in\Gamma$.
        \end{itemize}
        A valuation on $\mathfrak{K}$ (with values in $\Gamma$) is a map $\nu:\mathfrak{K}\rightarrow\Gamma\cup\{\infty\}$ such that:
        \begin{enumerate}
            \item $\nu(a) = \infty\iff a = 0$,
            \item $\nu(ab) = \nu(a) + \nu(b)$ for all $a,b\in\mathfrak{K}^\times$, and
            \item $\min\bigl(\nu(a),\nu(b)\bigr)\leq\nu(a+b)$, where the equality holds if $\nu(a)\neq\nu(b)$.
        \end{enumerate}
    }
    \begin{example}[Absolute values]\label{algebra:nonarchimedean_valuation}
        Let $|\cdot|:\mathfrak{K}\rightarrow\mathbb{R}$ be a non-Archimedean absolute value and consider a number $b\in\,]\,1,+\infty[$. The function
        \begin{gather}
            \nu_b(x) :=
            \begin{cases}
                -\log_b|x|&\cif x\neq0\\
            ...
            \end{cases}
        \end{gather}
        is a valuation on $\mathfrak{K}$.
    \end{example}

\section{Algebraic geometry}

    This section lays the foundations for \labelref{chapter:alggeom}.
    ...
    \begin{property}[Characterization by invertible complements]\label{algebra:local_ring_invertible}
        A ring $R$ is local if and only if there exists a maximal ideal $\mathfrak{m}\subset R$ such that every element in the complement $R\backslash\mathfrak{m}$ is invertible.
    \end{property}
    ...
    \newdef{Residue field}{\index{residue!field}
        Consider a local ring $R$ and let $\mathfrak{m}$ be its maximal ideal. The quotient ring $R/\mathfrak{m}$ forms a field, called the residue field.
    }

    The following definition is similar to the one for groups (cf.~\cref{group:presentation}).
     \newdef{Finite presentation}{\index{presentation}\index{finitely!generated}\label{algebra:presentation}
        Consider a ring morphism $f:R\rightarrow S$. This morphism is said to be \textbf{finitely generated} or \textbf{of finite type} ($S$ is also called an $R$-algebra of finite type) if $S$ is isomorphic to a quotient of $R[x_1,\ldots,x_n]$ for some $n\in\mathbb{N}$. The morphism is said to be \textbf{finitely presented} if there exist integers $m,n\in\mathbb{N}$ such that $S\cong R[x_1,\ldots,x_n]/(f_1,\ldots,f_m)$ for some polynomials $\{f_1,\ldots,f_m\}$ over $R$.
    }
    ...
    \newdef{Completion\footnotemark}{\index{completion!of rings}\label{algebra:ring_completion}\index{formal!completion}\index{adic}
        ...
        Let $R$ be a commutative ring together with a proper ideal $I$. The completion $\widehat{R}$ of $R$ with respect to $I$ is the inverse limit $\varprojlim_{n\in\mathbb{N}}(R/I^n)$.
    }
    \begin{property}[Adic topology]\index{topology!adic}\index{Krull!topology}
        When the filtration ends in the trivial ideal, for example when $R$ is a Noetherian local ring, the completion $\widehat{R}$ is a \textit{topological ring} (see \cref{chapter:topology}). The \textbf{Krull topology} or \textbf{$I$-adic topology} is the topology for which a basis of $0$ is given by the powers of $I$.
    \end{property}
...
\subsection{Modules}
    ...
    \begin{remark}[\difficult{Categorical perspective}]
        The definition of a ring can be defined more concisely in categorical terms, using the definition of an \textit{algebra over a monad} (see \cref{cat:algebra_monad}). Modules over a monoid object $A$ are defined as algebras over the monad $A\otimes-$. A ring $R$ is a monoid object in the category $\symbfsf{Ab}$ of Abelian groups. So a module $M$ over $R$ consists of a morphism $\alpha:R\otimes M\rightarrow M$ satisfying the algebra axioms. The distributivity laws come for free since $\alpha$ is a morphism in $\symbfsf{Ab}$ and, hence, is bilinear (in both arguments).
    \end{remark}

    These formulations can be restated similar to that of group modules (\cref{group:module}).
    \newadef{$R$-module}{
        Consider the Abelian group $(M,\oplus)$. Its endomorphism set $\End(M,\oplus)$ can be given the structure of a ring where the addition is induced by that on $M$ and the multiplication is given by composition. A left $R$-module structure is then simply a ring morphism $R\rightarrow\End(M,\oplus)$.
    }
    ...
    \begin{theorem}[Nakayama's lemma\footnotemark]\index{Nakayama}\index{Krull--Azumaya}\label{algebra:nakayama}
    ...
    \end{result}
    \begin{result}
        Let $M$ be a finitely generated $R$-module. If the elements $m_1,\ldots,m_k\in M$ whose image in $M/J(R)M$ generate this quotient, they generate $M$.
    \end{result}
...
\subsection{Polynomials}
    ...
    In the context of polynomials, \cref{algebra:irreducible_element} becomes the following definition.
    ...
    \newdef{Split}{\index{split}\index{root}\index{multiplicity}\index{simple!root}
        ...
        \end{gather}
        where $n=\deg(f)$. The numbers $r_i\in R$ are called the \textbf{roots} of the polynomial. If a root occurs only once, i.e.~its \textbf{multiplicity} is one, it is said to be \textbf{simple}.
    }
    ...
    \begin{formula}[Vieta]\index{Vieta}
        Consider a polynomial of degree $n\in\mathbb{N}$ over $\mathbb{R}$. By the fundamental theorem of algebra, this polynomial has $n$ complex roots. Vieta's formulas relate the coefficients of the polynomial to its roots $r_i$:
        \begin{gather}
    ...
    \begin{property}[Polynomial remainder theorem\footnotemark]\index{B\'ezout}
        \footnotetext{Also called \textbf{little B\'ezout's theorem}.}
        Consider a polynomial $f\in R[x]$ over a ring $R$ (possibly noncommutative). For every element $r\in R$, one can decompose $f$ as
        \begin{gather}
            f(x) = (x-r)g(x) + f(r)\,,
        \end{gather}
        for some polynomial $g\in R[x]$. In particular, $x-r$ is a factor of $f$ if and only if $f(r)=0$.
    \end{property}

    \begin{method}[Horner--Ruffini]\index{Horner--Ruffini}
        The Horner(--Ruffini) method for polynomial evaluation and division is based on the above property (which essentialy boils down to long division). Consider a polynomial $f(x)=a_nx^n+\cdots+a_1x+a_0$. The quotient and remainder with respect to a monomial $x-r$ can be obtained as follows:
        \begin{enumerate}[Step 1)]
            ...
            \item \begin{gather}
                    \begin{array}{c|cccc}
                        ...
                        &\mid&rb_n&&\\
                        \hline
                        r&\textcolor{red}{b_n}&a_{n+1}+rb_n&&
                    \end{array}
                \end{gather}
            \item[Step $n-1$)] \begin{gather}
                    \begin{array}{c|cccc}
                        &a_n&a_1&\cdots&a_0\\
                        &\mid&rb_n&\cdots&rb_1\\
                        \hline
                        r&b_n&b_{n-1}&\cdots&a_0+rb_1
                    \end{array}
                \end{gather}
            \item[Step $n$)] \begin{gather}
                    \begin{array}{c|cccc}
                        &a_n&a_1&\cdots&a_0\\
                        &\mid&rb_n&\cdots&rb_1\\
                        \hline
                        r&b_n&b_{n-1}&\cdots&\textcolor{red}{b_0}
                    \end{array}
                \end{gather}
        \end{enumerate}
        The quotient is given by the polynomial $b_nx^{n-1}+\cdots+b_2x+b_1$ and the remainder is given by $b_0$. By the foregoing result, the evaluation at $x=r$ is exactly this remainder: $f(r)=b_0$.
    \end{method}
...
\subsection{Field extensions}
    ...
    The following property is very similar to the tower rule~\ref{group:tower_rule_groups} for finite groups.
...
\subsection{Algebraicity}\label{section:algebraic_numbers}
    ...
    \newdef{Algebraic dependence}{\index{integral}\label{algebra:integral}
        Consider a commutative ring $R$ and a subring $S\subset R$. An element $r\in R$ is said to be algebraically dependent on $S$ if it is the root of a polynomial in $S[x]$. If $r$ is the root of a monic polynomial in $S[x]$, it is said to be \textbf{integrally dependent on} or \textbf{integral over} $S$.
    }
    \begin{remark}
    ...
    \newdef{Transcendence degree}{\index{degree!transcendence}\label{algebra:transcendence_degree}
        The transcendence degree of a field extension $L/K$ is given by the cardinality of a maximal set of algebraically independent elements of $L$ over $K$.
    }

    \newdef{Integral closure}{\index{integral!closure}\index{domain!integrally closed}\label{algebra:integrally_closed_domain}
        The integral closure of a subring $A\subseteq B$ (in $B$) consists of all integral elements of $B$ over $A$. An integral domain $A$ (\cref{algebra:integral_domain}) is said to be an \textbf{integrally closed domain} if it coincides with its integral closure in $\mathrm{Frac}(A)$. All UFDs and PIDs (\cref{algebra:ufd} and \cref{algebra:pid}) are integrally closed.
    }
    ...
    \newdef{Ideal class group}{\index{ideal!class group}\index{Dedekind!domain}
        Consider the ring of algebraic integers $\mathcal{O}_K$ for a number field $K$ (or \textit{Dedekind domain}). The fractional ideals of (\cref{algebra:fractional_ideal}) of $\mathcal{O}_K$ form a group $\mathcal{I}_K$ with the principal fractional ideals forming a subgroup $\mathcal{P}_K$. The quotient group
        \begin{gather}
            \mathcal{C}_K := \mathcal{I}_K/\mathcal{P}_K
        \end{gather}
        is called the ideal class group of $K$. The order of $\mathcal{C}_K$ is called the \textbf{class number} $h_K$.
    }
    \begin{property}[UFD]
        The ring of integers $\mathcal{O}_K$ is a UFD if and only if $h_K=1$.
    \end{property}
    \begin{property}[Abelian groups]
        Every Abelian group can be obtained as the ideal class group over a \textit{Dedekind domain}.
    \end{property}
    ...
    \newdef{Perfect field}{\index{perfect!field}\label{algebra:perfect_field}
...
\subsection{Galois extensions}\label{section:galois_extensions}
...
\subsection{Nullstellensatz}\index{Nullstellensatz}
    ...
    For the geometric incarnation, see \labelref{chapter:alggeom}.

...
\section{Group actions}\label{section:group_actions}
    ...
    \newdef{Crossed module}{\index{module!crossed}\index{Peiffer}\label{group:crossed_module}
        ...
        \begin{enumerate}
            ...
            \begin{gather}
                t\big(\alpha(g)h\big) = gt(h)g^{-1}.
            \end{gather}
            \item\textbf{Peiffer identity}:
            \begin{gather}
                \alpha\big(t(h)\big)h' = hh'h^{-1}.
            \end{gather}
        \end{enumerate}
    }
...
\subsection{Ideals}
    ...
    \newdef{Principal ideal domain}{\index{domain!principal ideal}
        An integral domain \ref{algebra:integral_domain} in which every ideal is principal. These are often abbreviated as \textbf{PID}s.
    }
    ...
    The following definition is similar to the one for groups \ref{group:presentation}:
...
\subsection{Field extensions}
    ...
    \newdef{Degree}{\index{degree!of extension}
        If $L/K$ is a field extension, $L$ can be given the structure of a $K$-vector space \ref{linalgebra:vector_space}. The dimension of this vector space is called the degree of the extension $K$. It is often denoted by $[L:K]$.
    }

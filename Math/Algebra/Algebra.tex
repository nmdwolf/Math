...
\subsection{Modules}
    ...
    \end{result}
    \begin{result}
        Let $M$ be a finitely generated $R$-module. If there exist elements $m_1,\ldots,m_k\in M$ whose image in $M/J(R)M$ generate this quotient, the elements generate $M$.
    \end{result}
...
\subsection{Polynomials}
    ...
    \begin{method}[Horner--Ruffini]\index{Horner--Ruffini}
        The Horner(--Ruffini) method for polynomial evaluation and division is based on the above property (which essentialy boils down to long division). Consider a polynomial $f(x)=a_nx^n+\cdots+a_1x+a_0$. By the above theorem, polynomial division and evaluation are more or less equivalent, so only division is covered. The quotient and remainder with respect to a monomial $x-r$ can be obtained as follows:

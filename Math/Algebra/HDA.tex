...
\section{Monoidal categories}\label{section:monoidal_categories}
    ...
    \newdef{Monoidal category}{\index{monoidal!category}\index{tensor product}\label{cat:monoidal_category}
        ...
        \end{itemize}
        These natural transformations are required to make the \textbf{triangle} and \textbf{pentagon} diagrams in \cref{fig:triangle_diagram} and \cref{fig:pentagon_diagram}, respectively, commute. A monoidal category for which the associator and the unitors are identity transformations is often said to be \textbf{strict}.
...
\subsection{Braided categories}
    ...
    \newdef{Symmetric monoidal category}{\label{cat:symmetric}
        A braided monoidal category in which the braiding $\sigma$ satisfies
        \begin{gather}
...
\section{Enriched category theory}\label{section:enriched_category_theory}
    ...
    The following definition is due to \indexauthor{B\'enabou}. It should represent the `ideal place in which to do category theory'.
...
\subsection{Baez--Crans 2-vector spaces}\label{section:baez_crans}
    ...
    \begin{property}[Chain complexes]
        The (2-)categories of 2-vector spaces and 2-term chain complexes are equivalent.
...
\subsection{Lie superalgebras}
    ...
    \newdef{Internal Lie algebra}{\index{Lie!algebra}
        Let $(\symbfsf{C},\otimes,\symbf{1})$ be a linear symmetric monoidal category with braiding $\sigma$. A Lie algebra internal to $\symbfsf{C}$ is an object $L\in\ob{C}$ and a morphism
        \begin{gather}
...
\subsection{\texorpdfstring{Lie $n$-algebras}{Lie n-algebras}}\index{Lie!algebra}\label{section:higher_lie_structures}
    ...
    \newdef{Semistrict Lie $\omega$-algebra}{\index{Lie!operad}
        ...
        It can be shown that these structures are equivalent to the $L_\infty$-algebras of \indexauthor{Stasheff} defined below.
    }
    ...
    \begin{example}\label{hda:gk_lie_2_algebra}
        For a finite-dimensional Lie algebra $\mathfrak{g}$ with the trivial representation on $\mathbb{R}$ (or, more generally, the underlying field of $\mathfrak{g}$), one obtains
        \begin{gather}
    ...
    \newadef{Lie algebra}{\index{Lie!algebra}
        Consider a finite-dimensional Lie algebra $\mathfrak{g}$. The dual of the Lie bracket $[\cdot,\cdot]:\mathfrak{g}\wedge\mathfrak{g}\rightarrow\mathfrak{g}$ is a morphism $\delta:\mathfrak{g}^*\rightarrow\mathfrak{g}^*\wedge\mathfrak{g}^*$:
        \begin{gather}
    ...
    More generally, by passing to graded vector spaces of finite type concentrated in positive degree, one can characterize $L_\infty$-algebras as semifree dgcas. (This is a form of \textbf{Koszul duality}.)\index{Koszul!duality}
    \newadef{$L_\infty$-algebra}{\label{hda:l_infinity_bis}
        ...
        \end{itemize}
        When written out it in full detail, it can be checked that this is exactly the definition of an $L_\infty$-algebra, i.e.~there is an equivalence $\mathfrak{g}\leftrightarrow\mathrm{CE}(\mathfrak{g})$.
    }

    \begin{remark}[$L_\infty$-algebroids]\index{Lie!algebroid}
        By passing from \textbf{connected} Chevalley--Eilenberg algebras, i.e.~those for which $V_0\cong\mathfrak{K}$, to general Chevalley--Eilenberg algebras, one obtains the larger category of \textit{$L_\infty$-algebroids} (see \cref{hdg:l_infty_algebroid}).
    \end{remark}
    ...
    \newdef{Maurer--Cartan element}{\index{Maurer--Cartan!equation}
        ...
        \end{gather}
        For dg-Lie algebras, this reduces to the ordinary \textbf{Maurer--Cartan equation} (see also \cref{bundle:mc_equation}):
        \begin{gather}
            \dr a + \frac{1}{2}[a,a] = 0\,.
        \end{gather}
        This is no coincidence since the complex $\Omega^\bullet(M)\otimes\mathfrak{g}$ of Lie algebra-valued \textit{differential forms} on a \textit{smooth manifold} $M$ carries a canonical dg-Lie algebra structure.
    }

    In fact, the following property can be shown for $L_\infty$-algebra-valued \textit{differential forms}.
    \begin{property}
        Let $M$ be a smooth manifold and $\mathfrak{g}$ a connected $L_\infty$-algebra of finite type.
        \begin{gather}
            \Omega^1\bullet(M;\mathfrak{g})_{\text{cl}}\cong\symbfsf{dgcAlg}\bigl(\mathrm{CE}(\mathfrak{g}),\Omega^\bullet(M)\bigr)
        \end{gather}
        \todo{CHECK THIS (YT course by Schreiber)}
    \end{property}

\chapter{Homological Algebra}\label{chapter:hom_alg}
    ...
    References for this chapter are~\citet{weibel_introduction_1994, hilton_course_1997}.
...
\section{Chain complexes}
	...
	\newdef{Chain complex}{\index{chain!complex}\index{boundary}\index{differential}\index{cycle}\index{exact!element}\index{closed!element}\label{homalg:chain_complex}
		Let $\symbfsf{A}$ be an additive category (often an Abelian category) and consider a collection $\{C_k\}_{k\in\mathbb{Z}}$ of objects and a collection $\{\partial_k:C_k\rightarrow C_{k-1}\}_{k\in\mathbb{Z}}$ of morphisms in $\symbfsf{A}$ such that for all $k\in\mathbb{Z}$:
		\begin{gather}
	...
	\end{gather}
	where $\partial_k,\partial_k'$ are the boundary operators of $C_\bullet$ and $D_\bullet$, respectively. Given an additive category $\symbfsf{A}$, one writes $\symbfsf{Ch(A)}$ for the category of chain complexes and chain maps in $\symbfsf{A}$.
    ...
    The following definitions use the language of \labelref{chapter:hda}.
    ...
    \newdef{Semifree DGCA}{
        A DGCA whose underlying graded-commutative algebra is isomorphic (as a graded-commutative algebra) to the exterior algebra over a graded vector space.
    }

    \newdef{Differential ideal}{\index{ideal!differential}\label{homalg:differential_ideal}
        A subset $I\subseteq(A,\dr)$ of a differential (graded) algebra satisfying:
        \begin{enumerate}
            \item $I$ is an ideal of the underlying associative algebra of $A_\bullet$.
            \item $I$ is differentially closed:$\dr I\subseteq I$.
        \end{enumerate}
    }
    ...
    \begin{remark}[\difficult{Model structure on DGCAs}]
        By \cref{topology:sullivan_algebras_specific}, the (minimal) models of DGCAs are (minimal) Sullivan algebras. From a model theory point of view, the (minimal) Sullivan algebras are the cofibrant objects and the (minimal) models are the cofibrant replacements.
    \end{remark}

\section{Exact sequences}

	\newdef{Exact sequence}{\index{exact!sequence}
		Let $\symbfsf{A}$ be an additive category and consider a sequence of objects and morphisms in $\symbfsf{A}$:
		\begin{gather}
        ...
        \end{gather}
        In particular this means that $\Phi_{k+1}\circ\Phi_k = 0$ for all $k\in\mathbb{N}$, which in turn implies that exact sequences are a special type of chain complexes (\cref{homalg:chain_complex}).
	}
...
\section{Resolutions}
	...
	\newdef{Acyclic complex}{\index{acyclic!complex}
		A chain complex $C_\bullet\in\symbfsf{Ch(A)}$ is said to be acyclic if the sequence \[\cdots\longrightarrow C_{k+1}\longrightarrow C_k\longrightarrow C_{k-1}\longrightarrow\cdots\] is exact or, equivalently, if the homology complex $H_\bullet(C_\bullet)$ vanishes.
	}
	...
	\newdef{Resolution}{\index{resolution}\index{augmentation}\label{homalg:resolution}
		Consider an object $X$ in $\symbfsf{A}$. A resolution of $X$ is given by an acyclic chain complex in $\symbfsf{A}$ of the form
		\begin{gather}
	...
	In practice, it is often convenient to restrict to a specific type of resolution. For example, by considering chain complexes with only injective or projective objects (\namecrefs{fig:injective_object}~\ref{fig:injective_object} and~\ref{fig:projective_object}), one obtains injective or projective resolutions. If every object in $A$ admits a projective (resp.~injective) resolution, $\symbfsf{A}$ is said to \textbf{have enough projectives} (resp.~\textbf{injectives}).
...
\section{Derived functors}
	...
	\newdef{Prolongation}{\index{prolongation}
		Let $\func{F}{A}{A'}$ be an additive functor. The prolongation of $F$ is a functor $\overline{F}:\symbfsf{Ch(A)}\rightarrow\symbfsf{Ch(A')}$ obtained by applying $F$ to every object in a chain complex and to every diagram in the definition of a chain map. As is common, by abuse of notation the prolongation will also often be denoted by $F$.
	}
...
\subsection{Modules}\label{section:tor_ext}
    ...
    \newdef{Flat module}{\index{flat!module}\index{flat!morphism}\label{homalg:flat_module}
        An $R$-module $M$ such that the induced tensor functor
        \begin{gather}
            -\otimes_R M:R\symbfsf{Mod}\rightarrow R\symbfsf{Mod}
        \end{gather}
        is exact. By \cref{homalg:exact_vanishing_derived_functor}, this implies that a flat module is $\otimes$-acyclic (\cref{homalg:acyclic_object}), which, in turn, implies by \cref{homalg:acyclic_derived_functors} that these modules can be used to construct a good resolution for calculating Tor-functors. A ring morphism $f:R\rightarrow S$ is said to be \textbf{flat} if it turns $S$ into a flat $R$-module.
    }

	\newdef{Bar resolution}{\index{resolution!bar}\index{resolution!standard|see{bar resolution}}\label{homalg:bar_resolution}
		Let $A$ be an associative algebra. The bar complex (or \textbf{standard complex}) of $A$ is the sequence
		\begin{gather}
			\cdots\rightarrow A^{\otimes 3}\rightarrow A^{\otimes 2}\rightarrow A\rightarrow 0\,,
		\end{gather}
		with $B_k(A):=A^{\otimes(k+2)}\cong A^e\otimes A^{\otimes n}$, where $A^e$ is the enveloping algebra (\cref{algebra:enveloping_algebra}) and the isomorphism is given by:
		\begin{gather}
			a_0\otimes\cdots\otimes a_{k+1}\mapsto(a_0\otimes a_{k+1})\otimes(a_1\otimes\cdots\otimes a_k)\,.
		\end{gather}
		The differentials are defined as follows:
		\begin{gather}
			\dr_k(a_0\otimes\cdots\otimes a_{k+1}) := \sum_{i=0}^ka_0\otimes\cdots\otimes a_ia_{i+1}\otimes\cdots\otimes a_{k+1}\,.
		\end{gather}
		If $A$ is free, so is the bar complex and, hence, it gives a (free) resolution of $A$.
	}

    \newdef{Koszul complex}{\index{Koszul!complex}\index{homology!Koszul}
        ...
        \end{gather}
        where the exterior powers $\Lambda^kM$ are defined as in \cref{section:wedge_product}, i.e.~they are the free modules spanned by totally antisymmetric $k$-tuples in $M$. The differentials are defined as
        \begin{gather}
    ...
    \end{example}
    \begin{property}[Koszul resolution]\index{regular!sequence}\index{Koszul!resolution}\index{resolution!Koszul}\label{homalg:koszul_resolution}
    ...
    \begin{property}[Koszul--Tate resolution]\index{Koszul--Tate resolution}\index{resolution!Koszul--Tate}\label{homalg:koszul_tate_resolution}
...
\subsection{Exact couples}
	...
	Now, consider a differential object $(C,\dr)$ together with a filtration $\{F_nC\}_{n\in\mathbb{N}}$. \Cref{set:filtration} of a filtration immediately gives a short exact sequence for every $n\in\mathbb{N}$:
	\begin{gather}
...
\subsection{Convergence}
    ...
    \newdef{Limit term}{\index{limit}\index{abut}
...
\subsection{Group cohomology}\index{group!cohomology}\label{section:group_cohomology}
	...
	\newdef{Group cohomology}{\label{homalg:group_cohomology}
        Consider a group $G$ together with a $G$-module $A$. (For simplicity only discrete, e.g.~finite, groups and Abelian coefficients will be considered.) For every $k\in\mathbb{N}$, define the $k^{\text{th}}$ \textbf{chain group} as
        \begin{gather}
            C^k(G;A) := \bigl\{\text{all set-theoretic functions from }G^k\text{ to }A\bigr\}.
        \end{gather}
    ...
    \todo{COMPLETE (ADD e.g. classification of extensions)}
...
\subsection{\difficult{Hochschild and cyclic homology}}\index{homology!Hochschild}
    ...
    \newdef{Hochschild homology groups}{\label{algebra:hochschild_homology}
        Let $A$ be an associative algebra and consider an $A$-bimodule $M$. The Hochschild chain complex is obtained by tensoring the bar complex of $A$ by $M$:
        \begin{gather}
            HC_k(A;M) := M\otimes A^{\otimes k}\cong M\otimes_{A^e}B_n(A)\,,
        \end{gather}
		where $B_\bullet(A)$ is the bar complex (\cref{homalg:bar_resolution}). The differential $\dr$ on the bar complex $B_\bullet(A)$ can be extended to $M\otimes_{A^e}B_\bullet(A)$ as $\mathbbm{1}_M\otimes\dr$ and the above isomorphism then induces a differential on $M\otimes A^k$. The boundary operator $\dr_k:HC_k(A;M)\rightarrow HC_{k-1}(A;M)$ is then given by:
        \begin{align}
            ...
            &+(-1)^ka_km\otimes\cdots\otimes a_{k-1}\,.\nonumber
        \end{align}
        The Hochschild homology of $A$ with coefficients in $M$ is then given by the homology of this chain complex:
        \begin{gather}
    ...
    \begin{remark}[Cohomology]\index{cohomology!Hochschild}
		The Hochschild cochain complex is obtained by applying the Hom-functor to the bar complex:
		\begin{gather}
			HC^k(A;M):=\hom_{A_e}\bigl(B_k(A);M\bigr)\,,
		\end{gather}
		where $\hom_{A_e}\bigl(B_k(A);M\bigr)\cong\hom(A^{\otimes k};M)$ by sending $f\in\hom(A^{\otimes k};M)$ to
		\begin{gather}
			\overline{f}:a_0\otimes\cdots\otimes a_{k+1}\mapsto a_0f(a_1\otimes\cdots\otimes a_k)a_{k+1}\,.
		\end{gather}
		The cohomology of this cochain complex is called the Hochschild cohomology $HH^\bullet(A;M)$ of $A$ with coefficients in $M$.
    \end{remark}

    \newdef{Cyclic homology}{\index{homology!cyclic}\index{Connes!complex}
        The cyclic chain complex $CC_\bullet(A)$ is the subcomplex of the Hochschild complex $HC_\bullet(A;A)$ by quotienting out the coinvariants of the permutation operator
        \begin{gather}
            \lambda:HC_k(A)\rightarrow HC_k(A):a_1\otimes\cdots\otimes a_k\mapsto(-1)^ka_k\otimes a_0\otimes\cdots\otimes a_{k-1}\,.
        \end{gather}
		This gives the \textbf{Connes complex}
		\begin{gather}
			CH_\bullet(A) := HH_\bullet(A;A)/\langle1-\lambda\rangle\,.
		\end{gather}
    ...
    \begin{theorem}[Hochschild--Kostant--Rosenberg]\index{Hochschild--Kostant--Rosenberg}
        Let $A=C^\infty(M)$ for $M$ a \textit{smooth manifold} (see \cref{diff:manifold}).
        \begin{gather}
    ...
    \begin{example}[Smooth algebras]
        ...
        \begin{gather}
            CH_k(A)\cong \Omega_{\text{dR}}^k(M)/\dr\Omega_{\text{dR}}^{k-1}(M)\oplus\bigoplus_{\substack{i=1\\k-2i\geq0}}H^{k-2i}_{\text{dR}}(M)\,.
        \end{gather}
        For this reason, cyclic cohomology is used in \textit{noncommutative geometry} to generalize \textit{de Rham cohomology}.
    \end{example}
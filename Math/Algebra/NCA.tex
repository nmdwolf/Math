...
\subsection{Differential calculi}
    ...
    \newdef{Cotangent dimension}{\index{dimension!cotangent}\index{parallelizable}
        ...
        This is in analogy with the case of function algebras $C^\infty(M)$ and the de Rham space $\Omega^1(M)$ on a smooth manifold. If $\Omega^1(M)$ is free over $C^\infty(M)$, this implies that there exists a global basis of one-forms or, equivalently, a global frame of the tangent bundle. This is the same as saying that $M$ is parallelizable. (The cotangent dimension will be explained later.)
    }

    \begin{example}[Universal FODC]\index{augmentation}
        Consider an algebra $A$ with multiplication $\mu$. The bimodule $\Omega_{\text{uni}}:=\ker(\mu)$, often called the \textbf{augmentation ideal}, equipped with the operator
        \begin{gather}
            \dr a:=1\otimes a-a\otimes1\,,
        \end{gather}
        defines a first-order differential calculus on $A$. Furthermore, every FODC over $A$ can be obtained as the quotient $\Omega_{\text{uni}}/\mathcal{N}$ for a subbimodule $\mathcal{N}$. The universal FODC is inner if and only if there exists a central element $F\in A\otimes A$ such that $\mu(F)=1$.
    \end{example}
    \begin{example}[K\"ahler differentials]\index{K\"ahler!differential}\label{nca:kahler_differential}
        In commutative algebra, an equivalent construction exists. Take the module $\Omega^1(A)$ to be the quotient $\Omega_{\text{uni}}/\Omega_{\text{uni}}^2$. The quotient enforces that the bimodule is symmetric and, hence, that the Leibniz rule becomes
        \begin{gather}
            \dr(ab) = a\dr b + b\dr a\,.
        \end{gather}
    ...
    \end{example}
    \begin{remark}[Relative K\"ahler differentials]
        The definitions above can also be extended to the relative setting, where a ring morphism $\phi:A\rightarrow B$ is provided. Instead of requiring $\dr\mathfrak{K}=0$, the relative condition $\dr\phi(A)=0$ is required. The space of relative K\"ahler differentials $\Omega_{B/A}$ satisfies a similar universal property in that it classifies all $A$-module derivations.
    \end{remark}

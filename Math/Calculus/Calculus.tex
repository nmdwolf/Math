\chapter{Calculus}
    ...
    For the section on umbral calculus, see \citet{di_bucchianico_introduction_1998}.

    \minitoc

\section{General definitions}

    \newdef{Domain}{\index{domain}\label{calculus:domain}
...
\section{Continuity}\index{continuity}
    ...
    \newdef{Darboux function}{\index{Darboux!function}
        A function satisfying the intermediate value property (\cref{topology:intermediate_value_theorem}).
    }
    ...
    \newdef{Function of bounded variation}{\index{bounded!variation}
        A function $f:[a,b]\rightarrow\mathbb{R}$ for which the following quantity is finite:
        \begin{gather}
...
\section{Series}
    ...
    \newdef{Series}{\index{series}\index{sum!partial}
        Consider a sequence $\seq{x}\subset\mathbb{R}$. For every index $n\in\mathbb{N}$, consider the \textbf{partial sum}
        \begin{gather}
            S_n := \sum_{k=0}^nx_n\,.
        \end{gather}
        The would-be limit $S_\infty:=\sum_{k=0}^{+\infty}x_n$ is called the series associated to $\seq{x}$. If the induced sequence $\seq{S}$ converges, the series is said to be \textbf{convergent} with limit $S_\infty$. Otherwise, the series is said to be \textbf{divergent}.
    }

    \newdef{Absolute/conditional convergence}{\index{convergence}
        Consider a series
        \begin{gather}
            S = \sum_{k=0}^{+\infty}x_k\,.
        \end{gather}
        If the associated series $S'=\sum_{k=1}^{+\infty}|x_k|$ converges, so does $S=\sum_{k=1}^{+\infty}x_k$. In this case, $S$ is said to be absolutely convergent. If $S$ converges but $S'$ does not, $S$ is said to be conditionally convergent.
    }

    \begin{property}[Riemann rearrangement theorem]\index{Riemann!rearrangement theorem}
        If a series $\sum_{k=0}^{+\infty}x_k$ is conditionally convergent, then for any number $\lambda\in\overline{\mathbb{R}}$, there exist a permutation $\pi$ of $\seq{x}$ such that
        \begin{gather}
            \sum_{k=0}^{+\infty}x_{\pi(k)} = \lambda\,.
        \end{gather}
    \end{property}
    ...
    \begin{example}
        ...
        \begin{gather}
            S=\sum_{k=0}^{+\infty}k \qquad\text{and}\qquad S'=\sum_{k=0}^{+\infty}2k+1\,.
        \end{gather}
        These are, respectively, the sums of all integers and all odd integers. Although the series are divergent, their partial sums satisfy important equations:
        \begin{gather}
            S_n = \frac{n(n+1)}{2} \qquad\text{and}\qquad S'_n = n^2\,.
        \end{gather}
    \end{example}
...
\subsection{Convergence tests}\index{convergence}
    ...
    \begin{property}
        A necessary condition for the convergence of a series $\sum_{k=1}^{+\infty}a_k$ is that
        \begin{gather}
    ...
    \newdef{Majorizing series}{\index{majorization}
        Let $S_a=\sum_{k=1}^{+\infty}a_k$ and $S_b=\sum_{k=1}^{+\infty}b_k$ be two series. The series $S_a$ is said to majorize $S_b$ if, for every $n\in\mathbb{N}_0$, the partial sums satisfy $S_{a,n}\geq S_{b,n}$, i.e.
        \begin{gather}
            \sum_{k=1}^na_k\geq\sum_{k=1}^nb_k
        \end{gather}
        for all $n\in\mathbb{N}$.
    }
    ...
    \begin{property}
        ...
        \begin{gather}
            \Int_n^{+\infty}f(x)\,dx\leq\sum_{k=n}^{+\infty}a_k \leq f(n) + \Int_n^{+\infty}f(x)\,dx\,.
        \end{gather}
    \end{property}
    ...
    \newdef{Asymptotic expansion}{\index{asymptotic!expansion}\label{calculus:asymptotic_expansion}
        Let $f:\mathbb{R}\rightarrow\mathbb{R}$ be a continuous function. A series $\sum_{k=0}^{+\infty}a_kx^k$ is called an asymptotic expansion of $f$ if there exists an $N\in\mathbb{N}$ such that
        \begin{gather}
            f(x) - \sum_{k=0}^na_kx^k = O(x^{n+1})
        \end{gather}
        for all $x\in\mathbb{R}$ and $n\geq N$.
    }

\subsection{Geometric series and resummation}

    \newdef{Geometric series}{\index{geometric!series}\index{growth rate}
        Consider a number $r\in\mathbb{R}$. The geometric series induced by $r$ is given by
        \begin{gather}
            S_n = \sum_{k=0}^nr^k\,.
        \end{gather}
        The parameter $r$ is often called the \textbf{growth rate}.

        It can be shown that the series is convergent for $|r|<1$ with value
        \begin{gather}
            \label{calculus:geoemtric_series}
            \sum_{k=0}^{+\infty}r^k = \frac{1}{1-r}\,.
        \end{gather}
    }

    \begin{example}[Grandi's series]\index{Grandi's series}
    ...
    However, there is a way to assign a value to Grandi's series. Either through an intuitive, nonformal approach, or through a rigourous analytic approach, both leading to the same result. For the former, one uses associativity of addition. For convergent series, associativity still holds, but for divergent series this can only be formally applied. Brackets can be added in two ways\footnote{This `trick' is very similar to the \textit{Eilenberg--Mazur swindle} from algebraic topology.\index{Eilenberg--Mazur swindle}}:
    \begin{align*}
    ...
    A similar issue arises for $r=2$. This gives the geometric series
    \begin{gather}
        S = 1 + 2 + 4 + 8 + \cdots\,.
    \end{gather}
    This series is clearly divergent, but analytic continuation also allows to assign a `renormalized' value:
    \begin{gather}
        S \doteqdot \lim_{r\rightarrow 2}\frac{1}{1-x} = -1\,.
    \end{gather}

    Some other approaches exist to assign finite, `renormalized' values to divergent series.
    \newdef{Summation}{\index{summation}
        A summation method is a function $\Sigma:\mathbb{R}^{\mathbb{N}}\rightarrow\mathbb{R}$ that assigns a definite value to a sequence of partial sums.

        To be deemed a `good' summation method, some properties are usually required:
        \begin{enumerate}
            \item\textbf{Regularity}: If $S_n\longrightarrow S$, then $\Sigma\seq{S}=S$.
            \item\textbf{Linearity}: $\Sigma(\lambda S_n+T_n)_{n\in\mathbb{N}}=\lambda\Sigma\seq{S}+\Sigma\seq{T}$ for all $\lambda\in\mathbb{R}$.
            \item\textbf{Stability}: $\Sigma\seq{S} = S_0+\Sigma(S_{n+1}-S_0)_{n\in\mathbb{N}}$. This means that if $\seq{S}$ are the partial sums of a sequence $\seq{x}$, one can calculate the series with $x_0$ omitted and add it afterwards.
            \item\textbf{Finite reindexability}: If $\seq{S},\seq{T}$ are two sequences such that $T_n=S_{\pi(n)}$ for some permutation $\pi\in\Aut(\mathbb{N})$ that only acts nontrivially on a finite initial segment of $\mathbb{N}$, then $\Sigma\seq{S}=\Sigma\seq{T}$.
        \end{enumerate}
    }

    \begin{method}[C\'esaro summation]\index{C\'esaro sum}
        Consider a sequence $\seq{x}\subset\mathbb{R}$ and its associated partial sums $\seq{S}$. The sequence $\seq{x}$ is said to be C\'esaro summable if the arithmetic means
        \begin{gather}
            \overline{S}_n := \frac{1}{n}\sum_{k=0}^nS_n
        \end{gather}
        converge, and its C\'esaro sum is given by $S\doteqdot\lim_{n\rightarrow\infty}\overline{S}_n$. If a series is convergent, it is C\'esaro summable and the results coincide.
    \end{method}
...
\subsection{Riemann integral}\index{integral!Riemann}\index{Riemann|seealso{integral}}
    ...
    \begin{theorem}[Second fundamental theorem of calculus\footnotemark]\index{Newton--Leibniz}\label{calculus:second_fundamental_theorem}
        ...
        Consider a function $f\in L^1\bigl([a,b]\bigr)$. If $F:[a,b]\rightarrow\mathbb{R}$ is an antiderivative of $f$, then
        \begin{gather}
            \Int_a^bf(x)\,dx = F(b) - F(a)\,.
        \end{gather}
    \end{theorem}

    \begin{formula}[Euler--MacLaurin]\index{Euler--MacLaurin formula}
...
\subsection{Euler integrals}\index{Euler!integral}
   ...
   \newformula{Gamma function}{\index{gamma!function}\label{calculus:gamma_function}
    ...
    \begin{property}[Euler's reflection formula]\index{Euler!reflection formula}\label{calculus:euler_reflection}
        The following formula holds for all $x\not\in\mathbb{Z}$:
        \begin{gather}
...
\subsection{Special functions}
    ...
    \newdef{Bernoulli polynomial}{\index{Bernoulli!polynomial}\label{calculus:bernoulli_number}
        ...
        \end{gather}
        The \textbf{Bernoulli numbers} are defined as
        \begin{gather}
            B_n := B_n(0)\,.
        \end{gather}
    }
    ...
    \newdef{Sheffer sequence}{\index{Sheffer sequence}\index{poweroid}
        ...
        \begin{gather}
            \deg(Qp) = \deg(p)-1
        \end{gather}
        for all $\lambda\in\mathbb{R}$ and $p\in\mathbb{R}[x]$. A polynomial sequence $\seq{p}\subset\mathbb{R}[x]$ is said to be a Sheffer sequence (or a \textbf{poweroid}) if
        \begin{gather}
            Qp_n = np_{n-1}
        \end{gather}
        for some delta operator $Q$.
    }
    \begin{example}
    ...
    \begin{property}[Umbral composition]\index{umbral!group}\label{calculus:umbral_composition}
        ...
        \end{gather}
        where $p_n=\sum_{k=0}^nb_{n,k}x^k$ and $q_n=\sum_{k=0}^na_{n,k}x^k$. The identity element of the \textbf{umbral group} is given by the standard monomials $p_n(x)=x^n$.
    \end{property}

    \newdef{Binomial type}{\index{binomial!type}\index{convolution}
        A polynomial sequence $\seq{p}\subset\mathbb{R}[x]$ is said to be of binomial type if\footnote{By rescaling the polynomials $p_n$ by $n!$, an ordinary convolution formula is obtained.}
        \begin{gather}
            p_n(x+y) = \sum_{k=0}^n{n\choose k}p_k(x)p_{n-k}(y)\,.
        \end{gather}
        A Sheffer sequence is a polynomial of binomial type if and only if
        \begin{itemize}
            \item $p_0(x)=1$, and
...
\subsection{Umbrae}
    ...
    The umbral composition rule is then simply given by the composition~\ref{calculus:umbral_composition} of two such linear operators. More generally, the umbral composition can be extended to the entire $\mathbb{R}$-vector space of polynomial functions, sometimes called the \textbf{umbral algebra} in this setting:\index{umbral!algebra}
    \begin{gather}
        (L_A\circ L_B)x^n := \sum_{k=0}^n{n\choose k}(L_Ax^k)(L_Bx^{n-k})\,.
    \end{gather}

    \begin{example}[Probability theory]

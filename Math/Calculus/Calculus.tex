...
\section{General definitions}
    ...
    \newdef{Domain}{\index{domain}
        A connected, open subset of $\mathbb{R}^n$. (Not to be confused with the domain of a function \ref{set:domain}.)
    }
...
\section{Continuity}\index{continuity}
    ...
    \begin{definition}[Darboux function]\index{Darboux!function}
        A function that has the intermediate value property \ref{topology:intermediate_value_theorem}.
    \end{definition}
...
\section{Differentiation}\index{differentiation}
    ...
    \begin{formula}[Derivative of \texorpdfstring{$f(x)^{g(x)}$}{f(x)^g(x)}]
        ...
        \begin{gather}
            ...
            \left(f(x)^{g(x)}\right)' = f(x)^{g(x)}\left(g'(x)\ln\big[f(x)\big] + \frac{g(x)}{f(x)}f'(x)\right).
        \end{gather}
    \end{formula}
...
\section{Integration theory}\index{Riemann!integral}
    ...
    \begin{formula}[Differentiation under the integral sign\footnotemark]\index{Leibniz!integral rule}\label{calculus:leibniz_integral_rule}
        ...
        \begin{gather}
            ...
            \deriv{}{x}\int_{a(x)}^{b(x)}f(x,y)\,dy = f\big(x,b(x)\big)b'(x) - f\big(x,a(x)\big)a'(x) + \int_{a(x)}^{b(x)}\pderiv{f(x,y)}{x}\,dy
        \end{gather}
    \end{formula}
    ...
    \begin{theorem}[Watson]\index{Watson}
        The Borel transform $F$ is unique if the function $f$ is holomorphic (Definition \ref{complexcalculus:holomorphic}) on the domain $\{z\in\mathbb{C}\mid|\arg(z)|<\frac{\pi}{2}+\varepsilon\}$.
    \end{theorem}
...
\subsection{Euler integrals}\index{Euler!integral}
   ...
   \newformula{Gamma function}{\index{gamma!function}\label{calculus:gamma_function}
    ...
    \newformula{Factorial}{
        For integers $n\in\mathbb{N}$ the gamma function can be expressed in terms of the factorial \ref{calculus:factorial}:
        \begin{gather}
...
\section{Convexity}
    ...
    \newdef{Convex set}{\index{convex}\index{hull}\label{calculus:convex}
        A subset of $X$ of a vector space $V$ (Definition \ref{linalgebra:vector_space}) is said to be convex if $x,y\in X$ implies that $\big\{\lambda x+(1-\lambda)y\mid\lambda\in[0,1]\big\}\subset X$, i.e.~if all straight lines connecting elements of the set are completely contained in that set. The \textbf{convex hull} of a subset $X$ is defined as the smallest convex subset containing $X$.
    }

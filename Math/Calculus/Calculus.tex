...
\subsection{Convergence tests}\index{convergence}
    ...
    \newprop{Absolute/conditional convergence}{
        If $S'=\sum_{i=1}^\infty|a_i|$ converges, so does the series $S=\sum_{i=1}^\infty a_i$. Moreover, $S$ is said to be absolutely convergent. If $S$ converges but $S'$ does not, $S$ is said to be conditionally convergent.
    }
...
\section{Differentiation}\index{differentiation}
    ...
    \newformula{Derivative}{\label{calculus:derivative}
        Consider a function $f:\mathbb{R}\rightarrow\mathbb{R}$. The following limit is called the derivative of $f$ at $x$ (if it exists):
        \begin{gather}
    ...
    \begin{formula}[Derivative of \texorpdfstring{$f(x)^{g(x)}$}{f(x)^g(x)}]
        Consider a function of the form \[u(x)=f(x)^{g(x)}.\] After taking the logarithm and applying the standard rules of differentiation, one can obtain the following expression:
        \begin{gather}
    ...
    \newdef{Euler operator}{\index{Euler!operator}\label{calculus:euler_operator}
        On the space $C^{r>1}(\mathbb{R}^n,\mathbb{R})$, the Euler operator $\mathbb{E}$ is defined as follows:
        \begin{gather}
        ...
        \begin{proof}
            \begin{mdframed}[roundcorner=10pt, linecolor=blue, linewidth=1pt]
                ...
                \begin{align*}
                    ...
                    &=\sum_{n=0}^Na_nx^n,
               \end{align*}
               where the definition of the Gamma function \ref{calculus:gamma_function} was used on line 3 and the relation \eqref{calculus:gamma_factorial_relation} between the factorial function and the Gamma function was used on line 4.\qed
           \end{mdframed}
        \end{proof}
    }
    \begin{theorem}[Watson]\index{Watson}
        The Borel transform $F$ is unique if the function $f$ is holomorphic on the domain $\{z\in\mathbb{C}\mid|\arg(z)| < \frac{\pi}{2} + \varepsilon\}$.
    \end{theorem}
...
\subsection{Gaussian integrals}\index{Gauss!integral}
    ...
    \begin{result}
        ...
        \begin{align}
            I(iA,iJ) &= \int[d\varphi]\exp\left(-i\int_{\mathbb{R}^n\times\mathbb{R}^n}\varphi(x)A(x,y)\varphi(y)\,d^nxd^ny + i\int_{\mathbb{R}^n}\varphi(x)J(x)\,d^nx\right)\nonumber\\
            &= C\det(A)^{-1/2}\exp\left(\frac{i}{2}\int_{\mathbb{R}^n\times\mathbb{R}^n}J(x)A^{-1}(x,y)J(y)\,d^nxd^ny\right),
        \end{align}
...
\section{Convexity}
    ...
    \newdef{Convex set}{\index{convex}\index{hull}\label{calculus:convex}
        A subset of $X$ of a vector space $V$ (Definition \ref{linalgebra:vector_space}) is said to be convex if $x,y\in X$ implies that $\{\lambda x+(1-\lambda)y\mid\lambda\in[0,1]\}\subset X$, i.e. if all straight lines connecting elements of the set are completely contained in that set. The \textbf{convex hull} of a subset $X$ is defined as the smallest convex subset containing $X$.
    }
    ...
    \begin{theorem}[Jensen's inequality]\index{Jensen's inequality}\label{calculus:jensen_inequality}
        Let $f$ be a convex function and consider a point $\{a_i\}_{i\leq n}$ in the probability simplex $\Delta^n$ (Definition \ref{topology:standard_simplex}).
        \begin{gather}

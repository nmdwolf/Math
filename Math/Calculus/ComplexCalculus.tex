...
\section{Complex algebra}
    ...
    The set of complex numbers $\mathbb{C}$ forms a 2-dimensional \textit{vector space} over the field of real numbers (see \labelref{chapter:linear_algebra}). At the same time, the operations of complex addition and complex multiplication also turn the complex numbers into a field.
    ...
    \newformula{Real/imaginary part}{
        ...
        \end{align}
        Note that the notations $\mathrm{Re}(z)$ and $\mathrm{Im}(z)$ are also frequently used.
    }
    \newdef{Argument and modulus}{\index{argument}\index{modulus}\index{polar!form}
        Since $\mathbb{C}\cong\mathbb{R}^2$, every complex number can be represented as a point in the plane. Now, consider this \textbf{polar form}of a complex number:
        \begin{gather}
            z = re^{i\theta}
        \end{gather}
        The number $\theta\in[0,2\pi[$ is called the argument of $z$ and it is denoted by $\arg(z)$. The number $r\in\mathbb{R}^+$ is called the modulus of $Z$ and is denoted by $|z|$.
    }
    ...
    \newdef{Poincar\'e plane}{\index{Poincar\'e!upper half plane}\label{complexcalculus:upper_half_plane}
        The Poincar\'e upper half plane is the subset of $\mathbb{C}$ with positive imaginary part:
        \begin{gather}
...
\section{Holomorphic functions}
    ...
    \newdef{Holomorphic function}{\index{holomorphic!function}\label{complex:holomorphic}
    ...
    \begin{property}[Laplace equation]\index{harmonic!function}
        The functions $u,v$ satisfying the Cauchy--Riemann conditions are harmonic functions, i.e.~they satisfy the \textit{Laplace equation}~\ref{pde:laplace_equation}.
    \end{property}
    ...
    \begin{theorem}[Identity theorem]\index{identity!theorem}
        If two holomorphic functions on a domain $D$ (\cref{calculus:domain}) coincide on a neighbourhood of an accumulation point of $D$, they coincide on all of $D$.
    \end{theorem}
...
\section{Laurent series}
    ...
    \newdef{Laurent series}{\index{Laurent!series}\index{annulus}\index{principal!part}\label{complex:laurent_series}
    ...
    \newdef{Analytic continuation}{\index{analytic!continuation}\label{complex:analytic_continuation}
...
\subsection{Poles}
    ...
    \newdef{Removable singularity}{\index{singularity!removable}
        Let $f:U\backslash\{a\}\rightarrow\mathbb{C}$ be a holomorphic function on a punctured open set. The point $a$ is called a removable singularity of $f$ if there exists a holomorphic function $g:U\rightarrow\mathbb{C}$ such that $f(z)=g(z)$ for all $z\in U\backslash\{a\}$.
    }
    \begin{theorem}[Riemann]\index{Riemann}\label{complex:riemann_theorem_singularities}
        A point $a\in\mathbb{C}$ is a removable singularity for a holomorphic function $f:U\backslash\{a\}\rightarrow\mathbb{C}$, with $U\subseteq\mathbb{C}$ an open set, if and only if there exists a neighbourhood of $a$ on which $f$ is bounded.
    \end{theorem}
    ...
    \newdef{Essential singularity}{\index{singularity!essential}
    ...
    \newdef{Cauchy principal value}{\index{Cauchy|seealso{principal value}}
        If $f:\mathbb{C}\rightarrow\mathbb{C}$ has a pole $z_0$ on the contour $C$, then
        \begin{gather}
            \mathcal{P}\Int_Cf(z)\,dz := \lim_{\varepsilon\downarrow0}\Int_{C_\varepsilon}f(z)\,dz\,,
        \end{gather}
        where $C_\varepsilon := C\backslash B(z_0,\varepsilon)$, assuming $f$ is integrable on all $C_\varepsilon$.
    }
...
\subsection{Branch cuts}\label{section:branch_cuts}
    ...
    \newformula{Complex logarithm}{\index{logarithm}
        The natural logarithm can be continued to the complex plane (as a multivalued function) as follows:
        \begin{gather}
            \mathrm{LN}(z) := \bigl\{\ln(r) + i(\theta + 2\pi k)\bigm\vert k\in\mathbb{Z}\bigr\}\,.
        \end{gather}
    }

    \newdef{Branch}{\index{branching}
        The problem with the previous two formulas is that they represent multivalued functions. To get an unambiguous image it is necessary to fix a value of the parameter $k$. By doing so there will arise curves, called \textbf{branch cuts}, in the complex plane where the function becomes discontinuous. A \textbf{branch} is defined as a particular choice of the parameter $k$.
    ...
    \newdef{Principal value}{\index{principal!value}
        The principal value\footnote{Not to be confused with the Cauchy principal value.} of a multivalued complex function is defined as the value associated with a choice of branch for which $\arg(f)\in\ ]\!-\pi,\pi]$.
    }
...
\subsection{Residue theorem}\index{residue}
    ...
    \begin{formula}[Argument principle]\index{argument!principle}
        ...
        \end{gather}
    \end{formula}
    \newdef{Winding number}{\index{winding number}\index{index!of map}

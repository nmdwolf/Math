\chapter{Distributions}\label{chapter:distributions}
	...
	The main references for this chapter are~\cite{georgiev_theory_2015,choquet-bruhat_analysis_1991,choquet-bruhat_analysis_2000}. Although this chapter is technically part of functional analysis and, hence, uses the language of normed spaces (see \labelref{chapter:functional}), it is located in the part on calculus due to its strong relation to measure and integration theory. (Terminology from the chapter on functional analysis will not be indicated in italic for simplicity.)
...
\subsection{Fourier series}
    ...
    \newdef{Hardy space}{\index{Hardy space}\index{Poisson!kernel}\label{distributions:hardy_space}
        The Hardy spaces $\mathcal{H}^k(S^1)$, for $k>0$, are defined as the subspaces of $L^k(S^1)$ with vanishing negative Fourier components.

        This can be generalized to the unit disk, since all functions in $L^k(S^1)$ can be obtained as limits of functions in $L^k(D^2)$ for $r\longrightarrow1$. This leads to
        \begin{gather}
            H^k := \left\{f\in L^k(D^2)\,\middle\vert\,\sup_{0\leq r<1}\left(\frac{1}{2\pi}\Int_0^{2\pi}\left|f(re^{i\theta})\right|^k\,d\theta\right)^{1/k}<+\infty\right\}\,.
        \end{gather}
        For $k\geq1$, the supremum in this definition also defines a norm $\|\cdot\|_{\mathcal{H}^k}$ Given a function $f\in\mathcal{H}^p(S^1)$, one can recover its representative $\widetilde{f}\in\mathcal{H}^k(D^2)$ by convolution with the \textbf{Poisson kernel}:
        \begin{gather}
            \widetilde{f}\left(re^{i\varphi}\right) = \frac{1}{2\pi}\Int_0^{2\pi}P_r(\varphi-\theta)f\left(e^{i\theta}\right)\,d\theta\,,
        \end{gather}
        where
        \begin{gather}
    ...
    \newdef{Toeplitz operator}{\index{Toeplitz!operator}
        Consider the projection $\pi_{\mathcal{H}}:L^2(S^1)\twoheadrightarrow\mathcal{H}^2(S^1)$ and let $M_f$ denote the multiplication operator induced by $f\in L^2(S^1)$. The Toeplitz operator with \textbf{symbol} $f$ is given by
        \begin{gather}
            T_f := \pi_{\mathcal{H}}\circ M_f|_{\mathcal{H}^2(S^1)}\,.
        \end{gather} 
    }

    \begin{property}
        A (bounded) linear map on $\mathcal{H}^p(S^1)$ is Toeplitz if and only if its matrix representation in the basis $(z^n)_{n\in\mathbb{N}}$ is a \textit{Toeplitz matrix} (see \cref{linalgebra:toeplitz_matrix}).
    \end{property}

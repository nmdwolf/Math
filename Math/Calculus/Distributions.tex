...
\subsection{Wave front sets}
    ...
    \newdef{Singular fibre}{\index{cut-off}
        ...
        This definition says that the singular fibre of a distribution at some point contains those directions such that the Fourier distribution (localized around the point) becomes singular in the given direction, hence, indicates that the function is not smooth along this direction.
    }
    \begin{property}
...
\section{\difficult{Analysis on groups}}
    ...
    \newdef{Haar measure}{\index{Haar measure}
        A left (resp.~right) Haar measure on a topological group is a regular Borel measure (\cref{measure:regular_measure}) that is finite on compact subsets and invariant under the left (resp.~right) group action. For locally compact groups, this is a Radon measure (\cref{measure:radon_measure}).
    }

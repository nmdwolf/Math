...
\subsection{Measurable functions}
    ...
    \newdef{Measure-preserving function}{\label{measure:measure_preserving}
        ...
        \begin{gather}
            T_\ast\mu = \mu\,.
        \end{gather}
        These functions form the morphisms in the category $\symbfsf{Meas}$ of measure spaces.
    }
...
\subsection{Limit operations}
    ...
    \begin{remark}[\difficult{Functional weak convergence}]\index{Stone--$\check{C}$ech compactification}
...
\subsection{Polish spaces}
    ...
    The content of this section heavily relies on that of \namecrefs{chapter:topology}~\nameref{chapter:topology} and~\nameref{chapter:metric}.
...
\subsection{\texorpdfstring{$L^p$}{Lp}-spaces}
    ...
    \end{formula}
    \begin{property}
        Equipped with the above norm the space $L^\infty$ becomes a \textit{Banach space} (see \cref{section:banach}).
    \end{property}

    \newdef{Weak type operator}{\index{weak!type}\label{measure:weak_type}
        Consider a function $T:L^p(X,\mu)\rightarrow L^q(Y,\nu)$. It is said to be of weak type-$(p,q)$ if it satisfies the following estimate: There exists a constant $C>0$ such that
        \begin{gather}
            \nu\bigl(\|Tf\|_q>\lambda\bigr)\leq C\left(\frac{\|f\|_p}{\lambda}\right)^q\,.
        \end{gather}
        It is said to be of strong type-$(p,q)$ if it is Lipschitz continuous.
    }

    \newdef{Hardy--Littlewood maximum function}{\index{Hardy--Littlewood maximal function}
        Consider the following function on (locally) integrable functions $f\in L^1_{\text{loc}}(\mathbb{R}^n)$:
        \begin{gather}
            Mf(x) := \sup_{r>0}\frac{1}{\lambda^n\bigl(B(x,r)\bigr)}\Int_{B(x,r)}|f(y)|\,dy\,,
        \end{gather}
        where $\lambda^n$ is the Lebesgue measure.
    }
    \begin{property}[Hardy--Littlewood maximal inequality]\label{measure:hardy_littlewood_inequality}
        The Hardy--Littlewood function is of weak (and even strong) type-$(1,1)$. Moreover, the constant in these estimates does not depend on the dimension $d\in\mathbb{N}$.
    \end{property}

    The following result should be compared to \cref{calculus:first_fundamental_theorem}.
    \begin{result}[Lebesgue differentiation theorem]\index{Lebesgue!differentiation theorem}\index{}
        Consider $f\in L^1_{\text{loc}}(\mathbb{R}^n)$. For almost all $x\in\mathbb{R}^n$, the following equality holds:
        \begin{gather}
            f(x) = \lim_{r\rightarrow0}\frac{1}{\lambda^n\bigl(B(x,r)\bigr)}\Int_{B(x,r)}f(y)\,dy\,.
        \end{gather}
    \end{result}
...
\section{Set-valued integral}

    For an introduction to hyperspaces, see \cref{section:hyperspace}.
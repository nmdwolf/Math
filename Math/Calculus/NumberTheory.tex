...
\section{Algebraic number theory}
    ...
    \newdef{Gaussian integers}{\index{integer!Gaussian}
        The integral domain (\cref{algebra:integral_domain}) of the form $\mathbb{Z}[i]$. The only units of this ring are $\pm1$ and $\pm i$.
    }

    \begin{theorem}[Prime factorization]\index{prime!factorization}\label{number:prime_factorization}
        Every integer $n\geq2$ can be factorized in a unique way (up to permutation of the factors) as the product of prime numbers.\footnote{1 is excluded since it is not a prime number.}
    \end{theorem}
    
    The following poroperty is related to the freshman's dream~\ref{algebra:freshmans_dream}.
    \begin{property}
        An integer $n\geq2$ is prime if and only if the following equation holds for all $0<k<n$:
        \begin{gather}
            {n \choose k}=0\bmod n\,.
        \end{gather}
    \end{property}
    ...
    Recall \cref{complexcalculus:upper_half_plane} of the Poincar\'e half plane $\mathfrak{h}$.
    ...
    \begin{theorem}[Gauss]\index{Gauss}
        For every $n\in\mathbb{N}$, the integer $8n+3$ is the sum of three odd squares.
    \end{theorem}
...
\subsection{Rational numbers}
    ...
    \begin{theorem}[Ostrowski]\index{Ostrowski}\label{number:ostrowski}
    ...
    \newdef{Id\`eles}{\index{id\`ele}
        The group of units of the ad\`ele ring: $\mathbb{I}_{\mathbb{Q}}:=\mathbb{A}^\times_{\mathbb{Q}}$.
    }
...
\subsection{Number and global fields}
    ...
    \end{gather}
    Moreover, taking $\mathbb{Q}_\infty:=\mathbb{R}$ and recalling Ostrowski's theorem~\ref{number:ostrowski}, one has
    \begin{gather}
        \mathbb{A}_{\mathbb{Q}}\cong\prod'_{p\in\mathrm{places}(\mathbb{Z})}\mathbb{Q}_p\,.
    \end{gather}

    Now, let $\mathfrak{K}$ be a number field (\cref{algebra:number_field}) and consider the following notations:
    \begin{itemize}
    ...
    \end{gather}
    and the \textbf{ad\`ele ring} is again defined as its `rationalization':
    \begin{gather}
    ...
    \newdef{Global field}{\index{field!global}
        A number field or a function field (\cref{alggeom:coordinate_ring}) over a finite field. Equivalently, a finite extension of the rational numbers $\mathbb{Q}$ or the rational functions with coefficients in a finite field.
    }

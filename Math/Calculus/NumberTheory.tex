...
\section{Algebraic number theory}
    ...
    \newdef{Jacobi $\theta$-function}{\index{Jacobi!$\theta$-function}\index{nome}
        ...
        \begin{gather}
            q = \exp(i\pi\tau)
        \end{gather}
        for some $\tau\in\mathbb{C}$ with $\Im(\tau)>0$. The parameter $q$ is often called the \textbf{nome}.

        The general Jacobi $\theta$-function is given by
        \begin{gather}
...
\section{Elliptic functions}
    ...
    \newdef{Elliptic curve}{\index{elliptic!curve}
        Consider a lattice of the form $\Gamma_\tau:=\mathbb{Z}+\tau\mathbb{Z}$ for some $\tau\in\mathfrak{h}$, where $\mathfrak{h}$ is the Poincar\'e upper half plane (\cref{complexcalculus:upper_half_plane}). An elliptic curve is a quotient $ \mathbb{C}/\Gamma_\tau$.
    }
    \begin{property}
    ...
    \newdef{Elliptic function}{\index{elliptic!function}
        A meromorphic function on $\mathbb{C}$ that is periodic with respect to a lattice $\Gamma_\tau$.
    }
...
\subsection{Modular forms}\label{section:modular_forms}
    ...
    \begin{remark}[Fourier expansion]
        Since $\begin{pmatrix}1&1\\0&1\end{pmatrix}$ is an element of $\mathrm{PSL}(2,\mathbb{2})$, every modular form is periodic and, hence, admits a \textit{Fourier expansion} (see \cref{distributions:fourier_series}):
        \begin{gather}

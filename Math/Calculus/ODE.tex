...
\section{First-order ODEs}
    ...
    \begin{formula}\label{ode:first_order_general_solution}
        ...
        \begin{gather}
            \varphi(t) = e^{-\int\!a(t)\,dt}\left(c + \int\!R(t)e^{\int\!a(s)\,ds}\,dt\right)\,,
        \end{gather}
...
\subsection{General solution}
    ...
    \begin{formula}\label{ode:second_order_general_solution}
        Let $\varphi:U\rightarrow\mathbb{R}$ be a nowhere zero solution of the homogeneous equation. The general solution of \eqref{ode:second_order_ODE} is given by
        \begin{gather}
            y(t) = c_1\varphi + c_2\varphi\int\frac{e^{-\int\!a(s)\,ds}}{\varphi^2(t)}\,dt + \psi_0,
        \end{gather}
        where $\psi_0$ is a particular solution of \eqref{ode:second_order_ODE}.
    \end{formula}

    \begin{property}
        Let $\psi_0$ be a solution of \eqref{ode:second_order_ODE}. The set of all solutions is given by the affine space
        \begin{gather}
            \big\{\psi_0 + \chi\,\big\vert\,\chi\text{ is a solution of the homogeneous equation}\big\}.
        \end{gather}
    \end{property}
    \begin{property}[Wronskian]\index{Wronskian}
...
\subsection{Method of Frobenius}\index{Frobenius!series}
    ...
    \newdef{Indicial equation}{\index{indicial equation}
        After inserting the ansatz \eqref{ode:frobenius_power_series} into the homogeneous equation and collecting all terms in $x^i$, an equation of the form $\sum_{i=n}^\infty H_i(k)x^i=0$ is obtained, where $n\in\mathbb{N}$ and the $H_i(k)$ are polynomials in $k$. This means that for every $i\in\mathbb{N}$, one obtains an equation of the form $H_i(k) = 0$, due to the independence of polynomial terms. The equation for the lowest-degree term will be quadratic in $k$ and is called the indicial equation.
    }
    ...
    \begin{theorem}[Fuchs]\index{Fuchs}
        If the functions $a,b:U\rightarrow\mathbb{R}$ in \eqref{ode:second_order_ODE} are analytic at $x=x_0$, the general solution can be expressed as a Frobenius series.
    \end{theorem}
...
\section{Sturm-Liouville theory}\index{Sturm-Liouville theory}
    ...
    \begin{property}
        The eigenfunctions corresponding to distinct eigenvalues are orthogonal with respect to the weight function $r:[a,b]\rightarrow\mathbb{R}$. This can be seen as an instance of Property \ref{linalgebra:diagonalizable_hermitian}.
    \end{property}
...
\subsection{Bessel functions}\index{Bessel!function}\index{Neumann!function}
    ...
    \end{property}
    \remark{For $n\not\in\mathbb{N}$ the limit in \eqref{ode:neumann_function} is not necessary since $\sin(n\pi)$ will never become 0 in this case.}

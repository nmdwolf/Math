...
\section{General linear equations}
    ...
    \newdef{Characteristic curve}{\index{characteristic!curve}
        A curve along which the highest-order partial derivatives is discontinuous. Equivalently, a curve along which the partial differential equation reduces to an ordinary differential equation.
    }
...
\section{First order PDE}
    ...
    \newformula{Characteristic curve}{
        Nonunique highest-order partial derivatives means that the following \textit{Pfaffian} system has no unique solution:
        \begin{gather}
            \begin{cases}
                P\pderiv{u}{x}+Q\pderiv{u}{y} = R\\
                \pderiv{u}{x}dx+\pderiv{u}{y}dy = du.
            \end{cases}
        \end{gather}
        Algebraically this gives the following condition:
        \begin{gather}
            \det
            \begin{pmatrix}
                P&Q\\
        ...
        \begin{gather}
            \frac{dx}{P} = \frac{du}{R}
        \end{gather}
        should hold as a consistency condition.
    }

    \begin{formula}[Lagrange-Charpit equations]\index{Lagrange-Charpit equations}
        The general solution of \eqref{pde:first_order_pde} is implicitly given by $F(\xi,\eta) = 0$, where $F(\xi,\eta)$ is an arbitrary differentiable function and $\xi(x,y,u) = c_1,\eta(x,y,u) = c_2$ are solutions of the Lagrange-Charpit equations:
        \begin{gather}
...
\section{Method of the characteristics}\label{section:characteristics}
    ...
    \newformula{Characteristic equation}{\index{characteristic}
        ...
        \end{gather}
        Accordingly this equation is often called the \textbf{characteristic equation} of the PDE \ref{pde:general_2order_pde}. The PDE has been reduced to an ODE (as before, Cramer's rule gives rise to additional ODEs).
    }
    ...
    \newformula{Canonical form}{\index{characteristic}
        Consider the general change of variables \[\xi = \xi(x,y)\qquad\eta = \eta(x,y)\qquad\zeta\equiv u.\] After this transformation, the PDE \ref{pde:general_2order_pde} becomes
        \begin{gather}

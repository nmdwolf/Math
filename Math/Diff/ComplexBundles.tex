...
\subsection{Riemann surfaces and orbifolds}
    ...
    \begin{example}[Riemann sphere]\label{complex:riemann_sphere}
        The sphere $S^2$ admits the structure of a Riemann surface, $\mathbb{CP}^1$. The automorphism group of $\mathbb{CP}^1$ is the M\"obius group $\mathrm{PSL}(2,\mathbb{C})$ from Definition \ref{alggeom:modular_group}. It can be proven that given any two triples of points, there exists a unique M\"obius transformation mapping them onto each other.
    \end{example}

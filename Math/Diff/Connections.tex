...
\subsection{Local representations}
    ...
    \begin{example}[General linear group]
        Let $G=\GL(n,\mathbb{R})$. The second term in \cref{bundle:local_compatibility} can be written as follows:\footnote{A derivation can be found in Lecture 22 of~\citet{schuller_lectures_2016}.}
        \begin{gather}
...
\subsection{Parallel transport}
    ...
    \begin{example}
        ...
        \begin{gather}
            g(t) = \left[\mathbbm{1} - \bigintsss_0^tA(\gamma'(t_1))\,dt_1 + \bigintsss_0^t\!\!\bigintsss_0^{t_1}A(\gamma'(t_1))A(\gamma'(t_2))\,dt_2\,dt_1-\cdots\right]g(0)\,,
        \end{gather}
        ...
        \begin{gather}
            g(t) = \left[\mathbbm{1} - \bigintsss_0^t(\gamma'(t_1))\,dt_1 + \frac{1}{2!}\bigintsss_0^t\!\!\bigintsss_0^t\mathcal{T}\Big(A(\gamma'(t_1))A(\gamma'(t_2))\Big)\,dt_2\,dt_1-\cdots\right]g(0).
        \end{gather}
        By noting that this formula is equal to the path-ordered exponential, one finds
        \begin{gather}
            \label{bundle:g0_to_gt}
            g(t) = \mathcal{T}\exp\left(-\bigintsss_0^tA(\gamma'(t'))\,dt'\right)g(0)\,.
        \end{gather}
    \end{example}
...
\section{Reduction of the structure group}\label{section:G-structure}
    ...
    \begin{property}[\difficult{Flat connections}]\label{bundle:flat_connection_cohomology}
        ...
        \begin{gather}
            \mathrm{hol}:H^1_{\text{flat conn}}(M;G)\cong\mathbf{Grp}\bigl(\pi_1(M),G\bigr)/G\,,
        \end{gather}
        where $H^1_{\text{flat conn}}(M;G):=\pi_0\mathbf{H}(M,\flat\mathbf{B}G)$ is the cohomology set (\cref{topos:cohomology}) of \textit{differentiable stacks} and $\flat$ is the flat modality (\cref{topos:cohesive_modalities}). By the adjunctions of cohesive modalities, this classifications is equivalent to giving a morphism
        \begin{gather}
...
\subsection{Complex bundles}
    ...
    \newdef{Theta characteristic}{\index{characteristic!$\theta$}
        Consider a smooth manifold $M$ together with its canonical class $K_M$. The theta characteristic, if it exists, is a characteristic class $\Theta$ such that $\Theta\cup\Theta=K_M$, where $\cup$ is the cup-product (\cref{bundle:cup_product}).
    }
...
\subsection{Deligne cohomology}
    ...
    \newdef{Deligne complex}{
        ...
        \end{gather}
        where $\check{H}^\bullet$ denotes \v{C}ech cohomology (\cref{sheaf:cech}) and the cochain complex $\mathbf{B}^k\mathrm{U}(1)_{\text{conn}}$ is turned into a chain complex by inverting the degrees.
    }
...
\section{\texorpdfstring{$\mathcal{D}$-geometry $\clubsuit$}{D-geometry}}
    ...
    \newdef{Sheaf of differential operators}{
        ...
        Let $M$ be a smooth manifold (or scheme, see \cref{chapter:alggeom}) and denote its structure sheaf by $\mathcal{O}_M$. $\mathcal{D}_M$ denotes the sheaf of $\mathcal{O}_M$-algebras of vector fields (interpreted as derivations) on $M$:
        \begin{gather}

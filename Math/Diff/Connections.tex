...
\subsection{Local representations}
    ...
    \begin{example}[Christoffel symbols]\index{Christoffel symbols}
        Let $\Gamma^i_{\ j\mu},\overline{\Gamma}^k_{\ l\nu}$ be the Yang-Mills fields corresponding to a connection on the frame bundle of some manifold $M$, where the sections are induced by a choice of coordinates ($x^i$ and $y^i$, respectively). In this case, the expansion coefficients of the Yang-Mills field are called the \textbf{Christoffel symbols} (compare to Definition \ref{bundle:christoffel_symbol}). Using Equations \eqref{bundle:local_compatibility} and \eqref{bundle:mc_pullback} this becomes:
        \begin{gather}
...
\subsection{Parallel transport}
    ...
    \begin{example}
        ...
        \end{gather}
        where $A$ is the Yang-Mills field associated to the local section $\sigma$. This can be rewritten using the standard Dyson trick \ref{perturbation:dyson_series} as
        \begin{gather}
...
\subsection{Koszul connections}
    ...
    \newdef{Covariant derivative}{\index{covariant!derivative}
        ...
        \end{gather}
        where $\gamma$ is any curve satisfying $\gamma(0) = x_0$ and $\gamma'(0) = X(x_0)$.
...
\subsection{Exterior covariant derivative}\label{section:exterior_covariant_derivative}
    ...
    \begin{construct}\index{covariant!derivative}\label{bundle:covariant_derivative_associated_bundle}
        Let $\prb$ be a principal bundle and let $P_V := P\times_G V$ be an associated vector bundle. Given a section $\sigma:M\rightarrow P_V$, one can construct a $G$-equivariant map $\phi:P\rightarrow V$ using \ref{bundle:section_bijection_phi}. The exterior covariant derivative of $\phi$ is given by Equation \eqref{bundle:derivative_horizontal_form}:
        \begin{gather}
...
\subsection{Curvature}
    ...
    \begin{property}[Integrability]
        Let $\omega$ be a principal connection one-form. The associated horizontal distribution \ref{bundle:distribution} \[p\mapsto\mathrm{Hor}(T_pP)\] is integrable if and only if the connection $\omega$ is flat. In contrast, the vertical distribution is always integrable.
    \end{property}
...
\subsection{Torsion}
    ...
    \newdef{Torsion}{\index{torsion}\index{Cartan!structure equation}
        ...
        \end{gather}
        where the wedge product is defined analogously to \ref{bundle:vector_valued_wedge} and \ref{bundle:lie_algebra_valued_wedge} using the induced representation of $\mathfrak{g}$ on $V$:
        \begin{gather}
...
\section{Reduction of the structure group}\label{section:G-structure}
    ...
    \begin{example}[Orientable manifold]\index{orientation}\label{bundle:orientable_structure}
...
\subsection{Complex bundles}
    ...
    \newdef{Theta characteristic}{\index{theta characteristic}
        Consider a smooth manifold $M$ together with its canonical class $K_M$. The theta characteristic, if it exists, is a characteristic class $\Theta$ such that $\Theta\smile\Theta=K_M$ where $\smile$ is the cup-product in cohomology \ref{bundle:cup_product}.
    }

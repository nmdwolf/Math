...
\section{Infinite-dimensional geometry}\label{section:infinie_dimensional}
    ...
    \newdef{$E$-manifold}{\index{manifold}
        Let $E$ be a locally convex TVS. A Hausdorff space is called an $E$-manifold if there exists an atlas of charts $(U,\varphi)$, where $\varphi:U\rightarrow\varphi(U)\subset E$ is a homeomorphism and the transition maps are Gateaux-smooth.
    }
    ...
    \newdef{Kinematic tangent bundle}{\index{tangent!bundle}
        ...
        \end{gather}
        by the equivalence relations $(x,w)\sim\big(x,\dr\psi_{ji}(\varphi_i(x);v)\big)$.
    }
    \begin{remark}
...
\subsection{Concrete sites}
    ...
    \newdef{Diffeological space}{\index{diffeology}\index{plot}
        ...
        \begin{enumerate}
            \item $\mathcal{D}$ contains all functions $f:\mathbb{R}^0\rightarrow X$ and all constant functions.
        ...
        \end{enumerate}
        The set $X$ can be turned into a topological space by equipping it with the \textbf{$\mathcal{D}$-topology}, the final topology with respect to $\mathcal{D}$.
    }
    ...
    \newdef{Smooth map}{\index{smooth!function}
        ...
        Let $(X,\mathcal{D})$ and $(Y,\mathcal{D}')$ be diffeological spaces. A map $g:X\rightarrow Y$ is said to be smooth if for every $f\in\mathcal{D}$ the composite $g\circ f\in\mathcal{D}'$. The diffeological spaces together with their differentiable morphisms form a category $\mathbf{DiffSp}$.
    }
...
\subsection{Supergeometry}
    ...
    \newdef{Superscheme}{\index{scheme!super-}
        The category of affine superschemes is defined as the opposite of the category of supercommutative superalgebras, i.e.~the commutative monoids internal to $\mathbf{sVect}$. More generally, one can define affine schemes internal to any symmetric monoidal category.
    }
    \begin{property}
        The category $\mathbf{CAlg}$ is a full subcategory of $\mathbf{sCAlg}$. Moreover, this inclusion is part of an adjoint triple, where the left and right adjoints are given by projection onto the even part and quotienting by the odd part.
    \end{property}
    ...
    \newdef{Reduction}{\index{reduction}\index{infinitesimal}
        Given an infinitesimally thickened space $\mathbb{R}^n\times\mathbb{D}$, its reduction $\mathfrak{R}$ is defined to be $\mathbb{R}^n$. Every reduction induces a canonical morphism $\mathbb{R}^n\hookrightarrow\mathbb{R}^n\times\mathbb{D}$. Plots can be reduced by precomposing with a reduction morphism.
...
\subsection{Graded manifolds}
    ...
    \begin{example}[de Rham differential]
        ...
        \end{gather}
        Note that the differentials $\drx^i$ are here regarded as coordinate functions on $\Pi TM$. The \textbf{degree} of a homogeneous element of $\Omega^\bullet(M)$ is defined as the difference of its graded degree and its form degree. The de Rham complex itself then corresponds to the algebra $C^\infty(\Pi TM)$.
    \end{example}
...
\subsection{Spaces}
    ...
    The following definition generalized Definition \ref{alggeom:algebraic_space}:

...
\subsection{Definitions}
    ...
    \begin{example}[Lie algebra of LIVFs]
        Consider the vector space $\mathfrak{X}^L(G)$ of LIVFs on a Lie group $G$. Using Property \ref{bundle:lie_bracket} one can show that the commutator (Lie bracket) also defines a LIVF on $G$. It follows that $\mathfrak{X}^L(G)$ is closed under Lie brackets and, hence, is a Lie algebra.
    \end{example}
    ...
    \newadef{Lie algebra of Lie group}{
        Let $G$ be a Lie group. The tangent space $\mathfrak{g}:=T_eG$ has the structure of a Lie algebra where the Lie bracket is induced by the commutator of vector fields \ref{bundle:lie_bracket} in the following way:
        \begin{gather}
...
\subsection{Lie algebras}
    ...
    \begin{result}[Commutator]\index{commutator}
        ...
        \begin{gather}
            [A,B] := AB-BA.
        \end{gather}
        This follows from \eqref{lie:bracket_as_adjoint_rep}: $[A,B] = \left.\deriv{}{t}\mathrm{Ad}_{\gamma(t)}(B)\right|_{t=0}$ with $\gamma(0) = e$ and $\gamma'(0) = A$.
    \end{result}
...
\subsection{Killing form}
    ...
    \newdef{Killing form}{\index{Killing!form}\index{Cartan-Killing|see{Killing form}}\label{lie:killing_form}
        ...
        \begin{gather}
            K:\mathfrak{g}\otimes\mathfrak{g}\rightarrow K:(x,y)\mapsto\mathrm{tr}(\mathrm{ad}_x\circ\mathrm{ad}_y),
        \end{gather}
        where the codomain $K$ is the underlying field. The trace can be calculated by choosing a (finite-dimensional) representation of the Lie algebra using Ado's theorem \ref{lie:ado}.
...
\subsection{Weights, roots and Dynkin diagrams}
    ...
    \newdef{Weyl group}{\index{Weyl!group}
        For every simple root $\lambda$ one can construct a Householder transformation $\sigma_\lambda$ (Definition \ref{linalgebra:householder_transformation}) as follows:
        \begin{gather}
            \sigma_\lambda:\mathrm{span}_\mathbb{R}(\Delta)\rightarrow\mathrm{span}_\mathbb{R}(\Delta):\mu\mapsto\mu-2\frac{\langle\lambda,\mu\rangle}{\langle\lambda,\lambda\rangle}\lambda.
        \end{gather}
...
\subsection{Universal enveloping algebra}
    ...
    \begin{property}[Casimir invariants of irreducible representations]
        ...
        \end{gather}
        By taking the trace of this formula and using Equation \eqref{lie:rho_killing_form} it can be seen that $c_\rho = \frac{\dim\mathfrak{g}}{\dim V}$.
    \end{property}

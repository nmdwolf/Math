...
\subsection{Metric}
    ...
    \newdef{Hodge Laplacian\footnotemark}{\index{Hodge!Laplacian}\index{Laplace-de Rham|see{Hodge Laplacian}}\index{Hodge-de Rham operator}\label{riemann:hodge_laplacian}
        ...
        \end{gather}
        It should be noted that, in contrast to the differential $d$, the Hodge Laplacian depends on the metric through the definition of the codifferential. The square root of this operator is the \textbf{Hodge-de Rham operator} $d+\delta$.
    }
    ...
    \begin{remark}\label{bundle:vector_calculus}
        ...
        \begin{align}
            \nabla f &= df\,^\sharp\\
            \nabla\times X &= \left(\ast\,dX^\flat\right)^\sharp\\
            \nabla\cdot X &= \left(\ast^{-1}d\ast X^\flat\right)^\sharp,
        \end{align}
        Property \ref{vector:mixed_properties} now follows from the nilpotency $d^2=0$.\\
        \begin{proof}[Explanation]
            \begin{mdframed}[roundcorner=10pt, linecolor=blue, linewidth=1pt]
                ...
                For simplicity we will give a derivation in $\mathbb{R}^3$ with the Euclidean metric. Consider a vector field $X = (f_1,f_2,f_3)$ where the $f_i$ are smooth. Using these functions $f_i$ one can construct a one-form $X^\flat = f_1dx_1 + f_2dx_2 + f_3dx_3$ and a two-form $\ast X^\flat = f_1dx_2\wedge dx_3 + f_2dx_3\wedge dx_1 + f_3 dx_1\wedge dx_2$. After applying the exterior derivative one obtains
                \begin{align}
                    dX^\flat &= \left(\pderiv{f_3}{x_2} - \pderiv{f_2}{x_3}\right)dx_2\wedge dx_3 + \left(\pderiv{f_1}{x_3} - \pderiv{f_3}{x_1}\right)dx_3\wedge dx_1 + \left(\pderiv{f_2}{x_1} - \pderiv{f_1}{x_2}\right)dx_1\wedge dx_2,\nonumber\\
                    d\ast X^\flat &= \left(\pderiv{f_1}{x_1} + \pderiv{f_2}{x_2} + \pderiv{f_3}{x_3}\right)dx_1\wedge dx_2\wedge dx_3.\nonumber
                \end{align}
            \end{mdframed}
        \end{proof}
    \end{remark}
...
\subsection{Dirac operators}
    ...
    \newdef{Dirac operator}{\index{Dirac!operator}\label{riemann:dirac_operator}
        ...
        \begin{gather}
            \underline{D} := \sum_{i=1}^n\gamma_i\cdot\nabla^E_{e_i}\sigma,
        \end{gather}
    ...
    \newdef{Topological index}{
        ...
        \begin{align}
            \mathrm{ind}(D) :=& (-1)^{n(n+1)/2} \int_M\mathcal{T}^{-1}(\mathrm{ch}(\sigma(D)))\cup\mathrm{td}(T_\mathbb{C}M)\nonumber\\
            =& (-1)^n \int_{TM}\mathrm{ch}(\sigma(D))\cup\mathrm{td}(T_\mathbb{C}M),
        \end{align}

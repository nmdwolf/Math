...
\subsection{Stiefel-Whitney classes}\index{Stiefel-Whitney class}
    ...
    \newdef{Stiefel-Whitney classes}{
        The characteristic classes $w_n(E)\in H^\bullet(M;\mathbb{Z}_2)$ of a rank-$n$ vector bundle over a paracompact space $M$ (hence admitting an $\mathrm{O}(n)$-reduction) are defined by the following conditions:
        \begin{enumerate}
    ...
    \newdef{Integral classes}{
        Consider the Bockstein morphism $\beta:H^i(M;\mathbb{Z}_2)\rightarrow H^{i+1}(M;\mathbb{Z})$ associated to the reduction $\mathbb{Z}\rightarrow\mathbb{Z}_2$. The integral Stiefel-Whitney classes are defined as the images $\beta w_i\in H^{i+1}(M;\mathbb{Z})$.
    }

...
\subsection{Levi-Civita connection}
    ...
    \newformula{Divergence}{\index{divergence}\label{riemann:divergence}
        ...
        \begin{gather}
            \mathrm{div}(X) = \mathrm{tr}(Y\mapsto \nabla_YX)\equiv\nabla_\mu X^\mu,
        \end{gather}
        where $\mathrm{tr}$ denotes the contraction (or trace) induced by $g$. This makes it clear that the covariant divergence is indeed a good generalization of the divergence \ref{vector:divergence} from vector calculus.
...
\subsection{Spin structures}
    ...
    \newdef{Spin bundle}{\index{spin!bundle}\index{spinor}
        A vector bundle associated to a spin frame bundle. Sections of a spin bundle are called spinor fields.
    }
    ...
    ?? DEFINE STIEFEL-WITHNEY CLASSES ??

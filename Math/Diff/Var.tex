...
\subsection{Classical systems}
    ...
    \newdef{Noether symmetry}{\index{Noether!symmetry}
        Consider an integral quantity $I$ defined by a Lagrangian function $L(q,u)$:
        \begin{gather}
...
\section{\difficult{Variational bicomplex}}\label{section:variational_bicomplex}
    ...
    \begin{example}[Euler-Lagrange operator]\index{Euler-Lagrange!operator}\index{Lie-Euler operator}\index{on-shell}
        ...
        \begin{gather}
            P_{\mathbf{L}}(X) := \mathcal{L}_{j^\infty X}\mathbf{L} = \sum_{|I|=0}^k(D_IX^\alpha)(\partial^I_\alpha L)\mathrm{Vol}.
        \end{gather}
        The coefficients from \eqref{var:Q_forms} associated to this operator are given by the following formula (to turn these coefficients into true forms one should multiple by the volume form):
        \begin{gather}
        ...
        \begin{gather}
            \delta_\mathrm{EL}\mathbf{L} := E_\alpha(L)\delta u^\alpha\wedge\mathrm{Vol}.
        \end{gather}
    ...
    \begin{construct}
        ...
        \end{gather}
        An equivalent definition can be given by a formula similar to \ref{bundle:k_form_exterior_derivative}, where an evolutionary vector field $X$ acts on a degree-$0$ functional by Lie derivation:
        \begin{gather}
    ...
    \newdef{Source form}{\index{source!form}
        ...
        \begin{gather}
            \omega = \omega_\alpha(x,u,u_I)\delta u^\alpha\wedge\mathrm{Vol}.
        \end{gather}
    ...
    \begin{property}
        ...
        \begin{gather}
            w^{|I|} := \delta u^\alpha\wedge\left(A^I_{\alpha\beta}\delta u^\beta_I + D_I(A^I_{\beta\alpha}\delta u^\beta)\right)\wedge\mathrm{Vol}
        \end{gather}
    ...
    \newdef{Lepage form}{\index{Lepage equivalent}
        ...
        \begin{gather}
            \pi_{\dim(M),0}(X\intmul\mathbf{d}\rho)=0
        \end{gather}
        for all $\pi_{\infty,0}$-vertical vector fields $X$. Given a local Lagrangian $\mathbf{L}\in\Omega^{\dim(M),0}(J^\infty(E))$, $\mathbf{L}$ and $\rho$ are said to be Lepage equivalent if $\pi_{\dim(M),0}\rho = \mathbf{L}$.
    }
...
\subsection{Inverse problem}
    ...
    \begin{formula}[Local expression]
        Consider a source form that admits the local expression \[\Delta=P_\alpha\delta u^\alpha\wedge\mathrm{Vol}.\] The Helmholtz conditions can locally be expressed as follows:
        \begin{gather}
            (-1)^{|I|}\partial^I_\alpha P_\beta = E^I_\beta(P_\alpha),
        \end{gather}
        where $E^I_\beta$ are the Lie-Euler operators \eqref{var:lie_euler_operator}.
    \end{formula}

    \begin{example}[Volterra-Vainberg formula]\index{Volterra-Vainberg formula}
        If $E$ is trivial and $\Delta=F_\alpha\delta u^\alpha\wedge\mathrm{Vol}$ satisfies the Helmholtz conditions, then
        \begin{gather}
...
\subsection{Structure of the bicomplex}
    ...
    \begin{formula}[Local Lagrangians]
        For local Lagrangians $\mathbf{L}\equiv L\mathrm{Vol}_{E'}$, the projected pullback acts as follows:
        \begin{gather}
            \Phi^\sharp\mathbf{L} = (L\circ\Phi)\det(D_\mu f^\mu)\mathrm{Vol}_E,
        \end{gather}
    ...
    \begin{formula}[Functionally projected Lie derivative]
        ...
        \end{gather}
        If $X$ is an evolutionary vector field, one can replace $X_\mathrm{ev}$ by $X$ on the right-hand side. For projectable vector fields one can replace the left-hand side by the ordinary Lie derivative $\mathcal{L}_{j^\infty X}\omega$.
    \end{formula}
    ...
    In the remainder of this section the homological properties of the variational bicomplex over a local chart (or equivalently, over a trivial bundle) will be studied. To prove the acyclicity of the various subcomplexes the usual approach of finding a null-homotopy (see property \ref{homalg:null_acyclic}) will be followed, i.e. a cochain map will be found $h:C_\bullet\rightarrow C_\bullet$ such that
    \begin{gather}

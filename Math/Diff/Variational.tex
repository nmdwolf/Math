...
\section{Constrained systems}\label{section:variational_calculus}
    ...
    \begin{method}[Holonomic constraints]\index{Lagrange!multiplier}
        ...
        \begin{gather}
            I_\mathrm{ext}[q,\lambda]:=\int_a^b\Big[L(q,\dot{q},t) + \sum_{i=1}^k\lambda_i(t)f_i(q,t)\Big]dt,
        \end{gather}
        where the $\lambda_i$ are are called \textbf{Lagrange multipliers}. Extrimizing with respect to these multipliers induce the constraints:
        \begin{gather}
            \frac{\delta I_\mathrm{ext}}{\delta\lambda_i}=0.
        \end{gather}
    \end{method}
...
\subsection{Differential structure}
    ...
    \newdef{Local Lagrangian}{\index{Lagrange!function}\index{local!functional}\label{var:local_lagrangian}
        ...
        \begin{gather}
            \mathbf{L} = L\vol,
        \end{gather}
        where $L$ is a smooth function on the infinite jet bundle (the Lagrangian density). By its very nature this implies that $L$ locally only depends on partial derivatives up to some finite order.
    ...
    \begin{formula}[Cartan calculus]
        ...
        \begin{enumerate}
            \item Exterior derivatives:
                \begin{align}
                    dx^\mu &= \mathbf{d}x^\mu,\\
                    \delta u^\alpha_I &= \mathbf{d}u^\alpha_I - u^\alpha_{I\mu}dx^\mu.
                \end{align}
            \item Interior product:
                \begin{gather}
                    \{d,\iota_{j^\infty Y}\}_+ = 0.
                \end{gather}
            \item Lie derivatives:
                \begin{align}
                    ...
                    [d,\mathcal{L}_{j^\infty Y}] &= [\delta,\mathcal{L}_{j^\infty Y}] = 0.
                \end{align}
        \end{enumerate}
    \end{formula}
...
\subsection{Euler operators}
    ...
    \begin{property}[Local variational formula]\index{variational!formula}\label{var:local_variational_formula}
        ...
        \begin{gather}
            \mathcal{L}_{j^\infty X}\mathbf{L} = j^\infty X\intmul\delta_\mathrm{EL}(\mathbf{L}) + d(j^\infty X\intmul\gamma)
        \end{gather}
...
\subsection{Structure of the bicomplex}
    ...
    \begin{property}[Augmented horizontal complex is exact]
        ...
        \begin{gather}
            0\longrightarrow\Omega^{0,q}\overset{d}{\longrightarrow}\Omega^{1,q}\overset{d}{\longrightarrow}\cdots\overset{d}{\longrightarrow}\Omega^{\dim(M),q}\overset{I}{\longrightarrow}\mathcal{F}^q\overset{I}{\longrightarrow}0
        \end{gather}
        are given by the following formula
        \begin{gather}
    ...
    \begin{result}[Functional decomposition]\label{var:functional_decomposition}
        ...
        \begin{gather}
            \Omega^{p,q}(J^\infty(E)) = d\Omega^{p-1,q}(J^\infty(E))\oplus\mathcal{F}^q(J^\infty(E)),
        \end{gather}
    ...
    \begin{gather}
        0\longrightarrow\mathbb{R}\longrightarrow\Omega^{0,0}\overset{d}{\longrightarrow}\Omega^{1,0}\overset{d}{\longrightarrow}\cdots\overset{d}{\longrightarrow}\Omega^{\dim(M),0}\overset{\delta_\mathrm{EL}}{\longrightarrow}\mathcal{F}^1\overset{\delta_V}{\longrightarrow}\mathcal{F}^2\overset{\delta_V}{\longrightarrow}\cdots.
    \end{gather}
...
\subsection{Variational problems}
    ...
    \begin{property}[First variational formula]\index{variational!formula}
        ...
        \begin{gather}
            ...
            \delta\mathbf{L} = \delta_\mathrm{EL}\mathbf{L} + d\sigma.
        \end{gather}
        ...
        \begin{gather}
            \delta_\mathrm{EL}\mathbf{L} + d\sigma = 0.
        \end{gather}
    ...
    \begin{result}[Global variational formula]\label{var:general_variational_formula}
        ...
        \begin{gather}
            \mathcal{L}^\sharp_{j^\infty X}\mathbf{L} = X_\mathrm{ev}\intmul\delta_\mathrm{EL}(\mathbf{L}) + d\sigma
        \end{gather}
    ...
    \newdef{Bessel-Hagen symmetry}{\index{symmetry!Bessel-Hagen}\index{divergence!symmetry}\label{var:bessel_hagen}
        A Bessel-Hagen or \textbf{divergence} symmetry of a Euler-Lagrange form $\delta_\mathrm{EL}(\mathbf{L})$ is a generalized vector field $X$ such that
        \begin{gather}
    ...
    \newdef{Local conservation law}{\index{conservation law}
        ...
        \begin{gather}
            X\intmul\Delta = d\eta
        \end{gather}
        for some local $\eta\in\Omega^{\dim(M),0}(J^\infty(E))$. Corollary \ref{var:general_variational_formula} shows that to every \underline{global} (generalized) symmetry, there corresponds a \underline{global} conservation law.
    }
    \begin{remark}
...
\section{Partial differential equations}\index{PDE}
    ...
    In this section the content of Chapter \ref{chapter:pde} is generalized using the language of differential geometry.
...
\subsection{Algebraic formulation}
    ...
    \newdef{Exterior system}{\index{exterior!system}
        ...
        \begin{align}
            ...
            &\ \ \vdots\nonumber
        \end{align}
        These differential one-forms generate an ideal in $\Omega^\bullet(\Sigma_0)$ that represents the differential equation in that it relates the variables in the algebraic condition $f=0$ by a set of differential relations. It is a Pfaffian system \ref{bundle:pfaff} or, combined with the PDE $f=0$ and its differential consequences, it gives a characteristic system \ref{bundle:characteristic_system}. The associated distribution becomes integrable (sometimes called \textbf{completely integrable}) if and only if the ideal is a differential ideal by Frobrenius's theorem \ref{bundle:differential_frobenius}. The integral manifolds of the distribution then give a solution of the PDE.\index{integrable!completely}
    }
    ...
    \begin{formula}[Lagrange-Charpit equations]\index{Lagrange-Charpit equations}
        ...
        \begin{gather}
            \begin{cases}
                f&=0\\
                df&=\left(\pderiv{f}{x^\mu}+u_\mu\pderiv{f}{u}\right)dx^\mu + \pderiv{f}{u_\mu}du_\mu=0\\
                \theta&=0\\
                d\theta&=0.
            \end{cases}
        \end{gather}
        ...
        \begin{gather}
            \frac{dx^1}{\smallpderiv{f}{u_1}}=\cdots=\frac{dx^n}{\smallpderiv{f}{u_n}}=\frac{-du_1}{\smallpderiv{f}{x^1}+u_1(\smallpderiv{f}{u})}\hspace{0.2\textwidth}\\
            \hspace{0.3\textwidth}=\cdots=\frac{du_n}{\smallpderiv{f}{x^n}+u_n(\smallpderiv{f}{u})}=\frac{du}{\sum_\mu u_\mu(\smallpderiv{f}{u_\mu})}.\nonumber
        \end{gather}
    \end{formula}

    \newdef{Monge cone}{\index{Monge!cone}
        Consider the characteristic system above. The rank of the Pfaffian system is $2\dim(M)$. At every point $(x^\mu,u)\in M\times\mathbb{R}$, the solutions $(x^\mu,u,u_\mu)$ of the PDE determine a tangent plane to $M$. The $n$-parameter family of tangent planes obtained by varying the derivative coordinates $u_\mu$ gives rise to the Monge cone with apex $(x^\mu,u)$. A solution to the PDE is a hypersurface that is everywhere tangent to the Monge cones. This approach to finding a solution is called the \textbf{method of the characteristics}.
    }

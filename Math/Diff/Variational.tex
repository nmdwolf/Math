...
\section{Constrained systems}\label{section:variational_calculus}
    ...
    \begin{method}[Holonomic constraints]\index{Lagrange!multiplier}
        ...
        \begin{gather}
            I_{\text{ext}}[q,\lambda]:=\bigintsss_a^b\left[L(q,\dot{q},t) + \sum_{i=1}^k\lambda_i(t)f_i(q,t)\right]\,dt\,,
        \end{gather}
...
\section{Introduction}
    ...
    \newdef{Variational symmetry}{\index{symmetry!varaitional}
        ...
        \begin{gather}
            \bigintsss_{M'}L(q',u',u'_I)\,dq' = \bigintsss_ML(q,u,u_I)\,dq
        \end{gather}
        for arbitrary $M$.
    }
...
\subsection{Differential operators}
    ...
    \newadef{Differential operator}{\index{differential!operator}\label{var:differential_operator}
        ...
        This definition can easily be framed in terms of the jet comonad, i.e.~a the category of differential operators over a base space $B$ is equivalent to the (co)Kleisli category (\cref{cat:kleisli_category}) of the jet comonad $J^\infty$.
    }
...
\subsection{Differential structure}
    ...
    \newdef{Local Lagrangian}{\index{Lagrange!function}\index{local!functional}\label{var:local_lagrangian}
        ...
        \begin{gather}
            S:\phi\mapsto\bigintsss_M(j^\infty\phi)^*\mathbf{L}\,.
        \end{gather}
...
\subsection{Variational problems}
    ...
    If the source form comes from a local Lagrangian, the above definition admits a specific case by \cref{var:projected_derivative_EL_operator} and the fact that $\delta_\mathrm{EL}$ annihilates $d$-exact forms (\cref{var:EL_operator_properties}).

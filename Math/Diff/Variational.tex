...
\subsection{Topology}
    ...
    \begin{property}[Jet comonad]\index{jet!comonad}
        The assignment of jet bundles constitutes a comonad: $J^\infty:\mathbf{Bundle}(B)\rightarrow\mathbf{Bundle}(X)$. The counit sends the prolongation of a section to the section evaluated at a point. The comultiplication reshuffles derivatives.
    \end{property}
...
\subsection{Differential operators}
    ...
    \newadef{Differential operator}{\index{differential!operator}\label{var:differential_operator}
        ...
        This definition can easily be framed in terms of the jet comonad, i.e.~a the category of differential operators over a base space $B$ is equivalent to the (co)Kleisli category~\ref{cat:kleisli_category} of the jet comonad $J^\infty$.
    }

    \begin{property}[Formal adjoints]\index{adjoint!operator}\label{var:formal_adjoint}
        Consider two differential operators
        \begin{gather}
            D,D^\dagger:\Gamma(E)\rightarrow\Gamma\big(E^*\otimes\Lambda^{\dim(M)}(M)\big)\,.
        \end{gather}
...
\subsection{Differential structure}
    ...
    \newdef{Evolutionary vector field}{\index{evolutionary vector field}
        ...
        \end{gather}
        Furthermore, by writing out Cartan's magic formula \ref{bundle:cartan_magic_formula} (with respect to $\mathbf{d}$), one can prove that the prolongation of an evolutionary vector field also satisfies this formula with respect to $\delta$ and that $\iota_{j^\infty X}$ and $\dr$ anticommute.
    }

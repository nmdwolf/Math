...
\section{Frobenius' theorem}
    ...
    \newdef{Foliation}{\index{foliation}\index{dimension!of foliation}\label{bundle:foliation}
        Let $M$ be an $n$-dimensional manifold. A foliation of $M$ is a decomposition
        \begin{gather}
            M = \bigsqcup_{j\in I}V_j
        \end{gather}
        by connected subsets $V_j\subset M$ such that there exists a cover $\{(U_i,\varphi_i)\}_{i\in I}$ of $M$ and an integer $p\leq n$, called the \textbf{dimension} of the foliation, such that for every $i\in I$ and $j\in J$ the intersection $V_j\cap U_i$ is given by the following system of equations:
        \begin{gather}
            \begin{cases}
                \phi_i^{p+1}(x) &= c_1\,,\\
                &\,\vdots\\

...
\section{Vector bundles}
    ...
    \begin{example}[Line bundle]\index{line!bundle}\index{wave!function}
        A vector bundle with a one-dimensional fibre. A common example in quantum mechanics is the $\mathbb{C}$-line bundle over some smooth manifold whose sections correspond to the wave functions of a given system. (See \cref{section:geometric_quantization} on geometric quantization for more information.)
    \end{example}
...
\subsection{Integral curves}
    ...
    \newdef{Flow}{\index{flow}\label{bundle:flow}
        ...
        \begin{gather}
            D(X) := \bigl\{(t,p)\in\mathbb{R}\times M\bigm\vert p\in M,t\in\ ]a_p,b_p[\bigr\}\,,
        \end{gather}
...
\subsection{Lie derivative}
    ...
    \begin{property}\index{Lie!bracket}\label{bundle:lie_bracket}
...
\subsection{Exterior derivative}
    ...
    \remark{\Cref{bundle:function_derivative} should be compared with the (informal) formula for the differential of a function that is often used in physics. The main difference is that here the quantities $dx^i$ are not infinitesimal quantities but vectors of unit norm.}
...
\subsection{Lie derivative}
    ...
    \begin{formula}[Lie derivative of smooth functions]
        Using definition~\eqref{bundle:function_derivative} of the exterior derivative of smooth functions and definition~\ref{bundle:dual_basis} of the dual basis, one can rewrite the Lie derivative~\ref{bundle:lie_derivative_functions} as follows:
        \begin{gather}
            Xf(p) = \dr f_p(X(p))\,.
        \end{gather}
    \end{formula}

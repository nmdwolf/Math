...
\section{Cohesion}\index{cohesion}\label{section:cohesion}
    ...
    \newdef{Local topos}{\index{local!topos}\label{topos:local_topos}
        An $\mathcal{S}$-topos $\mathbf{H}$ is said to be ($\mathcal{S}$-)local if the geometric morphism $(f^*\dashv f_*):\mathbf{H}\leftrightarrows\mathcal{S}$ admits a right adjoint $f^!$ such that one of the following equivalent statements holds:
        \begin{itemize}
    ...
    \newdef{Jet comonad}{\index{jet!comonad}
        ...
        The (co)algebras over the jet comonad correspond to common objects in geometry. The Eilenberg--Moore category $\mathbf{H}^{J^\infty}$ corresponds to PDEs with variables in $B$ and the Kleisli category $\mathbf{Kl}(J^\infty)$ corresponds to bundles over $B$ equipped with a differential operator.
    }

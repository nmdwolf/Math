...
\section{Lagrangian submanifolds}
    ...
    \begin{example}[Closed sections]\label{symplectic:closed_section_submanifold}
        Consider a closed section of a cotangent bundle $T^*M$, i.e.~a map $\sigma:M\rightarrow T^*M$ such that $\dr\sigma=0$. The graph of $\sigma$ is a Lagrangian submanifold. In particular, this implies that the graph of the differential $\dr f$ of any smooth function $f\in C^\infty(M)$ is a Lagrangian submanifold of $T^*M$.
    \end{example}
    \begin{property}
        Consider a Lagrangian submanifold $L\subseteq T^*M$. If $\pi|_L$ is a diffeomorphism, then $L=\im(\sigma)$ for some closed $\sigma\in\Omega^1(M)$.
    \end{property}
    ...
    \newdef{Exact Lagrangian}{\index{exact!Lagrangian}
        A Lagrangian submanifold for which the Liouville class vanishes, i.e.~on which the Liouville form trivialises.
    }

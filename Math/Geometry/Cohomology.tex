...
\subsection{de Rham complex}\index{de Rham!complex}
    ...
    \begin{theorem}[de Rham]\index{de Rham}\label{bundle:de_rham}
...
\subsection{\difficult{Equivariant cohomology}}\index{equivariant!cohomology}

    The de Rham theorem above can be generalized to the setting of equivariant cohomology (\cref{section:equivariant_cohomology}).
   
    \newdef{Weil model}{\index{Weil!model}
        Consider a smooth manifold $M$ with a smooth $G$-action and let $W(\mathfrak{g})$ be the Weil algebra (\cref{lie:weil_algebra}) of $G$. Construct the dgca $\Omega^\bullet(M)\otimes W(\mathfrak{g})$ with differential $\dr_\text{dR}+\dr_\text{W}$. The infinitesimal $G$-action gives a map $\mathfrak{g}\rightarrow\mathfrak{X}(M)$, so Cartan calculus can be extended to all of $\Omega^\bullet(M)\otimes W(\mathfrak{g})$. The space of \textbf{basic differential forms} is defined as the kernel of the Cartan operators $\iota_\xi,\mathcal{L}_\xi$ for all $\xi\in\mathfrak{g}$.
        
        The subcomplex, with the induced differential $\dr_\mathrm{dR}+\dr_\text{W}$, is called the \textbf{Weil model} of equivariant de Rham cohomology.
    }

    \newdef{Cartan model}{\index{Cartan!model}\index{Mathai--Quillen isomorphism}
        Consider the dgca $\Bigl(\Omega^\bullet(M)\otimes\mathfrak{g}^*[1]\Bigr)^G$ of $G$-invariant differential forms taking value in $\mathfrak{g}^*[1]$ with differential $\dr=\dr_{\text{dR}}-r^a\iota_a$. The Cartan model is equivalent to the Weil model through the \textbf{Mathai--Quillen isomorphism}, which is induced by the following projection operator
        \begin{gather}
            \Omega^\bullet(M)\rightarrow\left(\mathrm{CE}(\mathfrak{g})\otimes\Omega^\bullet(M)\right)_{\text{hor}}:\omega\mapsto\exp(-t^a\iota_a)\omega\,. 
        \end{gather}
    }
    \begin{remark}[BRST]\index{Becchi--Rouet--Stora--Tyutin}
        The Mathai--Quillen operator can be interpreted in the context of \textit{BRST theory} as a gauge-fixing procedure (see \cref{brst:gauge_fixing}). This gives rise to a third model, \indexauthor{Kalkman}'s BRST model.
    \end{remark}

    \begin{theorem}[Equivariant de Rham theorem]\index{de Rham}
        If $G$ is compact and connected, the cohomology of the Weil model, and hence of the Cartan model, is isomorphic to the $G$-equivariant cohomology $H_G^\bullet(M)$ from \cref{topology:equivariant_cohomology}.
    \end{theorem}
    ...
    \end{property}
    \begin{property}[Cartan map]\index{Cartan!map}
        Consider a principal $G$-bundle $\pbundle$. The map
        \begin{gather}
            \Bigl(\Omega^\bullet(P)\otimes\mathfrak{g}^*[1]\Bigr)^G\rightarrow\Omega^\bullet(B):\omega\otimes\langle\cdot\rangle\mapsto\omega_{\text{hor}}\otimes\langle F_\nabla\rangle
        \end{gather}
        is a quasi-isomorphism between the Cartan model of $H^\bullet_G(P)$ and $H^\bullet(M)$. (Note that this is essentially the Chern--Weil homomorphism from \cref{section:chern_weil}).
    \end{property}
...
\subsection{Integration}\index{co-!chain}
    ...
    \begin{remark}[$L^2$-cohomology]\index{cohomology!$L^2$}
        Given a \textit{Riemannian metric} (see \labelref{chapter:riemann}), one can define a notion of square-integrable differential forms. The cohomology theory of these forms is called $L^2$-cohomology.
    \end{remark}
...
\subsection{Cohomology with compact support}\index{cohomology!compact support}\label{section:compact_cohomology}
    ...
    The following definition gives a direct characterization of cohomology with compact support through its relation to compact sets.
    ...
    \begin{property}[Transversal intersections]
    ...
    \newdef{Intersection number}{\index{intersection!number}
        Consider two submanifolds $S,T\subseteq M$ that intersect transversely. When $\dim(S)+\dim(T)=\dim(M)$, the cup product $[\eta_S\wedge\eta_T]\in H^{\dim(M)}(M;\mathbb{Z})$ gives an integer, called the intersection number:
        \begin{gather}
            S\cdot T := \Int_M\eta_S\wedge\eta_T = \Int_S\eta_T = \Int_T\eta_S\,.
        \end{gather}
    }
    ...
    \begin{theorem}[Thom isomorphism]\index{Thom!isomorphism}\label{bundle:thom_isomorphism}
        ...
        \begin{gather}
            \pi_*:H^\bullet_{\mathrm{cv}}(E)\cong H^{\bullet-n}(M):\mathcal{T}\,.
        \end{gather}
    \end{theorem}
...
\subsection{\v{C}ech--de Rham complex}\label{section:cech_de_rham}
    ...
    \begin{theorem}[\v{C}ech--de Rham isomorphism]
    ...
    \begin{property}[Exponential sequence]
        ...
        \end{gather}
        Let $(M,\mathcal{O}_M)$ be a \textit{complex manifold} (see \labelref{chapter:complex_geometry}) or, for a smooth manifold, restrict the above sequence to the real numbers. The exact sequence induces an exact sequence of structure sheaves:
        \begin{gather}

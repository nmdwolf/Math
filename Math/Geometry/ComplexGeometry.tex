...
\section{Complex structures}
    ...
    \begin{remark}[Oka manifolds]\index{Oka!manifold}
        This principle has a more general homotopical/model-theoretic (recall \labelref{chapter:model_theory}) formulation. There exists a class of complex manifolds, the Oka manifolds, such that the inclusion
        \begin{gather}
...
\section{Holomorphic vector bundles}\index{holomorphic!vector bundle}
    ...
    \begin{property}[Holomorphic sections]\label{complex:holomorphic_sections}
    ...
    \newdef{Positive line bundle}{\index{positive!line bundle}\label{complex:positive_bundle}
        A holomorphic line bundle whose curvature form is, after multiplication by $i$, a positive-definite form.
    }
    ...
    \begin{formula}[K\"ahler identities]\index{K\"ahler!identities}\index{Lefschetz!operator}\index{contraction}
...
\subsection{Dirac operators}\index{Dirac!operator}\label{section:kahler_dirac_operator}
    ...
    \newdef{Dolbeault--Dirac operator}{\index{Dolbeault!operator}
        ...
        \end{gather}
        and that the Dirac operator can be identified with
        \begin{gather}
            \label{complex:dolbeault_dirac_operator}
            \overline{\partial}+\overline{\partial}\vphantom{\partial}^\dag\,,
        \end{gather}
        where $\overline{\partial}$ is the Dolbeault operator (\cref{complex:dolbeault_operator}). For the action on the $\theta$-characteristic, choose a connection $\nabla$ on the canonical line bundle:
        \begin{gather}
            \begin{aligned}
...
\subsection{Riemann surfaces}
    ...
    \newdef{Riemann surface}{\index{Riemann!surface}\label{complex:riemann_surface}
    ...
    \end{theorem}
    \begin{result}
        Because the universal cover of a Riemann surface is again a Riemann surface, the uniformization theorem implies that every Riemann surface can be obtained as the quotient of $\mathbb{CP}^1,\mathbb{C}$ or $\mathfrak{h}$ by a freely-acting discrete group.

        Moreover, $\mathbb{CP}^1$ only covers itself and $\mathbb{C}$ only admits $\mathbb{Z}$ or a discrete lattice as freely-acting discrete automorphism groups (leading to $\mathbb{C}$, $\mathbb{R}\times S^1\cong\mathbb{C}\backslash\{0\}$ and $\mathbb{T}^2$ as quotients). All other Riemann surfaces are obtained from the half plane $\mathfrak{h}$.

        This leads to the following classification of Riemann surfaces $\Sigma$, where $\widetilde{\Sigma}$ denotes the universal cover of $\Sigma$:
        \begin{itemize}
            \item Elliptic surface: $\widetilde{\Sigma}\cong\mathbb{CP}^1$ and, hence, $\Sigma\cong\mathbb{CP}^1$.
            \item Parabolic surface: $\widetilde{\Sigma}\cong\mathbb{C}$ and, hence, $\Sigma\cong\mathbb{C},\mathbb{C}\backslash\{0\}$ or $\mathbb{C}/(\mathbb{Z}+\tau\mathbb{Z})$ for some $\tau\in\mathfrak{h}$.
            \item Hyperbolic surface: $\widetilde{\Sigma}\cong\mathfrak{h}$ and, hence, $\Sigma\cong\mathfrak{h}/(\mathbb{Z}+\tau\mathbb{Z})$ for some $\tau\in\mathfrak{h}$. These are called \textbf{Fuchsian models} (cf.~\cref{number:fuchs_group}).\index{Fuchsian!model}
        \end{itemize}
    \end{result}

    \newdef{Riemann surface with punctures}{\label{complex:punctured_riemann_surface}
        A Riemann surface $\Sigma$ such that there exists an open subset $U$ satisfying:
        \begin{enumerate}
            \item $\Sigma\backslash U$ is compact, and
            \item $U$ is biholomorphic to the disjoint union of a finite number of punctured open disks.
        \end{enumerate}
        Its `closure' $\widehat{\Sigma}$ is uniquely determined by filling up the punctured disks. Hence, it is compact and $\widehat{\Sigma}\backslash\Sigma$ is finite.
    }

    The following notion generalizes the concepts of \cref{section:branch_cuts} to Riemann surfaces.
    \newdef{Ramification}{\index{ramification}\index{branching}
        Consider a holomorphic map of Riemann surfaces $f:M\rightarrow N$. Consider two points $p\in M,q\in N$ such that $f(p)=q$. Around these points, there exist coordinate charts with $z(p)=0$ and $\widetilde{z}(q)=0$ such that $\widetilde{z}\circ z = z^\kappa$ for some $\kappa\in\mathbb{N}_0$. This index is called the \textbf{ramification index} and, if $\kappa>0$, the morphism $f$ is called a \textbf{ramified} or \textbf{branched cover} over $p$.
    }
    \begin{property}
        For a holomorphic map $f:M\rightarrow N$, the cardinality of the preimage of all but finitely many points is constant. These remaining points are exactly the branch points.
    \end{property}
    ...
    \end{construct}
    \begin{remark}[Markings]\index{marking}
        A confusing point in Teichm\"uller theory (and the general study of Riemann surfaces) is that an element of the Teichm\"uller space, i.e.~an equivalence class of complex structures with biholomorphisms, is also called a marking or \textbf{marked Riemann surface}.
    \end{remark}
    \begin{property}[Alternative characterization]
...
\subsection{Orbifolds}
    ...
    Even when restricting to stable surfaces, the moduli space does not have the structure of a smooth manifold due to the existence of the marked points. To accurately describe the geometrical structure one needs to generalize the notion of a manifold (cf.~\labelref{chapter:manifolds}).
...
\subsection{Dolbeault cohomology}\index{Dolbeault!cohomology}
    ...
    The following theorem gives an analog of the de Rham theorem~\ref{bundle:de_rham} for complex (Dolbeault) cohomology, the cohomology induced by the (antiholomorphic) Dolbeault operator $\overline{\partial}$.
    \begin{theorem}[\v{C}ech--Dolbeault]\index{\v{C}ech--Dolbeault}
        Let $E\rightarrow M$ be a holomorphic vector bundle.
        \begin{gather}
            H^{\bullet,\bullet}_{\overline{\partial}}(M;E)\cong H^\bullet(\Omega^\bullet(M)\otimes E)
        \end{gather}
    \end{theorem}

    Infinitesimal deformations $J\longrightarrow J+\varepsilon$ of a complex structure are classified by \v{C}ech--Dolbeault cohomology.
    \begin{property}[Deformation of complex structures]
        Let $J$ be a complex structure on $M$ and denote the associated Dolbeault operator by $\overline{\partial}$. The infinitesimal deformations of $J$ are given by $H^1_{\overline{\partial}}(TM)\cong H^1(TM)$.
    \end{property} 
    ...
    \begin{theorem}[Kodaira vanishing theorem]\label{complex:kodaira_vanishing}
...
\subsection{\difficult{Lagrangian Floer homology}}
    ...
    In this section, a (co)homology theory is constructed from the intersection theory of Lagrangian submanifolds (see \labelref{chapter:symplectic} for an introduction).

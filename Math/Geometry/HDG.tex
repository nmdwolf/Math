\chapter{\difficult{Higher-dimensional Geometry}}\label{chapter:hdg}
    ...
    In this chapter, certain constructions and theorems introduced in the previous chapters are generalized to the setting of supergeometries and higher categories. As such, it can been as an analogue to \labelref{chapter:hda} for (differential) geometry.

    The main references are~\citet{schreiber_loop_2005,baez_higher_2005}. The section on spectral geometry is based on~\citet{sanders_commutative_2012}, whereas that on smooth spaces is inspired by~\citet{baez_convenient_2011}. For an introduction to (higher) category theory, see \labelref{chapter:cat}. \labelref{section:higher_lie_structures} gives a different approach to the higher-dimensional analogues of Lie algebras. For an introduction to supergeometry, see~\citet{cattaneo_introduction_2011}. The notion of cohesion was introduced in~\citet{lawvere_cohesive_1994}.
...
\section{Graded manifolds}
    ...
    In this section, some of the notions from \labelref{part:diffgeom} will be generalized to supermanifolds and general graded manifolds. The general notation $(x^i)$ will be used for the collection of both even and odd coordinates.
    ...
    \end{property}
    \begin{result}[dg-symplectic manifold]\index{master equation}\label{hdg:dg_symplectic_manifold}
        ...
        If $\omega$ is of degree 1, it was shown by \indexauthor{Schwarz} that $(M,\omega)$ is symplectomorphic to $\Pi T^*M$ such that the Poisson bracket is mapped to the Schouten--Nijenhuis bracket (\cref{bundle:schouten_nijenhuis_bracket}) and the Hamiltonian is mapped to a Poisson bivector field exactly if it satisfies the master equation.
    \end{result}

\section{Infinite-dimensional geometry}\label{section:infinite_dimensional}

    In many situations, when considering function spaces, the objects under consideration do not form a finite-dimensional manifold. However, with some care, one can omit this size condition. In \labelref{chapter:functional}, it was shown how calculus could be extended from $\mathbb{R}^n$ to infinite-dimensional vector spaces. Here, geometry is extended to that setting.
...
\section{Cohesion}\index{cohesion}\label{section:cohesion}
    ...
    \newdef{Local topos}{\index{local!topos}\label{topos:local_topos}
        An $\mathcal{S}$-topos $\symbfsf{H}$ is said to be ($\mathcal{S}$-)local if the geometric morphism $(f^*\dashv f_*):\symbfsf{H}\leftrightarrows\mathcal{S}$ admits a right adjoint $f^!$ such that one of the following equivalent statements holds:
        \begin{itemize}
            \item $f^!$ is fully faithful,
            \item $f^*$ is fully faithful,
            \item[$\ast$] $f^!$ is an $\mathcal{S}$-indexed functor (\cref{cat:indexed_category}), or
            \item[$\ast$] $f^!$ is Cartesian closed (\cref{cat:cartesian_closed_functor}),
        \end{itemize}
        where the starred items hold in the 1-categorical setting. If $\mathcal{S}=\symbfsf{Set}$ or $\mathcal{S}=\infty\symbfsf{Grpd}$, the conditions are automatically satisfied. The triple of functors $f^*\dashv f_*\dashv f^!$ is also called a \textbf{local geometric morphism}.

        The right adjoint is sometimes called the \textbf{codiscrete object functor} $\mathrm{coDisc}$ (in fact, this terminology is applied more generally when $\symbfsf{H}$ is just any category). If this functor exists, $\symbfsf{H}$ is said to have \textbf{codiscrete objects}.
    }
    \begin{property}
    ...
    \newdef{Locally connected topos}{\index{locally!connected topos}\index{fundamental!groupoid}\label{topos:locally_connected}
        ...
        \begin{gather}
            \Pi_0:\symbfsf{H}\rightarrow\symbfsf{Set}:\bigsqcup_{i\in I}X_i\mapsto I\,,
        \end{gather}
    ...
    \newdef{Cohesive topos}{\index{topos!cohesive}
        ...
        \begin{gather}
            \begin{tikzpicture}[baseline=(current bounding box.center)]
                ...
                \node (B) at (3, 0) {$\infty\symbfsf{Grpd}$};
                \node (A1) at (0, 1) {$\phantom{\symbfsf{H}}$};
                \node (B1) at (3, 1) {$\phantom{\infty\symbfsf{Grpd}}$};
                \node (A_1) at (0, -1) {$\phantom{\symbfsf{H}}$};
                \node (B_1) at (3, -1) {$\phantom{\infty\symbfsf{Grpd}}$};
                \node (A_2) at (0, -2) {$\phantom{\symbfsf{H}}$};
                \node (B_2) at (3, -2) {$\phantom{\infty\symbfsf{Grpd}}$};
    ...
    \begin{property}[Cohesive modalities]\index{flat!modality}\index{sharp!modality}\index{shape!modality}\index{discrete!object}\label{topos:cohesive_modalities}
        The adjoint quadruple of a cohesive topos induces an adjoint triple of modalities (\cref{type:modality}):
        \begin{gather}
    ...
    \newdef{Concrete object}{\index{concrete!object}\label{topos:concrete_object}
        Consider a cohesive topos $\symbfsf{H}$. An object $X\in\ob{H}$ is said to be concrete if $\eta^\sharp_X$ is monic.
    }
    ...
    \newdef{Differential cohesion}{\index{cohesion!differential}\index{elastic|see{cohesion, differential}}
        Consider a cohesive $(\infty,1)$-topos $\symbfsf{H}$. An \textbf{infinitesimal cohesive neighbourhood} of $\symbfsf{H}$ is a cohesive $(\infty,1)$-topos $\symbfsf{H}_{\text{th}}$ equipped with a strongly connected adjoint quadruple $(\iota_{\text{inf}}\dashv\Pi_{\text{inf}}\dashv\mathrm{Disc}_{\text{inf}}\dashv\Gamma_{\text{inf}}):\symbfsf{H}\rightarrow\symbfsf{H}_\text{th}$ such that $\iota_{\text{inf}}$ is also fully faithful (and, hence, so is $\mathrm{Disc}_{\text{inf}}$). If such a neighbourhood exists, $\symbfsf{H}_{\text{th}}$ is said to be differentially cohesive (or \textbf{elastic}) over $\symbfsf{H}$.\footnote{The subscript `th' indicates that $\symbfsf{H}_{\text{th}}$ can be seen as an `infinitesimal thickening' of $\symbfsf{H}$. (See \cref{hdg:infinitesimal_thickening} for more information.)}
    }
    \begin{remark}
    ...
    \newprop{Differential modalities}{\label{topos:differential_modalities}
        Consider a differentially cohesive $(\infty,1)$-topos $\symbfsf{H}$. The adjoint quadruple to its infinitesimal thickening induces an adjoint cylinder of modalities:
        \begin{gather*}
            (\mathfrak{R}\dashv\mathfrak{I}\dashv\&):=(\iota_{\text{inf}}\circ\Pi_{\text{inf}}\dashv\mathrm{Disc}_{\text{inf}}\circ\Pi_{\text{inf}}\dashv\mathrm{Disc}_{\text{inf}}\circ\Gamma_{\text{inf}})\,.
        \end{gather*}
    ...
    \newdef{Jet comonad}{\index{jet!comonad}
        ...
        The (co)algebras over the jet comonad correspond to common objects in geometry. The Eilenberg--Moore category $\symbfsf{H}^{J^\infty}$ corresponds to PDEs with variables in $B$ and the Kleisli category $\symbfsf{Kl}(J^\infty)$ corresponds to bundles over $B$ equipped with a differential operator.
    }
    ...
    \newdef{Formally \'etale morphism\footnotemark}{\index{local!diffeomorphism}\label{topos:formally_etale}
        ...
        A morphism $f:X\rightarrow Y$ in a differentially cohesive topos $\symbfsf{H}$ such that the naturality square of the shape modality is a pullback square:
        \begin{gather*}
            \begin{tikzpicture}
                \node (PB) at (0, 0) {pb};
                \node (X) at (-1, 1) {$X$};
    ...
    \newdef{Super-differential cohesion}{\index{cohesion!super-differential}\index{solid|see{cohesion, super-differential}}
        Consider a differentially cohesive $(\infty,1)$-topos $\symbfsf{H}_{\text{bos}}\rightarrow\symbfsf{H}_{\text{red}}$. A super-differentially cohesive (or \textbf{solid}\footnote{This terminology stems from the fact that supergeometry is necessary for treating fermions, which are in turn responsible for condensed matter physics and, hence, the study of solid matter.}) topos over $\symbfsf{H}_{\text{bos}}$ is a differentially cohesive $(\infty,1)$-topos $\symbfsf{H}\rightarrow\symbfsf{H}_{\text{red}}$ equipped with an adjoint quadruple $(\mathrm{even}\dashv\iota_{\text{sup}}\dashv\Pi_{\text{sup}}\dashv\mathrm{Disc}_{\text{sup}}):\symbfsf{H}\rightarrow\symbfsf{H}_{\text{bos}}$ such that $\iota_{\text{sup}}$ (and, hence, also $\mathrm{Disc}_{\text{sup}}$) is fully faithful.
...
\section{Smooth spaces}\index{smooth!space}\label{section:smooth_spaces}
    ...
    \end{gather}
    However, in general, this is not a homeomorphism, i.e.~currying does not define an adjunction and, therefore, $\symbfsf{Top}$ is not Cartesian closed (\cref{cat:closed}). This problem was treated by \indexauthor{Steenrod} and others, and the solution was simply to restrict to a smaller class of better behaved spaces: the compactly generated Hausdorff spaces.\footnote{In practice, this is not a problem since all interesting spaces, such as CW complexes, belong to this class.}

    Whilst studying varieties in algebraic geometry, people experienced similar problems. For this reason, \indexauthor{Grothendieck} invented schemes (see \labelref{chapter:alggeom} and \cref{section:schemes} in particular). The main takeaway of this approach was that ``it's better to work with a nice category containing some nasty objects, than a nasty category containing only nice objects'' as \indexauthor{Baez} phrases it succinctly.

    The category $\symbfsf{Diff}$ of finite-dimensional smooth manifolds suffers the same problems, namely the space of smooth functions $C^\infty(X,Y)$ is, in general, some kind of infinite-dimensional manifold and, hence, cannot be defined internally. It becomes even worse if one studies the mapping spaces between those. \indexauthor{Kriegl} and \indexauthor{Michor} have introduced a framework in which one can work safely, but the main problem with their solution is that not all spaces of interest are included. Certain other operations such as quotients and (co)limits are also not guaranteed to exist within that category.
...
\subsection{Fr\"olicher spaces}\index{Fr\"olicher space}
    ...
    \newdef{Fr\"olicher space}{
        ...
        \begin{itemize}
            \item a set $X$,
            \item a set of curves $C_X\subseteq\symbfsf{Set}(\mathbb{R},X)$, and
            \item a set of functionals $F_X\subseteq\symbfsf{Set}(X,\mathbb{R})$.
        \end{itemize}
        ...
        \begin{enumerate}
            ...
            \item If, given $c\in\symbfsf{Set}(\mathbb{R},X)$, the composite $f\circ c\in C^\infty(\mathbb{R})$ for all functionals $f\in F_X$, then $c\in C_X$.
            \item If, given $f\in\symbfsf{Set}(X,\mathbb{R})$, the composite $f\circ c\in C^\infty(\mathbb{R})$ for all curves $c\in C_X$, then $f\in F_X$.
        \end{enumerate}
...
\subsection{Smooth sets}
    ...
    \newdef{Concrete site}{\index{concrete!site}
        Consider a site $(\symbfsf{C},J)$. It is said to be concrete if
        \begin{enumerate}
            \item the functor $\symbfsf{C}(\ast,-):\symbfsf{C}\rightarrow\symbfsf{Set}$ is faithful, and
            \item for every covering family $\{f_i:U_i\rightarrow U\}_{i\in I}\in J(U)$, the morphism
            \begin{gather}
                \bigsqcup_{i\in I}\symbfsf{C}(\ast,f_i):\bigsqcup_{i\in I}\symbfsf{C}(\ast,U_i)\rightarrow\symbfsf{C}(\ast,U)
            \end{gather}
            is surjective.
        \end{enumerate}
    }
    ...
    \newdef{Concrete presheaf}{\index{concrete!sheaf}
        Consider a concrete site $(\symbfsf{C},J)$. For every presheaf $X\in\symbfsf{Psh}(\symbfsf{C})$ and object $U\in\ob{C}$, denote by
        \begin{gather}
            X_U:X(U)\rightarrow\symbfsf{Set}\bigl(\symbfsf{C}(\ast,U),X(\ast)\bigr)
        \end{gather}
        the adjunct of the restriction morphism
        \begin{gather}
            X(U)\times\symbfsf{C}(\ast,U)\rightarrow X(\ast)\,.
        \end{gather}
        ...
        \begin{gather}
            U\mapsto\symbfsf{Set}\bigl(\symbfsf{C}(\ast,U),X(\ast)\bigr)\,.
        \end{gather}
    ...
    \newdef{Concrete object}{\index{concrete!object}
        Consider a local geometric morphism $\Gamma:\symbfsf{H}\rightarrow\mathcal{S}$. Consider the class $V$ of local isomorphisms with respect to $\Gamma$. An object in $\symbfsf{H}$ is said to be concrete if it is $V$-separated, i.e.~its Yoneda embedding maps morphisms in $V$ to monos.
    }
    \begin{remark}
    ...
    \newdef{Diffeological space}{\index{diffeology}
        A diffeology on a set $X$ is a concrete sheaf $\mathcal{D}_X$ on $\symbfsf{CartSp_{\text{diff}}}$ such that $\Gamma\mathcal{D}_X=X$.
    }
    ...
    \newdef{Smooth map}{\index{smooth!function}
        Let $\mathcal{D}_X$ and $\mathcal{D}_Y$ be diffeological spaces. A map $g:X\rightarrow Y$ is said to be smooth if $g\circ f\in\mathcal{D}_Y$ for all $f\in\mathcal{D}_X$. The diffeological spaces together with their differentiable morphisms form a category $\symbfsf{DiffSp}$.
    }
    ...
    \newdef{Smooth set}{\index{smooth!set}\index{Nullstellensatz}
        \begin{gather}
            \symbfsf{SmoothSet}:=\symbfsf{Sh(CartSp_{\text{diff}})}
        \end{gather}
        The topology on this site is generated by the coverage of differentiably good covers (\cref{manifold:good_cover}). In fact, this topology coincides with the usual one consisting of open covers. The category of smooth sets is also denoted by $\symbfsf{C^\infty}$.

        To phrase this in the sense of \cref{section:cohesion}, smooth sets form a cohesive topos over $\symbfsf{Set}$, where $\mathrm{Disc}$ and $\mathrm{coDisc}$ assign the discrete and codiscrete structures from the preceding examples, respectively. Moreover, this cohesive topos satisfies the \textit{Nullstellensatz}~\ref{topos:nullstellensatz}.
    }
...
\subsection{Smooth algebras}
    ...
    \newdef{Smooth algebra}{\index{smooth!algebra}
        ...
        \begin{gather}
            C^\infty(M):=C^\infty(M,\mathbb{R})=\symbfsf{Diff}(M,\mathbb{R})\,.
        \end{gather}
        ...
        \end{gather}
        Furthermore, the hom-functor is covariant in the second argument and, hence, defines a copresheaf on the category $\symbfsf{CartSp_{\text{diff}}}$. Generalizing this situation, smooth algebras are defined as finite product-preserving copresheaves on $\symbfsf{CartSp_{\text{diff}}}$. This (functor) category is denoted by $\symbfsf{C^\infty Alg}$.
    }
    \newdef{Underlying algebra}{
        Given a smooth algebra $R\in\symbfsf{C^\infty Alg}$, its underlying algebra $U(R)$ is defined as the set $R(\mathbb{R})$ equipped with the canonically induced ring operations.
    }
    ...
    \newdef{Smooth locus}{\index{smooth!locus}
        Let $\symbfsf{C^\infty Alg^{fin}}$ denote the category of finitely generated smooth algebras. The category of \textbf{smooth loci} is defined as $(\symbfsf{C^\infty Alg^{fin}})^{\text{op}}$. The smooth locus corresponding to a smooth algebra $R$ is often denoted by $\ell R$.
    }
...
\subsection{Supergeometry}
    ...
    \newdef{Superscheme}{\index{super-!scheme}
        The category of affine superschemes is defined as the opposite of the category of supercommutative superalgebras, the commutative monoids internal to $\symbfsf{sVect}$:
        \begin{gather}
            \symbfsf{Aff(sVect)} := \symbfsf{sCAlg}^{\text{op}} := \symbfsf{CMon(sVect)}\,.
        \end{gather}
    ...
    \newdef{Fermionic modalities}{
        The adjoint cylinder induced by the inclusion $\symbfsf{CAlg}\hookrightarrow\symbfsf{sCAlg}$ in turn gives rise to the adjoint modalities
        \begin{gather}
    ...
    \newdef{Reduction}{\index{reduction}\index{infinitesimal}
        By \cref{topos:differential_modalities}, $\symbfsf{SuperSmoothSet}$ admits differential modalities to both $\symbfsf{FormalSmoothSet}$ and $\symbfsf{SmoothSet}$. The reduction modality $\mathfrak{R}$ simply drops all infinitesimal directions, both even- and odd-graded:
        \begin{gather}
    ...
    \newdef{Shape modality}{\index{modality|seealso{shape}}\index{shape}\index{de Rham!space|seealso{shape}}\label{hdg:de_rham_shape}
    ...
    \newdef{Local diffeomorphism\footnotemark}{\index{local!diffeomorphism}
        ...
        \begin{gather*}
            \begin{tikzpicture}
                \node (PB) at (0, 0) {pb};
                \node (X) at (-2, 1) {$\symbfsf{CRing}(A,B)$};
                \node (JX) at (2, 1) {$\symbfsf{CRing}(A,B/I)$};
                \node (Y) at (-2, -1) {$\symbfsf{CRing}(R,B)$};
                \node (JY) at (2, -1) {$\symbfsf{CRing}(R,B/I)$};
                \draw[->] (X) -- (JX);
                \draw[->] (Y) -- (JY);
                \draw[->] (X) -- (Y);
                \draw[->] (JX) -- (JY);
    ...
    \newdef{$V$-manifold}{
        Consider a group object $V$ in $\symbfsf{SuperSmoothSet}$. A $V$-manifold is a super smooth set $X$ equipped with a span
        \begin{gather*}
            \begin{tikzpicture}
                \node (U) at (0, 0) {$U$};
                \node (V) at (-2, -2) {$V$};
...
\subsection{Extending schemes}

    By interpreting objects in a big topos over a scheme (\cref{section:alggeom_topos_theory}) as generalized spaces, the following definition is obtained.
    \newdef{Algebraic space}{\index{algebraic!space}\index{atlas}\label{alggeom:algebraic_space}
        ...
        \begin{enumerate}
            \item The diagonal morphism $X\rightarrow X\times X$ is representable (\cref{topos:representable_diagonal}).
...
\subsection{\texorpdfstring{$\mathcal{D}$-geometry $\clubsuit$}{D-geometry}}

    \newdef{Sheaf of differential operators}{
        Let $X$ be a smooth manifold (\labelref{chapter:manifolds}) or scheme (\labelref{chapter:alggeom}). Denote its structure sheaf by $\mathcal{O}_X$ and denote the sheaf of $\mathcal{O}_X$-algebras of vector fields (interpreted as derivations) on $X$ by $\mathcal{D}_X$:
        \begin{gather}
            \mathcal{D}_X(U) := \bigl\{v\in\End(\mathcal{O}_X(U))\bigm\vert v(fg) = v(f)g + fv(g)\bigr\}\,.
        \end{gather}
    }

    \newdef{$\mathcal{D}_X$-module}{\index{D!module}\label{hdg:d_module}
        ...
        \begin{enumerate}
            ...
            \item\textbf{Flatness}: $\nabla_{[v,w]}\sigma = [\nabla_v,\nabla_w]\sigma$.
        \end{enumerate}
        If $X$ is locally free (\cref{sheaf:locally_free}), i.e.~corresponds to a locally trivial bundle, one obtains the algebraic reformulation of a vector bundle with flat connection.
    }

\subsection{Crystalline cohomology}\index{cohomology!crystalline}

    The following definition arises as an application of \cref{hdg:de_rham_shape} to the category of schemes. It was originally introduced by \indexauthor{Simpson}.
    \newdef{de Rham space}{\index{de Rham!space}
        Let $X$ be a scheme, interpreted through its functor of points $\func{X}{CRing}{Set}$. Its de Rham space $X_{\text{dR}}$ is obtained by reducing schemes (and sheafifying):
        \begin{gather}
            X_{\text{dR}}(R) := X(R_{\text{red}})\,.
        \end{gather}
    }

    \newdef{Crystalline site}{\index{site!crystalline}
        Let $X$ be a (smooth) scheme over a field of characteristic 0 and consider its de Rham space $X_{\text{dR}}$. The big site $\symbfsf{Sch}/_{\!X_{\text{dR}}}$ is called the \textbf{infinitesimal} or crystalline site $\symbfsf{Crys}(X)$ over $X$.\footnote{This is used both in the Zariski and \'etale sense.}
    }

    \newdef{Crystalline cohomology}{
        The Abelian sheaf cohomology on the crystalline site (as such, it can be interpreted as the algebraic de Rham cohomology of de Rham spaces).
    }

    \begin{property}[$\mathcal{D}$-modules]
        The category of $\mathcal{D}_X$-modules (\cref{hdg:d_module}) over a smooth scheme $X$ (over a field of characteristic 0) is equivalent to the category of quasicoherent sheaves over its de Rham space $X_{\text{dR}}$.
    \end{property}

    \newdef{Crystal}{\index{crystal}
        In essentially the same way as one defines a quasicoherent sheaf on a scheme, one can define a quasicoherent sheaf on a general sheaf $\func{X}{CRing}{Set}$ (\cref{sheaf:quasicoherent_base_change}). A quasicoherent sheaf $\mathcal{F}$ on $X$ sends every $R$-point $x\in X(R)$ to an $R$-module $\mathcal{F}(x)$ in a way that is compatible with base change:
        \begin{enumerate}
            \item For every ring morphism $R\rightarrow R'$, there is an isomorphism $\alpha_{x,x'}:\mathcal{F}(x')\cong\mathcal{F}(x)\otimes_RR'$.
            \item Given ring maps $R\rightarrow R'\rightarrow R''$: $\alpha_{x,x''}=\alpha_{x',x''}\circ\alpha_{x,x'}$.
        \end{enumerate}
        A crystal (of quasicoherent sheaves) on a scheme $X$ is a quasicoherent sheaf on its de Rham space $X_{\text{dR}}$.
    }
...
\section{\difficult{Gauge theory}}
    ...
    Recall the notions of \labelref{chapter:topos}, in particular the notions of stacks and higher topoi. The $(\infty,1)$-category of smooth $\infty$-stacks can be described in terms of the (left Bousfield) localization of a suitable presheaf category by Lurie's theorem~\ref{model:lurie_presentation}.
...
\subsection{Lie groupoids}
    ...
    \newdef{Lie groupoid\footnotemark}{\index{Lie!groupoid}\label{hdg:lie_groupoid}
        ...
        A groupoid internal to $\symbfsf{Diff}$.
    }
    \begin{remark}
        Note that \cref{cat:internal_category} requires the existence of pullbacks. In the category $\symbfsf{Diff}$, this is equivalent to assuming that the source and target morphisms are (surjective) submersions.
    ...
    \newdef{$L_\infty$-algebroid}{\index{Lie!algebroid}\label{hdg:l_infty_algebroid}
...
\section{Weak groups}
    ...
    \newdef{Weak 2-group}{\index{group!categorical}\index{2-!group}
        Let $(\symbfsf{C},\otimes,\symbfsf{1})$ be a monoidal category. This category is called a weak 2-group or \textbf{categorical group} if it satisfies the following conditions:
        \begin{enumerate}
        ...
        The definition of a weak 2-group can be strengthened to that of a \textbf{coherent 2-group} (sometimes called a \textbf{gr-category}), where the isomorphisms $x\otimes x^{-1}\rightarrow\symbfsf{1}$ and $x^{-1}\otimes x\rightarrow\symbfsf{1}$ glue together to form an adjoint equivalence.
    }

    \begin{example}[Automorphism 2-group]
        Consider a 2-category $\symbfsf{C}$. For every object $c\in\ob{C}$, the automorphism category $\symbfsf{Aut}(c)$ of autoequivalences of $c$ and invertible 2-morphisms between them forms a coherent 2-group.\footnote{Relaxing the objects to weakly invertible 1-morphisms gives a weak 2-group.}
    \end{example}
    ...
    \newdef{Strict 2-group}{
        ...
        This definition is equivalent to the following internal version. A (strict) 2-group is a group object in $\symbfsf{Cat}$ or an internal category in $\symbfsf{Grp}$. If $\symbfsf{Grp}$ is replaced by $\symbfsf{Lie}$, the notion of a (strict) Lie 2-group is obtained.
    }
...
\subsection{Spaces}
    ...
    \newdef{Smooth 2-space}{
        A category internal to $\symbfsf{C^\infty}$.
    }
    ...
    \newdef{2-group action}{\index{group!action}
        Consider a smooth 2-group $G$ and a smooth 2-space $E$. A strict action of $G$ on $E$ is a smooth morphism $G\rightarrow\symbfsf{Aut}(E)$, i.e.~a smooth (ana)functor preserving products and inverses.
    }
    ...
    \newdef{Algebraic stack}{\index{algebraic!stack}\index{Deligne--Mumford!stack}\index{Artin!stack}
        A stack $X\in\symbfsf{Sh}_{(2,1)}(\symbfsf{Sch}_\text{fppf})$ on the big fppf-site such that
        \begin{enumerate}
            \item The diagonal $\Delta_X:X\rightarrow X\times X$ is representable by an algebraic space (\cref{alggeom:algebraic_space}).
            \item There exists a scheme $S\in\symbfsf{Sch}$ and a morphism $h_S\rightarrow X$ that is surjective and smooth.
        \end{enumerate}
...
\section{2-Bundles}
    ...
    \newdef{$G$-bundle}{\index{principal!bundle}
        A locally trivial 2-bundle with typical fibre $F$ is said to have the 2-group $G$ as its structure (2-)group if the transition data factor through an action $G\rightarrow\symbfsf{Aut}(F)$. If $F=G$, the 2-bundle is called a \textbf{principal $G$-2-bundle}.
    }
    ...
    \newdef{$n$-connection}{\index{connection!principal}
        ...
        \begin{gather}
            \mathrm{hol}:\mathcal{P}_n(M)\rightarrow\symbfsf{Trans}_n(P)\,.
        \end{gather}
    }
...
\section{Space and quantity}\label{section:space_and_quantity}
    ...
    \newdef{Space}{\index{space}
        A (generalized) space modelled on a category $\symbfsf{C}$ is a presheaf on $\symbfsf{C}$.

        As before, the object $X(C)$ can be interpreted as the collection of `probes' from $C$ to $X$. The Yoneda lemma assures that ordinary test spaces in $\symbfsf{C}$ can be viewed as spaces modelled on $\symbfsf{C}$ and that their probes are indeed the ordinary maps in $\symbfsf{C}$.
    }

    In a similar vein, one can define observables as maps out of a space.
    \newdef{Quantity}{\index{quantity}
        A (generalized\footnote{It is generalized because it `measures' a category instead of a single object.}) quantity on a category $\symbfsf{C}$ is a copresheaf on $\symbfsf{C}$.
    }

\chapter{Principal Bundles}\label{chapter:principal_bundles}
    ...
    The main reference for this chapter is~\citet{sontz_principal_2015}. Ths chapter uses the language of (Lie) group theory quite heavily. For all things related to group theory, the reader is referred to \cref{section:groups,section:group_actions}. For more information on Lie groups and their associated Lie algebras, the reader is referred to \labelref{chapter:lie}.
...
\subsection{Ehresmann connections}
    ...
    \newdef{Tensorial form}{\index{tensorial}\index{basic}\label{bundle:tensorial_form}
        Consider a vector space $V$ equipped with a representation $\rho:G\rightarrow V$. A $V$-valued differential form $\theta\in\Omega^\bullet(P;V)$ on a principal $G$-bundle $\pbundle$ is said to be \textbf{tensorial} or \textbf{basic of type} $(V,\rho)$ if
        \begin{enumerate}
            \item it is horizontal, and
            \item if it is equivariant:
                \begin{gather}
                    R_g^*\theta = \rho(g^{-1})\circ\theta\,.
                \end{gather}
        \end{enumerate}
        The space of tensorial forms of type $(V,\rho)$ is sometimes denoted by $\Omega^\bullet_{\rho,\text{hor}}(P;V)$.
        ...
        \begin{gather}
            ...
            \tau|_u:\Omega^\bullet|_{\pi(u)}(M;P\times_\rho V)\rightarrow\Omega^\bullet_{\rho,\text{hor}}|_u(P;V):\phi\mapsto f^{-1}_u(\pi^*\phi)\,,
        \end{gather}
...
\subsection{Connection forms}
    ...
    \newdef{Connection 1-form}{\index{connection!form}
        Let $P$ be a principal $G$-bundle. A connection 1-form, associated to a given principal connection, is a $\mathfrak{g}$-valued 1-form $\omega\in\Omega^1(P;\mathfrak{g})$ that satisfies the following two conditions:
        \begin{enumerate}
            ...
            for all $g\in G$.
        \end{enumerate}
        The horizontal subspaces are recovered as the kernel of the connection 1-form:
        \begin{gather}
            H_pP = \ker(\omega|_p)\,.
        \end{gather}
    }
...
\subsection{Local representations}
    ...
    \begin{example}[General linear group]\index{Cartan!Maurer--Cartan connection}
...
\subsection{Parallel transport}
    ...
    \newadef{\difficult{Principal connection}}{\index{connection!principal}\label{bundle:holonomy_functor}
        Let $M$ be a smooth manifold and consider its path groupoid\footnote{See \cref{hdg:path_groupoid} for a rigorous exposition.} $\mathcal{P}_1(M)$ which has the points of $M$ as objects and homotopy classes of smooth paths in $M$ as morphisms. Consider a principal $G$-bundle $\pbundle$ and denote the delooping (\cref{cat:group_delooping}) of $G$ by $\symbfsf{B}G$. The assignment of holonomies to smooth paths defines a functor
        \begin{gather}
            \mathrm{hol}_i:\mathcal{P}_1(U_i)\rightarrow\symbfsf{B}G
        \end{gather}
        ...
        \begin{gather}
            \mathrm{hol}:\mathcal{P}_1(M)\rightarrow\symbfsf{Trans}_1(P)\subset G\symbfsf{Torsor}\,,
        \end{gather}
        where $\symbfsf{Trans}_1(P)$ is the full subcategory of the category of $G$-torsors on the fibres of $P$ (\cref{bundle:fibre_torsor}).
...
\subsection{Koszul connections}
    ...
    \newdef{Horizontal lifts on associated bundles}{\index{horizontal!lift}
        ...
        \end{gather}
        Note that, although the element $f$ seems to stay constant along the horizontal lift, it in fact changes according to \cref{bundle:associated_bundle_equivalence}.
    }
    ...
    \newdef{Covariant derivative}{
        Consider a vector bundle $\bundle$ with typical fibre $V$ and a connection 1-form $\omega$ on its associated principal $\GL(V)$-bundle. Let $\sigma:M\rightarrow E$ be a section of $E$ and let $X$ be a vector field on $M$. The covariant derivative of $\sigma$ with respect to $X$ is defined as follows:
        \begin{gather}
...
\subsection{Exterior covariant derivative}\label{section:exterior_covariant_derivative}
    ...
    \newdef{Exterior covariant derivative}{\index{exterior!covariant derivative}\label{bundle:exterior_covariant_derivative_P}
        Let $P$ be a principal bundle equipped with a connection 1-form $\omega$ and let $\theta\in\Omega^k(P)$ be a differential $k$-form. The exterior covariant derivative $\Dr\theta$ is defined as follows:
        \begin{gather}
            \Dr\theta(X_0,\ldots,X_k) := \dr\theta(X_0^H,\ldots,X_k^H)\,,
        \end{gather}
        where $\dr$ is the exterior derivative (\cref{bundle:exterior_derivative}) and $X_i^H$ is the projection of $X_i$ on the horizontal subspace $\mathrm{Hor}(T_pP)$. From this definition, it follows that the exterior covariant derivative $\Dr\theta$ is a horizontal form (\cref{bundle:horizontal_form}).
    }
    \begin{remark}
...
\subsection{Curvature}
    ...
    \newdef{Curvature}{\index{curvature}
        Let $\omega$ be a principal connection 1-form. The curvature $\Omega$ of $\omega$ is defined as the exterior covariant derivative $\Dr\omega$.
    }
    \newformula{Cartan structure equation}{\index{Cartan!structure equation}\label{bundle:cartan_structure_equation}
        Let $\omega$ be a connection 1-form and let $\Omega$ be its curvature form. The curvature can be expressed in terms of the connection as follows:
        \begin{gather}
            \Omega = \dr\omega + \frac{1}{2}[\omega\wedge\omega]\,.
        \end{gather}
        Recall the (geometric) fundamental theorem of calculus (\cref{bundle:mc_equation}). The Maurer--Cartan equation is equivalent to the vanishing of the (algebraic) curvature of a general $\mathfrak{g}$-valued 1-form.
    }
    ...
    \begin{property}[Tensorial]
        In contrast to a connection 1-form, the associated curvature is a tensorial $\mathfrak{g}$-valued 2-form or, equivalently, an $\ad(P)$-valued 2-form on the base manifold $M$.
    \end{property}
    ...
    \begin{property}[\difficult{Local systems}]\index{local!system}
    ...
    \begin{property}[Second Bianchi identity]\index{Bianchi identity}\label{bundle:second_bianchi_identity}
    ...
    \newdef{Field strength}{\index{Yang--Mills!field strength}
        Let $P$ be a principal bundle equipped with a principal connection 1-form $\omega$ and associated curvature $\Omega$. Given a local section $\sigma:U\rightarrow P$, one defines the (Yang--Mills) field strength $F$ as the pullback $\sigma^*\Omega$.
    }
...
\subsection{Torsion}
    ...
    \newdef{Solder form}{\index{solder form}
        Let $\pbundle$ be a principal $G$-bundle and let $V$ be a $\dim(M)$-dimensional vector space equipped with a representation\footnote{In general, this will be $V=\mathbb{R}^{\dim(M)}$ and $G=\GL(n,\mathbb{R})$.} $\rho:G\rightarrow\GL(V)$ such that $TM\cong P\times_G V$ as associated bundles. A solder(ing) form on $P$ is a tensorial 1-form (\cref{bundle:tensorial_form}) of type $(V,\rho)$.
    }

    \begin{example}[Tautological 1-form\footnotemark]\index{tautological!form}
        \footnotetext{\Cref{symplectic:tautological_form} gives another example in the setting of \textit{symplectic geometry}.}
        Consider a frame field $p:M\rightarrow FM$ (\cref{bundle:frame_bundle}) of the tangent bundle $TM$. A solder form for $FM\times_{{\GL(n)}}\mathbb{R}^n$ is given by
        \begin{gather}
            \theta := p^*(\pi_{FM,\ast})\,.
        \end{gather}
        In fact, one can prove that $\theta$ is actually the identity map $\mathbbm{1}_{TM}$.
    \end{example}

    \newdef{Torsion}{\index{torsion}\index{Cartan!structure equation}
        Let $\pbundle$ be a principal $G$-bundle equipped with a connection 1-form $\omega$ and a solder form $\theta$. The torsion $\Theta$ is defined as the exterior covariant derivative $\Dr\theta$. This is the content of the \textbf{Cartan structure equation}:
        \begin{gather}
            \Theta := \Dr\theta = \dr\theta + \omega\barwedge\theta\,.
        \end{gather}
    }

    \begin{property}[First Bianchi identity]\index{Bianchi identity}
        Let $\omega$ be a connection 1-form, $\Omega$ its associated curvature, $\theta$ a solder form and $\Theta$ its associated torsion.
        \begin{gather}
            \Dr\Theta = \Omega\barwedge\theta
        \end{gather}
    \end{property}

    \newdef{Contorsion}{\label{bundle:contorsion}
        Let $\omega$ be a connection 1-form. There exists an $\ad(P)$-valued 1-form $\sigma_\omega$ such that $\omega+\sigma_\omega$ is torsion-free. This 1-form is called the contorsion (tensor) of $\omega$.
    }
...
\section{Reduction of the structure group}\label{section:G-structure}
    ...
    \newdef{Reducible connection}{\index{connection!reducible}\label{bundle:reducible_connection}
        Consider a principal $G$-bundle $P$ equipped with a connection 1-form $\omega$. If a bundle map $F$ induces an $H$-reduction of $P$, the connection $\omega$ is said to be reducible (and to be compactible with the given reduction) if $F^*\omega$ takes values in $\mathfrak{h}$.
    }
...
\subsection{Chern--Weil theory}\label{section:chern_weil}
    ...
    \newdef{Chern--Weil morphism}{\index{Chern--Weil}
        Let $\pbundle$ be a principle $G$-bundle and choose a connection 1-form $\omega$ with curvature 2-form $\Omega$. There exists a morphism of algebras
        \begin{gather}
            \mathfrak{K}[\mathfrak{g}]^G\rightarrow\Omega^\bullet(P):f\mapsto f(\Omega)\,,
        \end{gather}
        where $\mathfrak{K}$ is the base field, satisfying:
        \begin{itemize}
            \item $f(\Omega)$ is closed.
            \item $f(\Omega)$ pulls back uniquely to a (closed) form $\overline{f}(\Omega) := \pi^*f(\Omega)$ on $M$.
            \item $\overline{f}(\Omega)$ does not depend on the chosen connection, i.e.~the difference $\overline{f}(\Omega)-\overline{f}(\Omega')$ for any two connection 1-forms $\omega,\omega'$ is exact.
        \end{itemize}
    }
...
\subsection{Complex bundles}
    ...
    On \textit{complex bundles}, one can always choose $\mathfrak{u}(n)$-valued connection 1-forms, since the structure group can be reduced to $\mathrm{U}(n)$. See \labelref{chapter:complex_geometry} for more information.

    \newdef{Chern class}{\index{Chern!class}\label{bundle:chern_class}
        Consider a rank-$n$ vector bundle $\bundle$ with curvature 2-form $\Omega$. Using Chern--Weil theory, one defines the Chern classes $c_k(E)\in H^{2k}(M)$ as follows:
        \begin{gather}
    ...
    \newdef{Chern character}{\index{Chern!character}\label{bundle:chern_character}
        Consider a rank-$n$ vector bundle $\bundle$ with curvature 2-form $\Omega$. Using Chern--Weil theory, one defines the Chern character as follows:
        \begin{gather}
    ...
    \begin{property}[Classification]\label{bundle:chern_classification}
        The classification~\ref{bundle:classification} of bundles and Chern--Weil theory imply that (complex) line bundles are entirely classified by their first Chern class, i.e.~by $H^2(M;\mathbb{Z})$.
    \end{property}
    \newdef{Picard group}{\index{Picard!group}
        The Abelian structure on $H^2(M;\mathbb{Z})$ also descends to a product on line bundles, the tensor product~\ref{bundle:tensor_product}. The set $H^2(M\mathbb{Z})\cong H^1(M;U(1))$ is in this context also called the Picard group of $M$ (see also \cref{sheaf:picard_group} for the sheaf-theoretic perspective).
    }
    ...
    \begin{example}[Todd class]\index{Todd class}\index{Bernoulli!number}\label{bundle:todd_class}
        ...
        \end{gather}
        where $B_i$ is the $i^{\text{th}}$ Bernoulli number (\cref{calculus:bernoulli_number}). Let $\pi:E\rightarrow M$ be a rank-$n$ vector bundle. If $x_i$ are the Chern roots of $E$, the Todd class is defined as
        \begin{gather}
...
\subsection{Real bundles}
    ...
    \newdef{Pontryagin class}{\index{Pontryagin!class}\label{bundle:chern_pontryagin}
        ...
        \begin{gather}
            \label{bundle:pontryagin_chern_formula}
            p_1(E)=c_1^2(E)-2c_2(E)=\mathrm{ch}_2(E)\,.
        \end{gather}
    }
...
\subsection{Lie group cohomology}\index{cohomology!Lie group}
...
\subsection{Chern--Simons forms}\label{section:chern_simons}\index{Chern--Simons!form}
    ...
    More generally, consider the \textit{concordance} $P\times[0,1]$ for some principal bundle $P$ with itself together with a connection $\widehat{A}$. This connection defines a path between two connections $A,A'$ on $P$. The relative Chern--Simons form is defined through transgression:\index{concordance}
    \begin{gather}
    ...
    \begin{example}[Killing form]\label{bundle:killing_transgression}
...
\subsection{Deligne cohomology}
    ...
    \newdef{Deligne complex}{\index{Deligne!complex}\label{bundle:deligne_complex}
        Let $\symbfsf{B}^k\mathrm{U}(1)_{\text{conn}}$ denote the cochain complex
        \begin{gather}
        ...
        \begin{gather}
            H_D^{k+1}(M;\mathbb{Z}) := \check{H}^0\bigl(M;\symbfsf{B}^k\mathrm{U}(1)_{\text{conn}}\bigr)\,,
        \end{gather}
        where $\check{H}^\bullet$ denotes \v{C}ech cohomology (\cref{sheaf:cech}). (Note that $\symbfsf{B}^k\mathrm{U}(1)_{\text{conn}}$ is turned into a chain complex by inverting the degrees.)
    }
    ...
    \begin{property}[Deligne--Beilinson product]\index{Deligne--Beilinson product}
        ...
        \begin{align}
            \cup&:\symbfsf{B}^k\mathrm{U}(1)_{\text{conn}}\otimes\symbfsf{B}^l\mathrm{U}(1)_{\text{conn}}\rightarrow\symbfsf{B}^{k+l+1}\mathrm{U}(1)_{\text{conn}}\nonumber\\
    ...
    \todo{COMPLETE (see book by Deligne)}
...
\subsection{Cartan geometry}
    ...
    \begin{property}
        The bundle $\pi:G\rightarrow G/H$ of a Klein geometry $G/H$ admits a 1-form $\omega:TG\rightarrow\mathfrak{g}$ that satisfies the following conditions:
        \begin{itemize}
        ...
        \end{itemize}
    \end{property}
    The last two conditions show that $\omega$ defines a principal connection 1-form, while the first condition states that this connection is flat. In fact, this 1-form is exactly the Maurer--Cartan form on $G$, where the last conditions are obtained by restricting to the subgroup $H\subset G$.
    ...
    \newdef{Cartan geometry}{\index{Cartan!geometry}\index{Cartan!connection}
        Consider a principal $H$-bundle $\pi:P\rightarrow M$ and a Lie algebra $\mathfrak{g}$ such that $\mathfrak{h}\leq\mathfrak{g}$ (in general, it is assumed that these form a model geometry). A Cartan geometry is characterized by a 1-form $\omega:TP\rightarrow\mathfrak{g}$ satisfying the following conditions:
        \begin{enumerate}
            \item $\omega$ restricts to an isomorphism on every fibre,
            \item $\omega$ is $H$-equivariant, and
            \item $\omega$ cancels $\mathfrak{h}$-fundamental vector fields.
        \end{enumerate}
        The 1-form $\omega$ is called the \textbf{Cartan connection}.
    }
    \newdef{Curvature}{\index{curvature}
...
\section{\texorpdfstring{$\mathcal{D}$-geometry $\clubsuit$}{D-geometry}}
    ...
    \newdef{Sheaf of differential operators}{
        Let $M$ be a smooth manifold (or scheme, see \labelref{chapter:alggeom}). Denote its structure sheaf by $\mathcal{O}_M$ and denote the sheaf of $\mathcal{O}_M$-algebras of vector fields (interpreted as derivations) on $M$ by $\mathcal{D}_M$:
        \begin{gather}

...
\subsection{Metric}
    ...
    \newdef{Bundle metric}{\index{metric!Riemannian}
    ...
    \begin{remark}\label{bundle:vector_calculus}
        ...
        \begin{mdframed}[roundcorner=10pt, linecolor=blue, linewidth=1pt]
            \begin{proof}[Explanation]
                ...
                \begin{align*}
                    \dr X^\flat &= \left(\pderiv{f_3}{x_2} - \pderiv{f_2}{x_3}\right)\drx_2\wedge\drx_3\\
                    &\qquad + \left(\pderiv{f_1}{x_3} - \pderiv{f_3}{x_1}\right)\drx_3\wedge\drx_1 + \left(\pderiv{f_2}{x_1} - \pderiv{f_1}{x_2}\right)\drx_1\wedge\drx_2\,,\\
...
\subsection{Riemannian manifolds}
    ...
    \newdef{Index and signature}{\index{signature}\index{index!of Riemannian manifold}
        Let $M$ be a connected pseudo-Riemannian manifold. The dimension of the `negative' subspace $N$ in the splitting $T_pP = P\oplus N$ is called the index of the manifold. The \textbf{signature} is defined as the pair $(\dim(P),\dim(N))$.
    }
    ...
    The following theorem by \indexauthor{Whitney} and \indexauthor{Cohen} is slightly stronger (although the name `conjecture' remained, it is actually a proven statement).\index[author]{Whitney}\index[author]{Cohen}
...
\subsection{Metric connections}
    ...
    \newdef{Levi-Civita connection connection}{\index{Riemann!connection}\index{Levi-Civita!connection|see{Riemann connection}}\label{riemann:levi_civita_connection}
        An affine connection $\nabla$ on a Riemannian manifold $(M,g)$ that satisfies the following two conditions:
        \begin{enumerate}
            ...
            \item\textbf{Torsion-freeness} (conform \cref{bundle:torsion}):
            \begin{gather}
                \nabla_XY - \nabla_YX = [X,Y]
            \end{gather}
            for all vector fields $X,Y$.
        \end{enumerate}
        The Levi-Civita connection is sometimes also called a \textbf{Riemannian connection}. Connections that are merely metric compatible are often simply called \textbf{metric connections}.
    }

    Because the existence of a metric is equivalent to the integrability of an $\mathrm{O}(n)$-structure by \cref{bundle:riemannian_G_structure}, \cref{bundle:integrable_torsion_free,bundle:connection_reducibility} imply that there exists a torsion-free connection that preserves this structure. The following theorem gives an even sharper result.
    \begin{theorem}[Fundamental theorem]\index{fundamental theorem!of Riemannian geometry}\label{riemann:unique}
        Every Riemannian\footnote{Or even pseudo-Riemannian.} manifold admits a unique Levi-Civita connection.
    \end{theorem}

    \newformula{Koszul formula}{\index{Koszul!formula}
        The Levi-Civita connection $\nabla$ is implicitly defined by the following formula:
        \begin{align}
            2g(\nabla_XY,Z) &= \mathcal{L}_Xg(Y,Z) + \dr(\iota_Xg)(Y,Z)\\
            &= X\bigl(g(Y,Z)\bigr) + Y\bigl(g(Z,X)\bigr) - Z\bigl(g(X,Y)\bigr)\nonumber\\
            &\hspace{3cm}+ g\bigl([X,Y],Z\bigr) - g\bigl([Z,X],Y\bigr) - g\bigl([Y,Z],X\bigr)\,.
        \end{align}
    }

    The following (local) formula is often useful in calculations (especially in \textit{general relativity}, see \cref{chapter:GR}).
    ...
    \newdef{Contorsion tensor}{\index{contorsion}
        Consider a metric connection $\nabla$. The difference between $\nabla$ and the unique Levi-Civita connection is called the contorsion (cf.~\cref{bundle:contorsion}):
        \begin{gather}
            K := \nabla-\nabla_{\text{LC}}\,.
        \end{gather}
        The contorsion can also be defined in terms of the torsion tensor (\cref{bundle:torsion}):
        \begin{gather}
...
\subsection{\difficult{Dirac operators}}
    ...
    \newdef{Index}{\index{index!of Dirac operator}

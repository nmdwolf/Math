...
\subsection{Dynamical systems}
    ...
    \newdef{Lagrangian}{\index{Lagrange!function}
        ...
        \begin{gather}
            L := \iota_{X_H}\theta - H\,,
        \end{gather}
        where $\theta$ is a symplectic potential. By Cartan's magic formula and the definition of Hamiltonian vector fields, one also has that
        \begin{gather}
            \mathcal{L}_{X_H}\theta = \dr L\,.
        \end{gather}
    }

    \newdef{Quasisymmetry}{\index{quasi-!symmetry}\index{Noether!charge}\label{symplectic:noether_charge}
        Consider a Lagrangian $L\in C^\infty(M)$. A quasisymmetry for $L$ is a vector field $X\in\mathfrak{X}(M)$ such that
        \begin{gather}
            X(L) = \dr f(X)
        \end{gather}
        for some $f\in C^\infty(M)$. The associated \textbf{Noether charge} is given by
        \begin{gather}
            Q := \sum_{i=1}^{2n}\pderiv{L}{\dot{z}^i}X^i - f\,.
        \end{gather}
        The first term is sometimes called the \textbf{bare Noether charge} $Q^0$. The Noether charge is the Hamiltonian generator of the quasisymmetry:
        \begin{gather}
            X^i = \{z^i,Q\}\,.
        \end{gather}
        This notion coincides with \cref{symplectic:noether} if one considers the `Hamiltonian' Lagrangian $L:=p_i\dot{q}^i-H$.
    }

...
\section{Neighbourhoods}
    ...
    \begin{example}[Monad]\index{monad}
        Given a neighbourhood relation $\sim$ on a set $M$, the (first-order) monad around $x\in M$ is defined as
        \begin{gather}
    ...
    \newdef{Geometric distribution}{\index{distribution!involutive}
        Let $M$ be a set equipped with a neighbourhood relation $\sim$. A (geometric) distribution on $M$ is a reflexive symmetric refinement $\approx$ of $\sim$. It is said to be \textbf{involutive} if
        \begin{gather}

\chapter{Calculus of Variations}\label{chapter:variation}
    ...
    The standard references for (global) variational calculus are~\cite{takens_global_1979,anderson_variational_1992}.
...
\subsection{Contact structure}
    ...
    \newdef{Contact form}{\index{contact!form}\index{Cartan!distribution}
        ...
        \begin{gather}
            \label{var:contact_form}
            \theta^\alpha_I := \symbfsf{d}u^\alpha_I - \sum_\mu u^\alpha_{I\mu}\symbfsf{d}x^\mu\,,
        \end{gather}
    ...
    \begin{remark}
        Note that, although the contact forms are often easier to work with, interior multiplication is only canonical with respect to the full differential:
        \begin{gather}
            \partial_\beta^J\intmul\symbfsf{d}u^\alpha_I = \delta^\alpha_\beta\delta^J_I = \partial_\beta^J\intmul\delta u^\alpha_I
        \end{gather}
        but
        \begin{gather}
            \partial_\mu\intmul\symbfsf{d}u^\alpha_I = 0 \qquad\neq\qquad \partial_\mu\intmul\delta u^\alpha_I=-u^\alpha_{I\mu}\,.
        \end{gather}
    \end{remark}
...
\subsection{Differential operators}
    ...
    \newdef{Generalized vector field}{\index{vector field!generalized}\index{order!of a vector field}
        Consider a vector bundle $\pi:E\rightarrow M$. A generalized vector field on $M$ is a `vector field' whose components are, locally, given by smooth functions on the infinite jet bundle $J^\infty(E)$, i.e.~it is a smooth map $X:J^\infty(E)\rightarrow TM$ and, hence, a smooth map $X:J^k(E)\rightarrow TM$ for some $k\in\mathbb{N}$ such that $X(\sigma)\in T_{\pi_k(\sigma)}M$. This minimal $k$ is called the \textbf{order} of $X$.

        Generalized vector fields on $E$ and $J^l(E)$ are given by replacing $TM$ by $TE$ or $T(J^l(E))$, respectively.
    }

    \begin{property}[Projectable vector field]\index{projectable}
        Generalized vector fields $X$ o $J^k(E)$ are projectable in that $(\pi_{k,l})_\ast X$ is again a vector field for all $l\leq k$. More generally, a vector field $X$ on $J^\infty(E)$ is said to be projectable if $(\pi_{\infty,k})_\ast X$ is a vector field on $J^k(E)$ for all $k\in\mathbb{N}$.
    \end{property}
...
\subsection{Differential structure}
    ...
    A smooth fibre bundle $\pi:E\rightarrow M$ over a base manifold $M$ will be considered.\footnote{In fact, the fibre bundle can be replaced with any fibred manifold.} First, the de Rham operator $\symbfsf{d}$ on the infinite jet bundle $J^\infty(E)$ is decomposed in a horizontal and a vertical part:
    \begin{gather}
        \symbfsf{d} = \dr+\delta\,.
    \end{gather}
    ...
    \begin{gather}
        \label{var:horizontal_derivative}
        \dr f := (D_\mu f)\drx^\mu\,,
    \end{gather}
    where the \textbf{total derivative}\footnote{This formula is virtually the same as the one for the true total derivative, with partial derivatives replaced by jet coordinates.}\index{total!derivative}
    \begin{gather}
    ...
    \newdef{Prolongation of vector fields}{\index{prolongation}\label{var:prolongation}
        Given a generalized vector field $X$ on a fibre bundle $\pi:E\rightarrow M$, there exists a unique vector field $j^kX$ on the jet bundle $J^k(E)$ defined by the following conditions:
        \begin{enumerate}
        ...
        \end{enumerate}
        Locally, the prolongation of a (generalized) vector field
        \begin{gather}
            \label{var:generalized_vector_field_on_E}
    ...
    A similar definition exists for vector fields on the base manifold.
    ...
    \newdef{Evolutionary vector field}{\index{vector field!evolutionary}\label{var:evolutionary_vector_field}
        ...
        The prolongation of an evolutionary vector field to $J^\infty(E)$ still projects to 0 on $M$. By extension, all vector fields on $J^\infty(E)$ that preserve the contact ideal and project to 0 on $M$ are called evolutionary vector fields. Such vector fields are of the form (cf.~\cref{var:generalized_vector_field_on_E})
        \begin{gather}
    ...
    \begin{property}[Preservation of bigrading]\label{var:preservation_bigrading}
        Evolutionary vector fields $Y\in\mathrm{Ev}(J^\infty(E))$ preserve the bigrading of the variation bicomplex under Lie derivatives:
        \begin{gather}
            \omega\in\Omega^{p,q}(J^\infty(E))\implies\mathcal{L}_{j^\infty Y}\omega\in\Omega^{p,q}(J^\infty(E))\,.
        \end{gather}
    \end{property}
    ...
    \begin{formula}[Cartan calculus]
        ...
        \begin{enumerate}
            \item Exterior derivatives:
                \begin{align}
                    \drx^\mu &= \symbfsf{d}x^\mu\,,\\
                    \delta u^\alpha_I &= \symbfsf{d}u^\alpha_I - u^\alpha_{I\mu}\drx^\mu \overset{\text{\cref{var:contact_form}}}{=} \theta_I^\alpha\,.
                \end{align}
            ...
            \item Lie derivatives:
                \begin{align}
                    \mathcal{L}_{j^\infty X} &= \symbfsf{d}\iota_{j^\infty X} + \iota_{j^\infty X}\symbfsf{d}\,,\\
...
\subsection{Euler operators}
    ...
    \begin{example}[Euler--Lagrange operator]\index{Euler--Lagrange!operator}\index{Lie--Euler operator}\index{shell}
        ...
        \end{gather}
        The set of sections $\phi$ that satisfy $\delta_{\text{EL}}L=0$ is called the \textbf{shell}:
        \begin{gather}
            \Sigma := \{u\in J^\infty(E)\mid\forall I:D_I\delta_{\text{EL}}L[u] = 0\}\,.
        \end{gather}
        These sections are also said to be \textbf{on shell}.
    \end{example}
...
\subsection{Functional complex}
    ...
    The interior Euler operator also induces a (global) decomposition of the variational bicomplex.
    \begin{property}[Functional decomposition]\label{var:functional_decomposition}
        \begin{gather}
            \Omega^{p,q}(J^\infty(E)) = \mathcal{F}^q(J^\infty(E)) \oplus \dr\Omega^{p-1,q}(J^\infty(E))
        \end{gather}
        In particular,
        \begin{gather}
            \omega = I(\omega) + \dr\sigma
        \end{gather}
        for some $\sigma\in\Omega^{p-1,q}(J^\infty(E))$.
    \end{property}
...
\subsection{Structure of the bicomplex}
    ...
    \begin{property}
        Consider a vector field $X$ on $J^\infty(E)$. Its Lie derivative will, in general, not respect the bigrading of the variational bicomplex (unless $X$ is evolutionary as in \cref{var:preservation_bigrading}) and, therefore, the `projected Lie derivative' is introduced:
        \begin{gather}
        ...
        \end{itemize}
        Note that this does not simply follow from the previous property since $X$ does not necessarily define a flow on $J^\infty(E)$. Moreover, it also satisfies
        \begin{gather}
            \mathcal{L}^\sharp_X\omega=\mathcal{L}_X\omega
        \end{gather}
        for all evolutionary or projectable vector fields $X$.
    \end{property}

    Aside from the differentials on the variational bicomplex, one should also look at the structure of the functional complex $(\mathcal{F}^\bullet,\delta_V)$ in \cref{var:functional_complex}. It can be shown that requiring both $[I,\Phi^\sharp]=0$ and $[\delta,\Phi^\sharp]=0$ is very restrictive. Furthermore, a complete characterization of those morphisms $\Phi$ that satisfy only $[I,\Phi^\sharp]=0$ is not fully understood. However, the infinitesimal version is easier to handle since it only involves linear equations:
    \begin{property}
    ...
    \begin{gather}
        \begin{aligned}
            \Phi^\natural &: \Omega^{\dim(M),q}(J^\infty(E))\rightarrow\mathcal{F}^q(J^\infty(E')):\omega\mapsto(I\circ\Phi^\sharp)\omega\,,\\
            \mathcal{L}^\natural_X &: \Omega^{\dim(M),q}(J^\infty(E))\rightarrow\mathcal{F}^q(J^\infty(E)):\omega\mapsto(I\circ\mathcal{L}^\sharp_X)\omega\,.
        \end{aligned}
    \end{gather}
...
\subsection{Variational problems}\label{section:variational_problems}
    ...
    \begin{property}[First variational formula]\index{variational!formula}
        ...
        \begin{gather}
            ...
            \symbfsf{dL} = \delta\symbfsf{L} = \delta_{\text{EL}}\symbfsf{L} + \dr\theta\,.
        \end{gather}
        The $\bigl(\dim(M)-1,1\bigr)$-form $\theta$ is often called the \textbf{presymplectic potential}.\index{potential!presymplectic}

        The reason why the above formula is called the first variational formula is the following. Given a local Lagrangian $\symbfsf{L}$, one can look at solutions of the associated variational principle obtained by extremizing over perturbations of a field configuration. Such perturbations are generated by evolutionary vector fields and, hence, one can write the extremality condition as
        \begin{gather}
            \forall X\in\mathrm{Ev}(J^\infty(E)):\mathcal{L}_{j^\infty X}\symbfsf{L} = 0\,.
        \end{gather}
        \Cref{var:first_variational_formula} then becomes:
        \begin{gather}
            \delta_\mathrm{EL}\symbfsf{L} + \dr\theta = 0\,.
        \end{gather}
    ...
    \begin{result}[Global variational formula]\label{var:general_variational_formula}
        ...
        \begin{gather}
            \mathcal{L}^\sharp_{j^\infty X}\symbfsf{L} = X_{\text{ev}}\intmul\delta_{\text{EL}}\symbfsf{L} + \dr\eta
        \end{gather}
    ...
    \newdef{Lepage form}{\index{Lepage equivalent}
        ...
        \begin{gather}
            \pi^{\dim(M),0}(X\intmul\symbfsf{d}\rho)=0
        \end{gather}
    ...
    \begin{property}[Lepage equivalent]
        Consider the first variational formula for a local Lagrangian $\symbfsf{L}$. A Lepage equivalent is given by the form $\symbfsf{L}-\theta$. Moreover, a form $\symbfsf{L}-\theta$, with $\symbfsf{L}\in\Omega^{\dim(M),0}(J^\infty(E))$ and $\theta\in\Omega^{\dim(M)-1,1}(J^\infty(E))$, is Lepage if and only if $\delta_{\text{EL}}\symbfsf{L}-\dr\theta\in\mathcal{F}^1(J^\infty(E))$.
    \end{property}

    The reason for considering Lepage equivalents is that, whereas one needs to take the Euler--Lagrange operator of $\symbfsf{L}$ to obtain the source form $\delta_{\text{EL}}\symbfsf{L}$, one only needs to take the exterior derivative of the Lepage equivalent to obtain that same source form.
...
\subsection{Symmetries}
    ...
    If the source form comes from a local Lagrangian, the above definition admits a specific case.
    \newdef{Bessel-Hagen symmetry}{\index{symmetry!Bessel-Hagen}\index{divergence!symmetry}\label{var:bessel_hagen}
        ...
        \end{gather}
        for some $\eta\in\Omega^{\dim(M)-1,0}(J^\infty(E))$.

        By \cref{var:projected_derivative_EL_operator}, if this condition only needs to hold locally, distinguished symmetries and Bessel-Hagen symmetries coincide. However, if the Bessel-Hagen condition is globally required, the Bessel-Hagen symmetries constitute a strict subalgebra of distinguished symmetries.
    }

    \newdef{Local conservation law}{\index{conservation law}
        A generator of a local conservation law of a source form $\Delta\in\mathcal{F}^1(J^\infty(E))$ is an evolutionary vector field $Y$ such that
        \begin{gather}
            \delta_{\text{EL}}(j^\infty Y\intmul\Delta) = 0
        \end{gather}
        or, again by \cref{var:EL_operator_properties} (or \cref{var:general_variational_formula}),
        \begin{gather}
            \iota_Y\Delta = \dr\eta
        \end{gather}
        for some local $\eta\in\Omega^{\dim(M)-1,0}(J^\infty(E))$. The reason for the terminology conservation law is that, on the shell $\Delta=0$, the form $\eta$ is $\dr$-closed and, hence, `conserved'.
    }
    \begin{remark}
    ...
    In fact, for locally variational source forms, \cref{var:general_variational_formula} implies that the two conditions in the result above are equivalent.
    ...
    \newdef{Noether current}{\index{Noether!current}
        Consider a Bessel-Hagen symmetry $X$ of the local Lagrangian $\symbfsf{L}$. By the variational formula~\eqref{var:first_variational_formula}, the following `current' is conserved in that it vanishes on shell:
        \begin{gather}
            j := \eta + \iota_{j^\infty X}\theta    \,.
        \end{gather}
    }

    Noether's theorem for local conservation laws can be globalized by passing from symmetries of the equations of motion to symmetries of the local Lagrangian.
    \begin{theorem}[Noether]
        Let $\symbfsf{L}$ be a local Lagrangian. Every global generalized symmetry $X$, i.e.~generalized vector field such that
        \begin{gather}
            \mathcal{L}^\sharp_{j^\infty X}\symbfsf{L}=0\,,
        \end{gather}
        generates a global conservation law for $\delta_{\text{EL}}\symbfsf{L}$ (through its evolutionary part).
    \end{theorem}
...
\subsection{Inverse problem}\label{section:inverse_problem}
    ...
    \indexauthor{Helmholtz} was the first to study the inverse problem, so the following sufficient conditions are named after him.
    \begin{property}[Helmholtz conditions]\index{Helmholtz!conditions}\index{variational!form}
        ...
        \begin{gather}
            \mathcal{L}_{j^\infty Y}\Delta = \delta_{\text{EL}}(Y\intmul\Delta)
        \end{gather}
        is satisfied for all evolutionary vector fields $Y$. This can also be rewritten in terms of the Helmholtz operator $\delta_V$ (hence its name):\footnote{This expression also immediately follows from \cref{var:I_properties}.}
        \begin{gather}
            \delta_V\Delta = 0\,.
        \end{gather}
    \end{property}

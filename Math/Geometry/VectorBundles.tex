\chapter{Vector Bundles}\label{chapter:vector_bundles}
    ...
    \epigraph{A tensor is something that transforms like a tensor.}{Every physics course ever.}
...
\section{Vector bundles}
    ...
    \end{construct}
    \begin{remark}
        As was also the case for tangent bundles, the choice of charts on a vector bundle is not arbitrary. To preserve the linear structure of fibres, the use of natural charts is imperative.
    \end{remark}
    ...
    \newdef{Weil divisor}{\index{Weil!divisor}\index{divisor|seealso{Weil}}
        Let $M$ be a smooth manifold. A (Weil) divisor on $M$ is a formal, integral combination of the form
        \begin{gather}
            D=\sum_{i\in I}n_iV_i\,,
        \end{gather}
        where the $V_i$ are irreducible codimension-1 hyperplanes and $\{V_i\}_{i\in I}$ is locally finite (\ref{topology:locally_finite}).
    }
    \begin{property}[Divisor--line bundle correspondence]
        Locally, on an open set $U_j$, every codimension-1 hypersurface $V_i$ can be obtained as the zero set of a nonzero, holomorphic function $f^i_j$. As such, every Weil divisor defines, locally, a nonzero, meromorphic function $f_j:=\prod_{i\in I}(f^i_j)^{n_i}$ which agrees on overlaps $U_j\cap U_{j'}$ with $f_{j'}$. These functions, in turn, induce transition functions $f_j/f_{j'}$ of a line bundle $\mathcal{O}(D)$.
    \end{property}

\subsection{Sections}

    \newdef{Frame}{\index{frame}\index{vielbein}\label{bundle:frame}
        A frame (field) or \textbf{vielbein} of a vector bundle $\pi:E\rightarrow B$ is a tuple $(s_1,\ldots,s_n)$ of sections such that $\bigl(s_1(b),\ldots,s_n(b)\bigr)$ is a basis for the fibre $\pi^{-1}(b)$ for all $b\in B$. By passing to local sections, one obtains a local frame.
    }
...
\subsection{Tubular neighbourhoods}
    ...
    \begin{theorem}[Tubular neighbourhood theorem]\label{bundle:tubular_neighbourhood_theorem}
        Every embedded submanifold admits a tubular neighbourhood: its normal bundle. Furthermore, all tubular neighbourhoods are diffeomorphic.
    \end{theorem}
...
\subsection{Sums and product}
    ...
    \newdef{Tensor product}{\index{tensor product!of vector bundles}\label{bundle:tensor_product}
...
\subsection{Koszul connections}
    ...
    \begin{example}[Riemannian geometry]
        Important examples in the case of the \textit{Levi-Civita connection} on a \textit{Riemannian manifold} are the volume form $\vol$ and the \textit{metric} $g$ (see \labelref{chapter:riemann}).
    \end{example}

    \begin{property}[Affinity]\label{bundle:affinity_koszul_connections}
...
\input{Math/Geometry/Integration}
\input{Math/Geometry/Cohomology}
\input{Math/Geometry/Morse}

\section{Differential operators}

    In this section, the study of PDEs is generalized to vector bundles. The case of PDEs on $\mathbb{R}^n$ was treated in \labelref{chapter:pde}.
    ...
    \newdef{Normally hyperbolic operators}{\index{hyper-!bolic operator}
        Consider a \textit{(pseudo)Riemannian} vector bundle $E$ (see \labelref{chapter:riemann}). A linear differential operator on $E$ is said to be normally hyperbolic if its principal symbol is proportional to the given metric.
    }

\subsection{Elliptic complexes}

    \newdef{Elliptic complex}{\index{elliptic!complex}
        Consider a sequence of differential operators $\bigl(D_n:C^\infty(E_n)\rightarrow C^\infty(E_{n+1})\bigr)_{n\in\mathbb{N}}$ on a collection of vector bundles $\{\pi_n:E_n\rightarrow M\}_{n\in\mathbb{N}}$. This system is called an elliptic complex if it is a cochain complex, i.e.~$D_{n+1}\circ D_n = 0$, and if the induced sequence $\bigl(\sigma_p(\xi)(D_n)\bigr)_{n\in\mathbb{N}}$ is exact for all $x\in M$ and $\xi\neq0$.
    }
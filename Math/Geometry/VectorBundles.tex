...
\subsection{Sections}
    ...
    \newdef{Frame}{\index{frame}\index{vielbein}\label{bundle:frame}
        A frame (field) or \textbf{vielbein} of a vector bundle $E$ is a tuple $(s_1,\ldots,s_n)$ of sections such that $\bigl(s_1(b),\ldots,s_n(b)\bigr)$ is a basis for the fibre $\pi^{-1}(b)$ for all $b\in B$. By passing to local sections, one obtains a local frame.
    }

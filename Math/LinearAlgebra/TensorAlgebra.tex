\chapter{Vector \& Tensor Calculus}
    ...
    References for this chapter are~\citet{jeevanjee_introduction_2015,choquet-bruhat_analysis_1991}. For a more geometric approach to some of the concepts and results in this chapter, see \namecrefs{chapter:curves_surfaces}~\nameref{chapter:curves_surfaces} and~\nameref{chapter:vector_bundles}, and \cref{section:integration_manifolds} (or \cref{bundle:vector_calculus}).
...
\section{Nabla-operator}\label{section:nabla}
    ...
    \begin{example}
        Let $\varphi:\mathbb{R}^n\rightarrow\mathbb{R}$ be a smooth function and let $\displaystyle\deriv{\vector{r}}{s}$ denote the tangent vector to a curve $\vector{r}(s)$ with \textit{natural parameter} (see \cref{diff:natural_parameter}). The variation of $\varphi$ along $\vector{r}(s)$ is given by
        \begin{gather}
            \pderiv{\varphi}{s} = \deriv{\vector{r}}{s}\cdot\nabla\varphi\,.
        \end{gather}
    \end{example}

    \begin{property}[Orthogonality]\label{vector:normal_vector}
        The gradient of a smooth real-valued function is perpendicular to its level sets (\cref{set:level_set}). Let $\gamma:\,]\!-\varepsilon,\varepsilon[\rightarrow\mathbb{R}^n$ be a differentiable curve in the level set $f^{-1}(c)$. Since $\varphi$ is constant on $\gamma$, one obtains:
        \begin{gather}
            \deriv{}{t}\varphi\bigl(\gamma(t)\bigr) = \nabla\varphi|_{\gamma(t)}\cdot\gamma'(t) = 0\,.
        \end{gather}
        It follows that $\nabla\varphi$ is everywhere normal to the level sets.
    \end{property}
    \begin{result}\label{vector:tangent_gradient}
        The gradient of a smooth real-valued function determines a normal vector to its graph (\cref{set:graph}) of its level sets. The graph of $\varphi$ is implicitly given by
        \begin{gather}
            f(x^1,\ldots,x^n,c):=\varphi(\vector{x})-c=0\,.
        \end{gather}
        The gradient to the function $f:\mathbb{R}^{n+1}\rightarrow\mathbb{R}$ is given by
        \begin{gather}
            \left(\pderiv{\varphi}{x^1},\ldots,\pderiv{\varphi}{x^{n-1}},-1\right)\equiv(\nabla \varphi,-1)\,.
        \end{gather}
        Since this result actually does not depend on the value $c$, it generalizes to all sets that can be characterized through an implicit equation $\varphi(x^1,\ldots,x^n)=c$.
    \end{result}
...
\subsection{Tensor product}\index{tensor product!of vector spaces}
    ...
    \newdef{Tensor component}{
        Let $\symbf{T}$ be a tensor that takes $r$ vectors and $s$ covectors as input and returns a scalar (element of the underlying field). The components of $\symbf{T}$ with respect to a frame $\{e_i\}_{i\leq n}$ and a coframe $\{e^i\}_{i\leq n}$ are defined as $T_{i\ldots j}^{\ \ \ \ k\ldots  l} := \symbf{T}(e_i,\ldots,e_j,e^k,\ldots,e^l)$.
    }
...
\subsection{Hodge star}
    ...
    \newdef{Selfdual form}{\index{dual!selfdual}\label{vector:self_dual_form}
        Let $V$ be a 4-dimensional inner product space and consider an element $\omega\in\Lambda^2(V)$. Since $\ast^2=1$ on $\Lambda^2(V)$, it has two eigenvalues: $+1$ and $-1$. The associated eigenvectors are said to be selfdual and antiselfdual, respectively.

        Every element $\omega\in\Lambda^2(V)$ can be uniquely decomposed as the sum of a selfdual and an antiselfdual two-form:
        \begin{gather}
            \omega \equiv \omega^++\omega^- = \frac{1}{2}\bigl((\omega + \ast\omega) + (\omega - \ast\omega)\bigr)\,.
        \end{gather}
    }
...
\section{Graded vector spaces}\label{section:graded_spaces}
    ...
    \newdef{Parity and suspension}{\index{parity!functor}\index{suspension}\label{hda:suspension}
        Consider the category $\symbfsf{sVect}$ of super-vector spaces. One can define the \textbf{parity functor} $\func{\Pi}{sVect}{sVect}$ as the functor that interchanges even and odd subspaces:
        \begin{gather}
            \begin{aligned}
                (\Pi V)_0 &:= V_1\,,\\
                (\Pi V)_1 &:= V_0\,.
            \end{aligned}
        \end{gather}
        A more general construction exists in $\mathbb{Z}\symbfsf{Vect}$. For every graded vector space $V$, the $k$-\textbf{shifted} vector space or $k$-\textbf{suspension} $V[k]$ is defined as follows (some authors use the opposite convention):
        \begin{gather}
            V[k]_i := V_{i-k}\,.
        \end{gather}
    }

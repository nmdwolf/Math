...
\subsection{Linear independence}
    ...
    \newdef{Linear combination}{\index{basis!Hamel}\label{linalgebra:linear_combination}
        ...
        \end{gather}
        for some $\{\lambda_i\}_{i\leq n}\subset\mathfrak{K}$. One can generalize this to general subsets $S\subseteq V$, but the number of nonzero elements $\lambda_i$ is always required to be finite.\footnote{Generalizations are possible in the context of topological vector spaces (see \cref{chapter:topology} and \cref{chapter:functional}), where one can define the notion of convergence.} (See \cref{linalgebra:hamel_remark} about \textit{Hamel bases}.)
    }
...
\subsection{Homomorphisms}
    ...
    \newdef{Homomorphism space}{\index{morphism!of vector spaces}\label{linalgebra:hom_space}
        ...
        \end{gather}
        The collection of $\mathfrak{K}$-vector spaces and linear maps between them form a category $\mathbf{Vect}_{\mathfrak{K}}$.
    }

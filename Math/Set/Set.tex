...
\section{Set operations}
    ...
    \newdef{Complement}{\index{complement}\label{set:complement}
        Let $\Omega$ be the universe of discours (see \cref{section:universes}) and consider $X\subseteq\Omega$. The complement of $X$ is defined as follows:
        \begin{gather}
...
\subsection{Families and filters}
    ...
    \newdef{Filtration}{\index{filtration}\label{set:filtration}
        Consider a set $X$ together with a collection of subsets $F_i$ indexed by a \textit{totally ordered set} $I$ (see \cref{set:total_order}). The collection is said to be a filtration of $X$ if
        \begin{gather}
...
\subsection{Algebra of sets}
    ...
    \begin{definition}[Generated $\sigma$-algebras]\label{set:generated_sigma_algebra}
    ...
    \begin{theorem}[Monotone class theorem]\label{set:monotone_class}
        Let $\mathcal{A}$ be an algebra of sets (\cref{set:algebra_of_sets}). If $\mathcal{G}_{\mathcal{A}}$ is the smallest monotone class containing $\mathcal{A}$, then it coincides with the $\sigma$-algebra generated by $\mathcal{A}$.
    \end{theorem}
...
\subsection{\difficult{Ordinals}}
    ...
    \begin{remark}\index{Burali--Forti paradox}
        The \textit{Burali--Forti paradox} says that the class of all ordinals (and, by extension, the class of all well-ordered sets) is not a set.
    \end{remark}
...
\section{Limits}
    ...
    \newdef{Direct system}{\index{direct!system}
        Let $(I,\leq)$ be a directed set (\cref{set:directed_set}) and let $\{A_i\}_{i\in I}$ be a family of objects. Consider a collection of morphisms $\{f_{ij}:A_i\rightarrow A_j\}_{i,j\in I}$ between these objects with the following properties:
        \begin{enumerate}
    ...
    \newdef{Inverse system}{\index{inverse!system}
        Let $(I,\leq)$ be a directed set (\cref{set:directed_set}) and let $\{A_i\}_{i\in I}$ be a family of objects. Consider a collection of morphisms $\{f_{ij}:A_j\rightarrow A_i\}_{i,j\in I}$ between these objects with the following properties:
        \begin{enumerate}
            \item for every $i\in I$: $f_{ii} = \mathbbm{1}_{A_i}$, and
...
\subsection{Partition}\label{section:partition}
    ...
    \newdef{Young tabloid}{
        A Young tabloid of shape $\lambda$ is defined as the equivalence class of Young tableaux that are connected by permuting the elements within a row. These are often drawn as in \cref{fig:young_tabloid}.
        \begin{figure}[!ht]
            \centering

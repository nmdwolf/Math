...
\subsection{Families and filters}
    ...
    \newdef{Power set}{\index{power!set}\label{set:power_set}
        ...
        Let $S$ be a set. The power set is defined as the set of all subsets of $S$ and is (often) denoted by $P(S)$ or $2^S$. The existence of this set is enforced by the power set axiom \ref{set:power_set_axiom}.
    }
...
\subsection{Posets}
    ...
    \newdef{Order-preserving map}{
        A function $f:(P,\leq)\rightarrow(P',\preceq)$ of posets such that $p\leq q\implies f(p)\preceq f(q)$ for all $p,q\in P$.
    }
    ...
    \newdef{Totally ordered set}{\index{totality}\label{set:total_order}
        A poset $P$ with the property that for all $a,b\in P$ either $a\leq b$ or $b\leq a$ is called a (nonstrict) totally ordered set. This property is called \textbf{totality}.
    }
    ...
    \newdef{Supremum}{\index{supremum}\label{set:supremum}
        The supremum $\sup(X)$ of a poset $X$ is the least upper bound of $X$.
    }
    ...
    \newdef{Cut}{\index{cut}\index{Dedekind cut}
        Let $(P,\leq)$ be a totally ordered set. A cut or \textbf{decomposition} of $P$ is a pair $(A,B)$ of disjoint subsets such that $P=A\cup B$ in the ordered sense.

        Cuts can be classified as follows:
        \begin{itemize}

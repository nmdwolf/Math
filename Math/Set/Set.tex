...
\subsection{Families and filters}
    ...
    \newdef{Filter}{\index{filter}\label{set:filter}
        Let $X$ be a \textit{partially ordered set} (see \cref{set:poset} below). A family $\mathcal{F}\subseteq P(X)$ is a filter on $X$ if it satisfies the following conditions:
        \begin{enumerate}
            \item\textbf{Empty set}: $\emptyset\not\in\mathcal{F}$.
            \item\textbf{Closed under intersections}: $\forall A,B \in\mathcal{F}:A\cap B\in\mathcal{F}$.
            \item\textbf{Closed under inclusions}: if $A\in\mathcal{F}$ and $A\subseteq B$, then $B\in\mathcal{F}$.
        \end{enumerate}
    }
...
\subsection{Algebra of sets}
    ...
    \begin{definition}[Product $\sigma$-algebras]\label{set:product_of_sigma_algebras}
        The product $\sigma$-algebra $\mathcal{F}_1\otimes\mathcal{F}_2$ on $X_1\times X_2$ can be defined in the following equivalently ways:
        \begin{itemize}

...
\section{Set operations}
    ...
    \newdef{Complement}{\index{complement}\label{set:complement}
        Let $\Omega$ be the universe of discours (Section \ref{section:universes}) and consider $X\subseteq\Omega$. The complement of $X$ is defined as follows:
        \begin{gather}
    ...
    \newdef{Converse relation}{\index{converse}\label{set:converse}
        Consider a relation $R\subset X\times Y$. The converse relation $R^t$ is defined as follows:
        \begin{gather}
...
\subsection{(Co)domain}
    ...
    \newdef{Level set}{\index{level set}\index{fibre}\label{set:level_set}
        ...
        \end{gather}
        For $X=\mathbb{R}^2$ the level sets are called \textbf{level curves} and for $X=\mathbb{R}^3$ they are called \textbf{level surfaces}. More generally, for functions $f:X\rightarrow Y$, one calls the preimage of a singleton $\{y\}\in Y$ the \textbf{fibre} over $y$.
    }
...
\section{Collections}
\subsection{Families and filters}

    \newdef{Power set}{\index{power!set}\label{set:power_set}
        \nomenclature[S_P]{$P(X),2^X$}{power set of a set $X$}
        Let $X$ be a set. The power set is defined as the set of all subsets of $X$ and is (often) denoted by $P(X)$ or $2^X$. The existence of this set is enforced by the power set axiom \ref{set:power_set_axiom}.
    }
    \result{All sets are elements of their power set: $X\in P(X)$.}

    \newdef{Collection}{\index{collection}
        Let $X$ be a set. A collection of elements in $X$ is a subset of $X$.
    }
    ...
    \newdef{Diagonal}{\index{diagonal}
        ...
        \begin{gather}
            \Delta_X := \big\{(x,x)\in X\times X\mid x\in X\big\}.
        \end{gather}
    }

    \newdef{Cover}{\index{cover}\label{set:cover}
        A cover of a set $X$ is a collection of sets $\mathcal{F}\subseteq P(X)$ such that
        \begin{gather}
            \bigcup_{V\in\mathcal{F}}V=X.
        \end{gather}
    }
    ...
    \newdef{Filtration}{\index{filtration}\label{set:filtration}
        Consider a set $X$ together with a collection of subsets $F_i$ indexed by a totally ordered set $I$. The collection is said to be a filtration of $X$ if
        \begin{gather}
            i\leq j\implies F_i\subseteq F_j.
        \end{gather}
        A filtration is said to be \textbf{exhaustive} if $\bigcup_iF_i=X$ and \textbf{separated} if $\bigcap_iF_i=\emptyset$.
    }
    \newdef{Associated grading}{
        In the case where one can define quotient objects every filtration $\{F_i\}_{i\in\mathbb{N}}$ of $X$ defines an associated graded object $\{G_i := F_i/F_{i-1}\}$.
    }
...
\subsection{Algebra of sets}
    ...
    \begin{definition}[Product $\sigma$-algebras]\label{set:product_of_sigma_algebras}
...
\subsection{Posets}
    ...
    \newdef{Preordered set}{\index{pre-!order|see{order}}
        A set equipped with a reflexive and transitive binary relation.
    }
    \newdef{Partially ordered set}{\index{poset}\label{set:poset}
        A set $P$ equipped with a binary relation $\leq$ is called a partially ordered set (or \textbf{poset}) if the following 3 axioms are fulfilled for all elements $x,y,z\in P$:
        \begin{enumerate}
    ...
    \newdef{Order-preserving map}{
        A function $f:(P,\leq)\rightarrow(P',\preceq)$ of posets such that $x\leq y\implies f(x)\preceq f(y)$ for all $x,y\in P$.
    }

    \newdef{Hasse diagram}{\index{Hasse diagram}
        Consider a finite poset $(P,\leq)$. A Hasse diagram for $P$ is a graph where the vertices are given by the elements of $P$ and two vertices $x,y$ are connected upwardly if and only if $x\leq y$ in $P$.
    }
    \begin{example}
    ...
    \newdef{Totally ordered set}{\index{totality}\label{set:total_order}
        A poset $P$ with the property that for all $x,y\in P$ either $x\leq y$ or $y\leq x$ is called a (nonstrict) totally ordered set. This property is called \textbf{totality}.
    }
    ...
    \newdef{Cut}{\index{cut}\index{Dedekind cut}
        Let $(P,\leq)$ be a totally ordered set. A cut or \textbf{decomposition} of $P$ is a pair $(A,B)$ of disjoint subsets such that $P=A\cup B$ in the ordered sense. Cuts can be classified as follows:
        \begin{itemize}
...
\subsection{\difficult{Ordinals}}
    ...
    \newdef{Well-ordering}{\index{well-!order}\index{well-!founded}
        A \textbf{well-founded} linear order, i.e.~a linear order $\leq$ such that every nonempty subset has a minimal element.
    }
    ...
    The most-used construction is the one by \textit{Von Neumann}. For every well-ordered set $W$ there exists a function $W\rightarrow P(W):w\mapsto W_{\leq w}:=\{w'\in W\mid w'\leq w\}$ that restricts to an order isomorphism on its image. This leads to the following definition:
    ...
    \begin{property}
        Every ordinal type is uniquely order-preserving isomorphic to an ordinal number. Consequently, every order-preserving isomorphism between an order type and itself is the identity.
    \end{property}
...
\subsection{\difficult{Cardinals}}
    ...
    \newadef{Cardinal number}{
        The cardinal number of a set is the smallest ordinal rank of any well-order on it, i.e.~any ordinal number bijective to it.\footnote{The well-ordering theorem (if assumed) assures that this definition coincides with the naive one above.} The cardinal numbers inherit a well-ordering from the ordinal numbers.
    }
    ...
    \newdef{Successor}{\index{successor}
        Given a cardinal $\kappa$, its successor $\kappa^+$ is defined as the smallest cardinal larger than $\kappa$.
    }
    \sremark{It should be noted that the successor of a cardinal number is not necessarily the same as its successor as an ordinal number (in fact this is only the case for finite cardinals).}
    ...
    \newdef{Regular cardinal}{\index{regular!cardinal}\label{set:regular_cardinal}
        ...
        \begin{gather}
            \kappa=\sum_{i\in I}\lambda_i\land\forall i\in I:\lambda_i<\kappa\implies|I|\geq\kappa.
        \end{gather}
    }

    The following theorem can easily be proven by a diagonal argument:
    \begin{theorem}[Cantor]\index{Cantor}
        Let $S$ be any set of cardinality $\kappa$. The power set $P(S)$ has cardinality greater than $\kappa$.
    \end{theorem}

\subsection{Lattices}\index{lattice}

    \newdef{Semilattice}{\index{join}\index{meet}
        A poset $(P,\leq)$ for which every 2-element subset has a supremum, called the \textbf{join}, is called a join-semillatice. Similarly, a poset $(P,\leq)$ for which every 2-element subset has an infimum, called the \textbf{meet}, called a meet-semilattice.
    }
    \begin{notation}
    ...
    \newadef{Lattice}{
        ...
        \end{enumerate}
        To go from this definition to the order-theoretic one, define the partial order \[x\leq y\iff x\land y=x.\] There exists an equivalent relation for the join.
    }

    \newdef{Complete lattice}{\index{complete!lattice}
        A lattice is said to be $\sigma$-complete (resp.~complete) if it admits all\footnote{When working with categories this has to be restricted to ``all small joins/meets'' or, equivalently, the index category should be a set.} countable (resp.~all) joins. It can be proven that complete lattices also admit all meets (as a special case of the \textit{adjoint functor theorem}).
    }
    ...
    \newdef{Frame}{\index{frame!set theory}\label{set:frame}
        A complete lattice $(P,\leq)$ for which the \textbf{infinite distributivity law} is satisfied:
        \begin{gather}
    ...
    \newdef{Boolean algebra}{\index{law!of excluded middle}\index{Boolean!algebra}
        A Boolean algebra $X$ is a Heyting algebra in which the \textbf{law of excluded middle} holds:
        \begin{gather}
            \forall x\in X:\neg\neg x=x.
        \end{gather}
...
\section{Limits}
    ...
    \newdef{Direct system}{\index{direct!system}
        Let $(I,\leq)$ be a directed set \ref{set:directed_set} and let $\{A_i\}_{i\in I}$ be a family of objects. Consider a collection of morphisms $\{f_{ij}:A_i\rightarrow A_j\}_{i,j\in I}$ between these objects with the following properties:
        \begin{enumerate}
    ...
    \newdef{Direct limit\footnotemark}{\index{direct!limit}\index{inductive!limit}\label{set:direct_limit}
        ...
        The operations on $A$ are defined such that the inclusion maps $\phi_i:A_i\rightarrow A$ are morphisms.
    }

    \newdef{Inverse system}{\index{inverse!system}
        Let $(I,\leq)$ be a directed set \ref{set:directed_set} and let $\{A_i\}_{i\in I}$ be a family of objects. Consider a collection of morphisms $\{f_{ij}:A_j\rightarrow A_i\}_{i,j\in I}$ between these objects with the following properties:
        \begin{enumerate}
            \item for every $i\in I$: $f_{ii} = \mathbbm{1}_{A_i}$, and

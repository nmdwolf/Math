...
\subsection{(Co)domain}
    ...
    \newdef{Graph}{\index{graph}\label{set:graph}
        Let $f:X\rightarrow Y$ be a function. The graph of $f$ is obtained by embedding the image $\im(f)$ as a new `dimension':
        \begin{gather}
...
\subsection{\difficult{Ordinals}}
    ...
    \end{gather}
    mapping an element to the \textbf{initial segment} bounded by it, that restricts to an order isomorphism on its image.\index{initial!segment} This leads to the following definition.
...
\subsection{Lattices}
    ...
    \newdef{Complemented lattice}{\index{complement}\index{lattice!orthomodular}\label{set:complemented_lattice}
        A bounded lattice in which, for every $a\in L$, there exists at least one $b\in L$ such that
        \begin{gather}
...
\subsection{Partition}\label{section:partition}
    ...
    \newdef{Composition}{\index{composition}
        Consider $k,n\in\mathbb{N}$. A $k$-composition of $n$ is a $k$-tuple $(t_1,\ldots, t_k)\subset\mathbb{N}$ such that $\sum_{i=1}^kt_k = n$.
    }
    \newdef{Partition}{\index{partition}

...
\subsection{Grothendieck topologies}
    ...
    \newdef{Sieve}{\index{sieve}
        ...
        A sieve $S$ on an object $x\in\symbfsf{C}$ is a sieve in the slice category $\symbfsf{C}_{/x}$. This means that $S$ is a subset of $\mathrm{ob}(\symbfsf{C}_{/x})$ that is closed under precomposition, i.e.~if $y\rightarrow x\in S$ and $z\rightarrow y\in\mathrm{hom}(\symbfsf{C})$, then $z\rightarrow y\rightarrow x\in S$.

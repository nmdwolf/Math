...
\subsection{Products}
    ...
    \newdef{Product}{\index{product}\index{recursion}\index{induction}\index{unit!type}\index{projection}\index{pairing}
        ...
        \begin{itemize}
            ...
            with the constraint
            \begin{gather}
                \mathrm{rec}_{A\times B}\big(C,f,(a,b)\big):\equiv f(a)(b).
            \end{gather}
        \end{itemize}
    }
...
\subsection{Identity types}
    ...
    \newdef{Equivalence}{\index{equivalence}\index{homotopy!equivalence}
        ...
        \begin{gather}
            \mathrm{isEquiv}(f) :\equiv \prod_{b:B}\mathrm{isContr}\big(\mathrm{hfibre}(b,f)\big)
        \end{gather}
        is inhabited.
        ...
        \begin{gather}
            p:\prod_{a:A}g\big(f(a)\big)=a \qquad\text{and}\qquad q:\prod_{b:B}f\big(g(b)\big)=b.
        \end{gather}
...
\subsection{Polynomial functors}
    ...
    \newdef{Polynomial}{\index{polynomial}
        ...
        \end{gather}
        where $\prod_g$ and $\sum_h$ are dependent product and sum functors \ref{topos:dependent_functors}.
    }
...
\subsection{Type theory}\label{section:modal_type_theory}
    ...
    \newdef{Modality}{\index{modality}\index{modal!operator}
        Consider an $(\infty,1)$-topos $\mathbf{H}$. A modality or \textbf{modal operator} on $\mathbf{H}$ is an idempotent\footnote{Sometimes, idempotency is not required.} (co)monad on $\mathbf{H}$ (see also Definition~\ref{cat:closure_operator}).
    }

    \newdef{Modal type}{\index{modal!type}
        A type $X\in\mathbf{H}$ is said to be modal for a modality $\Diamond$ if the unit $\eta_X:X\rightarrow\Diamond X$ or counit $\varepsilon_X:\Diamond X\rightarrow X$ is an equivalence.
    }
    \begin{property}
        The modal types of a (co)modality $\Diamond$ on $\mathbf{H}$ are exactly the $\Diamond$-(co)algebras or, equivalently, the objects of the Eilenberg-Moore category $\mathbf{H}^\Diamond$ (Definition \ref{cat:eilenberg_moore}). This property allows to define modal types for (co)monads that are not idempotent as their (co)algebras. It also means that one can recover the (co)monad objectwise as the composition of the (co)unit and the inclusion.
    \end{property}

    \newdef{Opposite}{\index{opposite}
        Consider a modality $\Diamond:\mathbf{H}\rightarrow\mathbf{H}$. Its (left) opposite is another modality $\bigcirc:\mathbf{H}\rightarrow\mathbf{H}$ such that $\bigcirc\dashv\Diamond$ together with an adjoint triple
        \begin{gather}
            \mathbf{H}^\bigcirc\cong\mathbf{H}^\Diamond\overset{\hookrightarrow}{\underset{\hookrightarrow}{\leftarrow}}\mathbf{H}\,,
        \end{gather}
        where the inclusions are the inclusions of modal types. Analogously, a right opposite is given by an adjunction $\Diamond\dashv\bigcirc$ together with an adjoint triple
        \begin{gather}
            \mathbf{H}^\bigcirc\cong\mathbf{H}^\Diamond\overset{\leftarrow}{\underset{\leftarrow}{\hookrightarrow}}\mathbf{H}.
        \end{gather}
        This, in particular, implies that the modalities $\Diamond$ and $\bigcirc$ are themselves adjoint.
    }
    ...
    \newdef{Negative}{\index{negative}
        Consider a comodality $\bigcirc:\mathbf{H}\rightarrow\mathbf{H}$. The negative of a modal type $X$ is obtained by removing its ``pure $\bigcirc$-part'', i.e.~its projection under $\bigcirc$. Categorically this means that one takes the cofibre:
        \begin{gather}
    ...
    \newdef{Aufhebung}{\index{Aufhebung}\label{type:aufhebung}
        Consider an inclusion of adjoint modalities:
        \begin{gather}
            \begin{tikzpicture}
                \node at (0, 0) {$\Diamond_1$};
                \node at (2, 0) {$\bigcirc_1$};
                \node at (0, -2) {$\Diamond_2$};
                \node at (2, -2) {$\bigcirc_2$};
                \node at (1, 0) {$\dashv$};
                \node at (1, -2) {$\dashv$};
                \node at (0, -1) {$\vee$};
                \node at (2, -1) {$\vee$};
            \end{tikzpicture}
        \end{gather}
        Right Aufhebung (of opposites) is given by an inclusion $\Diamond_2<\bigcirc_1$ of modal types.
    }

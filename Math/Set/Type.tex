\chapter{Logic and Type Theory}\label{chapter:type_theory}
    ...
    The main reference for this chapter is~\citet{program_homotopy_2013}. For a formal introduction to $\lambda$-calculus, see~\citet{selinger_lecture_2008}.
...
\section{Introduction to type theory}
    ...
    As in~\citet{program_homotopy_2013}, a universe hierarchy \`a la Russell will be adopted, i.e.~a sequence of universes $\seq{\mathcal{U}}$ will be used where the terms of every universe are types and every universe is cumulative in the sense that $A:\mathcal{U}_n\implies A:\mathcal{U}_{n+1}$. In general the subscripts will be omitted. However, one should take into account that every well-typed judgement should admit a formulation in which subscripts can be assigned in a consistent way.
...
\subsection{Functions}
    ...
    \begin{remark}[Scope]
        The $\Pi$-symbol scopes over all expressions to the right of the symbol, unless delimited (similar to $\lambda$-calculus), e.g.~\[\prod_{a:A}B(a)\rightarrow C(a)\equiv\prod_{a:A}\bigl(B(a)\rightarrow C(a)\bigr)\,.\]
    \end{remark}
...
\subsection{Identity types}
    ...
    \newdef{Equality type\footnotemark}{\index{equality}
        ...
        \begin{itemize}
            ...
            \item \textbf{Elimination and computation rules}: Here, the so-called \textbf{path induction principle} for equality types is presented, for the equivalent \textit{based path induction principle} see~\citet{program_homotopy_2013}.
...
\section{Modal logic}\index{logic!modal}

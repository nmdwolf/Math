...
\subsection{Propositional logic}
    ...
    \newdef{Logical connectives}{\index{conjunction}\index{disjunction}\index{implication}\index{negation}
        ...
        \end{itemize}
        Using implication, one can also define the logical `not' (\textbf{negation}):
        \begin{gather}
            \label{type:negation}
            \lnot P :\equiv P\implies\bot\,.
        \end{gather}
    }
    ...
    \newdef{Model}{\index{model}\index{interpretation}
        Consider a set of (propositional) formulas $\phi\equiv\{\phi_i\}_{i\in I}$. An \textbf{interpretation} of $\phi$ is an assignment $L$ of truth values to and a model for $\phi$ is an interpretation such that $\phi_i\vdash_L\top$ for all $i\in I$.
    }

    Let the (finite) universe $\Omega\equiv 2^n$ be generated by $n\in\mathbb{N}$ propositional variables $X_i\in\{x_i,\lnot x_i\}$. A set of formulas $\phi\equiv\{\phi_i\}_{i\in I}$ determines a subset $E_\phi\subseteq\Omega$, i.e. all models for $\phi$. Both $\phi$ and $E_\phi$, since they carry the same information, are also called \textbf{knowledge bases} or \textbf{epistemic states}.\index{knowledge}\index{epistemic!state}

    \newdef{Tautologies and inconsistencies}{\index{tautology}\index{consistent}
        A formula is said to be a tautology if it is true for every interpretation: $\phi\vdash\top$, i.e.~if $E_\phi=\Omega$. On the other hand, if, for a given set of formulas, there exists no interpretation such that all formulas are true, i.e.~if $E_\phi=\emptyset$, they are said to be (logically) inconsistent.

        Given a knowledge base $\phi$, a statement $A$ is said to be implied by $\phi$ if $E_\phi\subseteq A$. It is said to be consistent with $\phi$ if $A\cap E_\phi\neq\emptyset$.
    }
...
\subsection{Functions}
    ...
    \newdef{Function type}{\index{function}
        ...
        \begin{itemize}
            ...
            \item \textbf{Introduction rule}: A function can be defined by an explicit definition $f(x):\equiv\Phi$, where $\Phi$ is an expression possibly involving $x$, or by $\lambda$-abstraction $f:\equiv\lambda x.\Phi$.
...
\subsection{Products}
    ...
    \newdef{Nullary product}{\index{product}
        One can also define a nullary product. In this case, it is called the \textbf{unit type} $\symbf{1}$.
        \begin{itemize}
            \item \textbf{Formation rule}: $\type{\symbf{1}}$.
            \item \textbf{Introduction rule}: There is a unique nullary constructor $\ast:\symbf{1}$.
            ...
            \begin{gather}
                \mathrm{rec}_{\symbf{1}}:\prod_{\type{C}}C\rightarrow\symbf{1}\rightarrow C\,.
            \end{gather}
            On the constructor $\ast:\symbf{1}$, it is required to act trivially:
            \begin{gather}
                \mathrm{rec}_{\symbf{1}}(C,c_0,\ast):\equiv c_0\,.
            \end{gather}
        \end{itemize}
    }
    \newdef{Dependent functions}{\index{induction}\index{function}
        ...
        \begin{gather}
            \begin{aligned}
                ...
                &\\
                \mathrm{ind}_{\symbf{1}}&:\prod_{\typef{C}{\symbf{1}}}C(\ast)\rightarrow\prod_{x:\symbf{1}}C(x)\,.
            \end{aligned}
        \end{gather}
    }

    \begin{property}[Uniqueness principle]
        Using the induction principle, one can prove that every term $x:A\times B$ is necessarily of the form $(a,b)$ for some $a:A,b:B$. Furthermore, one can also prove that $\ast:\symbf{1}$ is the unique term in $\symbf{1}$.
    \end{property}
    ...
    \newdef{Nullary coproduct}{\index{coproduct}\index{ex falso quodlibet}
        As was the case for products, one can also define a nullary version of the coproduct, the \textbf{empty type} $\symbf{0}$:
        \begin{itemize}
            \item \textbf{Formation rule}: $\type{\symbf{0}}$.
            \item \textbf{Introduction rule}: There is no constructor for $\symbf{0}$.
            \item \textbf{Elimination and computation rules}: Since there is no constructor for $\symbf{0}$, one can always trivially `construct' a function out of $\symbf{0}$:
            \begin{gather}
                \begin{aligned}
                    \mathrm{rec}_{\symbf{0}}&:\prod_{\type{C}}\symbf{0}\rightarrow C\,,\\
                    \mathrm{rec}_{\symbf{0}}&:\prod_{\typef{C}{\symbf{0}}}\prod_{x:\symbf{0}}C(x)\,.
                \end{aligned}
            \end{gather}
...
\subsection{Polynomial functors}
    ...
    \newdef{Polynomial}{\index{polynomial}
        ...
        Given a polynomial in a locally Cartesian closed category $\symbfsf{C}$, the induced polynomial functor is given by the composition
        \begin{gather}
            \symbfsf{C}/W\xrightarrow{f^*}\symbfsf{C}/X\xrightarrow{\prod_g}\symbfsf{C}/Y\xrightarrow{\sum_h}\symbfsf{C}/Z\,,
        \end{gather}
...
\subsection{Equivalences}
    ...
    Since functions can be regarded as functors between $\infty$-groupoids and homotopies act between functions, one might expect that these can be regarded as (weak) natural transformations between the ($\infty$-)functors.
    \begin{property}
...
\subsection{Modalities}\index{possibility}\index{necessity}
    ...
    \begin{axiom}[K]
        ...
        \end{gather}
        In intuitionistic logic, where $\square$ and $\Diamond$ are not de Morgan dual, a similar axiom has to be introduced for $\Diamond$~\citep{simpson_proof_1994}:
        \begin{gather}
    ...
    \begin{axiom}[Necessitation]
        ...
        \end{gather}
        for all theorems $p$. If modus ponens is not included, this axiom (schema) should be replaced by the inference rule $p\Rightarrow\square p$. (For possibiltiy, the dual rule $\Diamond p\rightarrow p$ is used.)
    \end{axiom}

    \newdef{Modal system K}{\index{modal system}
        Classical propositional logic, together with axiom K and the necessitation rule. Modal fragments of modal system K are often said to be \textbf{normal}.
    }
    ...
    \begin{axiom}[S5]
        ...
        \end{gather}
        Note that this axiom schema does not follow from the necessitation rule, since the later ontly holds for provable statements.
    \end{axiom}

\subsection{Kripke frames}\index{Kripke frame}

    Another possible form of semantics for modal logic is given by Kripke frames\footnote{These are sometimes called \textbf{possible-world semantics}. However, Kripke frames are only an example of those.\index{possible worlds}}

    \newdef{Kripke frame}{
        A \textbf{Kripke model} on a \textit{vocabulary} $\mathbb{P}$ is a triple $(W,R,L)$ where:\footnote{A Kripke frame is obtained by dropping the valuation.}
        \begin{enumerate}
            \item $W$ is a set of states or possible worlds,
            \item $R:M\rightarrow P(W\times W)$ is an accessibility relation with $M$ the set of \textbf{modal contexts} ($|M|$ equals the number of modal operators $\square_i$), and
            \item a valuation $P:\mathbb{P}\rightarrow P(W)$, that says in which worlds a given proposition holds.
        \end{enumerate}
    }

    The valuation $P$ of a Kripke model induces a \textbf{forcing relation}\index{forcing} $\vDash\,:W\rightarrow\mathbb{P}$:
    \begin{gather}
        w\vDash p\iff w\iff P(p)\,.
    \end{gather}
    This relation can be extended to the entire modal language as follows:
    \begin{gather}
        \begin{aligned}
            w\vDash\lnot p &\iff w\not\vDash p\\
            w\vDash p\land q &\iff w\vDash p \land w\vDash q\\
            w\vDash p\lor q &\iff w\vDash p\lor w\vDash q\\
            w\vDash\square p &\iff \forall w'\in W:wRw'\implies w'\vDash p\\
            w\vDash\Diamond p &\iff \exists w'\in W:wRw'\implies w'\vDash p\,.
        \end{aligned}
    \end{gather}

    \newdef{Validity}{\index{validity}\index{satisfiable}
        A formula $\phi$ is said to be valid in a Kripke model $(W,R,P)$ if $w\vDash\phi$ for all $w\in W$. If it is valid in all Kripke models it is said to be valid. When there exists at least one Kripke model in which the formula is valid, it is also said to be \textbf{satisfiable}.
    }

    Kripke models give a semantics for normal modal logics, i.e.~modal fragments of modal system K. The additional axioms that can be introduced, restrict the accessibility relations in a Kripke frame as in \cref{table:modal_axioms}. For modal system S5, the accessibility relations are equivalence relations.
    \begin{table}[ht!]
        \centering
        \begin{tabular}{ccc}
            \toprule
            Axiom&Formula&Property\\
            \midrule
            D&$\Diamond\top$&Seriality, i.e.~$\forall w\in W,\exists w'\in W:wRw'$\\
            T&$\square p\rightarrow p$&Reflexivity\\
            B&$p\rightarrow\square\Diamond p$&Symmetry\\
            S4&$\square p\rightarrow\square\square p$&Transitivity\\
            S5&$\Diamond p\rightarrow\square\Diamond p$&Euclidean:\footnote{Cf.~triangle inequality.} $wRw'\land w'Rw''\implies wRw''$
            \\
            \bottomrule
        \end{tabular}
        \caption{Modal axioms.}
        \label{table:modal_axioms}
    \end{table}

\subsection{Type theory}\label{section:modal_type_theory}

    \newdef{Modality}{\index{modality}\index{modal!operator}\label{type:modality}
        Consider an $(\infty,1)$-topos $\symbfsf{H}$. A modality or \textbf{modal operator} on $\symbfsf{H}$ is an idempotent\footnote{Sometimes, idempotency is not required.} (co)monad on $\symbfsf{H}$ (see also \cref{cat:closure_operator}). For clarity, idempotent comonads will be called \textbf{comodalities} in this document (unless specifically mentioned, all statements can be dualized to pertain to comodalities).
    }

    \newdef{Modal type}{\index{modal!type}
        A type $X\in\symbfsf{H}$ is said to be $\Diamond$-modal if the unit $\eta_X:X\rightarrow\Diamond X$ is an equivalence.
    }
    \begin{property}
        The modal types of a modality $\Diamond$ on $\symbfsf{H}$ are exactly the $\Diamond$-algebras or, equivalently, the objects of the Eilenberg--Moore category $\symbfsf{H}^\Diamond$ (\cref{cat:eilenberg_moore}). This property allows to define modal types for monads that are not idempotent as their algebras. It also means that one can recover the monad objectwise as the composition of the unit and the inclusion.
    \end{property}
    ...
    \newdef{Opposite}{\index{opposite}\index{adjoint!cylinder}\index{unity}
        ...
        \begin{gather}
            \symbfsf{H}\overset{\hookrightarrow}{\underset{\hookrightarrow}{\leftarrow}}\symbfsf{H}'
        \end{gather}
        In the case where these functors are themselves modalities, these can be seen as full inclusions of modal types, turning $\symbfsf{H}$ into an essential subtopos of $\symbfsf{H}'$ (\cref{topos:essential_subtopos}), and such an adjoint cylinder represents \textbf{opposite} modalities. Such a situation also gives rise to a new pair of adjoint modalities $\iota_!\iota^*\dashv\iota_*\iota^*$, with the left adjoint being a monad (the other adjunction is trivial because of the fully faithfulness).

        A dual situation arises for an adjoint triple of the form
        \begin{gather}
            \symbfsf{H}\overset{\leftarrow}{\underset{\leftarrow}{\hookrightarrow}}\symbfsf{H}'\,,
        \end{gather}
        exhibiting $\symbfsf{H}$ as a bireflective subcategory of $\symbfsf{H}'$ (\cref{cat:reflective_inclusion}). Here, the resulting adjoint modalities have as left adjoint a comonad.
    ...
    \newdef{Negative}{\index{negative}
        Consider a comodality $\bigcirc:\symbfsf{H}\rightarrow\symbfsf{H}$. The negative of a modal type $X$ is obtained by removing its `pure $\bigcirc$-part', i.e.~its projection under $\bigcirc$. Categorically, this means that one takes the cofibre of the counit:
        \begin{gather}

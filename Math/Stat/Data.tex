...
\subsection{Prediction regions}
    ...
    \newdef{Validity}{\index{validity}\index{significance}\label{data:validity}
        Consider a measurable function $\Gamma:X\rightarrow2^Y$ and let $P$ be the joint distribution on the instance space $Z\equiv X\times Y$. $\Gamma$ is said to be valid (at \textbf{significance level} $\alpha\in[0,1]$ or \textbf{confidence level} $1-\alpha$) if it satisfies
        \begin{gather}
            \label{data:validity_condition}
            P\big(y\in\Gamma^\alpha(x)\big)\geq 1-\alpha.
        \end{gather}

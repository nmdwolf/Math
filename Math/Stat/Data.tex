...
\subsection{Kernel methods}
    ...
    \newdef{Reproducing kernel Hilbert space}{\index{Hilbert!space}
        ...
        A Hilbert space $\mathcal{H}\subset\mathrm{Map}(X,\mathbb{C})$ of functions over a set $X$ for which all evaluation functionals $\delta_x:f\mapsto f(x)$ are bounded (or continuous by Property \ref{functional:bounded_continuous}). Reproducing kernel Hilbert spaces are often abbreviated as \textbf{RKHS}s.
    }
    ...
    \newdef{Reproducing kernel}{
        ...
        \end{gather}
        Because $k$ is given by an inner product, it is not hard to see that the reproducing kernel is a kernel \ref{data:kernel}.
    }
...
\subsection{VC dimension}
    ...
    \newdef{Shatter coefficient}{\index{shatter}
        ...
        \begin{gather}
            S_n(C) := \max_{x_1,\ldots,x_n\in\Omega}\left|\big\{\{x_1,\ldots,x_n\}\cap c\,\big\vert\,c\in C\big\}\right|.
        \end{gather}
        ...
        \begin{gather}
            S_n(\mathcal{F}) := \max_{x_1,\ldots,x_n\in X}\left|\big\{\{f(x_1),\ldots,f(x_n)\}\,\big\vert\,f\in\mathcal{F}\big\}\right|.
        \end{gather}
...
\subsection{Prediction regions}
    ...
    \newdef{Validity}{\index{validity}\index{significance}\label{data:validity}
        Consider a measurable function $\Gamma:X\rightarrow P(Y)$ and let $P$ be the joint distribution on the instance space $Z\equiv X\times Y$. $\Gamma$ is said to be valid (at \textbf{significance level} $\alpha\in[0,1]$ or \textbf{confidence level} $1-\alpha$) if it satisfies
        \begin{gather}
...
\subsection{Conformal prediction}
    ...
    \newdef{Nonconformity measure}{\index{measure!conformity}
        Consider a bag $\mathcal{B}$ together with a new element $z^*$ in a probability space $(Z,\Sigma,P)$. A noncomformity measure $A:Z^\infty\times Z\rightarrow\mathbb{R}$ is a measurable function that gives a number indicating how different $z^*$ is from the content of $\mathcal{B}$.
    }
    ...
    \begin{example}[Point predictors]
        ...
        \begin{gather}
            A_\rho(\mathcal{B},(x,y)) := d\big(\rho(x),y\big).
        \end{gather}
    \end{example}
    \begin{example}[Interval predictors]
        ...
        \begin{gather}
            A_{u,l}(\mathcal{B},(x,y)) := \max\big(l(x)-y,y-u(x)\big).
        \end{gather}
    \end{example}

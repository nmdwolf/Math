...
\subsection{Prediction regions}
    ...
    \newdef{Validity}{\index{validity}\index{significance}
        Consider a region predictor $\Gamma$ and let $P$ be the joint distribution on the instance space $Z\equiv X\times Y$. $\Gamma$ is said to be valid (at \textbf{significance/confidence level} $\alpha\in[0,1]$) if satisfies
        \begin{gather}
            P(y\in\Gamma^\alpha(x))\geq 1-\alpha.
        \end{gather}
        One sometimes also distinguishes between exact validity and \textbf{conservative} validity, where the former is a subcase of the latter for which the inequality becomes an equality.
    }
...
\subsection{Conformal prediction}
    ...
    \begin{example}[Interval predictors]
        ...
        \end{gather}
    \end{example}
    \begin{example}[Nested interval predictors]
        Let $T$ be a totally ordered set \ref{set:total_order}. For every model $\tseq{C}\equiv(l_t,u_t)_{t\in T}:X\rightarrow\mathbb{R}^2$ that predicts a sequence of nested intervals, i.e. $s\leq t\implies l_s(x)\geq l_t(x)\land u_s(x)\leq u_t(x)$, that is trained on a bag $\mathcal{B}$, one can define a nonconformity measure as follows:
        \begin{gather}
    ...
    \begin{construct}[Conformal predictor]\label{data:cp}
        Consider a data sample given as a bag $\mathcal{B}\in P(Z)$ together with a nonconformity measure $A$ and let $\alpha$ denote the confidence level of the prediction region to be constructed. For any new element $z^*\in Z$ the algorithm proceeds as follows:
        \begin{enumerate}
    ...
    \begin{property}[Optimality]
        ...
        \begin{itemize}
            \item Regions are (conservatively) valid, i.e. $P(p^*\leq\alpha)\leq\alpha$ and thus also $P(y^*\in\Gamma^\alpha(x))\geq1-\alpha$.
    ...
    \begin{property}[Validity]
        ...
        \end{gather}
        i.e. the empirical quantiles are replaced by ``inflated'' quantiles. The smoothing of Property \ref{data:smooth_cp} can also be implied in this case. If one does not use a smoothened conformal predictor but assumes that all calibration scores are distinct, the following property is obtained:
        \begin{gather}
...
\subsection{Distribution-shift}
    ...
    \begin{method}[Power martingale]
        For every constant $\varepsilon\in[0,1]$ one defines the power martingale by
        \begin{gather}
        ...
        \end{gather}
        Because $\varepsilon\leq0$, the above martingales will start to become very large if the conformal predictor produces small $p$-values, i.e. when unlikely values are observed.
    \end{method}

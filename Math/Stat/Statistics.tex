...
\subsection{Common distributions}
    ...
    \end{remark}
    \remark{The sum of Gaussians is again Gaussian with the sum of the means and standard deviations as parameters.}
    ...
    \begin{formula}[$\chi^2$-distribution]\index{distribution!$\chi^2$}\label{statistics:chi_squared_distr}
        ...
        \end{gather}
    \end{formula}
    \remark{Due to the CLT \ref{statistics:CLT}, the $\chi^2$-distribution approximates a Gaussian distribution for large $n$: $f(\chi^2;n)\overset{n>30}{\longrightarrow}\mathcal{G}(\sqrt{2\chi^2};\sqrt{2n-1},1)$.
...
\section{Errors}
    ...
    \newformula{Error propagation}{\index{propagation!of errors}
        Let $\mathbf{X}$ be a vector random variable with \underline{small} (co)variance such that functions of $\mathbf{X}$ admit a good first-order Taylor approximation around the mean. The variance of a general function $f(\mathbf{X})$ is given by
        \begin{gather}
            \label{statistics:error_propagation}
            \variance{f} \approx \sum_p\left(\pderiv{f}{X_{(p)}}\right)^2\variance{X_{(p)}} + \sum_{p\neq q}\left(\pderiv{f}{X_{(p)}}\right)\left(\pderiv{f}{X_{(q)}}\right)\text{cov}[X_{(p)},X_{(q)}].
        \end{gather}
    }
    \begin{result}
    ...
    \newformula{Covariance of functions}{\index{covariance}
        \begin{gather}
            ...
            \mathrm{cov}[f,g] \approx \sum_{p,q}\left(\pderiv{f}{X_{(p)}}\right)\left(\pderiv{g}{X_{(q)}}\right)\mathrm{cov}[X_{(p)},X_{(q)}]
        \end{gather}
    }
    \begin{result}
        Let $\mathbf{f} = (f_1,\ldots,f_k)$ be a vector-valued function. The covariance matrix $\variance{\mathbf{f}(\mathbf{X})}$ is to first order given by
        \begin{gather}
            \variance{\mathbf{f}(\mathbf{X})} \approx J\variance{\mathbf{X}}J^T,
        \end{gather}

...
\subsection{Moment estimators}
    ...
    \begin{theorem}[Strong law of large numbers\footnotemark]\index{law!of large numbers}\index{Kolmogorov!law}\index{Kolmogorov!criterion}\label{prob:strong_lln}
        ...
        \begin{gather}
            \Prob\left(\lim_{n\rightarrow\infty}\overline{X}_n=\mu\right)=0\,.
        \end{gather}
    ...
    \newdef{Geometric mean}{\label{statistics:geometric_mean}
        Let $\{x_i\}_{i\leq N}$ be a data set taking values in $\mathbb{R}_+$. The geometric mean is used to average out \textit{normalized} measurements, i.e.~ratios with respect to a reference value.
        \begin{gather}
...
\subsection{Empirical distribution}
    ...
    \begin{theorem}[Glivenko--Cantelli]\index{Glivenko--Cantelli}\label{statistics:glivenko_cantelli}
        Consider a cumulative distribution function $F$ and denote the empirical distribution function of $n$ random variables by $F_n$. If the random variables are i.i.d.~according to $F$, then
        \begin{gather}
...
\subsection{Least squares estimation}
    ...
    \begin{property}
        The optimal $\chi^2$-value is asymptotically distributed according to a $\chi^2$-distribution with $n$ degrees of freedom. The parameter $n$ is equal to the number of events $N$ minus the number of fitted parameters $k$. (See more in \cref{section:chi_squared_test}.)
    \end{property}
...
\subsection{Interval types}
    ...
    \newdef{Two-sided confidence interval}{\label{statistics:two_sided_interval}
        \begin{gather}
            \Prob(x_-\leq X\leq x_+) = \bigintsss_{x_-}^{x_+}f(x)\,dx = C
        \end{gather}
        ...
        \begin{itemize}
            ...
            \item\textbf{central interval}: $\bigintsss_{-\infty}^{x_-}f(x)\,dx = \bigintsss_{x_+}^\infty f(x)\,dx = \frac{1-C}{2}$.
        \end{itemize}
    ...
    \newdef{One-sided confidence interval}{
        \begin{align}
            ...
            \Prob(x\geq x_-) &= \bigintsss_{x_-}^\infty f(x)\,dx = C\\
            \label{statistics:one_sided_interval2}
            \Prob(x\leq x_+) &= \bigintsss_{-\infty}^{x_+}f(x)\,dx = C
        \end{align}
    }
...
\subsection{Interval for a sample mean}
    ...
    \newformula{Interval with known variance}{
        If the sample size is large enough, the real distribution is unimportant, because the CLT ensures a Gaussian distribution of the sample mean $\overline{X}$. The $\alpha$-level confidence interval such that $\Prob(-z_{\alpha/2}<Z<z_{\alpha/2})$ with $Z = \frac{\overline{X} - \mu}{\sigma/\sqrt{N}}$ is given by
        \begin{gather}
    ...
    \newformula{Interval with unknown variance}{
        To account for the uncertainty of the estimated standard deviation $\hat{\sigma}$, the student-$t$ distribution (\cref{statistics:student_t_distr}) is used instead of a Gaussian distribution to describe the sample mean $\overline{X}$. The $\alpha$-level confidence interval is given by
        \begin{gather}
...
\subsection{\texorpdfstring{$\chi^2$}{Chi squared}-test}\index{test!$\chi^2$}\label{section:chi_squared_test}
    ...
    \begin{property}\label{statistics:chi_square}
        ...
        \begin{gather}
            \bigintsss_{\chi^2}^\infty f_{\chi^2}(x\mid n)\,dx\approx 1\implies
            \begin{cases}
                \bullet\text{ good fit}\\

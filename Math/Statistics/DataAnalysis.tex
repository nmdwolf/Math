...
\subsection{Linear problems}
    ...
    \newdef{Variance inflation factor}{\index{variance!inflation factor}
        ...
        \end{gather}
        where $R_i^2$ is the $R^2$-value obtained after regressing the predictor $\widehat{X}_i$ on all other predictors. The rule of thumb is that $\mathrm{VIF}\geq10$ implies that a significant amount of multicollinearity is present in the model.
    }
...
\subsection{Conjugate gradient}\index{conjugate!gradient}
    ...
    \newdef{Conjugate vectors}{\index{Arnoldi method}
        ...
        \end{gather}
        Two vectors $v,w$ are said to be ($A$-)conjugate if they are orthogonal with respect to this inner product. The general approach to obtain a basis of $A$-conjugate vectors is a modified version of the Gram--Schmidt procedure (\cref{linalgebra:gram_schmidt}) where the ordinary Euclidean inner product is replaced by~\eqref{data:A_inner_product}. This modification is called the \textbf{Arnoldi method}.
    }

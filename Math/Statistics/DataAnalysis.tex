...
\subsection{Bayes optimality}
    ...
    \newdef{Bayes risk}{\index{Bayes!risk}
        Let $\mathcal{H}\subseteq\mathcal{Y}^{\mathcal{X}}$ be a collection of models. The Bayes risk (with respect to $\mathcal{H}$) is the minimal risk over all models:
        \begin{gather}
            R^* := \inf_{f\in\mathcal{H}}R(f)\,.
        \end{gather}

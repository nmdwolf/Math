\chapter{Data Analysis \& Numerical Optimization}

    The main reference for the sections on optimization problems is~\citet{shewchuk_introduction_1994}. For the geometry of clustering methods, see~\citet{boissonnat_bregman_2010}. The main references for the section on conformal prediction are~\citet{shafer_tutorial_2008,vovk_algorithmic_2022,dewolf_comparative_2024}.\footnote{Yes, this is some gratuitous selfadvertisement.} Although a part of this chapter is a continuation of \labelref{chapter:statistics}, the focus here lies more on the computational aspect of analyses of large data sets. For this reason, the chapter starts with some sections on applied linear algebra (for a refresher, see \labelref{chapter:linear_algebra}).
...
\subsection{Inverse CDF sampling}
    ...
    Although it is one of the most straightforward sampling algorithms for distributions on $\mathbb{R}$, this approach makes the strong assumption that the cumulative distribution function (\cref{prob:cdf}) is invertible. However, it is very much worthwhile to introduce it.

    \begin{method}
        Sample a point $\lambda$ uniformly from the unit interval $[0,1]$. By \cref{prob:uniformization}, the CDF $F_X(X)$ is itself uniformly distributed and, hence, $\lambda$ can serve as a sample of $F_X(X)$. The new point $x'\in\mathbb{R}$ is, hence, simply given by $F^{-1}_X(\lambda)$.

        When $F_X$ is discrete, $\lambda$ might not lie in the image of $F_X$ and the inverse might not admit an algorithmically useful expression, so one should use a different approach. Given a point $x\in\mathbb{R}$ (the sampling algorithm used here is irrelevant and could even be deterministic) and its associated cumulative probability $F_X(x)$, one can sample a new point $x'$ by increasing (or decreasing) $x$ until the unique point $x'\in\mathbb{R}$ satisfying
        \begin{gather}
...
\subsection{Uniform rejection sampling}
    ...
    This method again uses the fact that the value of the cumulative distribution function is itself uniformly distributed on the unit interval $[0,1]$. The CDF does not have to be invertible for this method, but the probability density should be compactly supported and bounded.

    \begin{method}
        Consider an interval $[a,b]$ contained in the support of $f_X$ and let $q_0$ be an upper bound for $f_X$. Now, uniformly sample points $x\in[a,b]$ and $q\in[0,q_0]$. If $f_X(x)\geq q$, then $x$ is a good sample. If not, repeat this procedure.
    \end{method}
...
\subsection{Monte Carlo sampling}
    ...
    The general idea of (Markov chain) Monte Carlo methods is to construct a Markov chain of points such that, starting from a given index, all subsequent elements represent good samples.

    The most famous Monte Carlo algorithm uses an acceptance threshold analoguous to uniform rejection sampling.
    \begin{method}[Metropolis--Hastings]
        Choose a normal distribution $\mathcal{N}(\mu,\sigma^2)$. The first element of the sequence is taken to be $x_0=\mu$. Subsequent elements are constructed as follows:
        \begin{itemize}
            ...
            \item If not, uniformly sample $q\in[0,1]$. If $\lambda\geq q$, take $x_i=x'$, else take $x_i=x_{i-1}$.\footnote{This step could be merged with the previous one, since $\lambda\geq1\implies\lambda\geq q$.}
        \end{itemize}
    ...
    \begin{remark}[Normalization]
        It is clear from the definition of the acceptance ratio that one does not need $f_X$ to be normalized. This avoids costly calculations of the normalization factor. (This is of invaluable importance for heavy numerical calculations, as is common in computational physics/chemistry and machine learning.)
    \end{remark}
...
\subsection{Linear problems}
    ...
    \begin{method}[Normal equation]\index{normal!equation}\label{data:normal_equation}
        ...
        \begin{gather}
            Ax = b
        \end{gather}
        as in \cref{section:system_of_equations}, one can try to numerically solve for $x\in\mathbb{R}^n$ by minimizing the $\ell^2$-norm $\|Ax-b\|^2$:
        \begin{gather}
        ...
        \begin{gather}
            A^TAx = A^Tb\,.
        \end{gather}
        This is symbolically\footnote{Inverting the (symmetric) matrix can be done efficiently, but often up to numerical errors.} solved by $x=(A^TA)^{-1}A^Tb$, where $(A^TA)^{-1}A^T$ is the pseudoinverse of $A$.
    \end{method}
    ...
    \begin{method}[Regularization]{\index{Tikhonov regularization}\index{ill conditioned}
        Consider the same linear problem as above:
        \begin{gather*}
            Ax = b\,.
        \end{gather*}
        When the matrix $A$ is nearly singular --- in this case it is said to be \textbf{ill conditioned} --- the solution $x=(A^TA)^{-1}A^Tb$ is numerically unstable. In this case, a regularization term can be added to the minimization problem:
        \begin{gather}
            \|Ax-b\|^2+\|\Gamma x\|^2\,,
        \end{gather}
        where $\Gamma$ is called the \textbf{Tikhonov matrix}. When $\Gamma=\lambda\mathbbm{1}$, one speaks of \textbf{$\ell^2$-regularization} or \textbf{Tikhonov regularization}. This technique benefits solutions with smaller norms.
    }
    \end{method}
    \begin{remark}\index{regression!lasso}\index{regression!ridge}
    ...
    \newdef{Multicollinearity}{\index{collinearity}
        Let $\{X_i\}_{1\leq i\leq n}$ be a finite set of random variables. These random variables are said to be perfectly (multi)collinear if there exists an affine relation between them, i.e.~there exist coefficients $\{\lambda_i\}_{0\leq i\leq n}\subset\mathbb{R}$ such that
        \begin{gather}
            \lambda_0 + \lambda_1X_1 + \cdots + \lambda_nX_n = 0\,.
        \end{gather}
        The same concept can be applied to data samples. The data is said to be (multi)collinear if the above equation holds for all entries of the data set. However, in this case, one can also define `near multicollinearity' if the variables $X_i$ are related as above up to some error term $\varepsilon$. If the variance of $\varepsilon$ is small, the matrix $X^TX$ might have an ill-conditioned inverse which might render the algorithms unstable.
    }

    \newdef{Coefficient of determination}{\index{$R^2$}\index{coefficient!of determination}
        Consider a data set $\{x_1,\ldots,x_n\}\subset\mathbb{R}$ together with associated estimates $\{y_1,\ldots,y_n\}\subset\mathbb{R}$. The $R^2$ of the fit is defined as follows:
        \begin{gather}
            R^2 := 1 - \frac{\sum_{i=1}^n|x_i-y_i|^2}{\sum_{i=1}^n|x_i-\overline{x}|^2}\,.
        \end{gather}
        The denominator is an estimate of the population variance, whereas the numerator gives a measure of the residual variance.
    }
    \begin{property}
        For a correctly specified linear model, the $R^2$-value is bounded by $[0,1]$. Note that it is a widespread myth that these bounds hold in general!
    \end{property}
    ...
    \newdef{Variance inflation factor}{\index{variance!inflation factor}
        ...
        \end{gather}
        where $R_i^2$ is the $R^2$-value obtained after regressing the predictor $\widehat{X}_i$ on all other predictors. The general rule of thumb is that $\mathrm{VIF}\geq10$ implies that a significant amount of multicollinearity is present in the model.
    }

\subsection{Gradient descent}

    The gradient descent algorithm is first introduced in the case of quadratic forms.
    \begin{method}[Steepest descent]\index{residual}\index{condition number}\index{line!search}
        Consider the quadratic form
        \begin{gather}
            f(x) = \frac{1}{2}x^TAx - b^Tx + c\,,
        \end{gather}
        and assume that $A$ is symmetric and positive definite, such that the linear problem $Ax=b$ gives the minimum of $f$. Like most recursive algorithms, gradient descent starts from an arbitrary guess $x_0$. It then takes a step in the direction of steepest descent (or largest gradient), i.e.~in the direction opposite to $\nabla f(x_0) = Ax_0-b$:
        \begin{gather}
            x_{i+1} := x_i + \alpha r_i\,,
        \end{gather}
        where the \textbf{residual} is defined as
        \begin{gather}
            r_i := b - Ax_i\,.
        \end{gather}
        This procedure is repeated until convergence, i.e.~until the residual vanishes up to a fixed numerical tolerance.

        A naive gradient descent method would require to fine-tune the step size $\alpha\in\mathbb{R}^+$. However, a more efficient method is given by the \textbf{line search algorithm}, where the value of $\alpha$ is optimized in every step as to minimize $f$ along the line defined by $r_i$. A standard calculus argument leads to the following form of the step size:
        \begin{gather}
        ...
        \begin{gather}
            \deriv{}{\alpha}f(x_i) = -\nabla f(x_i)\cdot\nabla f(x_{i-1})\,.
        \end{gather}
        As a consequence, this minimization scheme often results in a chaotic zigzag trajectory through the configuration space. The higher the \textbf{condition number}
        \begin{gather}
            \kappa=\frac{|\lambda_{\max}|}{|\lambda_{\min}|}\,,
        \end{gather}
        the worse the zigzag motion will be. A very narrow valley (or some higher-dimensional analogue) will make the trajectory bounce back and forth between the walls, instead of moving towards the minimum.
    \end{method}

\subsection{Conjugate gradient}\index{conjugate!gradient}

    As noted in the previous section, a common problem with gradient descent is that the direction of steepest descent is often not the same as the direction pointing to the optimal solution and, hence, convergence might only be achieved after many iterations.
    ...
    \newdef{Conjugate vectors}{\index{Arnoldi method}
        ...
        \begin{gather}
            ...
            \braket{v}{w}_A := v^TAw\,.
        \end{gather}
        Two vectors $v,w$ are said to be ($A$-)conjugate if they are orthogonal with respect to this inner product. The general approach to obtaining a basis of $A$-conjugate vectors is a modified version of the Gram--Schmidt procedure (\cref{linalgebra:gram_schmidt}) where the ordinary Euclidean inner product is replaced by~\cref{data:A_inner_product}. This modification is called the \textbf{Arnoldi method}.
    }
    ...
    \begin{gather}
        \alpha_i = \frac{d_i^{\,T}r_i}{d_i^{\,T}\!Ad_i} = \frac{\braket{d_i}{r_i}\hphantom{_A}}{\braket{d_i}{d_i}_A}\,.
    \end{gather}
    ...
    \end{gather}
    Because of their prominence in the literature on numeric optimization techniques, these subspaces have earned their proper name.
    ...
    A different approach exists where the CG algorithm is not applied to the matrix $A^T\!A$, but the individual matrices $A,A^T$ are used directly. This way, not one Krylov space is generated, but two dual `copies' are constructed:
    \begin{gather}
        \begin{aligned}
            \mathcal{D}_i &:= \mathrm{span}\left\{r_0,Ar_0,\ldots,A^{i-1}r_0\right\}\,,\\
...
\subsection{Nonlinear conjugate gradients}\index{conjugate!gradient}
    ...
    At the level of the implementation, the structure of the algorithm remains more or less the same. What does change is the form of the Arnoldi method, in particular, the prefactor in \cref{data:beta}. For linear CG, there are multiple equivalent formulas, but for nonlinear CG these do not lead to the same algorithm. The two most common choices are given below.
    ...
    For linear CG, a simple formula for finding the optimal value of $\alpha_i$ was obtained. However, for nonlinear CG, one cannot solve \cref{data:argmin} as easily. The main idea, i.e.~that $\nabla f$ should be orthogonal to the previous search direction, is still valid. Here, only the \textbf{Newton--Raphson approach} is considered:\footnote{Another common method is the \textit{secant method}.}\index{Newton--Raphson algorithm}\index{secant method}
    \begin{gather}
...
\subsection{Krylov methods}
    ...
    \todo{COMPLETE (add e.g.~Lanczos)}

    From the preceding considerations, it is clear that, for invertible $A$, the set $\{x_0,Ax_0,.,\ldots,A^nx_0\}$ forms a basis for the Krylov space $\mathcal{K}_n(A,x_0)$. However, there is an issue with this particular basis, especially from a computational point of view. For large $n$, the vectors $A^nx_0$ will start to converge to (a linear combination of) the eigenvector(s) of $A$ with greatest eigenvalue. As such, this basis becomes numerically unstable.

    A better approach is to construct an orthonormal basis from $\{x_0,Ax_0,.,\ldots,A^nx_0\}$ as in the Arnoldi method, i.e.~using the $A$-inner product. This will also work in case $A$ is not invertible, since $\{x_0,Ax_0,.,\ldots,A^nx_0\}$ still spans the Krylov space and, hence, can be used to construct a basis. The Arnoldi relation
    \begin{gather}
        y_{n+1} = Ar_n - \sum_{i=0}^nr_i\braket{r_i}{r_n}_A\,,
    \end{gather}
    with
    \begin{gather}
        r_n := \frac{y_n}{\|y_n\|}\,,
    \end{gather}
    implies that $Ar_n\in\mathrm{span}\{r_0,\ldots,r_{n+1}\}$. If $Q$ is the transformation matrix onto the Arnoldi basis, i.e.~the matrix with the Arnoldi vectors $r_i$ as rows, then
    \begin{gather}
        A = Q^\dagger TQ\,.
    \end{gather}    
    The matrix $T$ has a so-called (lower) \textbf{Hessenberg form}\index{matrix!Hessenberg}, i.e.~$A_{ij}=0$ for all $j>i+1$ (all entries above the first `superdiagonal' are zero).

    Moreover, in case $A$ is Hermitian, this enhanced symmetry implies that $T$ is tridiagonal! Such matrices admit very efficient decompositions and eigenvalue constructions. This is essentially the \textbf{Lanczos method}.\index{Lanczos method} 
    
    The following is an easy, but very useful, corrolary of the Lanczos decomposition.
    \begin{property}
        \begin{gather}
            Av=\lambda v\implies TQv=\lambda Qv
        \end{gather} 
    \end{property}
    One of the most important observations from the Lanczos algorithm is that one actually does not need 

\section{Constrained optimization}
\subsection{Lagrange multipliers}\label{section:lagrange_multipliers}

    A common generalization of optimization problems is the addition of constraints involving equalities:
    \begin{gather}
        \label{data:constrained_problem}
        \argmin_x f(x) \qquad\text{such that}\qquad g_i(x)=0\qquad\forall 1\leq i\leq n\,.
    \end{gather}
    The general approach to solving such constrained problems is by extending the loss function $f$ (cf.~\cref{constraint:total_hamiltonian}).
    ...
    \newdef{Primal optimization problem}{
        ...
        \begin{gather}
            p^* := \min_x\theta_P(x) = \min_x\max_{\alpha,\beta\mid\beta_i\geq0}\mathcal{L}(x,\alpha,\beta)\,.
        \end{gather}
    }
    ...
    \newdef{Dual optimization problem}{
        \begin{gather}
            d^* := \max_{\alpha,\beta;\beta_i\geq0}\theta_D(\alpha,\beta) = \max_{\alpha,\beta\mid\beta_i\geq0}\min_x\mathcal{L}(x,\alpha,\beta)\,.
        \end{gather}
    }
    ...
    \newdef{Slater conditions}{\index{Slater!conditions}
        Consider a convex optimization problem, i.e.~a problem of the form of \cref{data:constrained_optimization} where $f$ is convex, the $g_i$ are convex and the $h_j$ are affine. This problem is said to satisfy the Slater condition(s) if there exists an $x$ that is strictly \textbf{feasible}, i.e.~$h_j(x)<0$ for all $1\leq j\leq n$.
    }
    ...
    The following property gives a set of sufficient conditions for strong duality.
    \begin{property}[Karush--Kuhn--Tucker conditions]\index{Karush--Kuhn--Tucker conditions}\label{data:kkt}
        There exist $x,\alpha$ and $\beta$ such that strong duality holds if and only if the following conditions are satisfied:
        \begin{align}
            \begin{cases}
                \ds\pderiv{\mathcal{L}}{x}=0\\
                \\
...
\subsection{Riemannian gradient descent}
    ...
    \todo{COMPLETE (also add references)}

\section{Approximation theory}
\subsection{Bayes optimality}

    \newdef{Bayes risk}{\index{Bayes!risk}
        Let $\mathcal{H}\subseteq\mathcal{Y}^{\mathcal{X}}$ be a collection of models and consider a loss function $\mathcal{L}:\mathcal{H}\times\mathcal{H}\rightarrow\mathbb{R}$. The Bayes risk (with respect to some prior distribution $\mathbb{P}(\mathcal{H})$) is the minimal risk over all models:
        \begin{gather}
            R^* := \inf_{f\in\mathcal{H}}R_\pi(f)\,,
        \end{gather}
        where the average risk of an estimate $f$ with respect to $\pi$ is defined as
        \begin{gather}
            R_\pi(f) = \mathrm{E}_{\theta\sim\pi}\bigl[\mathcal{L}(f,\theta)\bigr]\,.
        \end{gather}
        `The' Bayes risk is the Bayes risk with respect to $\mathcal{Y}^{\mathcal{X}}$.
    }
    ...
    \newdef{Bayes classifier}{\index{Bayes!classifier}
        ...
        \begin{gather}
            f^*:x\mapsto\argmin_{y\in\mathcal{Y}}\Int_{\mathcal{Y}}l(y,y')\,dP(y'\mid x)\,.
        \end{gather}
    }
    \begin{property}[Bayes optimality]
        The risk of the pointwise Bayes predictor is minimal, i.e.~$R(f^*)=R^*$. In practice, however, one cannot achieve Bayes optimality (through the pointwise Bayes predictor), since this would require the knowledge of the data-generating distribution.
    \end{property}

    \newdef{Approximation error}{\index{error}
        Given a data set $\mathcal{D}\subset\mathcal{X}\times\mathcal{Y}$, the empirical risk $R_{\text{emp}}$ is defined as follows (it is simply the empirical approximation of the pointwise Bayes risk):
        \begin{gather}
            R_{\text{emp}}:\widehat{y}\mapsto\frac{1}{|\mathcal{D}|}\sum_{(x,y)\in\mathcal{D}}l(\widehat{y}(x),y).
        \end{gather}
        Consider a hypothesis space $\mathcal{H}\subseteq\mathcal{Y}^{\mathcal{X}}$ (e.g.~determined by a choice of model architecture). The minimizer of the true risk in $\mathcal{H}$ is denoted by $h^*$, whereas the empirical risk minimizer is denoted by $\widehat{h}$. The \textbf{approximation error/uncertainty} is defined as the difference $R(\widehat{h})-R(h^*)$, whereas the \textbf{model uncertainty} is defined as the difference $R(h^*)-R^*$. The total error made by $\widehat{h}$ (with respect to the Bayes optimum) is then simply the sum of these two, i.e.~$R(\widehat{h})-R^*$.
    }

\subsection{PAC theory and empirical risk minimization}\index{PAC theory}\label{section:pac}

    \begin{property}
        Note that, by the (strong) law of large numbers~\ref{prob:strong_lln}, the empirical risk converges to the true risk almost surely whenever the data points are sampled i.i.d. However, in practice, the true question is how fast this convergence happens.
    \end{property}
    ...
    \begin{result}
        ...
        \begin{gather}
            \Prob\left(R(\widehat{f})\leq\inf_{f\in\mathcal{H}}R(f)+2\varepsilon\right)\geq1-\delta\,,
        \end{gather}
        i.e.~with high probability, the true risk for the minimizer is only slightly greater than the empirical risk and, hence, the minimizer is a reasonably good estimate.
    \end{result}

    \begin{property}[Bounded losses]
        Consider a hypothesis space $\mathcal{H}$ with a bounded loss function. The probability that, for a given number of data points $n\in\mathbb{N}_0$, there exists at least one model $f\in\mathcal{H}$ for which the empirical risk deviates significantly from the true risk is bounded as follows:
        \begin{gather}
            \begin{aligned}
        ...
        \begin{gather}
            \Prob\left(R(f)\leq R_{\text{emp}}(f) + \sqrt{\frac{\ln(|\mathcal{H}|)+\ln(1/\delta)}{2n}}\right)\geq1-\delta\,.
        \end{gather}
        Note that this bound is distribution free, i.e.~it does not depend on the data-generating distribution.
    ...
    \newdef{Complexity regularizer}{
        A function $c:\mathcal{X}\rightarrow\mathbb{R}^+$ such that
        \begin{gather}
            \sum_{x\in\mathcal{X}}e^{-c(x)}\leq1\,.
        \end{gather}
        A simple example would be the log-probabilities $c(x)=\ln(P(x))$ when a probability distribution $P$ on $\mathcal{X}$ is given.
    }
    Using a complexity regularizer on $\mathcal{H}$, the following risk bound can be obtained:
    \begin{gather}
        \Prob\left(R(f)\leq R_{\text{emp}}(f) + \sqrt{\frac{c(f)+\ln(1/\delta)}{2n}}\right)\geq1-\delta\,.
    \end{gather}
...
\subsection{Clustering}\index{clustering}
    ...
    \begin{method}[$k$-means algorithm]
        Consider an unlabelled dataset $\mathcal{D}\subset\mathbb{R}^n$. For every integer $k\in\mathbb{N}$, usually satisfying $k\ll|\mathcal{D}|$, and any choice of $k$ distinct \textbf{centroids} $\{c_i\in\mathbb{R}^n\}_{i\leq k}$, the $k$-means algorithm is defined through the following iterative scheme:
        \begin{enumerate}
            \item To every element $d\in\mathcal{D}$, assign a cluster $C_i$ based on the following criterion:
            \begin{gather}
    ...
    The cluster boundaries $H(c_1, c_2)=\{x\in\mathbb{R}^n\mid D_f(x\,\|\,c_1)=D_f(x\,\|\,c_2)\}$ admit a simple geometric construction (see \labelref{chapter:info} for an introduction to information geometry).
...
\subsection{Kernel methods}\label{data:kernel_methods}
    ...
    This section will introduce the mathematics of kernel methods. This mainly involves the language of Hilbert spaces (for a refresher, see \labelref{chapter:functional}).

    \newdef{Kernel\footnotemark}{\index{kernel}\index{Mercer|seealso{kernel}}\index{Gram matrix}\label{data:kernel}
        \footnotetext{Also called a \textbf{Mercer kernel}. See Mercer's theorem below for more information.}
        A function $k:X\times X\rightarrow\mathbb{C}$ that is (conjugate) symmetric and for which the \textbf{Gram matrix} $K_{ij}:=K(x_i,x_j)$ is positive definite for all $n\in\mathbb{N}$ and $\{x_i\in X\}_{i\leq n}$.
    }
    \newdef{Reproducing kernel Hilbert space}{\index{Hilbert!space}
        A Hilbert space $\mathcal{H}\subset\mathrm{Map}(X,\mathbb{C})$ for which all evaluation functionals $\delta_x:f\mapsto f(x)$ are bounded (or continuous by \cref{functional:bounded_continuous}). Reproducing kernel Hilbert spaces are often abbreviated as \textbf{RKHS}s.
    }
    ...
    \newdef{Reproducing kernel}{
        ...
        \end{gather}
        Because $k$ is given by an inner product, it is not hard to see that the reproducing kernel is a kernel in the sense of \cref{data:kernel}.
    }
    ...
    \newadef{RKHS}{
        ...
        \begin{enumerate}
            \item\textbf{Reproducing property}: For all $x\in X$ and $f\in\mathcal{H}$, the evaluation functional $\delta_x$ satisfies $\delta_x(f) = \langle k(\cdot,x)\mid f \rangle_{\mathcal{H}}$.
...
\subsection{VC dimension}
    ...
    \newdef{Shatter coefficient}{\index{shatter}
        Let $\mathcal{X}$ denote the universe of discours and consider a set $C\in P(\mathcal{X})$. $C$ \textbf{shatters} a set $A\in\mathcal{X}$ if, for every subset $a\subseteq A$, there exists a $c\in C$ such that
        \begin{gather}
            a = c\cap A\,.
        \end{gather}
        The shatter(ing) coefficients of $C$ are defined as follows:
        \begin{gather}
            S_n(C) := \max_{x_1,\ldots,x_n\in\mathcal{X}}\left|\bigl\{\{x_1,\ldots,x_n\}\cap c\,\bigm\vert\,c\in C\bigr\}\right|\,.
        \end{gather}
        ...
        Given a collection of binary functions $\mathcal{F}\subseteq\{0,1\}^{\mathcal{X}}$, the shatter(ing) coefficients (or \textbf{growth functions}) of $\mathcal{F}$ are given by:
        \begin{gather}
            S_n(\mathcal{F}) := \max_{x_1,\ldots,x_n\in\mathcal{X}}\left|\bigl\{\{f(x_1),\ldots,f(x_n)\}\,\bigm\vert\,f\in\mathcal{F}\bigr\}\right|\,.
        \end{gather}
    ...
    \newdef{VC dimension}{\index{Vapnik--Chervonenkis!dimension}
        ...
        \begin{gather}
            S_n(\mathcal{F})=2^n
        \end{gather}
        for all $n<\mathrm{VC}(\mathcal{F})$. A collection is called a \textbf{Vapnik--Chervonenkis class} if its VC dimension is finite.
    }
...
\subsection{Rademacher complexity}
    ...
    \newdef{Rademacher complexity}{\index{Rademacher!complexity}
        Consider a collection of functions $\mathcal{F}\subseteq\mathbb{R}^{\mathcal{X}}$. The Rademacher complexity of $\mathcal{F}$ is defined as follows:
        \begin{gather}
    ...
    \begin{property}[VC dimension]
        Shatter coefficient bounds the Rademacher complexity in the following way:
        \begin{gather}
...
\section{Time series analysis}
    ...
    \newdef{Time series}{\index{time series}
        An $\mathbb{N}$- or $\mathbb{Z}$-indexed stochastic process (\labelcref{chapter:stochastic_calculus}). Since $\mathbb{N}$ and $\mathbb{Z}$ are isomorphic, the two conventions for time series will be used interchangeably.
    }
...
\subsection{Correlation}
    ...
    \newdef{Autocorrelation function}{\index{autocorrelation}\index{correlation|seealso{autocorrelation}}
        Consider a time series $\seq{X}$. The associated autocovariance (resp.~autocorrelation) process is defined as the covariance (resp.~correlation) function of the random variables $\seq{X}$.
    }
    \begin{remark}
    ...
    \newdef{Spectral density}{\index{spectral!density}\index{memory}
        Let $\seq{X}$ be a (weakly) stationary time series. If the associated autocovariance is in $\ell^1$, one can define the spectral density as the discrete Fourier transform of the autocovariance function:
        \begin{gather}
    ...
    \newdef{Lag operator\footnotemark}{\index{lag}\index{backshift|see{lag}}
        ...
        The lag operator sends a variable in a $\mathbb{Z}$-indexed time series to the preceding value:
        \begin{gather}
            BX_t = X_{t-1}\,.
        \end{gather}
...
\subsection{Autoregressive models}
    ...
    \newdef{$\mathrm{AR}(p)$-model}{\index{autoregressive model}
        Consider a time series $\tseq{X}$. The autogressive model of order $p\in\mathbb{N}$ is defined as the multiple linear regression model of $X_t$ with respect to the first $p$ lagged values $X_{t-1},\ldots,X_{t-p}$ of the process:
        \begin{gather}
    ...
    \newdef{Partial autocorrelation function}{
        The $p^{\text{th}}$ partial autocorrelation function is defined as the $p^{\text{th}}$ coefficient in the $\mathrm{AR}(p)$-model.
    }
    \remark{The optimal order $p\in\mathbb{N}$ of an autoregressive model is the one for which all higher partial autocorrelation functions (almost) vanish.}

    \newdef{$\mathrm{MA}(p)$-model}{\index{moving average}
        Consider a time series $\tseq{X}$ where every variable $X_t$ contains a white noise contribution $\varepsilon_t\sim\mathcal{N}(0,\sigma^2)$. The moving average model of order $p\in\mathbb{N}$ is defined as the multiple linear regression model of $X_t$ with respect to the first $p$ lagged values $\varepsilon_{t-1},\ldots,\varepsilon_{t-p}$ of the error term:
        \begin{gather}
...
\subsection{Granger causality}
    ...
    \newdef{Granger causality}{\index{causality!Granger}
        Consider two time series $\seq{X}$ and $\seq{Y}$. The process $X_n$ is said to (Granger-)cause $Y_n$ if past values of $X_n$ help to predict future values of $Y_n$. More formally, this can be stated as follows:
        \begin{gather}
        ...
        This formulation of causality was introduced by \indexauthor{Granger} under the following two assumptions:
        \begin{itemize}
            \item The cause always happens prior to the effect.
            \item The cause carries unique information about the effect.
        \end{itemize}
    }
    \remark{A slightly different, but, for computational purposes often more useful\footnote{In fact, this was the original definition by \indexauthor{Granger}.} notion of Granger causality, is as follows. A time series $\seq{X}$ is said to \textbf{Granger-cause} a time series $\seq{Y}$ if the variance of predictions of $Y_n$ becomes smaller when the information contained in $X_n$ is taken into account.}

    \begin{remark}[Testing]\index{test!Diebold--Mariano}
        Assume that two uncorrelated models giving predictions of a time series are given. One way to check if they have the same accuracy is the \textit{Diebold--Mariano test}. However, when testing for Granger causality, one should pay attention. This test is not valid for nested models and, hence, is not applicable to two models that only differ by a set of extra predictors (in this case, an external process).
    \end{remark}
...
\subsection{Anomaly detection}\index{anomaly}

    \newdef{Pointwise anomaly}{
        A low-density region in configuration space.
    }

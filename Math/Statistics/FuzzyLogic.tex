...
\subsection{Capacities}
    ...
    The above notational remark also links back to the relation between hyperspaces and modal logic (cf.~\cref{metric:vietoris_modal_logic}). Consider an epistemic state $E\subseteq X$, i.e.~the true value of some quantity of interest lies in $E$. To this epistemic state, one can associate canonical possibility and neccesity measures:
    \begin{gather}
        \Pi_E(A) :=
        \begin{cases}
    ...
    Let $E\subseteq X$ be a model for a set of propositions $\mathfrak{P}:=\{p_i\}_{i\in I}$. If $A\subseteq X$ corresponds to the proposition $A$, the statements $N_E(A)=1$ and $\Pi_E(A)=0$ are expressible in classic propositional logic, where they correspond to $\mathfrak{P}\implies p$ and $\mathfrak{P}\cup\{p\}\vdash\bot$, respectively. However, the opposite statements, $N_E(A)=0$ and $\Pi_E(A)=1$ are not!
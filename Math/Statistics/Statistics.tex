\chapter{Statistics}\label{chapter:statistics}
    ...
    In this chapter, most definitions and formulas will be based on either a standard calculus approach or a data-driven approach. See \labelref{chapter:probability} for a measure-theoretic approach. For some sections, the language of information geometry will be used (as introduced in the previous chapter).
...
\subsection{Dispersion}
    ...
    \begin{formula}\label{statistics:variance_without_sum}
        The variance can also be rewritten in the following way (cf.~\cref{prob:covariance_as_expectation}):
        \begin{gather}
...
\section{Empirical distribution}
    ...
    \begin{remark}
        Most of the preceding section can be recovered by applying \cref{section:moments} to the empirical distribution function of a data sample.
    \end{remark}
...
\subsection{Likelihood function}
    ...
    \newdef{Likelihood}{\index{likelihood}\label{statistics:likelihood}
        The likelihood $\mathcal{L}(a;\symbf{x})$ is the joint density of a set of measurements $\symbf{x} := \{x_1,\ldots,x_n\}$:
        \begin{gather}
            \mathcal{L}(a;\symbf{x}) = \prod_{i=1}^nf(x_i;a)\,.
        \end{gather}
    }
...
\subsection{Least squares estimation}
    ...
    \newmethod{Binned least squares}{\label{statistics:binned_least_squares}
        ...
        \begin{enumerate}
            ...
            \item The true number of events has a Poisson distribution: $\overline{n}_j = \sigma_j^2 = f_j$.
...
\subsection{Geometric approach}
    ...
    Consider a sample $\symbf{x}:=\{x_1,\ldots,x_n\}$ drawn from a distribution $f(x;\theta)$ in an exponential family. The likelihood (\cref{statistics:likelihood}) is given by \[\mathcal{L}(\theta;\symbf{x}) := \prod_{i=1}^nf(x_i;\theta)\,.\] The $m$-coordinates of the observed point are
    \begin{gather}
...
\subsection{\texorpdfstring{$\chi^2$}{Chi squared}-test}\index{test!$\chi^2$}\label{section:chi_squared_test}
    ...
    The quantity $\chi^2$ in \cref{statistics:binned_least_squares} can be turned into a goodness-of-fit statistic.
    \newdef{Pearson $\chi^2$}{\index{Pearson!$\chi^2$}
        Consider a categorical distribution $p$ with $k\in\mathbb{N}$ classes. Denote the fitted counts by $E_i$ (for `expected') and the observed counts by $O_i$ (for `observed').
        \begin{gather}
            \chi^2 = \sum_{i=1}^k\frac{(O_i-E_i)^2}{E_i}
        \end{gather}
    }
    \begin{property}
        Asymptotically in the number of observations, $\chi^2$ follows a $\chi^2$-distribution (\cref{statistics:chi_squared_distr}).
        \begin{proof}
            For simplicity, assume that the $E_i$ are the true expectation values given $p$. In this case, the individual terms in the above sum are asymptotically normal with mean $0$, variance $1-p_i$ and covariance $\Sigma_{ij}=\delta_{ij}-\sqrt{p_ip_j}$. Note that $\Sigma$ is a projection matrix and, hence, can be diagonalized. In fact, its diagonal form is
            \begin{gather*}
                \Sigma'=
                \begin{pmatrix}
                    \mathbbm{1}_{k-1}&0\\
                ...
                \end{pmatrix}\,.
            \end{gather*}
            Accordingly, the observations can be transformed in such a way that $\chi^2$ becomes the sum (asymptotically) of $k-1$ standard normal distributions.
        \end{proof}
    \end{property}
    \begin{remark}[Degrees of freedom]
        When $P$ is modelled using $c\in\mathbb{N}$ parameters, the number of d.o.f.~should be reduced. In this case, $\chi^2$ follows a $\chi^2$-distribution with $k-c-1$ d.o.f.
    \end{remark}

    \begin{property}\label{statistics:chi_square}
        If there are $c\in\mathbb{N}$ fitted parameters one has:
        \begin{gather}
            \Int_{\chi^2}^{+\infty}f_{\chi^2}(x\mid c)\,dx\approx 1\implies
            \begin{cases}
                \circ\text{  good fit}\\

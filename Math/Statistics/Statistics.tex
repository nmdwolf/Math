...
\subsection{\texorpdfstring{$\chi^2$}{Chi squared}-test}\index{test!$\chi^2$}\label{section:chi_squared_test}
    ...
    \begin{property}
        ...
        \begin{proof}
            For simplicity, assume that the $E_i$ are the true expectation values given $p$. In this case, the individual terms in the above sum have mean $0$ and variance $1-p_i$, and their covariance is given by $\Sigma_{ij}=\delta_{ij}-\sqrt{p_ip_j}$. Note that $\Sigma$ is a projection matrix and, hence, can be diagonalized. In fact, its diagonal form is
            \begin{gather*}
                \Sigma'=
                \begin{pmatrix}
                    \mathbbm{1}_{k-1}&0\\

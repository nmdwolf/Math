...
\subsection{Homotopy}
    ...
    \newdef{Retraction}{\index{retract}
        Let $X$ be a topological space and let $A\subseteq X$ be a subspace. A continuous function $f:X\rightarrow A$ is called a retraction (and $A$ is called a \textbf{retract} of $X$) if\footnote{It is a retraction of the inclusion map $A\hookrightarrow X$ in the sense of Definition \ref{cat:retract}.}
        \begin{gather}
            f|_A=\mathbbm{1}_A.
        \end{gather}
    }
    \newdef{Homotopy}{\index{homotopy}\index{isotopy}\label{topology:homotopy}
...
\subsection{Homotopy groups}
    ...
    \newdef{Loop space}{\index{loop!space}
        ...
        The set of all \textbf{loops} in a pointed topological space, i.e.~all continuous functions $\gamma:(S^1,t_0)\rightarrow(X,x_0)$ for which $\gamma(t_0)=x_0$. This space is denoted by $\Omega_{x_0}X$ (if the base point is clear, the subscript is often omitted). It can be turned into a topological space by equipping it with the compact-open topology \ref{topology:compact_open_topology}.
    ...
    \newdef{Fundamental groupoid $\clubsuit$}{\index{fundamental!groupoid}\index{Poincar\'e!groupoid}\label{topology:fundamental_groupoid}
        ...
        \begin{itemize}
            ...
            \item\textbf{Morphisms}: Endpoint-preserving homotopy classes of continuous functions $f:S^1\rightarrow X$.
        \end{itemize}
        The fundamental group $\pi_1(X,x)$ can be recovered as the automorphism group of $x\in\mathrm{ob}(\mathbf{\Pi}_1(X))$.

        This construction can be generalized to an $\infty$\textit{-groupoid} (Definition \ref{topos:infty_groupoid}), where the class of $k$-morphisms is given by $k$-dimensional ``paths''. More explicitly, it is the \textit{Kan complex} (Definition \ref{model:kan_complex}) given by
        \begin{gather}
            \mathrm{Sing}(X):[k]\mapsto\mathbf{Top}(\Delta^k,X)\,,
        \end{gather}
        the \textit{singular simplicial complex} of $X$ (to be defined in Section \ref{section:singular_homology}).
    }
    ...
    \newdef{Semilocally simply connected}{
        A topological space is said to be semilocally simply-connected if every point admits a neighbourhood $U$ such that every loop in $U$ can be contracted to a point.
    }

    \newdef{Universal covering space}{
        A simply-connected covering space \ref{topology:covering_space}.
    }
    \begin{uproperty}
    ...
    \begin{property}[Automorphisms]\index{deck transformation}\label{topology:universal_cover_automorphisms}
        Consider a topological space $X$ and let $\widetilde{X}$ be its universal covering space. The group of deck transformations $\Aut(\widetilde{X})$ from Definition \ref{topology:deck_transformation} is isomorphic to the fundamental group $\pi_1(X)$. Hence, one obtains
        \begin{gather}
    ...
    \end{theorem}
    This theorem can be extended in the context of Banach spaces (Chapter \ref{chapter:functional}). There it is called the \textbf{Schauder fixed point theorem}.\index{Schauder!fixed point theorem}
    ...
    \end{theorem}
    \remark{The theorem is even stronger in that one can replace the volume by any other measure (See Chapter \ref{chapter:measure}).}
...
\subsection{Fibrations}
    ...
    \newdef{Homotopy extension property}{
        Consider a continuous function $\iota:A\rightarrow X$. This function is said to have the homotopy extension property with respect to a topological space $Y$ if for every homotopy $f:A\times[0,1]\rightarrow Y$ and extension $\widetilde{f}_0:X\rightarrow Y$ of $f_0:=f|_{A\times\{0\}}$ there exists a homotopy $\widetilde{f}:X\times[0,1]\rightarrow Y$ extending $f$ such that Diagram \ref{fig:homotopy_extension_property} commutes, where Property \ref{topology:internal_hom} is used to represent homotopies in terms of the path space of $Y$.
...
\subsection{\difficult{Rational homotopy theory}}
    ...
    \newdef{Polynomial differential forms}{
        Consider the standard $n$-simplex $\Delta^n$ (Definition \ref{topology:simplex}). On this topological space one can define differential forms analogous to those from Section \ref{section:forms}. Let $\{t_i\}_{0\leq i\leq n}$ be $n+1$ generators in degree 0 (the barycentric coordinates). Together with $n+1$ associated generators $\dr t_i$ in degree 1, one can construct the free GCA over $\mathbb{Q}$. To preserve the geometric structure of $\Delta^n$ one has to quotient out the following relations:
        \begin{gather}
        ...
        Morphisms $f:[m]\rightarrow[n]$ in the simplex category $\simplex$ (Definition \ref{model:simplex_category}) induce morphisms of simplices and these in turn induce morphisms $F:\Omega^\bullet_\text{poly}(\Delta^n)\rightarrow\Omega^\bullet_\text{poly}(\Delta^m)$ defined by the following action on generators:
        \begin{gather}
    ...
    There is also another approach due to \textit{Quillen}. Instead of working with differential graded algebras, \textit{Quillen} used differential graded Lie algebras, i.e.~\textit{strict $L_\infty$-algebras} (Section \ref{section:higher_lie_structures}).
...
\subsection{Simplicial homology}\index{homology!simplicial}
    ...
    In this section a chain complex \ref{homalg:chain_complex} and its associated homology theory are constructed for triangulable spaces.
...
\subsection{Relative homology}\index{homology!relative}
    ...
    \newdef{Relative homology groups}{
        ...
        \begin{gather}
            H_k(K,L) := \frac{\ker(\overline\partial_k)}{\im(\overline\partial_{k+1})}.
        \end{gather}
...
\section{Local coefficients}\index{local!coefficients}\label{section:local_coefficients}
    ...
    For some applications one needs to generalize the notion of (co)homology. For example, when considering integration theory on manifolds (Section \ref{section:compact_cohomology}), one in general needs to take into account the nonorientability.

    Consider a topological space $X$ that admits a universal cover $\widetilde{X}$. By Property \ref{topology:universal_cover_automorphisms} one then has
    \begin{gather}
        X\cong\widetilde{X}/\pi_1(X).
    \end{gather}
...
\section{\difficult{Axiomatic approach}}\label{section:eilenberg_steenrod}
    ...
    \begin{property}[Eilenberg-MacLane spectrum]\index{Eilenberg-MacLane!spectrum}
        The Eilenberg-MacLane spectrum of an Abelian group $A$ is the sequence of Eilenberg-MacLane spaces $\{K(A,n)\}_{n\in\mathbb{N}}$. By the Dold-Kan theorem \ref{model:dold_kan} Abelian groups form a subcategory of Abelian chain complexes: $\mathbf{Ab}\hookrightarrow\mathbf{Ch}^+(\mathbf{Ab})$. The Eilenberg-MacLane assignment extends to a functor $\mathcal{D}(\mathbf{Ab})\rightarrow\mathbf{Spectra}$ from the derived category of Abelian chain complexes to the category of ($\Omega$-)spectra.
    \end{property}
...
\section{\difficult{Equivariant cohomology}}\index{equivariant!cohomology}
    ...
    \newdef{Equivariant cohomology}{\index{Borel!construction}\label{topology:equivariant_cohomology}
        ...
        If the action is not free, a more general construction needs to be used. Consider the universal bundle $\pi:EG\rightarrow BG$ from Definition \ref{bundle:classifying_space}. From this bundle one can construct the associated bundle $EG\times_G X$ (this is sometimes called the \textbf{Borel construction}). It gives a model for the homotopy quotient $X/\!\!/G$. The $G$-equivariant cohomology of $X$ is then defined as the singular cohomology of the Borel construction:
        \begin{gather}

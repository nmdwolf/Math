...
\subsection{Homotopy}
    ...
    \newdef{Homotopy}{\index{homotopy}\index{isotopy}\label{topology:homotopy}
        Let $f,g\in C(X,Y)$ where $X,Y$ are topological spaces. If there exists a continuous function $H:X\times[0,1]\rightarrow Y$ such that $f(x) = H(x,0)$ and $g(x) = H(x,1)$, then $f$ and $g$ are said to be homotopic. This relation induces an equivalence relation on $C(X,Y)$ for which the quotient space is denoted by $[X,Y]$. A homotopy $H$ such that $H(\cdot,t)$ is a homeomorphism for all $t\in[0,1]$ is called an \textbf{isotopy}.
    }
    ...
    \newdef{Mapping class group}{\index{mapping class group}\label{topology:MCG}
        ...
        Consider a topological space $X$ with some structure. The mapping class group of $X$ is defined as the quotient
        \begin{gather}
            \mathrm{MCG}(X) := \Aut(X)/\Aut_0(X),
        \end{gather}
...
\subsection{\difficult{Rational homotopy theory}}
    ...
    \newdef{Polynomial differential forms}{
        ...
        \end{gather}
        The resulting graded algebra is denoted by $\Omega^\bullet_\mathrm{poly}(\Delta^n)$. In degree 0 this complex can be identified with the space of polynomial functions on $\Delta^n$. The ordinary differential forms can be obtained by taking the tensor product of $\Omega^\bullet_\mathrm{poly}$ with the space of smooth functions on $\Delta^n$. Under this isomorphism the generators $dt_i$ are identified with the de Rham differentials of the barycentric coordinates.

        Morphisms $f:[m]\rightarrow[n]$ in the simplex category $\simplex$ (Definition \ref{model:simplex_category}) induce morphisms of simplices and these in turn induce morphisms $F:\Omega^\bullet_\mathrm{poly}(\Delta^n)\rightarrow\Omega^\bullet_\mathrm{poly}(\Delta^m)$ defined by the following action on generators:
        \begin{gather}
            F(t_i) := \sum_{f(j)=i}t_j.
        \end{gather}
        It can be seen that this turns the above construction into a functor $\Omega^\bullet_\mathrm{poly}:\simplex^{op}\rightarrow\mathbf{dgcAlg}$. By passing to the opposite functor and taking a left Kan extension along the Yoneda embedding $\simplex\hookrightarrow\mathbf{sSet}$, one obtains a functor $\func{\Omega^\bullet_\mathrm{poly}}{sSet}{dgcAlg^{op}}$. Composition with the singular set functor $\func{\mathrm{Sing}}{Top}{sSet}$ gives the \textbf{piecewise-polynomial differential forms} functor~$\Omega^\bullet_\mathrm{pwpoly}$.
    }
...
\section{Local coefficients}\index{local!coefficients}\label{section:local_coefficients}
    ...
    \end{gather}
    $\pi_1(X)$, the automorphism group of $\widetilde{X}$, acts on singular simplices by postcomposition.

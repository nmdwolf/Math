...
\subsection{Homotopy groups}
    ...
    \newdef{Eilenberg--MacLane space}{\index{Eilenberg--MacLane space}\label{topology:eilenberg_maclane}
        ...
        \begin{gather}
            \pi_i\bigl(K(G,n)\bigr)=\begin{cases}
                G&i=n\,,\\
                0&i\neq n\,.
            \end{cases}
        \end{gather}
    ...
    \end{theorem}
    This theorem can be extended in the context of Banach spaces (see \cref{chapter:functional}). There it is called the \textbf{Schauder fixed point theorem}.\index{Schauder!fixed point theorem}
    ...
    \end{theorem}
    \remark{The theorem is even stronger in that one can replace the volume by any other measure (See \cref{chapter:measure}).}
...
\subsection{CW complexes}
    ...
    \end{construct}
    \remark{Infinite-dimensional CW complexes can be obtained by taking the direct limit (\cref{algebra:direct_limit}) of the sequence above.}
...
\subsection{\difficult{Rational homotopy theory}}
    ...
    \newdef{Rational homotopy category}{
        Consider the category \textbf{Top} of topological spaces. The rational homotopy category is obtained as the localization (see \cref{model:localization}) of \textbf{Top} with respect to the collection of rational homotopy equivalences.
    }
...
\subsection{Simplices}
    ...
    \end{property}
    \begin{result}
        \Cref{topology:homeomorphic_homotopy}, which states that homeomorphic spaces have the same homotopy groups, implies that the fundamental group of a triangulable space can be computed by looking at its triangulations.
    \end{result}
...
\subsection{Simplicial homology}\index{homology!simplicial}
    ...
    \newdef{Homology group}{
        From \cref{topology:boundary_operator_relation} it follows that $B_k(\mathcal{K})\subset Z_k(\mathcal{K})$ is a subgroup. One can thus define the $k^{th}$ homology group $H_k(\mathcal{K})$ as the following quotient group:
        \begin{gather}
...
\section{Singular homology}\label{section:singular_homology}
    ...
    \newdef{Singular simplex}{\index{simplex!singular}
        Recall the \textbf{standard} $k$-simplex $\Delta^k$ from \cref{topology:standard_simplex}. A singular $k$-simplex in a topological space $X$ is defined as a continuous function $\sigma:\Delta^k\rightarrow X$.
    }
...
\section{Local coefficients}\index{local!coefficients}\label{section:local_coefficients}
    ...
    For some applications one needs to generalize the notion of (co)homology. For example, when considering integration theory on \textit{manifolds} (see \cref{section:compact_cohomology}), one in general needs to take into account the nonorientability.
...
\section{\difficult{Equivariant cohomology}}\index{equivariant!cohomology}
    ...
    \newdef{Equivariant cohomology}{\index{Borel!construction}\label{topology:equivariant_cohomology}
        ...
        If the action is not free, a more general construction needs to be used. Consider the universal bundle $\pi:EG\rightarrow BG$ from \cref{bundle:classifying_space}. From this bundle one can construct the associated bundle $EG\times_G X$ (this is sometimes called the \textbf{Borel construction}). It gives a model for the homotopy quotient $X/\!\!/G$. The $G$-equivariant cohomology of $X$ is then defined as the singular cohomology of the Borel construction:
        \begin{gather}

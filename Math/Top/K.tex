\chapter{\texorpdfstring{\difficult{$K$-theory}}{K-theory}}\index{K-theory}
...
\subsection{Classification}
    ...
    \begin{property}[Homotopy classification]
        ...
        \begin{gather}
            K^0(X) = [X,B\!\GL(k)\times\mathbb{Z}].
        \end{gather}
        When specializing to $k=\mathbb{R},\mathbb{C}$ this becomes
        \begin{gather}
            K^0_{\mathbb{R}}(X) = [X,B\mathrm{O}\times\mathbb{Z}]
        \end{gather}
        and
        \begin{gather}
            K^0_{\mathbb{C}}(X) = [X,B\mathrm{U}\times\mathbb{Z}]
        \end{gather}
        due to homotopy invariance. Reduced $K$-theory can be obtained by considering basepoint-preserving homotopies. For connected spaces this is equivalent to $[X,B\!\GL]$.
    \end{property}
    ...
    \begin{theorem}[Atiyah-J\"anich]\index{Atiyah-J\"anich}\label{k:atiyah_janich}
        The space of Fredholm operators \ref{functional:fredholm} on a separable and infinite-dimensional Hilbert space forms a classifying space for $K$-theory.
    \end{theorem}
...
\subsection{Negative degree}
    ...
    \newdef{$K^{-1}$}{
        ...
        \begin{align}
            K^{-1}_\mathbb{R}(X) &:= [X,\mathrm{O}]\\
            K^{-1}_\mathbb{C}(X) &:= [X,\mathrm{U}].
        \end{align}
    }
...
\subsection{Bott periodicity}\index{Bott!periodicity}
    ...
    \end{theorem}
    \begin{result}
        Applying Bott periodicity to the case $X=\ast,Y=\emptyset$ and comparing to Example \ref{k:point}, one obtains $K^0_\mathbb{C}(D^2,S^1)\cong\mathbb{Z}$. One can also conclude that $\beta$ is a generator of $K^0_\mathbb{C}(D^2,S^1)$.
    \end{result}

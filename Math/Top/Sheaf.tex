...
\section{Sheaves}
    ...
    \begin{construct}[Associated sheaf]\index{sheafification}\index{section!of a (pre)sheaf}
...
\section{Ringed spaces}
    ...
    \newdef{Quasicoherent sheaf}{\index{sheaf!coherent}
        Let $(X,\mathcal{O}_X)$ be a ringed space. A quasicoherent sheaf $\mathcal{E}$ on $X$ is a sheaf of $\mathcal{O}_X$-modules that is locally the cokernel of a morphism of free modules.

        This means that $\mathcal{E}$ is locally presentable in the sense that it fits into an exact sequence of the form
        \begin{gather}
            \mathcal{O}^{k_i}_X|_{U_i}\longrightarrow\mathcal{O}^{l_i}_X|_{U_i}\longrightarrow\mathcal{E}|_{U_i}\longrightarrow0
        \end{gather}
        for some open cover $\{U_i\}_{i\in I}$ of $X$.

        A quasicoherent sheaf is said to be coherent if it is of \textbf{finite type}, i.e. every point has an open neighbourhood $U$ admitting a surjective morphism
        \begin{gather}
            \mathcal{O}^m_X|_U\longrightarrow\mathcal{E}|_U,
        \end{gather}
        and, for every open neighbourhood $V$ and integer $n\in\mathbb{N}$, any morphism
        \begin{gather}
            \mathcal{O}^n_X|_V\longrightarrow\mathcal{E}|_V
        \end{gather}
        has a kernel of finite type.
    }
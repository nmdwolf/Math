...
\section{Sheaves}
    ...
    \newdef{Stalk}{\index{stalk}\index{germ}
        Consider a point $x\in X$ together with the set of all neighbourhoods of $x$. This set can be turned into a directed set (\cref{set:directed_set}) by equipping it with the (partial) order relation \[U\geq V\implies U\subseteq V.\] This turns the sheaf $\mathcal{F}$ on $X$ into a directed system. The stalk at $x$ is defined as the following direct limit (\cref{algebra:direct_limit}):
        \begin{gather}

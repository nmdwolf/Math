...
\subsection{Properties}
    ...
    \newadef{Quasicoherent sheaf}{\label{sheaf:quasicoherent_base_change}
        A sheaf $\mathcal{F}$ on $\symbfsf{CRing}/_{\!X}$ that is compatible with base change:
        \begin{enumerate}
...
\subsection{Morphisms}
    ...
    \newdef{Finite morphism}{\index{morphism!finite}
        A scheme morphism $f:X\rightarrow Y$ such that there exists an affine cover $\{U_i\}_{i}$ of $Y$ on which $f$ satisfies the following conditions for all $i\in I$:
        \begin{enumerate}
            \item $f^{-1}(U_i)$ is affine, and
            \item the induced ring map $B_i\rightarrow A_i$ turns $A_i$ into a finitely generated $B_i$-algebra.
        \end{enumerate}
    }
    ...
    \newdef{Flat morphism}{\index{flat!morphism}
        A scheme morphism whose induced morphism on local rings is flat. If it is also surjective, it is said to be \textbf{faithfully flat}. In fact, it is sufficient to check this for closed points.
    }
    ...
    \begin{property}[Flat sheaves]
        A coherent sheaf over a Noetherian scheme is flat if and only if it locally free.

        Consider an affine morphism $f:X\rightarrow Y$ of $S$-schemes together with a quasicoherent sheaf $\mathcal{F}$ over $X$. $\mathcal{F}$ is flat over $S$ if and only if $f_*\mathcal{F}$ is flat over $S$.
    \end{property}
    \begin{property}[Flat schemes]
        Flatness of morphisms is preserved by composition and base change.

        Now, consider a scheme $f:X\rightarrow$ locally of finite type with $Y$ Noetherian. $f$ is flat if and only if
        \begin{gather}
    ...
    \begin{result}[Artin fibres]\index{length}
        ...
        \end{gather}
        Combining this formula with the preceding property, it follows that (under the same assumptions), the fibres of a flat morphism are spectra of Artin rings of constant length.
    \end{result}
    ...
    \newdef{\'Etale morphism}{\index{etale!morphism}\index{formally!\'etale}\index{formally!smooth}\index{unramified}\label{alggeom:etale_morphism}
        ...
        Since this definition does allow for some pathological examples, \'etale morphisms are defined as those morphisms that are formally \'etale and locally of finite presentation. (Likewise for smooth and unramified morphisms.) The subacategory of $\symbfsf{Sch}_{/X}$ on \'etale morphisms is denoted by $\symbfsf{Et}_X$.
    }
    ...
    \newadef{Unramified morphism}{
        A scheme morphism $f:X\rightarrow Y$ is unramified if
        \begin{enumerate}
            \item it is locally of finite presentation,
            \item the induced map $f^\sharp:\mathcal{O}_{Y,f(x)}\rightarrow\mathcal{O}_{X,x}$ satisfies
                \begin{gather}
                    f^\sharp(\mathfrak{m}_{f(x)})\mathcal{O}_{X,x} = \mathcal{O}_{X,x}\,,
                \end{gather}
            \item and $\kappa\bigl(f(x)\bigr)$ is a finite algebraic extension of $\kappa(x)$.
        \end{enumerate}
    }

    \begin{property}[Equivalent \'etale conditions]
        The following conditions are equivalent for a scheme morphism $f:X\rightarrow Y$:
        \begin{itemize}
            \item $f$ is \'etale,
            \item $f$ is flat and unramified,
            \item $f$ is flat and smooth,
            \item $f$ is smooth and of relative dimension 0, and
            \item $f$ is locally of finite presentation and, for every $x\in X$, there exist open affines $x\in\spec(A)\subseteq X$ and $f(U)\subseteq\spec(B)\subseteq Y$ with $A=R[x_1,\ldots,x_n]/(f_1,\ldots,f_n)$ such that the Jacobian of the local representative is nonsingular: $\det(\pderiv{f_i}{x_j})\neq0$.\footnote{This is the algebraic counterpart of the \textit{inverse function theorem} (see \cref{bundle:inverse_function_theorem}).}
        \end{itemize}
...
\subsection{Tangent space}
    ...
    \newdef{Tangent cone}{\index{tangent!cone}\index{initial!part}
        Consider a variety $X$ over an algebraically closed field $\mathfrak{K}$. The tangent cone to $X$ at the origin is defined as the zero locus of the `initial ideal' of $I$:
        \begin{gather}
            C_0X := V\bigl(\{f^{\text{in}}\bigm\vert f\in I(U)\}\bigr)\,,
        \end{gather}
        where $U\ni x$ is an affine chart around and $f^{\text{in}}$ denotes the \textbf{initial part} of $f$, i.e.~the homogeneous part of smallest degree of $f$.
    }

    \newdef{Tangent space}{\index{tangent!space}
        Consider a variety $X$ over an algebraically closed field $\mathfrak{K}$ and choose a point $x\in X$. By working in a suitable affine chart, one can assume that $x=0$. This implies that any polynomial $f\in I(X)$ has a vanishing constant term. The tangent space at $x$ is defined as follows:
        \begin{gather}
            T_xX := V\bigl(\{f^{[1]}\bigm\vert f\in I(U)\}\bigr)\,,
        \end{gather}
        where $U\ni x$ is an affine chart and $f^{[1]}$ denotes the linear part of the polynomial $f$.
    }
    \begin{property}
    ...
    \newadef{Tangent cone: Schemes}{\index{tangent!cone}
        Let $X$ be a Noetherian scheme. The tangent cone $C_xX$ is given by the spectrum of the graded ring $\mathrm{Gr}(\mathcal{O}_x)=\bigoplus_{k\in\mathbb{N}}\mathfrak{m}_x^k/\mathfrak{m}_x^{k+1}$.
    }
    ...
    \newdef{Regular scheme}{\index{regular!scheme}
        A (Noetherian) scheme is said to be regular or \textbf{nonsingular} if the local dimension at any point is equal to the dimension of the Zariski tangent space at that point. Equivalently, a scheme is regular if at every point the tangent space is isomorphic to the tangent cone.
    }
    ...
    \begin{property}[\'Etale morphisms]\label{alggeom:tangent_cone_etale}
...
\subsection{Algebraic de Rham complex}\index{de Rham!complex}
    ...
    \begin{property}[Unramified morphism]
        A morphism $f:X\rightarrow Y$ locally of finite type is unramified if and only if $\Omega_{X/Y}=0$.
    \end{property}
...
\section{\difficult{Topos theory}}\label{section:alggeom_topos_theory}

    This section heavily uses the content of \labelref{chapter:topos}.

\subsection{Sites of schemes}
    ...
    The following topology is finer than the Zariski topology.\footnote{There exists another topology in between the Zariski and \'etale topologies, the \textit{Nisnevich topology}, but this is of no interest here.}\index{topology!Nisnevich}
    ...
    \newdef{Flat topologies}{\index{flat!topology}
        The finest topologies on the category of schemes (or slices thereof) are given by considering flat morphisms. The two most widely used flat topologies are the `fppf-' and `fpqc-topologies'. The \textbf{fppf-topology} (for \textit{fid\`element plat de presentation finie}) is generated by jointly surjective families of flat morphisms that are locally of finite presentation.
        
        The \textbf{fpqc-topology} (for \textit{fid\`element plat et quasi-compacte}) was historically generated by jointly surjective families of flat and quasicompact morphisms. However, when doing so, the fpqc-topology is not comparable to the Zariski topology, since open immersions need not be quasicompact (unless one restricts to Noetherian schemes). A solution introduced by \indexauthor{Vistoli} and \indexauthor{Kleiman}~\citep{vistoli_notes_2004} is to replace quasicompactness by the following seemingly similar definition. A morphism is fpqc if it is locally of finite presentation and if there exists an open affine cover $\{U_i\}_{i\in I}$ of $X$ such that each $U_i$ is the image of a compact open subscheme.\footnote{Quasicompactness only says that the $U_i$ contain the image of their (compact) preimage, not that they coincide with it.} (If one drops the modifier `locally' in the definition of the fppf-topology, a similar problem arises.)
    }

    \begin{property}[Representable sheaves]
        Every representable presheaf satisfies descent in the fpqc-topology.
    \end{property}

    This also implies the same result for the fppf- and Zariski topologies due to the following property.
    \begin{property}[Refinement]
        The above topologies satisfy the following chain of refinements:
        \begin{gather}
            \text{fpqc}\supset\text{fppf}\supset\text{\'etale}\supset\text{Zariski}\,.
        \end{gather}
    \end{property}
...
\subsection{\'Etale cohomology}
    ...
    The following property is the analogue of \cref{sheaf:enough_injectives} for the \'etale site.
    \begin{property}
        The category of sheaves of Abelian groups on $\symbfsf{Et}_X$ has enough injectives (\cref{homalg:resolution}).
    \end{property}
    ...
    \begin{remark}[Finite \'etale covering]\index{etale!covering}
        When a finite \'etale morphism $f:X\rightarrow Y$ exists, $X$ is sometimes called a \textbf{finite \'etale cover(ing)} of $Y$. The full subcategory of the slice category $\symbfsf{Sch}/_{\!Y}$ on finite \'etale coverings is often denoted by $\symbfsf{FEt}_Y$.
    \end{remark}

    \newdef{\'Etale fundamental group}{\index{fundamental group!etale}
        Consider a connected scheme $X$. The (\'etale) fundamental group $\pi_1(X)$ is the unique (up to isomorphism) profinite group (\cref{topology:profinite_group}) such that $\symbfsf{FEt}_X$ is equivalent to the category $\symbfsf{Fin}\pi_1(X)\symbfsf{Set}$ of finite of $\pi_1(X)$-sets.
    }
    \begin{remark}
        Recall \cref{topology:equivalent_categories} for topological spaces. There, a stricter set of spaces was considered with the benefit that all $\pi_1(X)$-sets could be classified. By passing to profinite groups, a classification in the topological setting can be stated, virtually identical to the above one.
    \end{remark}

...
\subsection{Homotopy}
    ...
    \newdef{Null-homotopic}{
        A continuous function homotopic to a constant function.
    }
...
\subsection{Homotopy groups}
    ...
    \newdef{Loop group}{\index{loop!group}\label{topology:loop_group}
        In the case of topological groups, one can define a group structure on the (free) loop space. With the compact-open topology~\ref{topology:compact_open_topology}, it becomes a topological group.
    }
    ...
    \newdef{Fundamental groupoid $\clubsuit$}{\index{fundamental!groupoid}\index{Poincar\'e!groupoid}\label{topology:fundamental_groupoid}
        ...
        \begin{gather}
            \mathrm{Sing}(X):[k]\mapsto\symbfsf{Top}(\Delta^k,X)\,,
        \end{gather}
    ...
    The following conjecture is due to \indexauthor{Baez}. Proofs are available depending on which model is used for the definition of \textit{$\infty$-groupoids}.
    \begin{theorem}[\difficult{Homotopy hypothesis}]\index{homotopy!hypothesis}
        $\symbfsf{Top}$ and $\symbfsf{\infty Grpd}$ are equivalent as $(\infty,1)$-categories. In particular, this means that $n$-groupoids are equivalent to homotopy $n$-types.
    \end{theorem}
    \begin{remark}
        The statement of the theorem is sometimes used as a consistency condition for the definition of \textit{$\infty$-categories} and, in some cases, it is even used as the very definition of higher groupoids. It should also be noted that the relation is very important in \textit{homotopy type theory} as introduced in \labelref{chapter:type_theory}.
    \end{remark}
    ...
    \begin{theorem}[Brouwer fixed point theorem]\index{Brouwer!fixed point theorem}\label{topology:brouwer}
        Every continuous function from a convex subset of $\mathbb{R}^n$ to itself has a fixed point.
    \end{theorem}
    This theorem can be extended in the context of \textit{Banach spaces} (see \labelref{chapter:functional}). There, it is called the \textbf{Schauder fixed point theorem}.\index{Schauder!fixed point theorem}
    ...
    \begin{theorem}[Ham-sandwich theorem]\index{ham-sandwich theorem}
        For any $n\in\mathbb{N}$ connected subsets in $\mathbb{R}^n$ with nonvanishing volume, there exists a hyperplane that cuts all of these objects exactly in half (with respect to their volume).
    \end{theorem}
    \remark{The theorem is even stronger in that one can replace the volume by any other \textit{measure} (see \labelref{chapter:measure}).}
...
\subsection{\difficult{Rational homotopy theory}}
    ...
    For the theory on differential graded algebras, see \labelref{chapter:hda}.
    ...
    \newdef{Polynomial differential forms}{
        ...
        \end{gather}
        It can be seen that this turns the above construction into a functor $\Omega^\bullet_{\text{poly}}:\simplex^{op}\rightarrow\symbfsf{dgcAlg}$. By passing to the opposite functor and taking a left Kan extension along the Yoneda embedding $\simplex\hookrightarrow\symbfsf{sSet}$, one obtains a functor $\func{\Omega^\bullet_{\text{poly}}}{sSet}{dgcAlg^{op}}$. Composition with the singular set functor $\func{\mathrm{Sing}}{Top}{sSet}$ gives the \textbf{piecewise-polynomial differential forms} functor~$\Omega^\bullet_{\text{pwpoly}}$.
    }
    ...
    There is also another approach due to \indexauthor{Quillen}. Instead of working with differential graded algebras, \indexauthor{Quillen} used differential graded Lie algebras, i.e.~strict $L_\infty$-algebras (\cref{section:higher_lie_structures}).
...
\subsection{\difficult{Equivariant homotopy theory}}
    ...
    \newdef{Orbit category}{
        Let $G$ be a topological group. The orbit category $\symbfsf{Orb}_G$ is the category defined by the following data:
        \begin{enumerate}
...
\subsection{Simplices}
    ...
    \begin{property}[Fundamental group]\index{fundamental!group}
        Let $(\mathcal{K},a_0)$ be a pointed, path-connected polyhedron and consider a contractible, one-dimensional subpolyhedron $\mathcal{C}\subset\mathcal{K}$ containing all vertices of $\mathcal{K}$. Moreover, let $G$ be the free group (\cref{group:free_group}) with generators $g_{ij}$ corresponding to the ordered 1-simplices $[v_i,v_j]\in\mathcal{K}$. On this group, one can define the following relations:
        \begin{itemize}
...
\subsection{Simplicial homology}\index{homology!simplicial}
    ...
    \newdef{Chain group}{\label{topology:chain_group}
        Let $X$ be a simplicial $n$-complex. The $k^{\text{th}}$ chain group $C_k(X)$ is defined as the free Abelian group generated by the $k$-simplices in $X$:
        \begin{gather}
            C_k(X) := \left\{\sum_ia_i\sigma_i\,\middle\vert\,\sigma_i\text{ is a $k$-simplex in $X$}, a_i\in\mathbb{Z}\right\}\,.
        \end{gather}
        For $k>n$ and $k<0$, $C_k(X)$ is defined to be $\{0\}$.
    }

    \newdef{Boundary operator}{
        The boundary operator $\partial_k:C_k(X)\rightarrow C_{k-1}(X)$ is the group morphism defined by the following properties:
        \begin{itemize}
    ...
    \newdef{Cycle group}{
        The $k^{\text{th}}$ cycle group $Z_k(X)$ is defined as the set of $k$-chains $\sigma_k$ such that $\partial_k\sigma_k = 0$. These chains are called \textbf{cycles}.
    }
    \newdef{Boundary group}{
        The $k^{\text{th}}$ boundary group $B_k(X)$ is defined as the set of $k$-chains $\sigma_k$ for which there exists a $(k+1)$-chain $b$ such that $\partial_{k+1}b = \sigma_k$. These chains are called \textbf{boundaries}.
    }
    \newdef{Homology group}{\label{topology:simplicial_homology_group}
        From \cref{topology:boundary_operator_relation}, it follows that $B_k(X)\subset Z_k(X)$ is a subgroup (it is a normal subgroup because of the Abelian character). One can thus define the $k^{\text{th}}$ homology group $H_k(X)$ as the following quotient group:
        \begin{gather}
            H_k(X) := Z_k(X)/B_k(X)\,.
        \end{gather}
        \Cref{group:theorem:free_group} says that $H_k(X)$ can be written as $G_k\oplus T_k$. Both of these groups say something about $X$. The rank of $G_k$, denoted by $R_k(X)$, is equal to the number of $(k+1)$-dimensional holes in $X$. The torsion subgroup $T_k$ says how the space $X$ is `twisted'.
    }

    \newdef{Betti numbers}{\index{Betti number}\label{topology:betti_number}
        The ranks $R_k(X)$ are called the Betti numbers of $X$.
    }
    \newdef{Euler characteristic}{\index{Euler!characteristic}\index{Euler--Poincar\'e formula}\label{topology:euler_characteristic}
    ...
    \begin{property}[Vanishing torsion]
        When $G$ is a field, such as $\mathbb{Q}$ or $\mathbb{R}$, the torsion subgroups $T_k$ vanish and the (co)homology groups are vector spaces. The relation between integral homology and homology with coefficients in a group is given by the \textit{universal coefficient theorem}.
    \end{property}
    ...
    \begin{theorem}[Lefschetz fixed point theorem]\index{Lefschetz!fixed point theorem}\index{Lefschetz!number}\label{topology:lefschetz}
        Let $X$ be a compact triangulable space (equivalently, a finite simplicial complex) with a continuous endofunction $f:X\rightarrow X$. If the (rational) \textbf{Lefschetz number}
        \begin{gather}
            \Lambda(X,f) := \sum_{n\in\mathbb{N}}(-1)^n\tr\left(H^n(X;\mathbb{Q})\xrightarrow{f_*}H^n(X;\mathbb{Q})\right)\,,
        \end{gather}
        where $\tr$ is the trace of linear maps (\cref{linalgebra:trace}), is nonzero, there exists a fixed point of $f$. The converse, however, is not true.
    \end{theorem}
    \begin{result}[Brouwer fixed point theorem]
        For contractible spaces, higher cohomology groups vanish and $H^0(X;\mathfrak{K})\cong\mathfrak{K}$. It follows that the Lefschetz number of any continuous endofunction is 1. This implies Brouwer's fixed point theorem~\ref{topology:brouwer}.
    \end{result}

    \begin{remark}[Euler characteristic]
        Note that the Euler characteristic~\ref{topology:euler_characteristic} is the Lefschetz number of the identity.
    \end{remark}

\subsection{Relative homology}\index{homology!relative}

    In this section, the homology of a simplicial complex $X$ `modulo' a subcomplex $Y$ is considered.

    \newdef{Relative chain group}{
        The $k$-chain group of $X$ modulo $Y$ is defined as the following quotient group:
        \begin{gather}
            C_k(X,Y) := C_k(X)/C_k(Y)\,.
        \end{gather}
        Equivalence classes will be denoted by square brackets $[c]$, where $c\in C_\bullet(X)$.
    }
    \newdef{Relative boundary operator}{
        ...
        \end{gather}
        where $\partial_k$ on the right-hand side denotes the boundary operator in ordinary homology. This is a well-defined operation, since $\partial C_\bullet(Y)\subseteq C_\bullet(Y)$.
    }
    \newdef{Relative homology groups}{
        ...
        \begin{gather}
            H_k(X,Y) := \frac{\ker(\overline\partial_k)}{\im(\overline\partial_{k+1})}\,.
        \end{gather}
        Elements $h_k\in H_k(X,Y)$ can be represented as $h_k=z_k + C_k(Y)$ such that $\partial_kz_k\in C_{k-1}(Y)$.
    }

    \begin{property}[Homotopy invariance]
        Let $X,Y$ be two (triangulable) topological spaces and consider two subspaces $A\subset X,B\subset Y$. If a continuous function $f:X\rightarrow Y$ is a homotopy equivalence such that the restriction to $A$ gives a homotopy equivalence $f|_A:A\rightarrow B$, the relative homology complexes $H_\bullet(X,A)$ and $H_\bullet(Y,B)$ are isomorphic.
    \end{property}

    \begin{property}[Long sequence for a pair]
        The relative homology groups fit in the following (long) exact sequence:
        \begin{gather}
            \cdots\longrightarrow H_k(Y)\overset{i_\ast}{\longrightarrow}H_k(X)\overset{j_\ast}{\longrightarrow}H_k(X,Y)\overset{\partial_k}{\longrightarrow}H_{k-1}(Y)\longrightarrow\cdots\,,
        \end{gather}
        where $i_\ast$ and $j_\ast$ are the homology morphisms induced by the inclusions $i:Y\hookrightarrow X$ and $j:X\hookrightarrow(X,Y)$.
    \end{property}
    ...
    \todo{STREAMLINE TERMINOLOGY AND NOTATION (topological space vs. triangulable space vs. simplicial complex, ...)}
...
\subsection{Examples}
    ...
    \end{example}
    \begin{example}
        Let $X$ be a path-connected simplicial complex.
        \begin{gather}
            H_0(X) = \mathbb{Z}
        \end{gather}
        Furthermore, every point $p\in X$ determines a generator $\langle p \rangle\in H_0(X)$.
    \end{example}
    ...
    \newdef{Homology sphere}{\index{homology!sphere}
        An \textit{$n$-dimensional manifold} (see \labelref{chapter:manifolds}) having the same homology groups as the $n$-sphere.
    }
...
\section{Singular homology}\label{section:singular_homology}
    ...
    \begin{property}[Representability]
        ...
        \begin{mdframed}[roundcorner=10pt, linecolor=blue, linewidth=1pt]
            \begin{proof}
                Consider the Eilenberg--Maclane space $K(G,n)$ of $G$ (\cref{topology:eilenberg_maclane}). By the Hurewicz theorem, there exists an isomorphism $H_n\bigl(K(G,n)\bigr)\cong\pi_n\bigl(K(G,n)\bigr)\cong G$, and, by definition of cohomology, $H^n\bigl(K(G,n);G\bigr)=\hom\bigl(H_n(K(G,n)),G\bigr)$. It follows that $H^n\bigl(K(G,n);G\bigr)\cong\hom(G,G)$ and, in particular, there corresponds a cohomology class $\psi$ to the identity mapping on $G$. The cohomology class in $H^n(X;G)$ associated to a homotopy class of functions $f:X\rightarrow K(G,n)$ is given by $f^*\psi$.
            \end{proof}
        \end{mdframed}
    \end{property}
...
\section{\difficult{Axiomatic approach}}\label{section:eilenberg_steenrod}
    ...
    \newdef{Eilenberg--Steenrod axioms}{\index{Eilenberg--Steenrod axioms}\label{topology:eilenberg_steenrod_axioms}
        All (co)homology theories have a set of properties in common. By treating these properties as axioms, one can construct relative (co)homology theories as a sequence of functors $H_k:\symbfsf{Top}\times\symbfsf{Top}\rightarrow\symbfsf{Ab}$.\footnote{Technically, one should replace $\symbfsf{Top}\times\symbfsf{Top}$ by the category consisting of subspace inclusions $A\hookrightarrow X$.} The axioms are as follows:
        \begin{enumerate}
...
\section{\difficult{Equivariant cohomology}}\index{equivariant!cohomology}\label{section:equivariant_cohomology}

    In this section, the action of (topological) groups $G$ on topological spaces will be considered. These spaces will be called topological $G$-spaces or just $G$-spaces. See, for example, \citet{cartier_mad_2001,rozenblyum_bung_2023}. The content of \labelref{chapter:sheaf} is used.
...
\subsection{Borel construction}
    ...
    If the action is not free, a more general construction needs to be used. Consider the \textit{universal bundle} $\pi:EG\rightarrow BG$ from \cref{bundle:classifying_space}. From this bundle, one can construct the associated bundle $EG\times_G X$, which is called the \textbf{Borel construction}. It gives a model for the homotopy quotient $X/\!\!/G$, with $BG\cong\ast/\!\!/G$. The $G$-equivariant cohomology of $X$ is then defined as the singular cohomology of the Borel construction:\index{Borel!construction}
    \begin{gather}
        H_G^\bullet(X) := H^\bullet(EG\times_GX)\,.
    \end{gather}

    \begin{property}[Group cohomology]
        When $X=\ast$, equivariant cohomology reduces to group cohomology (\cref{section:group_cohomology}):
        \begin{gather}
            H^\bullet_G(\ast)\cong H^\bullet(G)\,.
        \end{gather}  
    \end{property}

    \begin{example}[$U(1)$]
        \begin{gather}
            H^\bullet_{U(1)}(\ast;\mathbb{Z}) \cong H^\bullet\bigl(U(1);\mathbb{Z}\bigr) = \mathbb{Z}[x]
        \end{gather}
        with $\deg(x)=2$.\footnote{Note the use of $U(1)$ instead of $S^1$ to distinguish the group and underlying topological space.}
    \end{example}

    \begin{property}[Trivial action]
        If $G$ acts trivially on $X$, then
        \begin{gather}
            H_G^\bullet(X)\cong H^\bullet(X)\times H^\bullet(G)\,.
        \end{gather}
    \end{property}

    \begin{property}
        Consider a topological $\mathbb{T}^n$-space $X$.
        \begin{gather}
            H^\bullet_{\mathbb{T}^n}(X;\mathbb{Q})\cong H^\bullet(X^{\mathbb{T}^n})[x_1,\ldots,x_n]\,,
        \end{gather}
        with $\deg{x_i}=2$ for all $i\leq n$.
    \end{property}
    This result is sufficient to determine the equivariant cohomology for arbitrary compact \textit{Lie groups} (see \cref{chapter:lie}) because of the following result.
    \begin{property}
        Let $G$ be a compact Lie group and choose a \textit{maximal torus} $T$ (see \cref{lie:maximal_torus}).
        \begin{gather}
            H^\bullet_G(X)\cong H^\bullet_T(X)^W\,,
        \end{gather}
        where $W$ is the \textit{Weyl group} of $G$ (see \cref{lie:torus_weyl_group}).
    \end{property}

    \begin{theorem}[Atiyah--Bott localization]\index{Atiyah--Bott localization}
        Consider a smooth manifold $M$ (\cref{chapter:manifolds}) equipped with a smooth $\mathbb{T}^n$-action. The kernel and cokernel of the restriction map
        \begin{gather}
            H^\bullet_{\mathbb{T}^n}(X)\rightarrow H^\bullet_{\mathbb{T}^n}(X^{\mathbb{T}^n})
        \end{gather}
        are torsion (as $H^\bullet(\mathbb{T}^n)$-algebras).
    \end{theorem}

    \todo{COMPLETE (e.g.~lectures by Tu)}
...
\subsection{Classifying topoi and stacks}
    ...
    \newdef{Classifying topos}{\index{classifying!topos}\index{equivariant!sheaf}
        First, consider a discrete group $G$. The classifying topos $\mathcal{B}G$ is defined as the category of $G$-sets together with equivariant maps. Equivalently, consider the delooping groupoid $\symbfsf{B}G$ (\cref{cat:group_delooping}). $\mathcal{B}G$ is equivalent to the (pre)sheaf topos on the classifying space $BG\cong|\symbfsf{B}G|$.
    ...
    \begin{example}\index{classifying!stack}
        When $X$ is trivial, the delooping groupoid $\symbfsf{B}G$ is recovered, which is also called the \textbf{classifying stack} in this form. On the other hand, when $G$ is trivial, the representable stack $\underline{X}$ is recovered.
    \end{example}
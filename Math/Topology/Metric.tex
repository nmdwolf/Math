...
\section{Definition}
    ...
    \newdef{Isometry}{\index{isometry}\label{metric:isometry}
        A function $f:(M,d)\rightarrow (M',d')$ between metric spaces is called an isometry or \textbf{distance-preserving function} if it satisfies
        \begin{gather}
...
\section{Topology}
    ...
    \begin{theorem}[Urysohn's metrization theorem]\index{Urysohn!metrization theorem}\label{topology:urysohn_metrization}
        Second-countable $T_3$-spaces are metrizable.
    \end{theorem}
...
\section{Examples}
    ...
    \begin{example}[Product space]\index{product!space}
        ...
        \end{gather*}
        where $(M_i,d_i)$ is a metric space for all $i\leq n$. This set becomes a metric space when equipped with the distance function
        \begin{gather}
...
\section{Hyperspaces}\label{section:hyperspace}
    ...
    \begin{property}[Hemicontinuity]\index{hemicontinuity}\index{semicontinuity|seealso{hemicontinuity}}\label{metric:hemicontinuity}
        $F:X\rightarrow P(Y)$ is lower (resp.~upper) hemicontinuous if and only if it is continuous with respect to the lower (resp.~upper) Vietoris topology (when extended to a topology on the entire power set).
    \end{property}
    \begin{remark}

...
\section{Ringed spaces}\label{section:ringed_spaces}
    ...
    \newdef{Locally ringed space}{\label{sheaf:locally_ringed_space}
        ...
        A morphism $f:(X,\mathcal{O}_X)\rightarrow(Y,\mathcal{O}_Y)$ of locally ringed spaces is given by a continuous function $f:X\rightarrow Y$ and a morphism of sheaves $f^\sharp:\mathcal{O}_Y\rightarrow f_*\mathcal{O}_X$ such that the induced map (its adjunct) on local rings preserves the maximal ideal:
        \begin{gather}
            f_\sharp(\mathfrak{m}_{f(x)})\subseteq\mathfrak{m}_x\,.
        \end{gather}
    }

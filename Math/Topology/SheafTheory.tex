...
\section{Abelian sheaf cohomology}
    ...
    In this section only \textbf{Abelian sheaves} on topological spaces will be considered, i.e.~e. sheaves with values in $\symbfsf{Ab}$ (unless stated otherwise).\index{sheaf!Abelian}
...
\subsection{Derived cohomology}
    ...
    \begin{property}[Injective resolutions]\label{sheaf:enough_injectives}
    ...
    Because of \cref{sheaf:left_exact_functor}, the (right) derived functors of $\Gamma(X,-)$ will be nontrivial.
...
\section{Ringed spaces}\label{section:ringed_spaces}

    \newdef{Ringed space}{\index{ringed space}\label{sheaf:ringed_space}
        A topological space equipped with a sheaf of rings.
    }

    The following remark uses \cref{topos:topological_spaces} for the notions of \textit{direct image} and \textit{inverse image}.
    \begin{remark}[Sheaves of modules]
        Consider a morphism of ringed spaces $(f,f^\sharp):(X,\mathcal{O}_X)\rightarrow(Y,\mathcal{O}_Y)$, together with a sheaf $\mathcal{F}$ of $\mathcal{O}_X$-modules and a sheaf $\mathcal{G}$ of $\mathcal{O}_Y$-modules. The direct image $f_*\mathcal{F}$ is a sheaf of $f_\sharp\mathcal{O}_X$-modules and, hence, through the morphism $f^\sharp$, a sheaf of $\mathcal{O}_Y$-modules. The inverse image $f^{-1}\mathcal{G}$, however, is a sheaf of $f^{-1}\mathcal{O}_Y$-modules and, hence, does not admit a natural $\mathcal{O}_X$-module structure. For this reason, the inverse image of a sheaf of $\mathcal{O}_Y$-modules is defined by an additional tensoring:
        \begin{gather}
            f^*\mathcal{G} := f^{-1}\mathcal{G}\otimes_{f^{-1}\mathcal{O}_Y}\mathcal{O}_X\,.
        \end{gather}
        It can be shown that $(f^*,f_*):\mathcal{O}_X\symbfsf{Mod}\leftrightarrows\mathcal{O}_Y\symbfsf{Mod}$ is still an adjunction.
    \end{remark}
    \newdef{Locally ringed space}{\label{sheaf:locally_ringed_space}
        ...
        A morphism $f:(X,\mathcal{O}_X)\rightarrow(Y,\mathcal{O}_Y)$ of locally ringed spaces is given by a continuous function $f:X\rightarrow Y$ and a morphism of sheaves $f^\sharp:\mathcal{O}_Y\rightarrow f_*\mathcal{O}_X$ such that the induced map on local rings preserves the maximal ideal:
        \begin{gather}
            f^\sharp(\mathfrak{m}_{f(x)})\subseteq\mathfrak{m}_x\,.
        \end{gather}
    }

    \begin{formula}[Residue field]\index{residue!field}
        Every locally ringed space $(X,\mathcal{O}_X)$ admits a function
        \begin{gather}
            \kappa:x\mapsto\mathcal{O}_{X,x}/\mathfrak{m}_x
        \end{gather}
        which assigns to every point the residue field of the stalk over it.

        Every morphism of locally ringed spaces descends to a morphism on residue fields, which is automatically an injection:
        \begin{gather}
            f_\sharp:\kappa\bigl(f(x)\bigr)\hookrightarrow\kappa(x)\,.
        \end{gather}
    \end{formula}

    \newdef{Quasicoherent sheaf}{\index{sheaf!coherent}\label{sheaf:coherent}
        Let $(X,\mathcal{O}_X)$ be a ringed space. A quasicoherent sheaf $\mathcal{F}$ on $X$ is an $\mathcal{O}_X$-module that is locally the cokernel of a morphism of free modules.
        ...
        \end{gather}
        for some open cover $\{U_i\}_{i\in I}$ of $X$, where the index sets $\{k_i\}_{i\in I}$ and $\{l_i\}_{i\in I}$ might be infinite.
    ...
    \begin{property}\label{sheaf:coherent_abelian_category}
        The coherent sheaves over a ringed space $(X,\mathcal{O}_X)$ form an Abelian category (\cref{section:abelian_categories}). However, quasicoherent sheaves do not!
    \end{property}
    ...
    \begin{property}\label{sheaf:locally_free_coherent}
        Locally free sheaves are quasicoherent. Moreover, locally free sheaves of finite rank are coherent if and only if the structure sheaf $\mathcal{O}_X$ is coherent. In algebraic geometry, this will for example be the case for \textit{locally Noetherian varieties} (see \cref{alggeom:noetherian_scheme}) and, in differential geometry, this is the case for \textit{complex manifolds} (see \labelref{chapter:complex_geometry}).
    \end{property}
    ...
    \begin{property}
        Over suitable spaces, such as \textit{schemes} (see \labelref{section:schemes}), locally free sheaves are invertible if and only if they are of rank 1.
    \end{property}
    \begin{remark}

...
\subsection{Locally convex spaces}
    ...
    \begin{adefinition}[Fr\'echet space]
        A topological vector space that admits a topology induced by a countable separated family of seminorms such that it is also complete with respect to the induced metric.
    \end{adefinition}
...
\subsection{Tensor products}\index{tensor product!of vector spaces}
    ...
    \newdef{Projective tensor product of locally convex spaces\footnotemark}{
        \footnotetext{There also exists a notion of injective tensor product for locally convex spaces, but this is too technical for this compendium.}
        Let $V,W$ be locally convex spaces. Definition \ref{functional:locally_convex_seminorm} gives rise to a family of projective seminorms as in the definition above. These define the projective tensor products $V\otimes_\pi W$. Note that in general these spaces are not complete, even when both $V$ and $W$ are.\footnote{In fact, if both $V$ and $W$ are infinite-dimensional Banach spaces, their tensor product (in this sense) will never be complete.}
    }

...
\section{Algebras}
    ...
    \newdef{Division algebra}{\index{algebra!division}
        A unital algebra in which every nonzero element has both a left and right multiplicative inverse. If the algebra is associative, these inverses coincide. A normed division algebra is a division algebra equipped with a multiplicative quadratic form $q$ such that $\langle a\mid b \rangle := \frac{1}{2}[q(a+b)-q(a)-q(b)]$ is a nondegenerate inner product (\cref{linalgebra:innerproduct}).
    }
    ...
    \begin{example}[Frobenius algebra]\index{algebra!Frobenius}\label{linalgebra:frobenius}
        ...
        \begin{gather}
            \eta(ab,c)=\eta(a,bc)\,.
        \end{gather}
        Equivalently, an associative algebra $(A,\mu)$ equipped with a linear form $\varepsilon:A\rightarrow K$ such that $\varepsilon\circ\mu$ is nondegenerate.\footnote{A third equivalent definition will be given in \cref{nca:frobenius}.}
    ...
    \newdef{Rota--Baxter algebra}{\index{algebra!Rota--Baxter}

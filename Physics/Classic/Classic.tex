\chapter{Newtonian, Lagrangian and Hamiltonian Mechanics}\label{chapter:lagrange}
...
\section{Euler-Lagrange equations}\index{Lagrange!equations of motion}\index{Euler-Lagrange|see{Lagrange}}
...
\section{Kepler problem}\index{Kepler!problem}\label{classic:kepler}
    ...
    \newdef{Laplace-Runge-Lenz vector}{\index{Laplace-Runge-Lenz vector}\label{classic:lrl_vector}
        ...
        Before explaining the symmetry itself, the associated conserved Noether charge is given:
        \begin{gather}
            \vector{A} := \vector{p}\times\vector{L} - mk\vector{r},
        \end{gather}
        where $m$ is the mass and $k$ is the scale factor of the potential:
        \begin{gather}
            V(r) = \frac{k}{r}.
        \end{gather}
...
\subsection{Contact structure}
    ...
    \newdef{Canonical transformation}{\index{canonical!transformation}\index{symplectization}
        A transformation of the trivial line bundle $T^*M\times\mathbb{R}$ that leaves both the fibres and the \textbf{symplectization}\footnote{This form turns $T^M\times\mathbb{R}$ into a symplectic manifold.}
        \begin{gather}
            \Omega := d(e^t\alpha)
        \end{gather}
        invariant.

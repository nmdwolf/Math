...
\section{Electricity}
    ...
    \newdef{Electric potential}{\index{potential}
        ...
        \end{gather}
        By fixing an endpoint (some fixed reference location\footnote{As usual the potential is only determined up to some constant.}), the electric field and electric potential are related in the same way as the ordinary potential and the associated (conservative) force (cf.~Equation \eqref{classic:conservative_force}):
        \begin{gather}
            E = -\nabla V.
        \end{gather}
    }
...
\subsection{Ohm's law}
    ...
    \newformula{Ohm's law in wires}{\label{em:ohms_law_linear}
        The following formula can be found by combining Definition \ref{em:resistivity} and Formulas \ref{em:pouillet} and \ref{em:ohms_law} and by assuming that the conductivity tensor is a scalar (this follows from the isotropic behaviour of most resistors):
        \begin{gather}
            V = RI.
        \end{gather}
    }

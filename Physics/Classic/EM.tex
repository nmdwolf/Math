...
\section{Electricity}
    ...
    \newdef{Electric potential}{\index{potential}
        The electric field gives the force experienced by a single (positive) charge. The electric potential is then defined as the work done by this charge when moving through the field:
        \begin{gather}
            V(\vector{a},\vector{b}) := \int_{\vector{a}}^{\vector{b}}\vector{E}\cdot d\vector{l}.
        \end{gather}
        By fixing the endpoint (some fixed reference location\footnote{As usual the potential is only determined up to some constant.}), the electric field and electric potential are related in the same way as the ordinary potential and the associated (conservative) force (cf. Equation \eqref{classic:conservative_force}):
        \begin{gather}
            E = -\nabla V.
        \end{gather}
    }
...
\subsection{Conductors and resistors}
    ...
    \begin{property}[Electric field of conductor]
        The electric field inside a conductor is vanishes at equilibrium. All free charges are located at the surface. Moreover, the electric field at the surface points in the direction of the normal vector.
    \end{property}
...
\subsection{Electric dipoles}
    ...
    \begin{formula}[Multipole expansion]
        ...
        \end{itemize}
        where $P_n$ is the $n^{th}$ \temptext{Legendre polynomial}. The first term gives an approximation as if all charge was concentrated in a single point (its called the \textbf{monopole approximation}). At very large distances, where the charge distribution appears as a single point, this gives a good approximation.

        When the position vector $\vector{r}$ is not assumed to be oriented along the $z$-axis, the Legendre polynomials have to be replaced by \temptext{spherical harmonics}.
    \end{formula}

\section{Magnetism}\index{magnetization}\index{susceptibility}\index{permeability}\index{induction}
...
\section{Differential-geometric perspective}
    ...
    \newdef{Field strength}{\index{field!strength}
        Define \[\mathbf{E} = E_1\drx^1 + E_2\drx^2 + E_3\drx^3\] and \[\mathbf{B} = B_1\drx^2\wedge\drx^3 + B_2\drx^3\wedge\drx^1 + B_3\drx^1\wedge\drx^2\] as the electric and magnetic field forms. Using these differential forms one can write the field strength as follows:
        \begin{gather}

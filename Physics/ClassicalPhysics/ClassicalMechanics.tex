...
\subsection{Linear motion}
    ...
    \newdef{Conservative force}{\index{conservative!force}\index{potential}\label{classic:conservative_force_def}
        ...
        To generalize this result from position space $\mathbb{R}^3$ to include velocities (or even higher-order derivatives), a variational approach has to be adopted since, in this case, the actual path is of relevance. For velocity-dependent forces, the (generalized) potential $U:\mathbb{R}^6\rightarrow\mathbb{R}$ generates the force as follows:\footnote{Note that, as with ordinary forces, not all velocity-dependent forces are conservative (in this genelized sense).}
        \begin{gather}

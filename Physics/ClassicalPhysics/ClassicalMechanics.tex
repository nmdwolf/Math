\chapter{Classical Mechanics}\label{chapter:lagrange}
    ...
    The section about the geometric framework is mainly based on~\citet{palais_symmetries_1997,arnold_mathematical_2013}. For an introduction to differential geometry, see \labelref{chapter:manifolds} and onwards.
...
\subsection{Linear motion}
    ...
    \newdef{Conservative force}{\index{conservative!force}\index{potential}\label{classic:conservative_force_def}
        ...
        \end{gather}
        for all paths $C:S^1\rightarrow\mathbb{R}^3$.
        ...
        To generalize this result from position space $\mathbb{R}^3$ to include velocities (or even higher-order derivatives), a variational approach has to be adopted since, in this case, the actual path is of relevance. For velocity-dependent forces, the (generalized) potential $U:\mathbb{R}^6\rightarrow\mathbb{R}$ generates the force as follows:\footnote{Note that, as with ordinary forces, not all velocity-dependent forces are conservative (in this generalized sense).}
        \begin{gather}
            \label{classic:velocity_dependent_force}
            \vector{F}(\vector{x},\vector{v}) = -\nabla U-\deriv{}{t}(\nabla_{\vector{v}}U)\,.
        \end{gather}
        In general, the functional derivative~\eqref{var:euler_lagrange_derivative} is used:
        \begin{gather}
            \vector{F}(\vector{r},\dot{\vector{r}},\ldots,t) = -\delta_{\text{EL}}U(\vector{r},\dot{\vector{r}},\ldots,t)\,.
        \end{gather}
    }
...
\subsection{Euler--Lagrange equations}\index{Euler--Lagrange!equations}
    ...
    \begin{remark}[Conservative forces]\index{conservative!force}
...
\subsection{Kepler problem}\index{Kepler!problem}\label{section:kepler}
    ...
    \begin{formula}[Gravitational potential for a point mass]\index{gravity}\label{classic:gravitational_potential}
        \begin{gather}
            V = -G\frac{M}{r}\,,
        \end{gather}
        where $G=6,67\times10^{-11}\frac{\mathrm{Nm}^2}{\mathrm{kg}^2}$ is the \textbf{gravitational constant}.
    \end{formula}
    ...
    \newdef{Laplace--Runge--Lenz vector}{\index{Laplace--Runge--Lenz vector}\label{classic:lrl_vector}
        ...
        Before explaining the symmetry itself, the associated Noether charge (\cref{symplectic:noether_charge}), the Laplace--Runge--Lenz vector, is given:
        \begin{gather}
        ...
        \begin{gather}
            V(r) = \frac{k}{r}\,.
        \end{gather}
        This vector points in the direction of the semi-minor axis of elliptical orbits and its magnitude corresponds to the \textit{eccentricity} of the orbit.
    }
    ...
    To see this, perform the following time transformation introduced by \indexauthor{Moser} in the 80s (so quite recently):
    \begin{gather}
        \deriv{s}{t} = \frac{1}{r}\,.
    \end{gather}
    THis way, time passes slower, the further away one is positioned from the center of mass. With respect to the time parameter $s$, the velocity does not depend on the radial parameter and the particle moves in a circular orbit in the 4-dimensional space $(t,x,y,z)$.
...
\section{Hamiltonian mechanics}
    ...
    \newdef{Rotation}{\index{rotation}
        A closed trajectory in phase space for which at least one of the variables takes on all possible values. Topologically, it is characterized by a noncontractible, closed trajectory.
...
\subsection{Poisson bracket}
    ...
    \newdef{Poisson bracket}{\index{Poisson!bracket}\label{classic:poisson_bracket}
...
\subsection{Hamilton--Jacobi equation}
    ...
    \newdef{Hamilton's characteristic function}{\index{Hamilton!characteristic function}\index{energy}\index{eikonal equation}
        ...
        \begin{gather}
            S(q,p,t) = W(q,p) - Et\,.
        \end{gather}
        The time-independent function $W$ is called Hamilton's characteristic function. The time-independent Hamilton--Jacobi equation is often called the \textbf{eikonal equation}\footnote{This name stems from optics.}.
    }
...
\subsection{Fluid mechanics}
    ...
    \newdef{Cauchy stress tensor}{\index{Cauchy!stress tensor}
        The Cauchy stress tensor is a $(0,2)$-tensor $\symbf{T}$ that gives the relation between a stress vector associated to a plane and the normal vector $\vector{n}$ to that plane:
        \begin{gather}
            \vector{t}_{(\vector{n})} = \symbf{T}(\vector{n})\,.
        \end{gather}
    }
    \begin{example}
...
\section{Koopman--von Neumann mechanics}\index{Koopman--von Neumann!mechanics}\index[author]{Koopman}\index[author]{von Neumann}
    ...
    \begin{property}[Generality]
        ...
        To recover \textit{quantum mechanics} (see \labelref{chapter:qm}), the \textit{canonical commutation relations} (see \cref{qm:CCR}) are postulated: $[\widehat{x},\widehat{p}]=i$. In this case, no additional operators have to be introduced and (up to a factor $\hbar$), the Liouville operator can be shown to coincide with standard Hamiltonian:
        \begin{gather}
    ...
    \begin{remark}[Linearity]
        Note that, in contrast to the \textit{Schr\"odinger equation}~\eqref{qm:TDSE_position}, both the Liouville and KdV equations are linear PDEs.
    \end{remark}

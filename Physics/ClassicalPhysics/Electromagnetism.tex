...
\subsection{Electric charges in a magnetic field}
    ...
    \begin{construct}[Amp\`ere dipole model]\index{Amp\`ere!model}\index{torque!magnetic dipole}
        ...
        \begin{gather}
            \label{em:energy_magnetic_dipole}
...
\subsection{Moving charge}
    ...
    Consider a (point) particle with charge $q$ moving with velocity $\vector{v}(t)$. To determine the electromagnetic field at a position $\vector{r}_0$, it is useful to introduce the following notion.
    \newdef{Retarded time}{\index{time!retarded}
        \begin{gather}
            t_r(\vector{r}_0,t) := t - \frac{\|\vector{r}_0-\vector{r}(t_r)\|}{c}
        \end{gather}
        Note that, as long as $v<c$, this implicit equation has exactly one solution for $t_r$.
    }
    
    The electromagnetic field of the moving particle is given by the following potentials.
    \newdef{Li\'enard--Wiechert potentials}{\index{Li\'enard--Wiechert potentials}
        Let $\vector{R}(t_r) := \vector{r}_0-\vector{r}(t_r)$ and $\hat{\mathbf{n}}:=\vector{R}/R$.
        \begin{gather}
            V(\vector{r}_0,t) = \frac{1}{4\pi\varepsilon_0}\frac{q}{R(t_r)\bigl(1 - \vector{\beta}(t_r)\cdot\hat{\mathbf{n}}(t_r)\bigr)}
        \end{gather}
        and
        \begin{gather}
            \vector{A}(\vector{r}_0,t) = \frac{\mu_0c}{4\pi}V(\vector{r}_0,t)\vector{\beta}(t_r)\,,
        \end{gather}
        where
        \begin{gather}
            \vector{\beta} := \frac{\vector{v}}{c}
        \end{gather}
        as in \cref{relativity:lorentz_factor} from special relativity.
    }

    The resulting expressions for the fields are:
    \begin{gather}
        \vector{E}(\vector{r}_0,t) = \underbrace{\left.\frac{q}{4\pi\varepsilon_0\gamma^2}\frac{\hat{\mathbf{n}}-\vector{\beta}}{R^2(1 - \vector{\beta}\cdot\hat{\mathbf{n}})^3}\right|_{t_r}}_{\text{static (Coulomb-like)}} + \underbrace{\left.\frac{q}{4\pi\varepsilon_0c}\frac{\hat{\mathbf{n}}\times\bigl((\hat{\mathbf{n}}-\vector{\beta})\times\dot{\vector{\beta}}\bigr)}{R(1-\vector{\beta}\cdot\hat{\mathbf{n}})^3}\right|_{t_r}}_{\text{radiative}}
    \end{gather}
    and
    \begin{gather}
        \vector{B}(\vector{r}_0,t) = \frac{\hat{\mathbf{n}}(t_r)\times\vector{E}(\vector{r}_0,t)}{c}\,,
    \end{gather}
    where $\gamma$ is the Lorentz factor~\eqref{relativity:lorentz_factor} from special relativity.

    \begin{property}[Instantaneous position]
        For constant velocity, there is no radiative term. Moreover, the static term points in the direction of the instantaneous direction of the particle, not the retarded position.
    \end{property}

    \begin{remark}[Radiation and uniform acceleration]
        There is an open and long-standing discussion whether uniformly accelerating charges produces radiation. This highly depends on the definition of radiation!
    \end{remark}
...
\subsection{Gauge theory}\index{gauge!transformation}\label{section:em_gauge_theory}
    ...
    \begin{example}[Lorenz gauge]\index{Lorenz gauge}
        A first example of a gauge fixing condition is the Lorenz gauge\footnote{Named after \textit{L.~\indexauthor{Lorenz}}, not to be confused with \textit{H.~\indexauthor{Lorentz}}.}:
        \begin{gather}

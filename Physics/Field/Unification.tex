...
\subsection{Donaldson--Witten theory}\index{Donaldson--Witten theory}
    ...
    According to \cref{bundle:connection_gauge_transformation}, one can characterize the stabilizer of $A$ as those sections of $\Aut_V(P)$ that are covariantly constant (with respect to $A$). The Lie algebra of this group is given by $\Gamma(\mathrm{ad}(P))$, hence the reducible connections are those that correspond to a nonzero kernel of
    \begin{gather}

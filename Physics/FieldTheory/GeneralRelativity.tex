...
\section{Causal structure}\index{causal}
    ...
    \begin{definition}[Fermi--Walker transport]
        Consider an accelerating observer with 4-momentum $u$ and associated 4-acceleration $a=\deriv{u}{\tau}$. Consider the following equation for some 4-vector $\xi$:
        \begin{gather}
            \deriv{\xi}{\tau} = -\Omega(\xi)\,,
        \end{gather}
        where the Fermi--Walker 2-form is defined as follows:
        \begin{gather}
            \Omega := a\wedge u\,.
        \end{gather}
        A tetrad field $\mathrm{e}$ that transforms in this way, is called a Fermi--Walker frame or \textbf{proper, nonrotating reference frame} because, in the rest frame of the observer, no rotation takes place. The 2-form $\Omega$ gives an instantaneous generator of the Lorentz transformation that the reference frame at (proper) time $\tau$ to that at $\tau+d\tau$.

        Fermi--Walker transport is characterized by three conditions:
        \begin{enumerate}
            \item $\mathrm{e}_0(\tau)=u(\tau)$,
            \item if $\xi\perp\mathrm{e}_0$, then Fermi--Walker transport implies $\dot{\xi}\sim\mathrm{e}_0$, and
            \item it is isometric.
        \end{enumerate}
        \todo{CONTINUE (link with Thomas precession)}
    \end{definition}

\chapter{Quantum Field Theory}\label{chapter:qft}
...
\subsection{BV-BRST theory}
    ...
    \newdef{BV Laplacian}{\index{Batalin--Vilkovisky!Laplacian}
        ...
        \end{gather}
        The canonical BV Laplacian in field-antifield coordinates $(\phi^I,\mathcal{P}_I)\equiv(\phi^i,\eta^A;\mathcal{P}_i,\mathcal{P}_A)$ is given by (the DeWitt convention is again used here)
        \begin{gather}
            \Delta_{\text{BV}} := (-1)^{\varepsilon_I+1}\frac{\partial^R}{\partial\phi^I}\frac{\partial^R}{\partial\mathcal{P}_I}\,.
        \end{gather}
    }
    ...
    \newdef{BV integral}{\index{Batalin--Vilkovisky!integral}\index{gauge!fixing}\index{integral|seealso{Batalin--Vilkovisky}}
        ...
        \begin{gather}
            L_K = \left\{(q^\mu,p_\mu)\in\Pi T^*M\,\middle\vert\,p_\mu=\pderiv{K}{q^\mu}\right\}\,.
        \end{gather}
...
\subsection{Ward identities}\index{Ward identities}
    ...
    In fact, the Ward identities can be generalized even further. Instead of just adding sources $j_I$ for the canonical coordinates, one can also add sources $L_I$ for the BRST variations $\mathcal{S}\phi^I$. \Cref{qft:pre_ward_equation} then becomes:
    \begin{gather}
    ...
    \begin{formula}[Comparison of Hamiltonian and Lagrangian actions]

...
\section{Differential-geometric perspective}
    ...
    \newdef{Field strength}{\index{field!strength}
        Let \[\mathbf{E} = E_1dx^1 + E_2dx^2 + E_3dx^3\] and \[\mathbf{B} = B_1dx^2\wedge dx^3 + B_2dx^3\wedge dx^1 + B_3dx^1\wedge dx^2\] be the electric and magnetic field forms respectively. Using these forms we can write the field strength as follows:
        \begin{gather}
    ...
    \newdef{Potential}{\index{potential}\index{gauge!field}
        The homogeneous equation \ref{maxwell:diff_homogeneous} together with Poincar\'e's lemma\footnote{See theorem \ref{forms:theorem:poincare}.} imply that there exists a differential 1-form $\mathbf{A}$ such that
        \begin{gather}
            \mathbf{F} = d\mathbf{A}.
        \end{gather}

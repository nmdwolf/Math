...
\subsection{Angular quantities}
    ...
    \newformula{Moment of inertia}{\index{inertia}
		...
		\end{equation}
		or more generally we can define the moment of inertia tensor:
		\begin{equation}
...
\newdef{Principal axes of inertia}{\index{principal axis}
	Let $[I]$ be the matrix of inertia. This is a real symmetric matrix, which means that it admits an eigendecomposition of the form (see \ref{linalgebra:diagonalization_properties}):
\[[I] = [Q][\Lambda][Q]^T\]
...
\end{formula}
    ...
    \remark{From the previous definitions it follows that both the angular momentum and torque vectors are in fact pseudo-vectors and thus change sign under coordinate transforms with $\det = -1$.}

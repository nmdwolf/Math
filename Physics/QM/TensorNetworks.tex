...
\subsection{Finite-dimensional lattices}
    ...
    \begin{remark}[Physical and virtual spaces]
        For each `physical' index $i_n$ one can regard the matrix $A^{i_n}(n)$ as a linear map between `virtual' (or ancilla) spaces $\mathbb{C}^{D_n}$.
    \end{remark}
...
\section{Matrix product operators}
    ...
    \newdef{Matrix product operator\footnotemark}{\index{matrix product!operator}
        ...
        \begin{gather}
            \hat{O} = \sum_{\{i_k,j_l\}}\tr\left(\prod_{m,n=1}^NO^{i_m,j_n}(n)\right)|i_1\cdots i_N\rangle\langle j_1\cdots j_n|\,.
        \end{gather}
        In terms of a basis $\{\hat{O}_i\}$ for the space of local operators this becomes:
        \begin{gather}
            \hat{O} = \sum_{\{i_k\}}\tr\left(\prod_n^NA^{i_n}(n)\right)\hat{O}_{i_n}\,.
        \end{gather}
    }

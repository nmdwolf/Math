...
\section{Schr\"odinger picture}
    ...
    \begin{formula}[TISE]\index{Schr\"odinger!equation}\index{Hamiltonian}
        The following partial differential equation is called the \textbf{time-independent Schr\"odinger equation}:
        \begin{gather}
            \label{schrodinger:TISE}
            \hat{H}\psi(x) = E\psi(x).
        \end{gather}
    ...
    \begin{example}[Massive particle in a stationary potential]
        \begin{gather}
            \label{schrodinger:TDSE_position}
            i\hbar\pderiv{}{t}\psi(x,t) = \left(\frac{\hat{p}^2}{2m} + \hat{V}(x)\right)\psi(x,t)
        \end{gather}
    \end{example}

    \begin{formula}[General solution]
        A general solution of the time-dependent Schr\"odinger equation (for time-independent Hamiltonians) is given by the following formula (cf. Equation \eqref{diffeq:first_order_general_solution}):
        \begin{gather}
            \label{schrodinger:general_solution}
            \psi(x,t) = \sum_Ec_E\psi_E(x)e^{-\frac{i}{\hbar}Et},
        \end{gather}
        where the functions $\psi_E(x)$ are the eigenfunctions of the TISE \eqref{schrodinger:TISE}. The coefficients $c_E$ can be found using the orthogonality relations
        \begin{gather}
            \label{schrodinger:general_solution_coefficients}
...
\section{Heisenberg picture}
    ...
    \begin{formula}[Matrix representation]
        ...
        \begin{gather}
            \label{schrodinger:matrix_entry}
...
\section{Uncertainty principle}
    ...
    \newformula{Uncertainty relation}{\index{Heisenberg!uncertainty relation}\index{Robertson|see{Heisenberg uncertainty relation}}
        ...
        \begin{gather}
            \label{schrodinger:uncertainty_relation}

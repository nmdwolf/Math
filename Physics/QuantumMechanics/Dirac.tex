...
\subsection{Chiral spinors}
    ...
    \newdef{Chiral matrix}{\index{chiral!matrix}\index{helicity}
        ...
        \begin{gather}
            \gamma^{d+1} := \gamma^1\gamma^2\cdots\gamma^d\,.
        \end{gather}
        In an odd number of dimensions ($d=2m+1$), a generating set for the Clifford algebra can be obtained by taking the generating set from one dimension lower and adjoining the element $k\gamma^{d+1}$, where $k^2=(-1)^{n+d/2}$. This gives two inequivalent representations of the Clifford algebra (depending on the sign). From here on, the following redefinition will be used:
        \begin{gather}
            \gamma^{d+1}\longrightarrow k\gamma^{d+1}\,.
        \end{gather}
        This has the benefit that $(\gamma^{d+1})^2 = \mathbbm{1}$. In $d=3+1$, one generally takes the following representation for $\gamma^5$ (in the Dirac representation):\footnote{Such a block diagonal form can always be chosen by working in a \textit{helicity-adapted basis}.}
        \begin{gather}
            \gamma^{d+1} :=
            \begin{pmatrix}
                0&\mathbbm{1}\\
    ...
    \begin{remark}[Weyl representation]

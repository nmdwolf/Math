...
\section{Introduction}
    ...
    The simplest example is again $T^*\mathbb{R}^n$. Here, the usual choice of observables are the coordinate and momentum functions $\{q^i,p_i\}_{i\leq n}$ and the group $G$ is given by the \textit{Heisenberg group}, a $\mathrm{U}(1)$-central extension of the translation group $\mathbb{R}^{2n}$. Irreducible representations are characterized by the Stone--von Neumann theorem~\ref{qm:stone_von_neumann}.\index{Heisenberg!group}

    Although the above procedure sounds reasonable, it is known to be inconsistent. \indexauthor{Groenewold} and \indexauthor{Van Hove} showed that there does not exist a map $\mathcal{O}$, satisfying the axioms~\ref{quantization:axioms}, that takes the entire (Lie) algebra of classical observables to the (Lie) algebra of corresponding quantum observables. As such, a different approach should be followed.\index{Groenewold--Van Hove}
...
\section{Deformation quantization}\index{quantization|seealso{deformation}}
    ...
    \begin{remark}[Kontsevich quantization]\index{Kontsevich quantization}
        \indexauthor{Kontsevich} had previously proven that every finite-dimensional Poisson manifold admits a deformation quantization of its algebra of smooth functions. However, a slight variation of \indexauthor{Fedosov}'s approach can be used even for infinite-dimensional algebras such as those occuring in field theory (where Kontsevich's approach is invalid).
    \end{remark}
...
\subsection{Prequantization}
    ...
    \begin{property}[Weil integrality]\index{Weil!integrality}\index{Dirac!condition}\index{Bohr--Sommerfeld condition}\label{quantization:weil_integrality}
        Complex line bundles are classified by the (first) Chern class $c_1\in H^2(M;\mathbb{Z})$, which is proportional to the curvature form through Chern--Weil theory (\cref{bundle:chern_classification}). Therefore, a prequantum line bundle exists if and only if the symplectic form is integral (up to a factor of $2\pi$). For simply connected manifolds, this is equivalent to
        \begin{gather}
            \forall S\in H_2(M):\Int_S\omega\in 2\pi\mathbb{Z}\,.
        \end{gather}
        This condition resembles the `old' \textbf{Bohr--Sommerfeld condition}\footnote{In fact, it is closer to Einstein's quantization rule~\citep{stone_einsteins_2005}.} and is, in general, known as the Weil integrality condition.
    ...
    \end{property}
    \begin{result}[Dirac quantization condition]\index{quantization!Dirac condition}\label{quantization:dirac_condition}
        The minimal coupling procedure, where $\nabla$ is coupled to an external gauge field, is given by the substitution $\omega\longrightarrow\omega+eF$. Weil integrality then implies that $e$ is an integer.

        Classically, the Dirac quantization condition is obtained by assuming the existence of a magnetic charge $g$. To obtain a well-defined holonomy when transporting an electric charge $q$ around $g$, one needs
        \begin{gather}
            qg \in 2\pi\mathbb{Z}\,.
        \end{gather}
        As soon as one magnetic monopole is present in the universe, the quantization of electric charges would be evident!
    \end{result}
    ...
    \newdef{Bohr--Sommerfeld leaf}{\index{Bohr--Sommerfeld leaf}
        ...
        \begin{gather}
            \begin{tikzpicture}[baseline = (current bounding box.center)]
                ...
                \node (H) at (4, 2) {$\symbfsf{B}\mathrm{U}(1)_{\text{conn}}$};
                \draw[->] (M) -- node[below]{$\omega$} (cl);
                \draw[->] (H) -- node[right]{$F$} (cl);
                \draw[->, dashed] (M) -- node[above left]{$\nabla$} (H);
            \end{tikzpicture}
        \end{gather}
        where $\symbfsf{B}\mathrm{U}(1)_{\text{conn}}$ is the Deligne complex (\cref{bundle:deligne_complex}). A submanifold $L\subset M$ is isotropic (Lagrangian if it is maximal) if the lift
        \begin{gather}
            \begin{tikzpicture}[baseline = (current bounding box.center)]
                \node (L) at (-2, 0) {$L$};
                \node (M) at (0, 0) {$M$};
...
\subsection{Coadjoint orbits}
    ...
    \todo{ADD (SEE ALSO \cref{section:coadjoint_orbits})}
...
\section{Toeplitz quantization}\label{section:toeplitz_quantization}
    ...
    \newdef{Toeplitz quantization}{\index{Toeplitz!quantization}\index{Toeplitz!operator}\index{Hardy space}
        The map $\mathfrak{T}_n:C^\infty(M)\rightarrow\mathrm{End}(\mathcal{H}_n)$ given by\footnote{This particular form, first multiplying by a smooth function and then projecting onto holomorphic sections, is very similar to the definition of \textit{Toeplitz operators}, where the projection is onto the \textit{Hardy space}~(\cref{distributions:hardy_space}), hence the name.}
        \begin{gather}
...
\subsection{Dirac procedure}
    ...
    \todo{COMPLETE (e.g.~Henneaux \& Teitelboim) + see also \cref{section:qft_gauge_theory}}
...
\subsection{BRST quantization}
    ...
    \begin{axiom}[BRST-invariant states]\index{observable}
    ...
    There is, however, a problem. \Cref{quantization:nilpotent_charge} can only be satisfied for nontrivial operators if there are states with negative norm.
    ...
    \begin{property}[Lefschetz trace formula]\index{Lefschetz!trace formula}
        The trace of an operator in the extended phase space and in BRST cohomology coincide.
    \end{property}

    \begin{construct}[Jordan representation]
    ...
    \begin{property}[Time evolution]\label{quantization:brst_time_evolution}
    ...
    \begin{construct}[Constraint operators]

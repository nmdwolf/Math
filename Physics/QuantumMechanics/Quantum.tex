...
\subsection{WKB approximation}\index{WKB approximation}\index{Green--Liouville method|see{WKB approximation}}
    ...
    \begin{property}\index{admissibile!solution}
    ...
    \end{gather}
    Since Lie derivatives pull back under diffeomorphisms (\cref{bundle:pullback_lie_derivative}) and the image $\im(\dr S)$ gives a trivial subbundle of $T^*M$, this is also equivalent to
    \begin{gather}
    ...
    \begin{quote}
        A second-order approximate solution to the Schr\"odinger equation is given by a pair $(S,a)$, where $S$ is an admissible phase function and $a\in\Omega^{1/2}(\im(\dr S))$ is a half-form that is invariant under the (classical) Hamiltonian flow.
    \end{quote}

\section{\difficult{Curved backgrounds}}

    \todo{FIND GOOD SPOT FOR THIS SECTION}

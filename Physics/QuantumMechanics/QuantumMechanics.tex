...
\subsection{Dirac--von Neumann postulates}\index{Dirac--von Neumann postulates}\label{section:qm_postulates}
    ...
    But now, given the abstract state vectors $\ket{\psi}$, how does one recover the \textbf{position (configuration) representation} $\psi(x)$, which is the standard tool in introductory courses of QM? This is simply the projection of the state vector $\ket{\psi}$ on the `basis function' $\delta(x)$, i.e.~$\psi(x)$ represents an expansion coefficient in terms of a `basis' for the physical Hilbert space. In the same way, one can obtain the \textbf{momentum representation} $\psi(p)$ by projecting onto the plane waves $e^{ipx}$.

    \begin{remark}\index{Hilbert!rigged Hilbert space}
        It should be noted that neither the `basis states' $\delta(x)$, nor the plane waves $e^{ipx}$ are square integrable and, hence, they are not elements of the Hilbert space $L^2(\mathbb{R},\mathbb{C})$. This issue can be resolved through the concept of \textit{rigged Hilbert spaces} (see \cref{qm:rigged} further below).
    \end{remark}
...
\section{Heisenberg--Born--Jordan picture}
    ...
    Taking the expectation value of this expression for the Schr\"odinger equation (using its linearity), the following (interaction-independent) result is obtained.
    \begin{theorem}[Ehrenfest]\index{Ehrenfest}\label{qm:ehrenfest}
        ...
        \begin{gather}
            \deriv{\langle\widehat{O}\rangle}{t} = \frac{1}{i\hbar}\bigl\langle[\widehat{O},\widehat{H}]\bigr\rangle + \Bigl\langle\pderiv{\widehat{O}}{t}\Bigr\rangle\,.
        \end{gather}
    \end{theorem}
...
\subsection{Hydrogen atom}
    ...
    \end{gather}
    It is not hard to see that this is the quantum mechanical version of the Kepler problem (\cref{section:kepler}). The special property of the Kepler problem was that it contained a `hidden' symmetry that gave rise to the conserved Laplace--Runge--Lenz vector (\cref{classic:lrl_vector}). As is the case for all conserved charges in quantum mechanics, this symmetry induces a degeneracy of the energy eigenvalues.
    
    Degeneracy of the magnetic quantum number $m\in\mathbb{N}$ follows from the rotational $\mathrm{SO}(3)$-symmetry, but this is not the only degeneracy in this system. The energy levels of the hydrogen atom only depend on the principal quantum number $n\in\mathbb{N}$. It is the degeneracy of the total angular quantum number $l\in\mathbb{N}$ that is due to the `hidden' $\mathrm{SO}(4)$-symmetry. This is often called an `accidental degeneracy' for this reason.
...
\subsection{Molecular dynamics}
    ...
    \begin{property}[Hund's rules]\index{Hund's rules}\index{term}
        This set of three rules determines the lowest energy terms $^{2S+1}L_J$ of an atomic system within the Russell--Saunders scheme:
        \begin{enumerate}
            \item Given an electron configuration, the states with maximal spin multiplicity $2S+1$ have the lowest energy.
            \item Given a multiplicity, the states with maximal orbital angular momentum $L$ have the lowest energy.
            \item When the outermost shell is at most half filled, the states with minimal total angular momentum $J$ have the lowest energy. If the outermost shell is more than half filled, the states with maximal $J$ have the lowest energy.
        \end{enumerate}
    \end{property}
...
\subsection{Spin-orbit coupling}\label{section:spin_orbit_coupling}
    ...
    In composite systems, there are two contributions to the angular momentum of the particles: their instrinsic spin $\widehat{S}$ and their orbital angular momentum $\widehat{L}$. These give rise to the total angular momentum:
    \begin{gather}
    ...
    In the case where the particles interact mainly amongst each other electromagnetically (e.g.~electrons for light particles), the best approximation is given by coupling all spin degrees of freedom and all orbital degrees of freedom: $\widehat{S}$ and $\widehat{L}$.  When the particles spin-orbit coupling becomes important, it is better to work with the total angular momenta of the individual particles: $\widehat{j}_i$.
...
\subsection{Spin-orbit coupling}
    ...
    The potential energy of a magnetic dipole $\vector{\mu}$ in a magnetic field $\vector{B}$ is given by \cref{em:energy_magnetic_dipole}:
    \begin{gather}
    ...
    \end{gather}
    where $\vector{E}$ is the electric field of the nucleus in the nucleus' rest frame. This electric field is radial, i.e.~$\vector{E}\sim\vector{r}$, which implies that
    \begin{gather}
    ...
    \end{gather}
    where the factor $g$ is called the (electron) \textbf{$g$-factor}.\index{g!factor} Combining the formulas above leads to the following energy contribution:
    \begin{gather}
    ...
    \begin{remark}[Anomalous magnetic moment]\index{magnetic moment!anomalous}
        The \textit{Dirac equation} (see \labelref{chapter:dirac}) predicts that $g=2$. However, \textit{quantum electrodynamics} (see \labelref{chapter:qft}) predicts a small deviation:
        \begin{gather*}
            g=2+\varepsilon\approx2.00159...\,,
        \end{gather*}
        where $\varepsilon$ is called the anomalous magnetic moment.
    \end{remark}
...
\subsection{WKB approximation}\index{WKB approximation}\index{Green--Liouville method|see{WKB approximation}}
    ...
    \end{gather}
    with $S:\mathbb{R}^n\rightarrow\mathbb{R}$ a phase function to be determined. Inserting this in the TDSE (in the configuration representation) gives:
    \begin{gather}
        \left[\frac{\|\vec{\nabla}S(\vector{q})\|^2}{2m} + \bigl(V(\vector{q})-E\bigr) - \frac{i\hbar \Delta S(\vector{q})}{2m}\right]\exp\left(iS(\vector{q})/\hbar\right) = 0\,.
    \end{gather}
    To first order, i.e.~for slowly varying potentials, the last term can be ignored. In this case, the phase function satisfies the (time-independent) Hamilton--Jacobi equation~\eqref{classic:time_independent_hje}:
    \begin{gather}
        H\bigl(\vector{q},\vec{\nabla}S(\vector{q})\bigr) = \frac{\|\vec{\nabla}S(\vector{q})\|^2}{2m}+V(\vector{q})=E\,.
    \end{gather}
    This leads to the following result.

    \begin{property}[Admissible solutions]\index{admissible!solution}\label{quantum:admissible_solution}
        A function $S:\mathbb{R}^n\rightarrow\mathbb{R}$ is a phase function for a first-order solution to the Schr\"odinger equation if its differential lies in a level set of the classical Hamiltonian $H:T^*\mathbb{R}^n\rightarrow\mathbb{R}$. These solutions are said to be admissible.
    \end{property}
    ...
    \begin{gather}
        ...
        a\Delta S + 2\vec{\nabla}a\cdot\vec{\nabla}S = 0\,.
    \end{gather}
    If $a$ satisfies this equation, $\psi$ is called a \textbf{semiclassical state}.\index{semiclassical} Note that this equation is equivalent to $a^2\vec{\nabla}S$ being divergence free. The generalization to curved spaces, i.e.~replacing $\mathbb{R}^{2n}$ by a symplectic manifold $M$, will be covered in \cref{section:curved_wkb} below.
...
\subsection{Extending quantum mechanics}
    ...
    \begin{remark}[Rigged Hilbert spaces]\index{Hilbert!rigged Hilbert space}\index{Gel'fand!triple}\label{qm:rigged}
...
\subsection{WKB approximation}\label{section:curved_wkb}
    ...
    \Cref{quantum:admissible_solution} is generalized quite trivially after replacing $\mathbb{R}^n$ by a configuration manifold $Q$ and noticing that the image $\im(\dr S)$, for an admissible solution $S$, is a projectable Lagrangian submanifold of $H^{-1}(E)$ by \cref{symplectic:closed_section_submanifold} and \cref{symplectic:projectable_lagrangians}. This leads to the following generalization.
    \begin{definition}[Admissible solution]\index{admissible!solution}
    ...
    To obtain a second-order solution, one also needs a prefactor for the semiclassical states. The homogeneous transport equation~\eqref{quantum:homogeneous_transport_equation} is generalized as follows:
    \begin{gather}
        a\Delta S + 2\mathcal{L}_{\nabla S}a = 0\,,
    \end{gather}
    where $\Delta$ is the Laplace--Beltrami operator on $Q$ or, equivalently,
    \begin{gather}
        \mathcal{L}_{\pi_*X^H}(a^2\vol) = 0\,.
    \end{gather}
    Since Lie derivatives pull back under diffeomorphisms (\cref{bundle:pullback_lie_derivative}) and $X^H$ is tangent to $L$ by the Hamilton--Jacobi equation, this is also equivalent to
    \begin{gather}
        \mathcal{L}_{X^H}(a^2\pi^*\vol) = 0\,.
    \end{gather}
    The quadratic behaviour in $a$ leads to the idea that the correct object for representing quantum states is a half-density (see also \cref{quantization:half_form}).
    
    \begin{property}[Second-order solutions]
        A pair $(L,a)$ where $L$ is an admissible phase function and $a\in\Omega^{1/2}(L)$ is a half-form that is invariant under the (classical) Hamiltonian flow:
        \begin{gather}
            \mathcal{L}_Ya = 0\,,
        \end{gather}
        where $Y$ is the (nonsingular) vector field on $L$ induced by $X^H$
    \end{property}
    
    This then gives a second-order solution on $Q$ by pulling back along the inverse $(\pi\circ\iota)^{-1}$, which is a diffeomorphism since $L$ is projectable. Moreover, if $L$ is exact (\cref{symplectic:exact_lagrangian}), then $S$ is induced by a primitive of the induced Liouville form $\iota^*\alpha$. If both the exactness and projectability conditions are dropped, the notion of a \textbf{geometric solution} is obtained.\index{geometric!solution}

    To pass to this more general situation, some more structure is needed. If $L$ is not exact, the Liouville form does not admit a global primitive. However, $L$ does admit a (good) cover $\{U_i\}_{i\in I}$ such that, on every patch, a second-order solution can be found, in which case the problem becomes how to glue these together. The gluing condition corresponds to the following integrality condition (see also \cref{quantization:weil_integrality}):
    \begin{gather}
        \label{quantum:integrality_condition}
        \phi_i(x)-\phi_j(x)\in2\pi\hbar\mathbb{Z}\,,
    \end{gather}
    where $\phi_i$ is the phase function on $U_i$, for all $x\in U_i\cap U_j$. Note that this condition can only be satisfied for all $\hbar\in\mathbb{R}^+$ if $[\alpha]=0$. However, this is exactly the condition that should be relaxed, since $\hbar$ will be a value fixed by experiment (Planck's constant).
    
    \newdef{Quantizable Lagrangian}{
        A projectable Lagrangian submanifold $L\subset T^*M$ for which there exists an $\hbar\in\mathbb{R}^+$ such that the restriction of the Liouville class to $L$ is $\hbar$-integral, i.e.~the integrality condition~\eqref{quantum:integrality_condition} is satisfied. All values $\hbar$ for which the integrality condition is satisfied are said to be \textbf{admissible}.
    }
    \begin{remark}

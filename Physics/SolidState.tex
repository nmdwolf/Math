...
\section{Crystals}
	\begin{theorem}[Steno's law]\index{Steno's law}
		The angles between corresponding faces of a type of crystal are constant and do not depend on the total shape of the crystal.
	\end{theorem}
...
\section{Symmetries}
	\newdef{Equivalent planes/axes}{
    	When applying certain symmetries to a plane or axis, it often occurs that we obtain a set of equivalent planes/axes. The are denoted respectively by $\{h\ k\ l\}$ and $<h\ k\ l>$.
    }
...
\subsection{Bravais lattice}
	\newdef{Bravais lattice}{\index{Bravais lattice}
    	A crystal lattice with a certain point group symmetry. There are 14 different Bravais lattices in 3 dimensions. These are the only possible ways to place (infinitely) many points in a 3 dimensional space by applying symmetry operations to a point group.
    }
...
\subsection{Reciprocal lattice}
	\newformula{Reciprocal basis vectors}{\index{reciprocal lattice}
    	The reciprocal lattice corresponding to a given Bravais lattice with primitive basis $\{\vec{a},\vec{b},\vec{c}\}$ is defined by the following reciprocal basis vectors
        \begin{equation}
			\vec{a}^* = 2\pi\stylefrac{\vec{b}\times\vec{c}}{\vec{a}\cdot(\vec{b}\times\vec{c})}
		\end{equation}
		The vectors $\vec{b}^*$ and $\vec{c}^*$ are obtained by permutation of $(a,b,c)$. 
    }
    \newdef{Reciprocal lattice vector}{
    	The reciprocal lattice vector $\vec{r}^{\ *}_{hkl}$ is defined as follows:
        \begin{equation}
    ...
    \begin{property}
		The reciprocal lattice vector $\vec{r}^*_{hkl}$ has the following properties:
        \begin{itemize}
			\item $\vec{r}^{\ *}_{hkl}$ is perpendicular to the family of planes $(h\ k\ l)$ of the direct lattice.
            \item $||\vec{r}^{\ *}_{hkl}|| = \stylefrac{2\pi n}{d_{hkl}}$
		\end{itemize}
	\end{property}
...
\subsection{Constructive interference}
	...
	\newformula{Laue conditions}{\index{Laue!conditions}
    	Suppose that the incident beam makes angles $\alpha_0,\beta_0$ and $\gamma_0$ with the lattice axes. A diffracted beam making angles $\alpha,\beta$ and $\gamma$ with the axes will be observed if following conditions are satisfied:
    	\[
        	\begin{array}{c}
	...
	\newformula{Bragg's law}{\index{Bragg's law}
    	An equivalent formulation of the Laue conditions is given by following formula:
    	\begin{equation}
        ...
        \begin{flalign*}
            ...
            &\theta:\text{the 'Bragg' angle}&\\
            &d_{hkl}:\text{distance between neighbouring planes}&
		\end{flalign*}
	}
    \remark{The angle between the incident and diffracted beams is given by $2\theta$.}

	\newmethod{Ewald sphere}{\index{Ewald sphere}
...
\subsection{Intensity of diffracted beams}
	\newdef{Systematic extinctions}{\index{extinction}
    	As every particle in the motive emits its own waves they will interfere and some waves can cancel out. This leads to the absence of certain diffraction spots. These absences are called systematic exctinctions.
    }
    
	\newdef{Atomic scattering factor}{\index{scattering!atomic scattering factor}
    	The waves produced by the individual electrons of an atom, which can have a different phase, can be combined into a resulting wave. The ampltiude of this wave is called the atomic scattering factor.
	}
    ...
    \begin{example}
        ...
        If $h+k+l$ is odd, then $F(hkl) = 0$ for a BCC lattice. If $h,k$ and $l$ are not all even or all odd then $F(hkl) = 0$ for an FCC lattice.
	\end{example}
...
\section{Alloys}
	\subsection{Hume-Rothery conditions}\index{Hume-Rothery conditions}
...
\subsection{Point defects}
	...
	\newdef{Frenkel pair}{\index{Frenkel pair}
		An atom displaced from a lattice point to an interstitial location and thus creating a vacancy-interstitial pair is called a Frenkel defect.
	}
	\begin{formula}
	...
	\sremark{The existence of these defects creates the possibility of diffusion.}
...
\subsection{Charge carriers}
    ...
    \sremark{The order of magnitude across different materials can fluctuate enormously. It can differ by up to 20 orders of magnitude.}
...
\subsection{Band structure}
    ...
    \newdef{Band gap}{\index{band gap}
    	The energy difference between the valence and conduction band if they do not overlap. It is the energy zone\footnotemark\ where no electron states can exist.
    }
    ...
    \newformula{Fermi function}{\index{Fermi!function}
    	The following distribution gives the propability of a state with energy $E_i$ being occupied by an electron:
        \begin{equation}
...
\subsection{Intrinsic semiconductors}
	\begin{formula}
		Let $n$ denote the charge carrier density as before. The following temperature dependence exists:
		\begin{equation}
			n \propto e^{-E_g/2kt}
		\end{equation}
		where $E_g$ is the energy of the band gap. This formula can be directly derived from the Fermi function by noting that for intrinsic semiconductors the Fermi level sits in the middle of the band gap, i.e. $E_c - E_f = E_g / 2$, and that for most semiconductors $E_g\gg kT$.
	\end{formula}
...
\subsection{Extrinsic semiconductors}
    ...
    \newdef{Acceptor}{
    	Element with 3 valence electrons added to create an excess of holes in the valence band. The resulting semiconductor is said to be a \textbf{p-type semiconductor}.
    }
    \newdef{Donor}{
    	Element with 5 valence electrons added to create an excess of electrons in the valence band. The resulting semiconductor is said to be an \textbf{n-type semiconductor}.
    }
...
\subsection{Ferroelectricity}
    ...
    \remark{Ferroelectricity can only occur in crystals with unit cells that do not have a center of symmetry. This would rule out the possiblity of having the asymmetry needed for the dipole moment.}
    

	\newdef{Saturation polarization}{\index{polarization}
    	The maximum polarization obtained by a ferroelectric material. It coincices with the maximum domain formation. 
    }
    \newdef{Remanent polarization}{
    	The residual polarization of the material when the electric field is turned off.
    }
    ...
    \remark{All ferroelectric materials are piezoelectric, but the converse is not true. All crystals without a center of symmetry are piezoelectric. This property is however only a necessary (and not a sufficient) condition for ferroelectricity, as mentioned above.}
...
\subsection{Paramagnetism}
	...
	\newformula{Curie-Weiss law}{\index{Weiss!Curie-Weiss law}\index{Curie!Curie-Weiss law}
		...
		\end{equation}
        where $\theta = CN_W$ with $N_W$ the Weiss-constant. This deviation of the Curie law is due to the intermolecular interactions that induce an internal magnetic field $H_m = N_WM$.
	}

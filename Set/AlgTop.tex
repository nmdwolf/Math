...
\subsection{Homotopy groups}
	...
	\newdef{Fundamental groupoid}{\index{groupoid!fundamental}
		Let $X$ be a topological space. The fundamental groupoid $\Pi_1(X)$ is the groupoid which has the points of $X$ as objects and the basepoint-preserving homotopy classes of continuous function $f:X\rightarrow X$ as morphisms. The fundamental group $\pi_1(X, a)$ is then defined as the automorphism group of $a\in\text{ob}(\Pi_1(X))$.
	}
...
\subsection{Rational homotopy theory}
	...
	\newdef{Rational homotopy equivalence}{\index{rational!homotopy equivalence}
		...
		\end{equation}
		is an isomorphism for all $n\in\mathbb{N}$.
	}

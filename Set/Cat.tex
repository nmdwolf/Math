...
\subsection{Operads}
	...
	\begin{example}[Little $k$-cubes operad]\index{operad!little $k$-cubes}
		A topological operad for which every topological space $\mathcal{P}(n)$ consists all possibe configurations of $n$ embedded $k$-cubes in a (unit) $k$-cube. Composition is given by the obvious way of inserting one unit $k$-cube in one of the smaller embedded $k$-cubes.
	\end{example}
	\begin{property}[Boardman\&Vogt - May]

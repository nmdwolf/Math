...
\subsection{Operads}
	...
	\newdef{Plain operad\footnotemark}{\index{operad}\index{arity}
		...
		Let $\mathcal{O} = \{P(n)\}_{n\in\mathbb{N}}$ be a set of $n$-ary operations ($n$ is called the \textbf{arity}). The set $\mathcal{O}$ is called a plain operad if it satisfies following axioms:
		\begin{enumerate}

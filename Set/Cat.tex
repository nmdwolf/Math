...
\section{Categories and subcategories}
	...
	\newdef{Subcategory}{\index{full!subcategory}
		...
		\begin{itemize}
			\item For every object in ob$_S$ the identity morphism is an element of hom$_S$.
			\item For every morphism in hom$_S$ both the source and target are elements of ob$_S$.
	...
	\newdef{Category with weak equivalences}{\index{weak!equivalence}\label{category:weak_equivalence}
		A category $C$ with a subcategory $W$ such that:
		\begin{itemize}
			\item $W$ contains all isomorphisms in $C$.
			\item Any two composable morphisms $f, g\in\text{hom}_W$ satisfy the 2-out-of-3 property: If any two of $\{f, g, f\circ g\}$ are in $W$ then so is the third.
		\end{itemize}
	}
...
\section{Functors}
	...
	\remark{A contravariant functor can also be defined as a covariant functor from the opposite category. Therefore from now on we will drop the word \textit{covariant} when talking about functors. Furthermore, a contravariant functor $G:C^{op}\rightarrow\text{Set}$ is often called a \textbf{presheaf}.\index{presheaf}}
	...
	\newdef{Dagger category\footnotemark}{\index{category!dagger}\index{involution}\label{cat:dagger_category}
		...
		A category equipped with a contravariant endofunctor, i.e. a functor $\dag:C^{op}\rightarrow C$, such that:
		\begin{itemize}
			\item $\forall a\in\ob(C): \mathbbm{1}_a^\dag = \mathbbm{1}_a$
	...
	\newdef{Faithful functor}{\index{faithful}
		A functor $F:C\rightarrow D$ is said to be faithful if the map \[\text{hom}_C(X, Y)\rightarrow \text{hom}_D(FX, FY)\] is injective for all objects $X, Y\in\ob(C)$.
	}
	\newdef{Full functor}{\index{full}
		A functor $F:C\rightarrow D$ is said to be full if the map \[\text{hom}_C(X, Y)\rightarrow \text{hom}_D(FX, FY)\] is surjective for all objects $X, Y\in\ob(C)$.
	}
...
\section{Natural transformations}
	...
	\remark{A similar definition holds for presheafs $G:C^{op}\rightarrow\text{Set}$. In this case hom$(X, -)$ has to be replaced by hom$(-, X)$.
...
\section{Initial and terminal objects}
	...
	\newdef{Global element}{\index{global!element}\label{category:global_element}
		Let $C$ be a category with terminal object $\mathbf{1}$. A global element of an object $x\in\ob(C)$ is a morphism $\mathbf{1}\rightarrow x$.
	}
	\begin{example}
		In the category Set a global element coincides with the usual notion of \textit{element}.
	\end{example}
...
\section{Diagrams and limits}
	...
	\begin{property}
		Consider an equalizer $(E, e)$. The equalizing morphism $e$ is a mono.
	\end{property}
	...
	\newdef{Pullback\footnotemark}{\index{pullback}\label{cat:pullback}
		...
		The pullback of two morphisms $f:a\rightarrow c$ and $g:b\rightarrow c$ is defined as the limit of diagram \ref{fig:pullback}.
		\begin{figure}[ht!]
			\centering
			\begin{tikzpicture}
				\matrix (m) [matrix of math nodes,row sep=2em,column sep=4em, minimum width=1em, ampersand replacement=\&]{
					a\&\&b\\
					\&c\&\\
				};
	...
	\newdef{Span}{\index{span}
		...
		Let $\Lambda$ be the category with three objects $\{-1, 0, 1\}$ and two morphisms $i:0\rightarrow -1$ and $j:0\rightarrow 1$. By the above definition of a diagram a span in $C$ is equivalent to a functor $S:\Lambda\rightarrow C$.
		\begin{figure}[!ht]
			\centering
			\begin{tikzpicture}
...
\section{Morphisms}
	...
	\newdef{Monomorphism}{\index{monomorphism}
		Let $C$ be a category. A morphism $\mu\in\text{hom}_C(A, B)$ is called a mono\footnote{Sometimes just a \textbf{mono} or a \textbf{monic} morphism.} if for every object $X\in\ob(C)$ and every two morphisms $\alpha_1, \alpha_2\in\text{hom}_C(X, A)$ such that $\mu\circ\alpha_1 = \mu\circ\alpha_2$ we can conclude that $\alpha_1=\alpha_2$.
	}
	...
	\begin{property}
		Every global element\footnote{See definition \ref{category:global_element}.} is a mono.
	\end{property}
	
	\newdef{Injective object}{\index{injective!object}
		Let $C$ be an category. An object $I\in\ob(C)$ is said to be injective if for every $A, B\in\ob(C)$, mono $f:A\rightarrow B$ and morphism $g:A\rightarrow I$ there exists a morphism $\phi:B\rightarrow I$ such that $\phi\circ f = g$.
		\begin{figure}[ht!]
			\centering
			\begin{subfigure}[b]{0.49\textwidth}
				\centering
	...
	\newdef{Projective object}{\index{projective!object}
		Let $C$ be an category. An object $P\in\ob(C)$ is said to be projective if for every $A, B\in\ob(C)$, epi $f:A\rightarrow B$ and morphism $g:P\rightarrow A$ there exists a morphism $\phi:P\rightarrow B$ such that $f\circ\phi = g$.
	}
	
	\newdef{Subobject}{\index{subobject}
		Let $C$ be a category and let $a\in\ob(C)$ be any object. A subobject $b$ of $a$ is a mono $b\hookrightarrow a$.
		
		In fact one should work up to isomorphism and hence the complete definition goes as follows. A subobject $b$ of $a$ in the category $C$ is an isomorphism class of monos $i:b\hookrightarrow a$ in the slice category $C/a$.
	}
	\newdef{Well-powered category}{\index{power}
		A category $C$ is said to be well-powered if for every object $a\in\ob(C)$ the class of subobjects Sub$(a)$ is small.
	}
	
	\newdef{Generator\footnotemark}{\index{generator}\label{cat:generator}
		\footnotetext{Also called a \textbf{separator}.}
		Let $C$ be a category. A collection of objects $\{o_i\in\ob(C)\}$ is called a collection of generators for $C$ if given any two objects $a, b\in\ob(C)$ and any two morphisms $f, g:a\rightarrow b$ we have that $f\neq g\implies\exists j\in I, h_j\in\hom_C(o_i, a): f\circ h_j\neq g\circ h_j$.
	}
	
	\newdef{Decategorification}{
		Let $C$ be a (essentially) small category. The set of isomorphism classes of $C$ is called the decategorification of $C$. This is given by a functor \[K:\text{Cat}\rightarrow\text{Set}\]
	}
	...
	\newdef{Lifting property}{\index{orthogonal}
		A morphism $f:a\rightarrow b$ has the left lifting property with respect to a morphism $g:c\rightarrow d$ if for every commutative diagram \[\begin{array}{ccc}
			a&\longrightarrow&c\\
			f\Big\downarrow&&\Big\downarrow g\\
			b&\longrightarrow&d
		\end{array}\]
		there exists a morphism $\psi:b\rightarrow c$ such that the triangles commute. $g$ is also said to have the right lifting property with respect to $f$. If the morphism $\psi$ is unique then $f$ and $g$ are said to be \textbf{orthogonal}.
	}
	
	\newdef{Weak factorization system}{\index{weak!factorization}\label{category:wfc}
		Consider a category $C$. A pair $(L, R)$ of classes of morphisms in $C$ is called a weak factorization system (WFS) if it satisfies the folllowing 3 properties:
		\begin{itemize}
			\item Every morphism in $C$ factors as a composition $g\circ f$ where $f\in L$ and $g\in R$.
			\item $L$ consists of the morphisms in $C$ that have the left lifting property with respect to morphisms in $R$.
			\item $R$ consists of the morphisms in $C$ that have the right lifting property with respect to morphisms in $L$.
		\end{itemize}
	}
...
\section{Monoidal categories}
	...
	\newdef{Monoidal category\footnotemark}{\index{category!monoidal}\index{category!tensor}\index{tensor!product}
		...
		A category $C$ equipped with a bifunctor \[-\otimes -:C\times C\rightarrow C\] called the \textbf{tensor product} or \textbf{monoidal product}, together with a distinct object $\mathbf{1}$, called the \textbf{unit object}, and 3 natural isomorphisms, called the \textbf{coherence maps}:
		\begin{itemize}
		...
		\end{itemize}
		such that the following \textit{triangle} and \textit{pentagon} diagrams commute. (See \ref{fig:triangle_diagram} and \ref{fig:pentagon_diagram}.)
	...
	\newdef{Weak inverse}{\index{weak!inverse}
		Let $(C,\otimes, \mathbf{1})$ be a monoidal category. Consider an object $x\in\ob(C)$. An object $y\in\ob(C)$ is called a weak inverse of $x$ if it satisfies $x\otimes y\cong\mathbf{1}$.
	}
	\remark{One can show that the existence of a one-sided weak inverse (as in the definition) is sufficient to prove that it is in fact a two-sided weak inverse, i.e. $y\otimes x\cong\mathbf{1}$.}
	...
	\newdef{Braided monoidal category}{\index{braiding}
		Let $(C, \otimes, \mathbf{1})$ be a monoidal category. $C$ is called a braided monoidal category if it comes equipped with a natural isomorphism \[\sigma_{a, b}:a\otimes b\cong b\otimes a\]
		such that the following two hexagon diagrams commute:
		...
		\begin{figure}[ht!]
			...
			\begin{subfigure}[b]{0.49\textwidth}
				...
				\begin{tikzpicture}
					...
					\draw[->] (m-3-1) -- (m-4-2) node[pos=0.5, below left]{$\sigma_{a, c}\otimes b$};
	...
	\begin{property}\index{Yang-Baxter equation}
		...
		\end{equation}
	\end{property}
	\remark{When drawing the above equality using string diagrams one sees that the Yang-Baxter equation is equal to the invariance of string diagrams under a Reidemeister III move.}
	...
	\newdef{Monoidal functor}{\index{monoidal!functor}\index{coherence!maps}\index{structure!morphisms}
		...
		\begin{itemize}
			\item A natural isomorphism $\psi_{a, b}: Fa\circledast Fb\implies F(a\otimes b)$ such that the following diagram commutes:
			\begin{figure}[ht!]
				\centering
				\begin{tikzpicture}
	...
	\newdef{Dual object}{\index{dual!object}
		...
		If the object $a^*$, together with the morphisms $\eta, \varepsilon$, exists then $a^*$ is called the \textbf{left dual}\footnote{Analogously, $a$ is called the \textbf{right dual} of $a^*$. The right dual of $b$ is often denoted by $^*b$.} of $a$ and $a$ is said to be \textbf{dualizable}.
	}
	\begin{property}
	...
	\newdef{Trace}{\index{trace}
		Let $(C, \otimes, \mathbf{1})$ be a rigid category. Let $f\in\hom_C(a, a^{**})$. The left and right traces of $f$ are defined as the following morphisms in End$_C(\mathbf{1})$:
		\begin{align}
	...
	\newdef{Symmetric monoidal dagger category}{\index{category!dagger}
		A monoidal category $(C, \otimes)$ which also carries the structure of a dagger category\footnote{See definition \ref{cat:dagger_category}.} such that:
		\begin{equation}
...
\section{Groupoids}
	...
	\newdef{Groupoid}{\index{groupoid}\label{cat:groupoid}
		\nomenclature[S_Grpd]{$\text{Grpd}$}{Category of groupoids.}
		A groupoid $\mathcal{G}$ is a small category in which all morphisms are invertible.
	}
	
	\newdef{Core}{\index{core}
		Let $C$ be a (small) category. The core Core$(C)\in$Grpd of $C$ is defined as the maximal subgroupoid of $C$.
	}
	
	\newdef{Orbit}{\index{orbit}
		Let $\mathcal{G}$ be a groupoid with $O, M$ respectively the set of objects and morphisms. On $O$ one can define an equivalence $a\sim b$ if there exists a morphism $\phi:a\rightarrow b$. The equivalence classes are called orbits and the set of orbits is dentoed by $O/M$.
	}
	...
	\newdef{Lie groupoid\footnotemark}{\index{Lie!groupoid}
		...
		A groupoid internal to Diff. As noted in the remark below definition \ref{cat:internal_category} the pullbacks of the source and target morphisms should exist. In Diff this is equivalent to assuming that the source and target morphisms should be (surjective) submersions.
	}

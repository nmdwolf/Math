...
\subsection{Functors}
	...
	\newdef{Dagger category\footnotemark}{\index{category!dagger}\label{cat:dagger_category}\index{involution}
...
\subsection{Monoidal categories}
	...
	\newdef{Dual object}{\index{dual!object}
		Let $(C, \otimes, \mathbf{1})$ be a monoidal category and let $a\in\ob(C)$. A left dual $a^*$ of $a$ is an object in $C$ together with two morphisms $\eta:\mathbf{1}\rightarrow a\otimes a^*$ and $\varepsilon:a^*\otimes a\rightarrow\mathbf{1}$, called the \textbf{unit} and \textbf{counit} morphisms\footnote{Also called the \textbf{coevaluation} and \textbf{evaluation} morphisms.}, such that the following diagrams commute:
		\begin{figure}[ht!]
			\centering
			\begin{tikzpicture}

...
\subsection{Symmetric and alternating groups}
    ...
    \newdef{Symmetric group}{\index{symmetric!group}\index{degree}
        ...
        The symmetric group $S_n$ of the set $V_n = \{1, 2, ..., n\}$ is defined as the set of all permutations of $V_n$. The number $n$ is called the \textbf{degree} of the symmetric group. The symmetric group $\text{Sym}(X)$ on a finite set $X$ is defined similarly (by first numbering the elements and then acting by $S_n$).

        For infinite sets $Y$ one defines the the symmetric group Sym$(Y)$ as the group of all bijections from $Y$ to itself (multiplication is given by function composition).
    }

    \begin{theorem}[Cayley's theorem]\index{Cayley}
        Every group (even the infinite ones) is isomorphic to a subgroup of the permutation group Sym$(G)$.
    \end{theorem}
...
\subsection{Direct products}
    ...
    Semidirect products can even be generalized a bit further:

...
\subsection{Direct product}
	...
	\newdef{Inner semidirect product}{\index{split}
		...
		\begin{itemize}
			...
			\item There exists a group morphism $\rho:G\rightarrow H$ which satisfies $\rho|_H = \mathbbm{1}$ and $\ker(\rho)=N$.
	...
	\begin{remark}
		The direct product of groups is a special case of the outer semidirect product where the group morphism is given by the trivial map $\varphi:h\mapsto \mathbbm{1}_G$.
	\end{remark}
...
\subsection{Free groups}
	...
	\begin{theorem}\label{group:theorem:free_group}
		...
		\end{equation}
		where $A$ is a free and finitely generated group of rank $n-m$ and all $Z_{h_i}$ are cyclic groups of order $h_i$. The group $T$ is called the torsion subgroup\footnote{See also definition \ref{group:torsion_group}.}.
	\end{theorem}
...
\subsection{Group actions}
	...
	\newdef{Faithful action\footnotemark}{\index{faithful!action}\label{group:faithful_action}
		...
		A group action is faithful if the morphism $G\rightarrow\text{Sym}(X)$ is injective. Alternatively, a group action is faithful if for every two group elements $g, h\in G$ there exists an element $x\in X$ such that $g\cdot x\neq h\cdot x$.
	}

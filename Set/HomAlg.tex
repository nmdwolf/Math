...
\section{Chain complexes}
	...
	\newdef{Chain complex}{\index{chain!complex}\index{boundary}\index{differential}\index{cycle}\label{homalg:chain_complex}
		...
		\nomenclature[S_CHCat]{$\textbf{Ch}_\bullet(\textbf{A})$}{category of chain complexes with objects in the additive category $\textbf{A}$}
		Let $\mathbf{A}$ be an additive category (often an Abelian category). Consider a sequence $(C_k)_{k\in\mathbb{Z}}$ of objects and a sequence $\{\partial_k:C_k\rightarrow C_{k-1}\}_{k\in\mathbb{Z}}$ of morphisms in $\mathbf{A}$ (called the \textbf{boundary operators} or \textbf{differentials}) such that for all $k\in\mathbb{Z}$:
		\begin{gather}
			\partial_k\circ\partial_{k+1} = 0.
		\end{gather}
		This structure is called a chain complex\footnote{A \textbf{cochain complex} is constructed similarly. For this structure we consider an ascending order, i.e. $\partial_k:C_k\rightarrow C_{k+1}$.}. Elements of $\text{im}(\partial_k)$ are called \textbf{boundaries} and elements of $\text{ker}(\partial_k)$ are called \textbf{cycles}. The chain complex $\{(C_k, \partial_k)\}_{k\in\mathbb{Z}}$ is often denoted by $(C_\bullet,\partial_\bullet)$ or simply by $C_\bullet$ if the choice of boundary operators is clear.
	}

	The morphisms of chain complexes are called \textbf{chain maps} and they are defined as a collection of morphisms $\{f_k:C_k\rightarrow D_k\}_{k\in\mathbb{Z}}$ such that for all $k\in\mathbb{Z}$ the following equation holds:
	\begin{gather}
		\partial'_k\circ f_k = f_{k-1}\circ\partial_k
	\end{gather}
	where $\partial_k,\partial_k'$ are the boundary operators of respectively $C_\bullet$ and $D_\bullet$. Given an additive category $\mathbf{A}$ one can define the category $\mathbf{Ch}_\bullet(\mathbf{A})$ of chain complexes and chain maps in $\mathbf{A}$.
	...
	\newdef{Quasi-isomorphism}{\index{quasi!isomorphism}
		A chain map $f:C_\bullet\rightarrow D_\bullet$ for which the induced morphisms on homology are isomorphisms.
	}
	...
	\newdef{Chain homotopy}{\index{chain!homotopy}
		...
		\end{gather}
        A chain map homotopy equivalent to the zero map is said to be \textbf{null-homotopic}. Given two maps $f:C_\bullet\leftrightarrows D_\bullet:g$, such that both $f\circ g$ and $g\circ f$ are chain-homotopic to the identity, we say that $C_\bullet$ and $D_\bullet$ are (chain-)homotopy equivalent.
	}

	\begin{property}
		Chain-homotopic maps induce the same maps in homology. In particular, every (chain-)homotopy equivalence is a quasi-isomorphism.
	\end{property}
    ...
    \newdef{Differential modulo differential}{
        ...
        \begin{enumerate}
            ...
            \item $d^2 = [\partial,D]$ for some other morphism $D$.
        \end{enumerate}
        The first condition states that $d$ descends to a morphism on $H_\bullet(C_\bullet)$. The second condition, which says that $d$ is $\partial$-exact, states that $d^2$ is a differential on $H_\bullet(C_\bullet)$. The resulting homology theory is denoted by $H_\bullet(d|H_\bullet(C_\bullet))$.
    }
    ...
    \newdef{Differential graded algebra}{\index{dg-algebra}\label{homalg:dg_algebra}
        ...
        A differential graded algebra (often called a \textbf{dg-algebra} or even \textbf{dga}) is a (co)chain complex that carries the structure of an algebra where the differential acts as a derivation. Using the terminology of Chapter \ref{chapter:hda} this can equivalently be defined as a graded algebra equipped with a nilpotent derivation of degree $\pm1$.
    }

    The following three definitions use the language of Chapter \ref{chapter:hda}:
...
\section{Exact sequences}
	...
	\newdef{Exact sequence}{\index{exact!sequence}
		Let $\mathbf{A}$ be an additive category. Consider a sequence (finite or infinite) of objects and morphisms in $\mathbf{A}$:
		\begin{gather}
        ...
        \end{gather}
        In particular this means that $\Phi_{k+1}\circ\Phi_k = 0$ for all $h\in\mathbb{N}$ which in turn implies that exact sequences are a special type of chain complex as defined in definition \ref{homalg:chain_complex}.
	}
...
\section{Resolutions}
	...
	\newdef{Acyclic complex}{\index{acyclic}
		...
		is exact or, equivalently, if the homology complex $H_\bullet(C_\bullet)$ vanishes.
	}
    ...
    \begin{example}[Koszul-Tate resolution]\index{Koszul-Tate resolution}\index{Koszul!complex}\label{homalg:koszul_tate}
    ...
    \begin{theorem}[Homological perturbation]

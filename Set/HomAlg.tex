...
\section{Chain complexes}
	...
	\newdef{Chain complex}{\index{chain!complex}\index{boundary}\index{differential}\index{cycle}\label{homalg:chain_complex}
		...
		Let $\mathbf{A}$ be an additive category (often an Abelian category). Consider a sequence $(C_k)_{k\in\mathbb{Z}}$ of objects and a sequence $\{\partial_k:C_k\rightarrow C_{k-1}\}_{k\in\mathbb{Z}}$ of morphisms in $\mathbf{A}$ such that for all $k\in\mathbb{Z}$:
		\begin{gather}
			\partial_k\circ\partial_{k+1} = 0.
		\end{gather}
		This structure is called a chain complex\footnote{A \textbf{cochain complex} is constructed similarly with an ascending order $\partial_k:C_k\rightarrow C_{k+1}$.} and the morphisms $\partial_k$ are called the \textbf{boundary operators} or \textbf{differentials}. Elements of $\text{im}(\partial_k)$ are called \textbf{boundaries} and elements of $\text{ker}(\partial_k)$ are called \textbf{cycles}. The chain complex $\{(C_k, \partial_k)\}_{k\in\mathbb{Z}}$ is often denoted by $(C_\bullet,\partial_\bullet)$ or simply by $C_\bullet$ if the choice of boundary operators is clear.
	}
	...
	\remark{Given a chain (resp. cochain) complex $C$ one can easily construct a cochain (resp. chain) complex $\widetilde{C}$ by setting $\widetilde{C}_k := C_{-k}$.}

	\newdef{Chain homology}{\index{homology}\label{homalg:homology}
		Given a chain complex $C_\bullet$, one can define its homology groups $H_n(C_\bullet)$. Since $\partial^2=0$, the kernel $\ker(\partial_k)$ is a subgroup of the image $\im(\partial_{k+1})$. This way one can define the quotient groups as follows:
		\begin{gather}
			H_n(C_\bullet) := \frac{\ker(\partial_k)}{\im(\partial_{k+1})}.
		\end{gather}
		The kernel in this definition is often called the \textbf{cycle} group, denoted by $Z_k(C_\bullet)$. The image in this definition is often called the \textbf{boundary} group, denoted by $B_k(C_\bullet)$. The homology groups themselves form a chain complex $H_\bullet(C_\bullet)$.
	}
	...
	\newdef{Chain homotopy}{\index{chain!homotopy}
		...
		\end{gather}
        A chain map homotopy-equivalent to the zero map is said to be \textbf{null-homotopic}. Given two chain maps $f:C_\bullet\leftrightarrows D_\bullet:g$, such that both $f\circ g$ and $g\circ f$ are chain-homotopic to the identity, $C_\bullet$ and $D_\bullet$ are said to be (chain-)homotopy equivalent.
	}
    ...
    \newdef{Differential modulo differential}{\label{homalg:differential_modulus_differential}
        Consider a chain complex $(C_\bullet,\partial)$ together with a morphism $d$. This morphism is said to be a differential modulo $\partial$ if it satisfies the following conditions:
        \begin{enumerate}
            \item $d\partial+\partial d=0$, and
            \item $d^2 = [\partial,D]$ for some other morphism $D$, i.e. $d^2$ is $\partial$-exact in the endomorphism ring on $C_\bullet$.
        \end{enumerate}
        The first condition states that $d$ descends to a morphism on $H_\bullet(C_\bullet)$. The second condition states that $d$ is a differential on $H_\bullet(C_\bullet)$. The resulting homology theory is denoted by $H_\bullet(d|H_\bullet(C_\bullet))$.
    }
    ...
    \newdef{Differential graded algebra}{\index{dg-algebra}\label{homalg:dg_algebra}
        ...
        A differential graded algebra (often called a \textbf{dg-algebra}, \textbf{DGA} or even \textbf{dga}) is a (co)chain complex that carries the structure of an algebra where the differential acts as a derivation. Equivalently, this can be defined as a graded algebra equipped with a nilpotent derivation of degree $\pm1$.
    }
    \newdef{Connective DGA}{\index{connective}
        A DGA $A_\bullet$ with vanishing higher (co)homology, i.e. $H_{\geq1}(A_\bullet)=0$. For every connective DGA, one can find a quasi-isomorphic DGA concentrated in negative degree.
    }
    ...
    \newdef{Differential graded-commutative algebra}{
        ...
        A graded-commutative differential graded algebra. This is often abbreviated as \textbf{DGCA} (or even \textbf{dgca}).
    }
    \newdef{Semifree dgca}{
        A DGCA is said to be semifree if the underlying graded-commutative algebra is isomorphic (as a graded-commutative algebra) to the Grassmann algebra \ref{hda:symmetric_algebra} over a graded vector space.
    }

    \newdef{Minimal model}{\index{model}
        Let $(C,\partial)$ be a (cohomological) DGCA of finite type. A model for $C$ is a quasi-isomorphism $\rho:(A,d)\rightarrow(C,\partial)$ from a semifree DGCA $(A,d)$. This model is said to be \textbf{minimal} if $A$ is freely generated in degrees $\geq2$ and satisfies $dA\subseteq\Lambda^{\geq2}A$.
    }
    \begin{remark}[\difficult{Model structure on DGCAs}]
        The (minimal) models of DGCAs are (minimal) Sullivan algebras by Property \ref{topology:sullivan_algebras_specific}. From a model theory point of view, the (minimal) Sullivan algebras are the cofibrant objects and the (minimal) models are the cofibrant replacements.
    \end{remark}
...
\section{Exact sequences}
	...
	\newdef{Exact sequence}{\index{exact!sequence}
        ...
        \end{gather}
        In particular this means that $\Phi_{k+1}\circ\Phi_k = 0$ for all $h\in\mathbb{N}$, which in turn implies that exact sequences are a special type of chain complexes \ref{homalg:chain_complex}.
	}
...
\section{Resolutions}
	...
	\newdef{Resolution}{\index{resolution}\index{augmentation}\label{homalg:resolution}
		...
		\end{gather}
        This also implies that $X$ is the zeroth homology of the chain complex $C_\bullet := \{C_k\}_{k\geq0}$, i.e. $H_0(C_\bullet)\cong X$. The morphism $\varepsilon:C_0\rightarrow X$ is often called the \textbf{augmentation map} and the complex $C_\bullet\rightarrow X\rightarrow 0$ is called the \textbf{augmentation} of $C_\bullet$.
	}
    ...
    \begin{theorem}[Homological perturbation]\label{homalg:homological_perturbation}
        ...
        \end{gather}
    \end{theorem}
    \begin{remark}[Ghost number]\index{ghost!number}
...
\section{Derived functors}
	...
	\newdef{Prolongation}{\index{prolongation}
		Let $\func{F}{A}{A'}$ be an additive functor. The prolongation of $F$ is a functor $\overline{F}:\textbf{Ch}(\textbf{A})\rightarrow\textbf{Ch}(\textbf{A}')$ obtained by applying $F$ to every object in a chain complex and to every diagram in the definition of a chain map.
	}
	...
	One of the motivating properties of derived functors are the long exact sequences in (co)homology theories. All of these are a result of the following property:
...
\subsection{Modules}\label{section:tor_ext}
    ...
    \newdef{Koszul complex}{\index{Koszul!complex}
        ...
        \begin{gather}
            0\longrightarrow\Lambda^rM\overset{r}{\longrightarrow}\Lambda^{r-1}M\longrightarrow\cdots\longrightarrow M\longrightarrow R\longrightarrow0,
        \end{gather}
...
\section{Spectral sequences}
	...
	\newdef{Spectral sequence}{\index{spectral!sequence}\index{page}
		Consider a sequence $\{(E_i,d_i)\}_{i\in\mathbb{N}}$ of differential objects. This sequence is called a spectral sequence if it satisfies
		\begin{gather}
			H(E_i,d_i)\cong E_{i+1}
		\end{gather}
		for every $i\in\mathbb{N}$. A morphism of spectral sequences is a collection of morphism $(\varphi_i)_{i\in\mathbb{N}}$ satisfying
		\begin{itemize}
...
\subsection{Exact couples}
	...
	\newdef{Exact couple}{\index{exact!couple}
		A tuple $(A,B,\alpha,\beta,\gamma)$ such that diagram \ref{fig:exact_couple} commutes.
		\begin{figure}[!ht]
			\centering
...
\subsection{Convergence}
    ...
    \newdef{Convergence}{\index{convergence}
        ...
        \begin{gather}
            E^\infty_{p,q}\cong G_p H_q\qquad\forall p,q\in\mathbb{Z}.
        \end{gather}
    }

    \newdef{Bounded sequence}{\index{bounded}

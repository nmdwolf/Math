...
\subsection{Functions}
    ...
    \newdef{Bijection}{\index{bijection}
        A function that has an inverse. This is equivalent to stating that there is a one-to-one correspondence between elements of the domain and of the codomain.
    }
    \newnot{Isomorphic}{
...
\subsection{Posets}
    ...
    \newdef{Linear order}{\index{order!linear}
        ...
        \begin{enumerate}
            ...
            \item \textbf{Comparison}: $x<z\implies x<y\lor y<z$;
            \item \textbf{Connectedness}: $x\not< y\land y\not<x\implies x=y$.
        \end{enumerate}
    }
    \remark{Using negation one can pass freely between linear orders and total orders. However, without the law of the excluded middle, there exists no bijection between these two.}
...
\subsection{\difficult{Ordinals and cardinals}}
    ...
    \newdef{Von Neumann ordinal}{\index{Von Neumann!ordinal}
        ...
        We give as example the first few finite ordinals:
        \begin{itemize}
            \item $0 := \emptyset$
            \item $1 := \{0\} = \{\emptyset\}$
...
\section{Galois theory}
    ...
    \newdef{Degree}{\index{degree}
        If $L/K$ is a field extension then $L$ can be given a $K$-vector space structure\footnote{See definition \ref{linalgebra:vector_space}.}. The dimension of this vector space is called the degree of the extension $L$. It is often denoted by $[L:K]$.
    }
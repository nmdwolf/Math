...
\subsection{Functions}
    ...
    \newdef{Bijection}{\index{bijection}
        A function that has an inverse. Equivalently, there exists a one-to-one correspondence between the elements of the domain and those of the codomain.
    }
    \newnot{Isomorphic}{
    ...
    \begin{theorem}[Cantor-Bernstein-Schr\"oder]\index{Cantor-Bernstein-Schr\"oder}
        Consider two sets $A,B$. If there exist injections $A\hookrightarrow B$ and $B\hookrightarrow A$, then there exists a bijection $A\cong B$.
    \end{theorem}
...
\section{Set operations}
    ...
    \newdef{Complement}{\index{complement}\label{set:complement}
        Let $\Omega$ be the universe of discours\footnote{See section \ref{section:universes}.} and let $E\subseteq\Omega$. The complement of $E$ is defined as follows:
        \begin{gather}
    ...
    \newdef{Relation}{\index{relation}\label{set:relation}
        A relation between sets $X$ and $Y$ is a subset of the Cartesian prdouct $X\times Y$. A relation on $X$ is then simply a subset of $X\times X$. This definition can easily be extended to $n$-ary relations by working with subsets of $n$-fold products.
    }
...
\subsection{\difficult{Ordinals and cardinals}}
    ...
    \newdef{Well-ordering}{\index{well-order}
    ...
    The most used such construction is that by \textit{Von Neumann}: For every well-ordered set $W$ there exists an isomorphism $W\rightarrow P(W)$ that maps an element to the set of all subsets bounded from above by it. By analogy we inductively define the Von Neumann ordinals as those well-ordered sets containing all smaller ordinals:
    \newdef{Von Neumann ordinal}{\index{von Neumann!ordinal}
        ...
        We give as example the first few finite von Neumann ordinals:
        \begin{itemize}
            \item $0 := \emptyset$,
            \item $1 := \{0\} = \{\emptyset\}$,
    ...
    \begin{remark}\index{Burali-Forti paradox}
        The \textit{Burali-Forti paradox} is exactly the statement that the class of all ordinals (and by extension the class of all well-ordered sets) is not a set.
    \end{remark}
...
\section{Limits of algebraic structures}
    ...
    \newdef{Direct limit\footnotemark}{\index{direct!limit}\index{inductive limit|see{direct limit}}\label{set:direct_limit}
    ...
    \newdef{Inverse limit\footnotemark}{\index{inverse!limit}\index{projective!limit|see{inverse limit}}\label{set:inverse_limit}

...
\section{Collections}
	...
	\begin{notation}\label{set:function_set}
		\nomenclature[S]{$Y^X$}{The set of functions from a set $X$ to a set $Y$.}
...
\subsection{Inverse systems}
	...
	\newdef{Inverse limit\footnotemark}{\index{inverse!limit}\index{projective!limit}
		...
		\begin{equation}
			\label{inverse_limit}
			\varprojlim_{k\in I} A_k = \left.\left\{\vec{a}\in\prod_{i\in I}A_i\right\vert a_i=f_{ij}(a_j), \forall i\leq j\in I\right\}
		\end{equation}
		For all $i\in I$ there exists a \textbf{natural projection} $\pi_i:\varprojlim_{k\in I} A_k\rightarrow A_i$.
	}

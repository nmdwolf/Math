...
\section{Simplicial sets}
    ...
    \remark{Note that the notion of \textbf{simplicial category} can mean two distinct things. In general it will mean a category enriched in $\mathbf{sSet}$. However, conform the above definition, it can also mean a simplicial object in the (2-)category $\mathbf{Cat}$.}
    ...
    \newdef{Standard simplex}{\index{simplex}\label{sheaf:standard_simplex}
        For every $n$ we define the standard (simplicial) $n$-simplex $\Delta[n]$ as the simplicial set given by $\Delta(-, [n])$.
    }
    \begin{property}
    ...
    \begin{construct}[Geometric realisation]\index{geometric!realisation}

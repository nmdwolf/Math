...
\section{Grothendieck topos}\label{section:grothendieck_topos}
    ...
    \newdef{Global sections functor}{\index{global!sections}
        ...
        \begin{gather}
            \Gamma:\mathcal{E}\rightarrow\mathbf{Set}:X\mapsto\Gamma(1,X).
        \end{gather}
        When $\mathcal{E}$ is the sheaf topos over a topological space, this is exactly the global sections functor \ref{sheaf:global_sections_functor}. The right adjoint assigns to every set $S$ the copower $S\cdot1\equiv\bigsqcup_{s\in S}1$. When $\mathcal{E}$ is a sheaf topos, this right adjoint is exactly the constant sheaf functor.
    }
...
\subsection{Topological sheaves}
    ...
    \newdef{Petit and gros topoi}{\index{topos!petit \& gros}
        Consider a topological space $X$ together with its category of opens $\mathbf{Op}(X)$. The petit topos over $X$ is defined as the usual sheaf topos $\mathbf{Sh}(X)$. It represents $X$ as some kind of generalized space. However, we can also build a topos whose objects are generalized spaces. To this end we choose a site $S$ of ''probes'' and call the sheaf topos $\mathbf{Sh}(S)$ a gros topos. (See section \ref{section:smooth_spaces} for more information.)
    }
...
\section{Cohesion}
    ...
    In this section we will often talk about topoi \textbf{over} some base topos $\mathcal{S}$. These are topoi equipped with a geometric morphism to $\mathcal{S}$.

    \newdef{Local topos}{\index{local!topos}
        Consider a topos $\mathcal{E}$ over a base topos $\mathcal{S}$. $\mathcal{E}$ is said to be ($\mathcal{S}$-)local if the geometric morphism $(f^*\dashv f_*):\mathcal{E}\leftrightarrows\mathcal{S}$ admits a fully faithful right adjoint $f^!$. This latter property can be replaced by either of the following (necessarily equivalent) statements:
        \begin{itemize}
            \item $f^*$ is fully faithful.
            \item $f^!$ is an $\mathcal{S}$-indexed functor.
            \item $f^!$ is Cartesian closed.
        \end{itemize}
        If we take $\mathcal{S}=\mathbf{Set}$, the condition on $f^!$ is automatically satisfied since all functors are $\mathbf{Set}$-indexed.
    }

    \newdef{Connected topos}{\index{connected!topos}
        An object in a topos is said to be \textbf{connected} if its representable functor preserves finite coproducts. A Grothendieck topos is said to be \textbf{locally connected} is all objects can be written as coproducts of connected objects. If the topos is a Grothendieck topos, we obtain a left adjoint to the constant sheaf functor. In particular, a Grothendieck topos is locally connected if and only if its global sections geometric morphism is esssential. More generally we say a topos over some base topos $\mathcal{S}$ is \textbf{locally connected} if its associated geometric morphism is essential and the left adjoint is $\mathcal{S}$-indexed.

        A topos over a base topos is said to be \textbf{connected} if the inverse image part of the associated geometric morphism is fully faithful. For locally connected topoi this amounts to the property that the left adjoint in its adjoint triple preserves the terminal object. A locally connected Grothendieck topos is said to be \textbf{strongly connected} if the left adjoint in its adjoint triple preserves finite products.
    }

    \newdef{Cohesive topos}{\index{topos!cohesive}
        A local strongly connected topos.
    }

...
\section{Grothendieck topos}\label{section:grothendieck_topos}
    ...
    \newadef{Sheaf}{\index{descent}\label{topoi:local_object_sheaf}
        By definition every covering sieve admits a morphism into the Yoneda embedding: $\eta:U\hookrightarrow\mathcal{Y}c$. If we denote the collection of all these morphisms by $S$, then a presheaf is a sheaf if and only if it is $S$-local, i.e. if the following morphism is an isomorphism for all $\eta\in S$:
        \begin{gather}
            Fc\cong\mathbf{Psh}(\mathcal{Y}c, F)\xrightarrow{\mathbf{Psh}(\eta, F)}\mathbf{Psh}(U, F).
        \end{gather}
        This is also called the \textbf{descent condition} of ordinary sheafs.
    }

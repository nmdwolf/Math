...
\subsection{Group}
	...
	\end{enumerate}
        ...
        \newdef{Order of an element}{
        	The order of an element $a\in G$ is the smallest integer $n$ such that \eq{a^n = e} where $e$ is the identity element of $G$.
        }
...
\subsection{Group actions}
	...
	\newdef{Free action}{\index{free}
		\label{group:free_action}
	...
	\newdef{Faithful action\footnotemark}{\index{faithful!action}\label{group:faithful_action}
		...
		A group action is faithful if it is injective.
	}
	...
	\begin{property}
		Let $X$ be a set and let $G$ be a group such that the action of $G$ on $X$ is transitive. Then their exists a bijection $X\cong G/G_x$ where $G_x$ is the stabilizer of any element $x\in X$.\footnote{This can be proven by generalizing \ref{proof:stabilizer}}.
	\end{property}
...
\section{Rings}\index{ring}
	...
	\subsection{Ideal}\index{ideal}
    	\newdef{Ideal}{\label{linalgebra:ideal}Let $(R,+,\cdot)$ be a ring with $(R,+)$ its additive group. A subset $I\subseteq R$ is called an ideal\footnotemark\ of $R$ if it satisfies the following conditions:
        	\begin{enumerate}
...
\subsection{Graded ring}
	\newdef{Graded ring}{\index{graded}
		\label{group:graded_ring}

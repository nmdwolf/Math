\chapter{Set theory}
	\newdef{Power set}{\index{power set}
    	\label{set:power_set}
    	Let $S$ be a set. The power set is defined as the set of all subsets of $S$ and is (often) denoted by $P(S)$ or $2^S$.
    }
    \result{$S\subset P(S)$}
    
    \newdef{Partition}{\index{partition}
    	A  partition of $X$ is a collection of disjoint subsets $P = (A_i)_{i\in I} \subset X$ such that $\bigcup_{i\in I}A_i = X$.
    }
	...
	\newdef{Cover}{\index{cover}
		A cover of $S$ is a family of sets $\mathcal{F}\subseteq2^S$ such that
		\begin{equation}
			\label{set:cover}
...
\subsection{Posets}
	...
	\newdef{Totally ordered set}{\index{order}\index{ordered set}
...
\section{Algebra of sets}
	\newdef{Algebra of sets}{\index{algebra!of sets}
    	\label{set:algebra_of_sets}
    	A collection $\mathcal{F}$ of subsets of $X$ is a called an algebra over $X$ if it is closed under finite unions, finite intersections and complements. The pair $(X,\mathcal{F})$ is called a field of sets.
    }
	
\subsection{\texorpdfstring{$\sigma$}\ -algebra}
    \newdef{$\sigma$-algebra}{\index{$\sigma$-algebra}
    	\label{set:sigma_algebra}
    	A family of sets $\Sigma$ is a $\sigma$-algebra over a set $X$ if it satisfies the following 3 axioms:
        \begin{enumerate}
			\item $X\in\Sigma$
            \item Closed under complements: $\forall E\in\Sigma: E^c\in\Sigma$
            \item Closed under countable unions: $\{E_i\}_{i=1}^n\subset\Sigma:\bigcup_{i=1}^nE_i\in\Sigma$
		\end{enumerate}
    }
	\begin{remark}
    		Due to axioms $(2)$ and $(3)$ together with de Morgan's laws\footnotemark, a $\sigma$-algebra is also closed under countable intersections.
	\end{remark}
    ...
    \newdef{Borel set}{\index{Borel!set}
    	Let $\mathcal{B}$ be the $\sigma$-algebra generated by all open\footnotemark\ intervals $O\subseteq\mathbb{R}$. The elements $B\in\mathcal{B}$ are called Borel sets.
    }
    \footnotetext{Closed and half-open (both types) intervals generate the same $\sigma$-algebra.}

	\newdef{Product \texorpdfstring{$\sigma$-}\ algebra}{\label{set:product_of_sigma_algebras}
...
\subsection{Monotone class}
    \newdef{Monotone class}{\index{monotone class}
    	Let $\mathcal{A}$ be a family of sets. $\mathcal{A}$ is called a monotone class if it has the following two properties:
        \begin{itemize}
    ...
    \begin{theorem}[Monotone class theorem]
    	\label{set:theorem:monotone_class}
...
\input{Set_theory/Group}

\section{Vector spaces \& algebras}
	See definitions \ref{linalgebra:vector_space} and \ref{linalgebra:algebra} for the definitions of a vector space and algebra respectively.

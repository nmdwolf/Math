...
\section{Probability densities}
    ...
    \newdef{Cumulative distribution function}{\index{cumulative distribution function}
        Let $X$ be a random variable and let $P(X)$ be the associated PDF. The cumulative distribution function (CDF) $F(X)$ is defined as follows:
        \begin{gather}
...
\subsection{Multivariate distributions}
    ...
    \result{If $X$ and $Y$ are independent, then by remark \ref{prob:remark_independence} the marginal PDF is equal to the conditional PDF.}
...
\subsection{Important distributions}
    ...
    \begin{formula}[$\chi^2$ distribution]\index{distribution!$\chi^2$}
        ...
        \end{gather}
    \end{formula}
    \remark{Due to the CLT the $\chi^2$ distribution approximates a Gaussian distribution for large $k$: $P(\chi^2;k)\xrightarrow{k>30}\mathcal{G}(\sqrt{2\chi^2};\sqrt{2k-1},1)$.
...
\subsection{Estimation error}
    ...
    \newformula{Variance of estimator of variance}{
        \begin{gather}
            V\left(\widehat{V[x]}\right) =  \stylefrac{(N-1)^2}{N^3}\langle(x - \langle x \rangle)^4\rangle - \stylefrac{(N-1)(N-3)}{N^3}\langle(x - \langle x \rangle)^2\rangle^2
        \end{gather}
    }
...
\section{Bayesian modelling}
    ...
    \newdef{Conjugate distributions}{\index{conjugate!distribution}
        Consider a prior distribution $P(\theta)$ and a posterior distribution $P(\theta|X)$. If these distributions belong to the same family, e.g. they are both Gaussians, then they are said to be conjugate. In this case the prior $P(\theta)$ is said to be a \textbf{conjugate prior} for the likelihood $P(X|\theta)$.
    }

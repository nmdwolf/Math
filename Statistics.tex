...
\subsection{Important distributions}
	...
	\newformula{Uniform distribution}{\index{distribution!uniform}
		\begin{equation}\label{statistics:uniform_distr}
			\boxed{P(x;a,b) = \left\{
			\begin{array}{lcr}
...
\subsection{Extra conditions}
	...
	\newmethod{Bayesian statistics}{\index{Bayesian statistics}
    		\begin{equation}
		...
		where we remark that:
	        \begin{itemize}
        		\item The denominator $p(\text{result})$ is only a normalization constant.
        		\item The probability $p(x|X)$ to obtain a measurement $x$ when the true parameter is $X$ is a Gaussian distribution $\mathcal{G}(x;X,\sigma)$
        	\end{itemize}
	}
	\sremark{If nothing is known about the theory, we often assume that $p(X)$ is a uniform probability \ref{statistics:uniform_distr}.}

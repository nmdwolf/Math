\chapter{Statistics}
In this chapter, all (or most) definitions and formulas will be based on either a standard calculus approach or a data-driven approach. For a measure-theory based approach see chapter \ref{chapter:propability}.
...
\section{Data sample}
    ...
    \subsection{Average}\index{mean}\index{average|see{mean}}
        \newdef{Arithmetic mean}{The arithmetic mean is used to average out differences between measurements. It is also equal to the $1^{st}$ moment:
        	\begin{equation}
        ...
        \result{If the data has been grouped in bins, the weight function is given by the number of elements in each bin. Knowing this the mean becomes:
        	\begin{equation}
        ...
        \remark{
        	The measurements $x_i$ can be replaced by function values $f(x_i)$ to calculate the mean of the function $f(x)$.
		}
        \remark{
        	It is also important to notice that $\overline{f}(x) \neq f(\overline{x})$. The equality only holds for linear functions.
		}
        
        \newdef{Geometric mean}{
        	The geometric mean is used to average out \textit{normalized} measurements, i.e. ratios with respect to a reference value.
        	\begin{equation}
         ...
         \newdef{Harmonic mean}{
        	\begin{equation}
                ...
                h = \left(\stylefrac{1}{N}\prod_{i=1}^Nx_i^{-1}\right)^{-1}
            \end{equation}
	...
	\subsection{Dispersion}
        ...
        \result{The variance can also be written in the following way:
        	\begin{equation}
			\label{statistics:variance_without_sum}
	                \boxed{V(X) = \overline{x^2} - \overline{x}^2}
		\end{equation}
        }
        \remark{A better estimator for the variance of a sample is the following formula:
        	\begin{equation}
			\label{statistics:bessel_correction}
        	        \hat{s} = \stylefrac{1}{N-1}\sum_{i=1}^N(x_i - \overline{x})^2
		\end{equation}
        	When using equation \ref{statistics:sample_variance} it is more correct to exchange the sample mean $\overline{x}$ by the "true" mean $\mu$. Otherwise one should use the estimator \ref{statistics:bessel_correction}. 
        }
        ...
        \begin{remark}[Excess kurtosis]
		The excess kurtosis is defined as $c-3$. All univariate normal distributions have 0 excess. A positive excess is an indicator for long "fat" tails, a negative excess indicates short "thin" tails.
	\end{remark}
        
        \newdef{Percentile}{\index{percentile}
        	The $p^{th}$ percentile $c_p$ is defined as the number such that it is larger than $p\%$ of the measurements.
        }
        \sremark{From the previous definition it follows that the median is the $50^{th}$ percentile.}
        ...
        \newdef{FWHM}{\index{FWHM}
        	The Full Width at Half Maximum is the difference between the two values of the independent variable where the dependent variable is half of its maximum.
        }
        \begin{property}
	...
	\subsection{Multivariate datasets}
		...
		\sremark{The covariance of $X$ and $Y$ is often denoted as $\sigma_{XY}$. Furthermore it is important to observe the following equality: \[\sigma_X^2 = \sigma_{XX}\]}
        ...
        \newdef{Correlation coefficent}{\index{correlation}
            ...
            \noindent The correlation coefficient is bounded to the interval $[-1,1]$. Furthermore its magnitude is an indicator only for the linear dependence.
        }
        \remark{
        	For multivariate distributions these two definitions can be generalized using the following matrices:
            \begin{equation}
            ...
            Using these notations we get the following general equality:
            \begin{equation}
            	\label{statistics:general_variance_formula}
				\boxed{V_{ij} = \rho_{ij}\sigma_i\sigma_j}
			\end{equation}
        }
        \remark{
        	From equation \ref{statistics:general_variance_formula} it is clear that the diagonal elemets $\rho_{ii}$ are equal to 1.
        }

\section{Law of large numbers}
	\begin{theorem}[Law of large numbers]\index{Large numbers}
    	\label{statistics:theorem:large_numbers}
    ...
    \begin{result}[Frequentist propability\footnotemark]
		\begin{equation}
			\label{statistics:frequentist_propability}
            P(X) = \lim_{n\rightarrow\infty}\stylefrac{f_n(X)}{n}
		\end{equation}
	\end{result}
    \footnotetext{Also called the empirical propability.}

\section{Propability densities}
	\sremark{In the following sections and subsections, all distributions will be taken to be continuous. The formulas can be modified for use with discrete distributions by replacing the integral with a summation.}
    
	\newdef{Propability density functions p.d.f}{\index{pdf|see{propability density function}}\index{propability density function}
    	Let $X$ be a random variable and $P(X)$ the associated propability distribution. The p.d.f. $f(X)$ is defined as follows:
        \begin{equation}
			\label{statistics:pdf}
            \boxed{P(x_1\leq X \leq x_2) = \int_{x_1}^{x_2}f(X)dX}
		\end{equation}
        \noindent An alternative definition\footnotemark\ is the following:
        \begin{equation}
			\label{statistics:pdf_derivative}
            f(X) = \lim_{\delta x\rightarrow 0} \stylefrac{P(x\leq X\leq x+\delta x)}{\delta x}
		\end{equation}
    }
    \footnotetext{A more formal definition uses measure-theory and the Radon-Nikodym derivative.}
    ...
    \begin{theorem}
		Let $X$ be a random variable. Let $P(X)$ and $F(X)$ be the associated propability and c.d.f.
        \begin{equation}
	...
	\end{remark}
    \begin{formula}
		The $p^{th}$ percentile $c_p$ can be computed as follows\footnotemark:
        \begin{equation}
			c_p = F^{-1}(p)
		\end{equation}
	\end{formula}
    \footnotetext{This is clear from the definition of a percentile, as this implies that $F(c_p) = p$.}
    
    \newdef{Parametric family}{\index{parametric family}
    	A parametric family of propability densities $f(X;\vec{\theta})$ is a set of densities described by one or more parameters $\vec{\theta}$.
    }
    
\subsection{Theoretical moments}
    \newformula{Moment generating function}{\index{moment!generating function}
    	\begin{equation}
...
\subsection{Multivariate distributions}
	...
	\newdef{Joint density}{\index{propability density function!joint pdf}
    ...
    \remark{As $f_{XY}$ is a propability density, the normalization condition \ref{statistics:normalization} should be fulfilled.}
    
    \newdef{Conditional density}{\index{propability density function!conditional pdf}
...
\subsection{Likelihood function}
	\newdef{Likelihood}{\index{likelihood}
    	\label{likelihood}
    	The likelihood $\mathcal{L}(a;\vec{x})$ is the propability to find a set of measurements $\vec{x} = \{x_1,...,x_N\}$ given a distribution $P(X;a)$:
        \begin{equation}
...
\subsection{Extra conditions}
	\newmethod{Bayesian statistics}{
        ...
        \begin{itemize}
        	...
        	\item The propability $p(x|X)$  to have measurement $x$ when the true parameter is $X$ is a Gaussian distribution $\mathcal{G}(x;X,\sigma)$
        \end{itemize}
    }
    \remark{If nothing is known about the theory, $p(X)$ is (exaggerated assumption) a uniform propability \ref{statistics:uniform_distr}.}
...
\subsection{Testing}
    ...
    \newdef{Significance}{\index{significance}
    	The propability of making a type I error:
        \begin{equation}
    ...
    \newdef{Power}{\index{power}
    	The propability of not making a type II error:
        \begin{equation}
...
\section{Goodness of fit}\index{Goodness of fit}
	...
	\subsection{\texorpdfstring{$\chi^2$}\ -test}\index{Goodness of fit!$\chi^2$-test}
    	\begin{formula}
            \begin{equation}
...
\subsection{Runs test}\index{Goodness of fit!Runs test}
    ...
    \remark{The $\chi^2$-test and runs test are complementary. The $\chi^2$-test only takes the absolute value of the differences between the fit and data points into account, the runs test only takes the signs of the differences into account.}
...
\subsection{Kolmogorov test}\index{Kolmogorov!goodness of fit test}
	\newdef{Empirical distribution function}{\index{empirical distribution function}
    	\begin{equation}

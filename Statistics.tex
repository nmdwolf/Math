...
\subsection{Mean}\index{mean}
        ...
        \newdef{Geometric mean}{
        	Let $\{x_i\}$ be a positive data set\footnote{A negative data set is also allowed. The real condition is that all values should have the same sign.}. The geometric mean is used to average out \textit{normalized} measurements, i.e. ratios with respect to a reference value.
        	\begin{gather}
...
\subsection{Dispersion}
	...
	\begin{remark}
        ...
        \newdef{Pearson's mode skewness}{\index{Pearson}\label{statistics:pearsons_skewness}
        	\begin{gather}
...
\section{Probability densities}
	...
	\newdef{Probability density function}{\index{probability!density function}
		...
		\end{gather}
		An alternative definition\footnote{A more formal definition uses \textit{measure theory} and the \textit{Radon-Nikodym derivative}.} is the following:
	        \begin{gather}
	...
	\newdef{Parametric family}{\index{parametric family}
    		A parametric family of probability densities $f(X;\theta)$ is a set of densities described by one or more parameters $\theta$.
	}

\subsection{Function of a random variable}\index{random variable}

	\begin{formula}
	    	Let $X$ be random variable and $f(X)$ the associated PDF. Let $a(X)$ be a function of $X$. The random variable $A = a(X)$ has an associated PDF $g(A)$. If the function $a(x)$ can be inverted, then $g(A)$ can be computed as follows:
		\begin{gather}
...
\subsection{Important distributions}
	...
	\begin{property}\index{memory}\label{statistics:theorem:memoryless_exponential_distribution}
		The exponential distribution is \textit{memoryless}:
	        \begin{gather}
...
\subsection{Least squares estimation}
	...
	\newmethod{Least squares}{$ $\index{least squares}
			...
			\item Fitting a function $y = f(x;a)$ to a set of 2 variables $(x, y)$ where the $x$ values are exact and the $y$ valiues have an uncertainty $\sigma_i$ to estimate the value $a$.
		        ...
		        \item Determine (analytically) the $\chi^2$-statistic:
		        	\begin{gather}
...
\section{Comparison tests}
	...
	\newdef{McNemar test}{
		...
		\end{array}
		\end{gather}
		The null hypothesis of the McNemar test is that there is no significant difference between the predictive power of the model, i.e. $ p_a+p_c = p_a+p_b \land p_b+p_d = p_c+p_d$ where $p_\alpha$ indicates the proportion of the variable $\alpha$. By noting that the diagonal values are redundant in this description one can write the hypotheses more concisely:
		$$H_0: b=c$$$$H_1: b\neq c.$$
		The test statistic is the McNemar chi-squared statistic: $\chi^2 = \frac{(b-c)^2}{b+c}$. When the values of $b$ and $c$ are large enough ($>25$) one can approximate this distribution by an ordinary $\chi^2$-distribution with 1 degree of freedom.
	}
	\begin{remark}
		...
		\end{gather}
		The Edwards-correction gives a better approximation to this exact value.
	\end{remark}
	
	\newdef{Wilcoxon signed-rank test}{
	...
	\newdef{Nemenyi test}{
		...
		\end{gather}
		where again $k$ is the number of models and $N$ is the number of data sets. The exact critical values can either be found in the literature or one can approximately use a normal distribution.
	}
	
	\newdef{Bonferroni-Dunn test}{
...
\section{Goodness of fit}\index{goodness of fit}
	...
	\newdef{Akaike information criterion}{\index{Akaike}

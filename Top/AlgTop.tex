...
\subsection{Homotopy groups}
    ...
    \newdef{Loop space}{\index{loop!space}
        ...
        The set of all \textbf{loops} in a pointed topological space $(X,\ast)$, i.e. all continuous maps\footnote{The mapping space is equipped with the compact-open topology.} $\delta:(S^1,t_0)\rightarrow (X,\ast)$ for which $\delta(t_0)=\ast$. This space is denoted by $\Omega X$.
    ...
    \begin{property}[Universal cover]\index{deck transformation}
        Consider a topological space $X$ and let $\widetilde{X}$ be its universal covering space. The group of deck transformations $\text{Aut}(\widetilde{X})$ from definition \ref{topology:deck_transformation} is isomorphic to the fundamental group $\pi_1(X)$. Hence we obtain
        \begin{gather}
    ...
    \newdef{Homotopy group}{\index{homotopy!group}
        ...
        The homotopy group $\pi_n(X,x_0)$ is defined as the set of homotopy classes of continuous maps $f:S^n\rightarrow X$ based at $x_0\in X$. The set $\pi_0(X,x_0)$ can be seen to be the set of path-connected components of $X$. (This explains why the notation $\pi_0(X)$ was introduced before.)
    }
    ...
    \end{property}
    \begin{property}[Abelianity]\label{topology:abelian_homotopy_groups}
        For $n\geq2$  the homotopy groups $\pi_n(X,x_0)$ are Abelian. This follows from an Eckmann-Hilton argument \ref{algebra:eckmann_hilton}.
    \end{property}
    ...
    \newdef{Weak homotopy equivalence}{\index{homotopy!weak equivalence}\index{homotopy!type}
        A continuous map that induces isomorphisms on all homotopy groups. Two spaces connected via a weak homotopy equivalence are said to have the same \textbf{homotopy type}.
    }

    \newdef{$n$-connected space}{\index{connected}
        A topological space is said to be $n$-connected if it is path-conencted and if its first $n$ homotopy groups are trivial. A continuous function is said to be $n$-connected if its induced maps on homotopy groups are isomorphisms in degrees $k<n$ and surjective in degree $n$.
    }
    \newdef{Homotopy $n$-type}{\index{homotopy!type}
        A topological space for which homotopy groups $\pi_i$ vanish for $i>n$.
    }

    \newdef{Eilenberg-MacLane space}{\index{Eilenberg-MacLane}
        Let $G$ be a group (regarded as a discrete topological space) and choose a positive integer $n\in\mathbb{N}_0$. The Eilenberg-MacLane space $K(G,n)$ is a topological space with the following property:
        \begin{gather}
    ...
    \begin{property}[Uniqueness]
        For every group $G$ and integer $n$, the space $K(G,n)$ is unique up to weak homotopy equivalence.
    \end{property}
    \begin{property}[Loop spaces]
        For all groups $G$ and integers $n\in\mathbb{N}$, the loop space $\Omega K(G,n)$ is homotopy equivalent to $K(G,n-1)$.
    \end{property}
    ...
    \end{theorem}
    \remark{The statement of the above theorem is sometimes used as a consistency condition for the definition of $\infty$-categories and in some cases it is even used as the very definition of higher groupoids. It should also be noted that the relation is very important in homotopy type theory (see Chapter \ref{chapter:type_theory}).}
    ...
    \begin{theorem}[Freudenthal suspension theorem]\index{Freudenthal suspension theorem}
        The suspension morphism on homotopy
        \begin{gather}
            \pi_{n+k}(S^n)\rightarrow\pi_{n+k+1}(S^{n+1})
        \end{gather}
        is an isomorphism for all $k\leq n-2$. This can be generalized to $n$-connected CW complexes\footnote{See the next section for a definition.} as follows:
        \begin{gather}
    ...
    \end{theorem}
    \begin{remark}
        The second statement follows from the fact that the unit $X\rightarrow\Omega\Sigma X$ of the Eckmann-Hilton adjunction is $(2n+1)$-connected.
    \end{remark}

\subsection{CW complexes}

    \newdef{$n$-cell}{\index{cell}
        An open $n$-cell is a subset of a topological space homeomorphic to the $n$-dimensional open ball. A closed $n$-cell is the image of an $n$-dimensional closed ball under an attaching map \ref{topology:attaching_space}.
    }
...
\subsection{Fibrations}
    ...
    \newdef{Homotopy lifting property}{
        ...
        \begin{figure}[ht!]
            ...
            \begin{tikzpicture}
                ...
                \node (B) at (4,-2) {$B$};
                \draw[->] (X) -- (E) node[pos=0.5, above]{$\widetilde{f}_0$};
                \draw[right hook ->] (X) -- (XI) node[pos=0.5, left]{$X\times\{0\}$};
                \draw[->] (XI) -- (B) node[pos=0.5, below]{$f$};
                \draw[->] (E) -- (B) node[pos=0.5, right]{$\pi$};
                \draw[dashed, ->] (XI) -- (E) node[pos=0.5, above left]{$\widetilde{f}$};
            \end{tikzpicture}
    ...
    \newdef{Fibration}{\index{fibration}\label{topology:fibration}
        A map $\pi$ satisfying the homotopy lifting property with respect to every topological space $X$ is called a \textbf{Hurewicz fibration}. If the homotopy lifting property only holds with respect to CW complexes then it is called a \textbf{Serre fibration}.
    }
    ...
    \end{example}
    \begin{example}[Homotopy fibre]\index{homotopy!fibre|see{mapping fibre}}\index{mapping!fibre}
...
\subsection{\difficult{Rational homotopy theory}}
    ...
    \begin{property}\label{topology:sullivan_algebras_specific}
...
\subsection{\difficult{Equivariant homotopy theory}}
    ...
    \newdef{Equivariant homotopy equivalence}{\index{homotopy!equivalence}
        A continuous function that is both equivariant and a homotopy equivalence in the ordinary sense.
    }
    ...
    \begin{theorem}[Whitehead\footnotemark]\index{Whitehead}
        \footnotetext{This equivariant version is due to \textit{Bredon}.}
...
\subsection{Simplices}
    ...
    \begin{remark}[Barycentric coordinates]\index{barycenter}
        The coordinates $\lambda_i$ from previous definition are called barycentric coordinates. This terminology stems from the fact that the point $\sum_{i=0}^k\lambda_it_i$ represents the \textit{barycenter} of a gravitational system consisting of masses $\lambda_i$ placed at the points $t_i$.
    \end{remark}
    \newdef{Standard simplex}{\index{simplex!standard}\label{top:standard_simplex}
        \begin{gather}
    ...
    \newnot{Face}{\index{face}
        Consider a $k$-simplex $[v_0,\ldots,v_k]$. The \textbf{face} opposite to the vertex $v_i$ is the $(k-1)$-simplex $[v_0,\ldots,\hat{v}_i,\ldots,v_k]$ obtained by removing the vertex $v_i$.
    }
    ...
    \newdef{Path-connected}{\index{path!connected}
        Let $\mathcal{K}$ be a simplicial complex. It is said to be path-connected if every two vertices in $\mathcal{K}$ are connected by an edge in $\mathcal{K}$.
    }
    ...
    \newdef{Triangulable spaces}{\index{triangulation}
        Let $X$ be a topological space and let $\mathcal{K}$ be a polyhedron. If there exists a homeomorphism $\varphi:\mathcal{K}\rightarrow X$, we say that $X$ is triangulable and we call $\mathcal{K}$ a \textbf{triangulation} of $X$.
    }
    \begin{theorem}
        ...
        The group $G$ is isomorphic to the fundamental group $\pi_1(\mathcal{K}, a_0)$ if the generators $g_{ij}$ satisfy the following two relations:
        \begin{itemize}
...
\subsection{Simplicial homology}
    ...
    \newdef{Chain group}{\index{chain!group}\label{topology:chain_group}
        ...
        \begin{gather}
            C_k(\mathcal{K}) := \left\{\sum_ia_i\sigma_i\,\middle\vert\,\sigma_i\text{ is a $k$-simplex in $\mathcal{K}$}\text{ and }a_i\in\mathbb{Z}\right\}.
        \end{gather}
...
\subsection{Relative homology}
    ...
    \newdef{Relative boundary operator}{\index{boundary}
        ...
        \end{gather}
        where $c_k\in C_k(K)$ and $\partial_k$ denotes the boundary operator in ordinary homology. Because $\partial C_\bullet(L)\subseteq C_\bullet(L)$ this is a well-defined operation.
    }
    ...
    \begin{property}[Homotopy invariance]
        Consider two topological spaces $X,Y$ with subspaces $A\subset X,B\subset Y$. If a continuous function $f:X\rightarrow Y$ is a homotopy equivalence such that the restriction to $A$ gives a homotopy equivalence $f|_A:A\rightarrow B$, then the relative homology complex $H_\bullet(X, A)$ and $H_\bullet(Y, B)$ are isomorphic.
    \end{property}

    \begin{property}[Long sequence for a pair]
        Using the relative homology groups we obtain the following (long) exact sequence:
        \begin{gather}
            \cdots\longrightarrow H_k(L)\xrightarrow{i_\ast}H_k(K)\xrightarrow{j_\ast}H_k(K, L)\xrightarrow{\partial_k}H_{k-1}(L)\longrightarrow\cdots
        \end{gather}
        where $i_\ast$ and $j_\ast$ are the homology morphisms induced by the inclusions $i:L\rightarrow K$ and $j:K\rightarrow (K,L)$.
    \end{property}

    \begin{theorem}[Excision theorem]\index{excision}\label{topology:theorem:excision}
    ...
    \newdef{Reduced homology}{\index{homology!reduced}
        Consider the chain complex $C_\bullet(X)$ from ordinary homology. Example \ref{topology:point_homology} says that a point $\ast$ has homology $\mathbb{Z}$ concentrated in degree $0$. However, it would be nice if the point $\ast$ would have vanishing homology. To this end we can augment\footnote{See definition \ref{homalg:resolution}.} the chain complex $C_\bullet(X)$ of any topological space by the group $\mathbb{Z}$ to obtain the reduced homology complex $\widetilde{H}_\bullet$:
        \begin{gather}
            H_n(X) =
            \begin{cases}
...
\subsection{Examples}
    ...
    \newdef{Degree}{\index{degree!of map}\label{topology:degree}
        From the examples above we know that $H_n(S^n)=\mathbb{Z}$. Given a map $f:S^n\rightarrow S^n$ the induced map $f_*$ on homology is an endomorphism of $\mathbb{Z}$ and hence is of the form $f(x)=dx$ where $d\in\mathbb{Z}$. This coefficient is called the degree of $f$.
    }
    \begin{property}
...
\section{Singular homology}\label{section:singular_homology}
    ...
    \newdef{$\Delta$-complex}{\index{$\Delta$-complex}
        Let $\{\sigma_\alpha\mid\Delta^n\rightarrow X\}$ be a collection of morphisms from simplices to $X$ where the dimension $n$ may depend on the subscript $\alpha$. This collection forms a $\Delta$-complex on $X$ if it satisfies the following conditions:
        \begin{itemize}
    ...
    \begin{property}[Representability]
        Let $X$ be a CW-complex. There exists an isomorphism $[X, K(G, n)]\rightarrow H^n(X;G)$ between the homotopy classes of maps $X\rightarrow K(G,n)$ and the $n^{th}$ singular cohomology of $X$ with coefficients in $G$. (This result can be widely generalized cf. Theorem \ref{topology:brown} further below.)
...
\section{\difficult{Axiomatic approach}}
    ...
    \newdef{Eilenberg-Steenrod axioms}{\index{Eilenberg-Steenrod axioms}\label{topology:eilenberg_steenrod_axioms}
        All homology theories have a set of properties in common. By treating these properties as axioms we can construct homology theories as a sequence of functors $H_k:\mathbf{Top}\times\mathbf{Top}\rightarrow\mathbf{Ab}$ (technically one should replace $\mathbf{Top}\times\mathbf{Top}$ by the category consisting of subspace inclusions $A\hookrightarrow X$). The axioms are as follows:
        \begin{enumerate}
            \item \textbf{Homotopy invariance}: If $f,g$ are homotopic maps then their induced homology maps are the same: \[f\cong g\implies H_k(f) = H_k(g),\forall k\in\mathbb{N}.\]
    ...
    \begin{remark}
        If the dimension axiom is removed from the set of axioms, we obtain a so-called \textit{extraordinary (or generalized) homology theory}.
    \end{remark}

    Although the following theorem sounds to be more of a purely category-theoretic statement, its main application is to the subject of cohomology theories:
    \begin{theorem}[Brown's representability theorem]\index{Brown's representability theorem}\label{topology:brown}
    ...
    \newdef{Spectrum\footnotemark}{\index{spectrum}\label{topology:spectrum}
        ...
        A sequence op pointed topological spaces $\seq{X}$ such that $X_n\cong\Omega X_{n+1}$. (The isomorphism can be a weak homotopy equivalence or homeomorphism depending on the context.)
    }
    \begin{property}
...
\section{Equivariant cohomology}\index{equivariant!cohomology}
    ...
    \newdef{Equivariant cohomology}{\index{Borel!construction}\label{topology:equivariant_cohomology}
        ...
        In case the action is not free, a more general construction needs to be used. Consider the universal bundle $\pi:EG\rightarrow BG$ from Definition \ref{diff:classifying_space}. From this bundle one constructs the associated bundle $EG\times_G X$, this is also called the \textbf{Borel construction}. It gives a model for the homotopy quotient $X/\!\!/G$. The $G$-equivariant cohomology of $X$ is then defined as the singular cohomology of the Borel construction:
        \begin{gather}

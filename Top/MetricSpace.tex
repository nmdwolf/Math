...
\section{Examples of metrics}
    ...
    \end{example}
    \begin{property}[Projections determine convergence]\index{projection}
...
\section{Completeness}
    ...
    \newdef{Complete metric space}{\index{complete!space}\label{topology:complete_space}
...
\subsection{Convex metric spaces}
    ...
    \begin{property}[Convex sets]
        A closed subset of Euclidean space is a convex metric space if and only if it is a convex set (see definition \ref{linalgebra:convex}).
    \end{property}

    \newdef{Hyperconvex space}{\index{hyperconvex}
        A convex space for which the set of closed balls has the Helly property \ref{set:helly_family} is called a hyperconvex space.
    }

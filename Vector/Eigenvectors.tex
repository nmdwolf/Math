...
\subsection{Diagonalization}
    ...
    \newdef{Diagonalizable map}{
        Let $V$ be a finite-dimensional $K$-vector space. A linear map $f\in\text{End}(V)$ is diagonalizable if there exists a matrix representation $A\in M_n(K)$ of $f$ such that $A$ is a diagonal matrix.
    }
...
\subsection{Multiplicity}
    ...
    \begin{theorem}\label{linalgebra:theorem:diagonalizable_multiplicity}
        Let $f\in\text{End}(V)$ be a linear map. $f$ is diagonalizable if and only if for every eigenvalue the algebraic multiplicity is equal to the geometric multiplicity.
    \end{theorem}

...
\section{Graded vector spaces}\label{section:graded_spaces}
    ...
    \begin{remark}[Different conventions and d\'ecalage]\index{d\'calage}\index{Koszul!sign}\label{hda:decalage}
        ...
        \begin{gather}
            p_{\Sym}:=\sum_{\sigma\in S_n}\varepsilon(\sigma)\sigma\qquad\text{and}\qquad p_{\text{Alt}}:=\sum_{\sigma\in S_n}\chi(\sigma)\sigma,
        \end{gather}
...
\section{Tensor and fusion categories}
    ...
    \newdef{Tensor category}{\index{tensor!category}
        ...
        \end{enumerate}
        Some authors (such as \cite{etingof}) also add ''locally finite'' as a condition (see definition \ref{cat:locally_finite}).
    }

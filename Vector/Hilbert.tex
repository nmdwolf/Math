...
\section{Seminorms}
	...
	\newdef{Seminorm}{\index{seminorm}
		...
		\begin{itemize}
			\item \textbf{Homogeneity:} $p(a\vec{v}) = |a|p(\vec{v})$ for all scalars $a\in K$
			\item \textbf{Triangle equality (subadditivity):} $p(\vec{v} + \vec{w}) \leq p(\vec{v}) + p(\vec{w})$
		\end{itemize}
	}

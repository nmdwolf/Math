\chapter{Normed vector spaces}
	...
	In this chapter the term "linear operator", which is normally reserved for isomorphisms of the form $f:V\rightarrow V$, is used instead of "linear map". This was done to keep the vocabulary in track with that of the standard literature on Banach spaces and operator spaces.
...
\section{Banach spaces}
	...
	\newdef{Banach space}{\index{Banach!space}\label{linalgebra:banach_space}
	    	A normed vector space that is complete\footnotemark\ with respect to the norm.
...
\subsection{Bounded operators}
	...
	\begin{notation}
		\nomenclature[S]{$\mathcal{B}(V, W)$}{Space of bounded continuous maps from the space $X$ to the space $Y$.}
...
\subsection{Spectrum}
	...
	\newdef{Resolvent set}{
		\nomenclature[S]{$\rho(A)$}{The resolvent set of a bounded linear operator $A$.}
		The resolvent set $\rho(A)$ consists of all scalars $\lambda\in\mathbb{C}$ for which the resolven operator of A is a bounded linear operator on a dense subset of $V$. These scalars $\lambda$ are called \textbf{regular values} of $A$.
	}
	...
	\newdef{Continuous spectrum}{
		\nomenclature[S]{$\sigma_r(A)$}{The residual spectrum of a bounded linear operator $A$.}
		The set of scalars $\mu\in\mathbb{C}$ for which $A - \mu\mathbbm{1}_V$ fails to be surjective but for which the range of the resolvent is dense in $V$ is called the continuous spectrum of $A$. The scalars for which the range is not dense is called the \textbf{residual spectrum} $\sigma_r(A)$.
	}
	\newdef{Compression spectrum}{
		\nomenclature[S]{$\sigma(A)$}{The compression spectrum of a bounded linear operator $A$.}
	...
	\newdef{Essential spectrum}{\index{essential!spectrum}
		The set of scalars $\mu\in\mathbb{C}$ for which $A - \mu\mathbbm{1}_V$ for which $A-\mu\mathbbm{1}_V$ is not a Fredholm operator is called the essential spectrum $\sigma_{ess}(A)$.
	}
	\begin{property}

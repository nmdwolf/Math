\subsection{Homomorphisms}
	...
	\newdef{Homomorphism space}{\index{morphism!of vector spaces}
		\nomenclature[S_Hom]{$\hom(V, W)$}{Set of morphisms from a set $V$ to a set $W$.}
    		Let $V,W$ be two $K$-vector spaces. The set of all linear maps between $V$ and $W$ is called the homomorphism space from $V$ to $W$, or shorter the \texttt{hom-space} from $V$ to $W$.
	    	\begin{equation}
...
\subsection{Dual space}
	...
	\newdef{Linear form}{\index{linear!form}
	    	The elements of $V^*$ are called linear forms.
	}
	...
	\end{property}
	\begin{remark}
		If $V$ is infinite-dimensional, theorem \ref{linalgebra:dual_space_dimension} is \underline{never} valid. In the infinite-dimensional case we \textbf{always} have $|V^*|>|V|$ (where we now use the cardinality instead of the dimension).
	\end{remark}

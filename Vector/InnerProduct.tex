...
\subsection{Inner product space}
    ...
    \newdef{Metric dual}{\label{linalgebra:metric_dual}
        ...
        \end{gather}
        (See Definition \ref{riemann:flat_map} for a generalization.)
    }
    \newdef{Adjoint operator}{\index{adjoint}\index{Hermitian}\index{self-adjoint}\label{linalgebra:adjoint_operator}
        ...
        \end{gather}
        If $A=A^\dag$, $A$ is said to be \textbf{Hermitian} or \textbf{self-adjoint}. (In Chapter \ref{chapter:normed_spaces} a distinction will be made between these two notions.)
    }
...
\subsection{Orthogonality}\label{linalgebra:section:orthogonality}
    ...
    \newdef{Orthogonal projection}{\index{projection}\label{linalgebra:orthogonal_projection}
        ...
        \begin{align}
            \mathrm{proj}_W(v) &:= \sum_{i=1}^k\langle v|w_i \rangle w_i\\
            \mathrm{proj}_w(v) &:= \frac{\langle v|w \rangle}{\langle w|w \rangle}w.
        \end{align}
    }
    \begin{property}
        Orthogonal projections satisfy the following conditions:
        \begin{gather}
            \forall w\in W:\mathrm{proj}_W(w) = w \qquad\text{and}\qquad \forall u\in W^\perp:\mathrm{proj}_W(u) = 0.
        \end{gather}
    \end{property}
        ...
        \begin{enumerate}
            \item Orthogonalization:
                \begin{gather}
                    \begin{aligned}
                        w_1& = u_1&\\
                        w_2& = u_2 - \frac{\langle u_2|w_1\rangle}{\|u_2\|^2}w_1&\\
                        &\vdots&\\
                        w_n& = u_n - \sum_{i=1}^{n-1}\frac{\langle u_n|w_i\rangle}{\|u_n\|^2}w_i&
                    \end{aligned}
                \end{gather}
            \item Normalization:
                \begin{gather}

...
\subsection{Coordinates and matrix representations}
        ...
        \newdef{Coordinate isomorphism}{\label{linalgebra:coordinate_isomorphism}
        	...
        	\begin{gather}
			\beta:V\rightarrow K^n:\sum_{i=1}^n\lambda_ib_i\mapsto(\lambda_1, ..., \lambda_n)^T.
		\end{gather}
        }
        
        \begin{construct}[Matrix representation]\label{linalgebra:matrix_representation}
        	Let $V$ be an $n$-dimensional vector space and $W$ an $m$-dimensional vector space. Let $f:V\rightarrow W$ be a linear map and consider two bases $\mathcal{B}=\{b_1, ..., b_n\}, \mathcal{C} = \{c_1, ..., c_m\}$ for respectively $V$ and $W$. The matrix representation of $f$ with respect to $\mathcal{B}$ and $\mathcal{C}$ can be derived as follows:
        	
        	\hspace{1cm} For every $j\in\{1, ..., n\}$ we can write $f(b_j) = \sum_{i=1}^ma_{ij}c_i$. Based on this observation we define the matrix $(a_{ij})\in M_{m,n}(K)$ as the matrix representation of $f$. This way the $j^{th}$ column of $A_{f, \mathcal{B}, \mathcal{C}}$ coincides with the coordinate vector of $f(b_j)$ (with respect to $\mathcal{C}$). Using this relation we can construct $A_{f, \mathcal{B}, \mathcal{C}}$ by writing for every $j\in\{1, ..., n\}$ the coordinate vector of $f(b_j)$ in the $j$-th column.
        \end{construct}
...
\subsection{Coordinate transformations}
        ...
        \newdef{Transition matrix}{\label{linalgebra:transition_matrix}
        	Let $\mathcal{B} = \{b_1, ..., b_n\}$ and $\mathcal{B}' = \{b_1', ..., b_n'\}$ be two bases of $V$. By definition every element of $\mathcal{B}'$ can be written as a linear combination of elements in $\mathcal{B}$:
        	\begin{gather}
...
\subsection{Linear groups}\label{linalgebra:section:linear_groups}
	...
	\end{property}
		...
		\footnotetext{An involution is an operator that is its own inverse.}
        }
	...
	\newdef{Symplectic group}{\index{symplectic!group}
		...
		\end{gather}
		On the real or complex numbers one can define the canonical \textbf{symplectic} matrix \[\Omega_{st} = \begin{pmatrix}0&-\mathbbm{1}\\\mathbbm{1}&0\end{pmatrix}\] The group of matrices that preserve this matrix are often denoted by Sp$_n(\mathbb{R})$ or Sp$_n(\mathbb{C})$.
	}
	\begin{property}
...
\subsection{Matrix decompositions}
	...
	\begin{method}[QR Decomposition]\index{QR!decompositon}
		...
		\hspace{1cm} Let $\{v_i\}_{i\leq n}$ be a basis for the column space of $M$. By applying the Gram-Schmidt process to this basis one obtains a new orthonormal basis $\{e_i\}_{i\leq n}$. The matrix $M$ can then be written as $QR$ where:
		\begin{itemize}

\section{Matrices}
    ...
    \begin{property}\label{linalgebra:trace_commutative}
        ...
        \begin{itemize}
            ...
            \item $\text{tr}(AB) \neq \text{tr}(A)\text{tr}(B)$, and
            \item $\text{tr}(A^T) = \text{tr}(A)$.
        \end{itemize}
    \end{property}

    \newformula{Hilbert-Schmidt norm}{\index{Frobenius!norm|see{Hilbert-Schmidt}}\index{Hilbert-Schmidt!norm}
    ...
    \newdef{General linear group}{\index{general linear group}\label{linalgebra:GL_matrices}
        \nomenclature[S_GLn]{$\text{GL}(n,K)$}{general linear group: group of all invertible $n\times n$-matrices over the field $K$}
...
\subsection{Determinant}
    ...
    \newdef{Determinant of a linear operator}{\index{determinant}\label{linalgebra:operator_determinant}
        The previous property allows us to unambiguously define the determinant of $f\in\textup{End}(V)$ as follows:
        \begin{gather}
            \det(f) := \det(A)
        \end{gather}
...
\subsection{Linear groups}\label{linalgebra:section:linear_groups}
    ...
    \newdef{Special linear group}{\label{linalgebra:special_linear_group}
        ...
        The following subset of $\text{GL}_n(K)$ is called the special linear group:
        \begin{gather}
            \text{SL}_n(K) := \{A\in \text{GL}_n(K):\det(A) = 1\}.
        \end{gather}
    }
    \begin{property}
        Every $A\in\text{SL}_n(K)$ can be written as a product of elementary matrices.\footnote{This readily follows from theorem \ref{linalgebra:theorem:elementary_matrices}.}
    \end{property}
    ...
    \newdef{Orthogonal group}{\label{linalgebra:orthogonal_group}\index{orthogonal!group}
        ...
        \begin{align}
            \text{O}(n,K) &:= \{A\in \text{GL}_n(K):AA^T = A^TA = \mathbbm{1}_n\}\nonumber\\
    ...
    \newdef{Unitary group}{\label{linalgebra:unitary_group}\index{involution}
        ...
        Consider a field $K$ equipped with an \textbf{involution}\footnote{An involution is an operator that is its own inverse.} $\sigma:\lambda\mapsto\overline{\lambda}$. The unitary and special unitary group are defined as follows:
        \begin{align*}
            \text{U}_n(K,\sigma) &:= \{A\in \text{GL}_n(K):AA^\dagger = A^\dagger A = \mathbbm{1}_n\}\\
            \text{SU}_n(K,\sigma) &:= \text{U}_n(K)\cap\text{SL}_n(K).
        \end{align*}
        where $A^\dag:=\overline{A}^T$.
    }

    \begin{remark*}
        If $K=\mathbb{C}$ where the involution is taken to be the complex conjugate, the $\sigma$ is often omitted in the definition: $\text{U}(n,K)$ and $\text{SU}(n,K)$.
    \end{remark*}
    ...
    \newdef{Symplectic group}{\index{symplectic!group}
        ...
        Consider a vector space $V$ with an antisymmetric nonsingular matrix $\Omega$. The symplectic group $\text{Sp}_n(V,\Omega)$ is defined as follows:
        \begin{gather}
            \label{linalgebra:symplectic_group}
            \text{Sp}(V,\Omega) := \{A\in\text{GL}(V):A^T\Omega A = \Omega\}.
        \end{gather}
        Over the real or complex numbers we can define the canonical \textbf{symplectic} matrix
        \begin{gather}
            \Omega_{st} := \begin{pmatrix}0&-\mathbbm{1}\\\mathbbm{1}&0\end{pmatrix}.
        \end{gather}
        The groups of matrices that preserve this matrix are often denoted by $\text{Sp}(n,\mathbb{R})$ or $\text{Sp}(n,\mathbb{C})$.
    }
    \begin{property}
    ...
    \begin{example}
        $\text{Sp}(1)\cong\text{SU}(2)$ for $n=1$.
    \end{example}

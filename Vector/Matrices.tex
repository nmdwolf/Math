\section{Matrices}
	...
	\newdef{General linear group}{\index{general linear group}\label{linalgebra:GL_matrices}
		\nomenclature[S]{GL$_n(K)$}{General linear group: group of all invertible $n$-dimensional matrices over the field $K$.}
...
\subsection{Determinant}
	...
	\end{theorem}
        \begin{theorem}\label{linalgebra:theorem:determinant_inverse}
            For all $A\in\text{GL}_n(K)$ we find $A^{-1} = \det(A)^{-1}\ \text{adj}(A)$.
	\end{theorem}

...
\subsection{Determinant}
	...
	\end{theorem}
        \begin{theorem}\label{linalgebra:theorem:determinant_inverse}
            For all $A\in\textup{GL}_n(K)$ we find $A^{-1} = \det(A)^{-1}\ \text{adj}(A)$.
	\end{theorem}
...
\subsection{Linear groups}\label{linalgebra:section:linear_groups}
	...
	\end{property}
		...
		Every matrix $A\in\textup{GL}_n(K)$ can be written in the following way: \[A = SD\] where $S$ is a product of elementary matrices and $D=\text{diag}(1,\dotso,1,\det(A))$.
	\end{theorem}

...
\section{Banach spaces}
    ...
    \newdef{Weak topology}{\index{topology!weak}\label{hilbert:weak_topology}
        The initial topology\footnote{See definition \ref{topology:initial_topology}.} on a TVS with respect to its dual, i.e. a sequence $\seq{x}$ in $X$ converges to $x$ if and only if $\lambda(x_n)\longrightarrow\lambda(x)$ for all $\lambda\in X^*$.
    }
    ...
    \newdef{Norm}{\index{norm}
        ...
        \begin{enumerate}
            ...
            \item \textbf{Homogeneity:} $||\lambda v|| = |a\lambda|||v||$ for all scalars $\lambda\in K$, and

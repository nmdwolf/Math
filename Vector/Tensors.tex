\chapter{Vector \& Tensor Calculus}
    ...
    \newdef{Vector field}{\index{vector field}
        A vector field will mean a smooth vector-valued function $\vector{f}(\vector{x})\equiv\big(u(\vector{x}),v(\vector{x}),w(\vector{x})\big)$ on $\mathbb{R}^3$, where smooth implies that all three functions $u,v$ and $w$ are smooth.
    }
...
\section{Nabla-operator}\label{vectorcalculus:nabla}
    ...
    \newdef{Gradient}{\index{gradient}\label{vectorcalculus:gradient}
        Let $\varphi$ be a smooth real-valued function on $\mathbb{R}^3$.
        \begin{gather}
    ...
    \begin{example}
        Let $\varphi:\mathbb{R}^3\rightarrow\mathbb{R}$ be a smooth real-valued function and let $\deriv{\vector{r}}{s}$ denote the tangent vector to a curve $\vector{r}(s)$ with natural parameter\footnote{See \ref{diff:natural_parameter} for a formal definition.} $s$. The variation of $\varphi$ along $\vector{r}(s)$ is given by
        \begin{gather}
    ...
    \newdef{Gradient of a tensor}{\index{gradient}
        Let $T$ be a tensor field on $\mathbb{R}^3$ and let $\vector{e}^{\,i}$ be a (curvilinear) orthogonal frame\footnote{See \ref{diff:frame} for a formal definition.}. The gradient of $T$ is defined as follows:
        \begin{gather}
    ...
    \newdef{Solenoidal vector field}{\index{solenoidal}
        A vector field $\vector{A}$ is said to be solenoidal or \textbf{divergence-free} if it satisfies
        \begin{gather}
    ...
    \newdef{Irrotational vector field}{\index{irrotational}
        A vector field $\vector{A}$ is said to be irrotational if it satisfies
        \begin{gather}
    ...
    \begin{property}[Mixed properties]\label{vectorcalculus:mixed_properties}
        ...
        \end{align}
        In Cartesian coordinates equation \eqref{vectorcalculus:vector_laplacian} can be rewritten as follows:
        \begin{gather}
    ...
    \end{property}
    \begin{result}
        All conservative vector fields are irrotational. However, the converse is only true if the domain is simply-connected \ref{topology:simply_connected}. (All of this is formalized in the Poincar\'e lemma \ref{diff:poincare}.)
    \end{result}

    \newformula{Helmholtz decomposition}{\index{Helmholtz!decomposition}\label{vectorcalculus:helmholtz_decomposition}
        If $\vector{P}$ be a vector field that decays faster than $1/r$ when $r\longrightarrow\infty$, it can be written as
        \begin{gather}
            \vector{P} = \nabla\times\vector{A} + \nabla\varphi.
        \end{gather}
        for some vector field $\vector{A}$ and smooth function $\varphi$
    }

\subsection{Curvilinear coordinates}

    In this section the differential operators are generalized to curvilinear coordinates. To do this one needs the scale factors as formally defined in equation \eqref{diff:scale_factor}. Furthermore, the Einstein summation convention will not be used to make everything as explicit as possible.
    ...
    \newformula{Gradient}{\index{gradient}
        \begin{gather}
            \nabla\varphi = \sum_{i=1}^3\stylefrac{1}{h_i}\pderiv{\varphi}{q^i}\hat{e}_i
        \end{gather}
    }
    ...
    \newformula{Rotor}{\index{rotor}
        \begin{gather}
            \left(\nabla\times\vector{A}\right)_i = \sum_{j,k=1}^3\stylefrac{\varepsilon_{ijk}}{h_jh_k}\left(\pderiv{}{q^j}(A_kh_k) - \pderiv{}{q^k}(A_jh_j)\right),
        \end{gather}
        where $\varepsilon_{ijk}$ is the 3-dimensional Levi-Civita symbol \ref{tensor:levi_civita_symbol}.
    }

    \newformula{Laplacian in different coordinate systems}{\index{Laplace!operator}\label{tensor:laplace_operator}
...
\subsection{Line integrals}\index{line!integral}\index{path!integral|see{line integral}}
    ...
    \newformula{Line integral of a continuous function}{\label{vectorcalculus:line_integral_scalar}
        Let $f:\mathbb{R}^3\rightarrow\mathbb{R}$ be a continuous function and let $\Gamma$ be a piecewise smooth curve with parametrization $\vector{\varphi}(t),t\in[a,b]$. The line integral of $f$ along $\Gamma$ is defined as follows:
        \begin{gather}
            \int_\Gamma f(s)ds = \int_a^bf(\vector{\varphi}(t))||\vector{\varphi}'(t)||dt.
        \end{gather}
    }
    \newformula{Line integral of a continuous vector field}{\label{vectorcalculus:line_integral_vector}
        Let $\vector{F}$ be a continuous vector field on $\mathbb{R}^3$ and let $\Gamma$ be a piecewise smooth curve with parametrization $\vector{\varphi}(t),t\in[a,b]$. The line integral of $F$ along $\Gamma$ is defined as follows:
        \begin{gather}
            \int_\Gamma\vector{F}(\vector{s})\cdot d\vector{s} = \int_a^b\vector{F}(\vector{\varphi}(t))\cdot\vector{\varphi}'(t)dt.
        \end{gather}
    }

    \begin{property}[Conservative vector fields]\index{conservative}
...
\subsection[Integral theorems]{Integral theorems\footnotemark}
    \footnotetext{These theorems follow from a more general theorem by Stokes \ref{diff:stokes_theorem}.}
...
\section{Tensors}\index{tensor!product}\label{section:tensors}
    \nomenclature[O_zsymbinten]{$V\otimes W$}{Tensor product of the vector spaces $V$ and $W$.}

    There are two possible (equivalent) ways to introduce the concept of a \textit{tensor} on finite-dimensional vector spaces. One is to interpret tensors as multilinear maps, while the other is to interpret the components as expansion coefficients with respect to a chosen basis.

    \begin{definition}[Tensor product space]\index{outer!product}\label{tensor:tensor_product}
        The tensor product of two (finite-dimensional!!) vector spaces $V$ and $W$ is defined as\footnote{''\textit{isomorphic to}'' would be a better terminology. See the ''universal property'' \ref{tensor:prop:universal_property} below. For a complete proof and explanation, see \cite{jeevanjee}.} the set of bilinear maps on the Cartesian product $V^*\times W^*$. Let $v,w$ be vectors in respectively $V$ and $W$ and let $g,h$ be vectors in the corresponding dual spaces. The tensor product of $v$ and $w$ is then defined as follows:
        \begin{gather}
    ...
    \begin{notation}[Tensor power]\index{tensor!power}\index{tensor!type}\label{tensor:type}
        \begin{gather}
        ...
        \end{gather}
        These tensors are said to be of \textbf{type} $(r,s)$.
    \end{notation}
    ...
    \begin{remark}
        For finite-dimensional vector spaces the space $\mathcal{T}^1_1V$ is isomorphic to $\text{End}(V)$ and the space $\mathcal{T}^1_0V$ is isomorphic to $V$ itself.

        However, when including infinite-dimensional spaces, the space $\mathcal{T}^1_1V$ is only isomorphic to the endomorphism space $\text{End}(V^*)$ of the dual. This isomorphism is given by the map $\hat{T}:V^*\rightarrow V^*:\omega\mapsto\mathbf{T}(\cdot, \omega)$ for every $\mathbf{T}\in\mathcal{T}^1_1V$. Moreover, in this general setting, the spaces $\mathcal{T}^0_1V$ and $V^*$ are also isomorphic.
    \end{remark}
    ...
    \begin{proof}
        Consider a general tensor $\mathbf{T} = T_{i\ldots j}^{\ \ \ \ k\ldots l} e_k\otimes\cdots\otimes e_l\otimes\varepsilon^i\otimes\cdots\otimes\varepsilon^j$. Combining Definition \ref{tensor:tensor_product} and the pairing of dual vectors \eqref{linalgebra:dual_basis_2} gives
        \begin{align*}
...
\subsection{General operations}
    ...
    \begin{property}[Gradient of tensor products]\index{gradient}
        The gradient of a tensor product is defined through the Leibniz rule:
        \begin{gather}
...
\subsection{Complexification}
    ...
    \newdef{Complexification}{\index{complexification}\label{tensor:complexification}
        ...
        \end{gather}
        This space can still be considered a real vector space, but it can also be turned into a complex vector space by generalizing the scalar product as follows:
        \begin{gather}
    ...
    \begin{property}\label{tensor:complexification_decomposition}
        By noting that every element $v_{\mathbb{C}}\in V^{\mathbb{C}}$ can be written as \[v_{\mathbb{C}} = (v_1\otimes1) + (v_2\otimes i),\] the complexification can be decomposed as
        \begin{gather}
...
\subsection{Antisymmetric tensors}
    ...
    \end{notation}
    \begin{notation}[Antisymmetric tensors]\label{tensor:not:antysimmetric_space}
...
\subsection{Determinant}
    ...
    \newdef{Determinant}{\index{determinant}
        Consider a finite-dimensional vector space $V$ with basis $\{e_i\}_{i\leq n}$. Let $\varphi$ be a tensor in $\mathcal{T}^1_1V\cong\text{End}(V)$ and let $\omega$ be a volume form on $V$. The determinant of $\varphi$ is defined as follows:
        \begin{gather}
        ...
        One should note that the determinant is only well-defined for $(1,1)$-tensors. Although other types of tensors can also be represented as matrices, it would not be independent of a choice of basis anymore. A more general concept can be defined using the language principal bundles (Section \ref{chapter:principal_bundles}).
    }
...
\subsection{Levi-Civita symbol}
    ...
    \newformula{Cross product}{\index{cross product}\label{linalgebra:cross_product}
        Using the Levi-Civita symbol, one can define the $i^{th}$ component of the cross product\footnote{The previous remark also implies that the cross product is in fact not a vector, but a instead it is a ''pseudovector''.} as follows:
        \begin{gather}
...
\subsection{Wedge product}\label{section:wedge_product}
    ...
    \begin{definition}[Antisymmetrization]\label{tensors:antisymmetrization}
        Let $S_k=\{P_i\}_{i\leq k}$ denote the permutation group on $k$ elements, i.e. the group of all permutations of the ordered set $\{1,\ldots,k\}$. The antisymmetrization operator is defined as follows:
        \begin{gather}
            \text{Alt}(e_1\otimes\cdots\otimes e_k) := \sum_i \sgn(P_i)e_{P_i(1)}\otimes\cdots\otimes e_{P_i(k)}.
        \end{gather}
        Note that many authors introduce a factor $(1/k!)$. This convention is not adopted here to keep the following definition clean. If the factor is included, Formula \ref{tensor:general_wedge_product} below should be modified.
    \end{definition}
    ...
    \begin{construct}
        ...
        \[\{e_{i_1}\wedge\ldots\wedge e_{i_r}\ :\ \forall k: 1\leq i_k \leq \dim(V)\}.\] Accordingly, the dimension of this space is given by
        \begin{gather}
    ...
    \begin{formula}\label{tensor:general_wedge_product}
        ...
        \end{gather}
        where the antisymmetrization operator $\text{Alt}$ was defined in \ref{tensors:antisymmetrization}.
    \end{formula}
    ...
    \begin{formula}\index{cross!product}
        ...
        \end{gather}
        where $\lambda\in\Lambda^2(\mathbb{R}^3)$. See also the Hodge $\ast$-operator \ref{tensor:hodge_star} further below.

        Looking at the definition of the cross product \ref{linalgebra:cross_product}, one can see that $v\times w$ is actually the same as $J(v\wedge w)$. One can thus use the wedge product to generalize the cross product to higher dimensions.
    \end{formula}
...
\subsection{Exterior algebra}
    ...
    \newdef{Exterior algebra}{\index{exterior!algebra}\index{Grassmann!algebra}\label{tensor:exterior_algebra}
        One can define a graded vector space\footnote{Definition \ref{linalgebra:graded_vector_space}.} $\Lambda^\bullet(V)$ as follows:
        \begin{gather}
    ...
    \newadef{$\dag$}{\label{tensor:adef_exterior_algebra}
        Let $T(V)$ be the tensor algebra \ref{tensor:tensor_algebra} over the vector space $V$, i.e.
        \begin{gather}
            T(V) = \bigoplus_{k\geq0} V^{\otimes k}.
        \end{gather}
        The exterior algebra over $V$ is defined as the quotient of $T(V)$ by the two-sided ideal $I$ generated by the elements $\{v\otimes v:v\in V\}$.
    }
    ...
    \begin{property}
        Graded-commutativity implies that the wedge product of any odd exterior form with itself is identically 0. The wedge product of an even exterior form with itself vanishes if and only if the form can be decomposed as a product of 1-forms, i.e. if it is a pure $k$-form.
    \end{property}

\subsection{Hodge star}

    Equation \eqref{tensor:wedge_dimension} says that the spaces $\Lambda^k(V)$ and $\Lambda^{n-k}(V)$ have the same dimension and , hence, that there exists a linear isomorphism between them. This map is given by the Hodge star operator. Note that this map can only be defined independently of the choice of (ordered) basis if one restrict to vector spaces equipped with a nondegenerate Hermitian form \ref{linalgebra:NDH_form}.
    ...
    \newdef{Hodge star}{\index{Hodge!star}\label{tensor:hodge_star}
        ...
        \end{gather}
        where $\langle\cdot|\cdot\rangle_{n-k}$ is the inner product \eqref{tensor:wedge_inner_product} on $\Lambda^{n-k}(V)$. The element $\ast\omega$ is often called the \textbf{(Hodge) dual} of $\omega$.
        \begin{proof}
            Fix an element $\omega\in\Lambda^k(V)$. For every element $\rho\in\Lambda^{n-k}(V)$ one can see that $\omega\wedge\rho$ is an element of $\Lambda^n(V)$ and as such is a scalar multiple of $\text{Vol}(V)$. This implies that it can be written as \[c_\omega(\rho)\text{Vol}(V).\] The map $c_\omega:\Lambda^{n-k}(V)\rightarrow\mathbb{R}:\rho\mapsto c_\omega(\rho)$ is a bounded (and thus continuous) linear map, so Riesz's representation theorem \ref{hilbert:riesz} can be applied to identify $c_\omega$ with a unique element $\ast\omega\in\Lambda^{n-k}(V)$ such that \[c_\omega(\rho) = \langle\ast\omega|\rho\rangle_{n-k}.\]
        \qed
        \end{proof}
    }
    ...
    \end{result}
    \begin{remark}
        Formula \eqref{tensor:wedge_to_cross} is an explicit evaluation of the first equation \eqref{tensor:cross_by_hodge_star}.
        \begin{proof}
    ...
    \newdef{Self-dual}{\index{dual!self-dual}
        Let $V$ be a 4-dimensional inner product space and consider $\omega\in\Lambda^2(V)$. Then $\omega$ is said to be self-dual if $\ast\omega = \omega$. Furthermore, every $\rho\in\Lambda^2(V)$ can be uniquely decomposed as the sum of a self-dual and an anti-self-dual 2-form.
    }
...
\subsection{Tensor product}\index{tensor!product}
	...
	\newprop{Dimension of tensor product}{
		...
		This is exactly the same result as the one we get by applying the first definition.\qed
      	...
      	\newdef{Tensor algebra}{\index{tensor!algebra}
...
\subsection{Wedge product}
	...
	\begin{formula}[Antisymmetrization]
		...
		\begin{equation}
			...
			\text{Alt}(e_1\otimes...\otimes e_k) = \frac{1}{k!}\sum_i \sgn(P_i)e_{P_i(1)}\otimes...\otimes e_{P_i(k)}
		\end{equation}
	\end{formula}

	\newdef{Wedge product}{\index{wedge!product}
		Let $f,g$ be two vectors.
		\begin{equation}
	...
	\begin{formula}
		Let $v\in\Lambda^k(V)$ and $w\in\Lambda^m(V)$.
		\begin{equation}
			v\wedge w = \frac{(k+m)!}{k!m!}\text{Alt}(v\otimes w)
		\end{equation}

...
\subsection{Quotient space}
	...
	On infinite-dimensional spaces there exists a more general definition (that coincides with the previous one on finite-dimensional spaces):
	\begin{construct}[Tensor product]\index{tensor!product}
		Consider two vector spaces $V, W$ over a field $K$. First construct the free vector space $F(V\times W)$ over $K$. Then construct the two-sided ideal $I$ generated by the following elements:
		\begin{itemize}
			\item $(v+v', w) - (v, w) - (v', w)$
			\item $(v, w+w') - (v, w) - (v, w')$
			\item $(kv, w) - k(v, w)$
			\item $(v, lw) - l(v, w)$
		\end{itemize}
		where $k,l\in K$. The tensor product $V\otimes W$ is then given by the quotient $F(V\times W)/I$.
	\end{construct}
...
\subsection{Determinant}
	...
	\newdef{Form}{\index{form}
		An $n$-form is a totally anti-symmetric element $\omega\in\mathcal{T}^0_nV$.
	}

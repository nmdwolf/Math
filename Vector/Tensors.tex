\chapter{Vector \& Tensor Calculus}
    ...
    \newdef{Vector field}{\index{vector field}
        A smooth vector-valued function $\vector{f}(\vector{x})\equiv\big(u(\vector{x}),v(\vector{x}),w(\vector{x})\big)$ on $\mathbb{R}^3$, where smooth indicates that the component functions $u,v$ and $w$ are smooth.
    }
...
\subsection{Antisymmetric tensors}
    ...
    \end{notation}
    \begin{notation}[Antisymmetric tensors]\label{vector:not:antysimmetric_space}
...
\subsection{Exterior algebra}
    ...
    \newdef{Exterior algebra}{\index{exterior!algebra}\index{Grassmann!algebra}\label{vector:exterior_algebra}
        One can define a graded vector space \ref{linalgebra:graded_vector_space} as follows:
        \begin{gather}
...
\subsection{Hodge star}
    ...
    \newdef{Hodge star}{\index{Hodge!star}\label{vector:hodge_star}
        ...
        \begin{proof}
            Fix an element $\omega\in\Lambda^k(V)$. For every element $\rho\in\Lambda^{n-k}(V)$ one can see that $\omega\wedge\rho$ is an element of $\Lambda^n(V)$ and as such it is a scalar multiple of $\mathrm{Vol}(V)$. This implies that it can be written as \[c_\omega(\rho)\mathrm{Vol}(V).\] The map $c_\omega:\Lambda^{n-k}(V)\rightarrow\mathbb{R}:\rho\mapsto c_\omega(\rho)$ is a bounded (and thus continuous) linear map, so Riesz's representation theorem \ref{hilbert:riesz} can be applied to identify $c_\omega$ with a unique element $\ast\omega\in\Lambda^{n-k}(V)$ such that \[c_\omega(\rho) = \langle\ast\omega|\rho\rangle_{n-k}.\]
        \qed
        \end{proof}
    }

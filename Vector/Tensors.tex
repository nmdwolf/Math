...
\subsection{Levi-Civita tensor}
    ...
    \remark{
    	\label{tensor:remark:levi_civita_symbol}The Levi-Civita symbol is not a tensor, but a pseudotensor. This means that the sign changes under reflections (or any transformation with determinant $-1$).
    	}

	\newformula{Cross product}{By using the Levi-Civita symbol, we can define the $i$-th component of the cross product\footnotemark\ of two vectors $\vector{v},\vector{w}$ as follows:
		\begin{equation}
...
\subsection{Grassmann numbers}
	...
	\begin{definition}\index{DeWitt!convention for Grassmann variables}

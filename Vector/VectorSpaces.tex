...
\subsection{Algebras}
    ...
    \newdef{Algebra}{\index{algebra}\label{linalgebra:algebra}
        ...
        \begin{enumerate}
            \item \textbf{Right distributivity}: $(\vec{x} + \vec{y})\star\vec{z} = \vec{x}\star\vec{z} + \vec{y}\star\vec{z}$
            \item \textbf{Left distributivity}: $\vec{x}\star(\vec{y} + \vec{z}) = \vec{x}\star\vec{y} + \vec{x}\star\vec{z}$
            \item \textbf{Compatibility with scalars}: $(a\vec{x})\star(b\vec{y}) = ab(\vec{x}\star\vec{y})$
        \end{enumerate}

...
\subsection{Subspaces}
	...
	\begin{property}[$\dag$]\label{linalgebra:grassmannian_construction}
		The action of $GL(V)$ acts transitively\footnote{See definition \ref{group:transitive}} on all $k$-dimensional subspaces of $V$. It follows that the coset space\footnotemark\ $GL(V)/H_W$ for any stabilizer $H_W$ of some $W\in \text{Gr}(k, V)$ is isomorphic (as a set) to $\text{Gr}(k, V)$.
		\footnotetext{See definition \ref{group:coset}.}
	\end{property}
    
    \subsection{Algebra}
        \newdef{Algebra}{\index{algebra}
            \label{linalgebra:algebra}
    ...
    \subsection{Sum and direct sum}
    	\newdef{Sum}{
        	\label{linalgebra:sum}Let $V$ be a K-vector space. Let $W_1, W_2,..., W_k$ be subspaces of $V$. The 'sum' of the subspaces $W_1,..., W_k$ is defined as follows:
        	\begin{equation}
				W_1+...+W_k:=\left\{\sum_{i=1}^kw_i : w_i\in W_i\right\}
			\end{equation}
        }
        \newdef{Direct sum}{\index{direct sum}
        	\label{linalgebra:direct_sum}If every element $v$ of the sum as defined in definition \ref{linalgebra:sum} can be written as a unique linear combination, then the sum is called a direct sum.
		}
        \newnot{Direct sum}{

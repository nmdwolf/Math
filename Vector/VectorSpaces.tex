...
\subsection{Sum and direct sum}
	...
	\begin{theorem}\label{linalgebra:theorem:direct_sum}
	...
	\end{theorem}
        ...
        \newdef{Complement}{\index{complement}\label{linalgebra:complement}
...
\subsection{Algebras}
        ...
        \newdef{Temperley-Lieb algebra}{\index{Temperley-Lieb}\index{Jones!relations}
        	...
        	Multiplication in TL$_n(\delta)$ is performed diagrammatically by placing two diagrams side by side. Closed loops can be replaced by a factor $\delta$.
        }
...
\section[Linear maps]{Linear maps\footnote{Other names are \textbf{linear mapping} and \textbf{linear transformation}.}}
	...
	\newdef{General linear group\footnotemark}{\index{general linear group}
		\nomenclature[S_GL]{GL$(V)$}{General linear group: group of all automorphisms on a vector space $V$.}
	    	The set of all automorphisms $f:V\rightarrow V$ is called the general linear group GL$_K(V)$ of GL$(V)$.

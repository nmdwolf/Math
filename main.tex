\documentclass[12pt]{report}
\usepackage{Custom}
\pagestyle{headings}
\makeindex


\begin{document}

\reversemarginpar
\setcounter{tocdepth}{2}

\title{Summary: Mathematics \& Physics}
%\date{}
%\author{}
\author{Nicolas Dewolf}
\maketitle

\tableofcontents

\chapter{Introduction}
Definitions, properties and formulas marked by a dagger symbol $^\dag$ are explained and derived in one of the appendices. These derivations are placed in appendices such that the course itself contains only core notions and formulas.

Definitions of words in the middle of a text will be indicated by \textbf{bold text}. Terminology that has been defined in the past but that receives a new meaning/nuance will be indicated by \textit{italic text}.

\part{Set theory}
\input{Set_theory/Set}
\input{Topology/MetricSpace}
\input{Topology/Topology}

\part{Calculus}
\input{Calculus}
\input{ComplexCalculus}
\input{Lebesgue/Lebesgue}
\input{Transforms}
\input{Distributions}
\input{OrdinaryDiffEq}%
\input{PartialDiffEq}%
\input{Bessel}%

\part{Vectors \& tensors}
\input{Vector/LinearAlgebra}
\input{Vector/VectorCalculus}
\input{Vector/Hilbert}
\input{Vector/Operators}
\input{Vector/Tensors}
\input{Vector/Clifford}

\part{Differential geometry}
\input{Diff/Surface}
\input{Diff/Manifolds}
\input{Diff/Lie}
\input{Set_theory/Rep}
\input{Diff/Bundles}
\input{Diff/Forms}

\part{Statistics}
\input{Propability}
\input{Statistics}

\part{Classical mechanics}
\input{Physics/Mechanics}
\input{Physics/Lagrange}
\input{Physics/Optics}%
\input{Physics/Astronomy}%

\part{Electromagnetism}
\input{Physics/Electricity}
\input{Physics/Magnetism}
\input{Physics/MaxwellEquations}
\input{Physics/Fluidum}%

\part{Relativistic Mechanics}
\input{Physics/Relativity}

\part{Quantum Mechanics}
\input{QM/Schrodinger}
\input{QM/Formalism}
\input{QM/AngularMomentum}
\input{QM/Dirac}
\input{QM/Perturbation}
\input{QM/Scattering}
\input{QM/Computing}
\input{QM/QFT}

\part{Thermal physics \& Statistical mechanics}%
\input{Physics/Thermodynamics}%
\input{Physics/StatisticalMechanics}%
\input{Physics/PhotonGas}%

\part{Material physics}%
\input{Physics/SolidState}%

\part{Appendices}
\begin{appendices}
\input{DerivMath}
\input{Physics/deriv1}
\input{Units}
\end{appendices}

\begin{thebibliography}{99}
	\bibitem{AMP1}
	Analysis, Manifolds and Physics, Part 1: Basics (Y. Choquet-Bruhat, C. DeWitt-Morette \& M. Dillard-Bleick)
	\bibitem{AMP2}
	Analysis, Manifolds and Physics, Part 2 (Y. Choquet-Bruhat, C. DeWitt-Morette)
	\bibitem{Peskin}
	An Introduction to Quantum Field Theory (Michael E. Peskin \& Daniel V. Schroeder, Westview Press)
	\bibitem{jeevanjee}
    	An Introduction to Tensors and Group Theory for Physicists (Nadir Jeevanjee, Birkh\"auser)
	\bibitem{branchcut}
    	Branch cuts: http://www.math.odu.edu/~jhh/ch45.PDF
	\bibitem{Goldstein}
	Classical Mechanics (Goldstein, Safko \& Poole, Pearson New International Edition)
	\bibitem{symplectic}
	Elementary Symplectic Topology and Mechanics (Franco Cardin, Springer)
	\bibitem{greiner}
	Field Quantization (Reinhardt Greiner, Springer)
	\bibitem{invariant_sub}
	Invariant Subspaces (H. Radjavi \& P. Rosenthal)
	\bibitem{principal_bundles}
	Principal Bundles: The Classical Case (Stephen Bruce Sontz, Springer)
	\bibitem{bransden}
    	Quantum Mechanics (B.H. Bransden \& C.J. Joachain, Prentice Hall)
	\bibitem{fultonharris}
	Representation Theory: A First Course (William Fulton \& Joe Harris, Springer)
	\bibitem{sennash}
	Topology and Geometry for Physicists (Charles Nash \& Siddharta Sen, Dover Publications)
\end{thebibliography}

\printindex

\end{document}
